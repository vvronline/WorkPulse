...
const meetingLeaveKey = (tenantId: number | null | undefined, userId: number, meetingId: number): string =>
...
async function _checkMembership(db: DbLike, key: string, sql: string, params: unknown[]): Promise<boolean> {
    const now = Date.now();
...
async function isConversationMember(db: DbLike, conversationId: number, userId: number): Promise<boolean> {
...
async function isMeetingMember(db: DbLike, meetingId: number, userId: number): Promise<boolean> {
    ...
    wss.on("connection", async (ws: ExtWS, req: IncomingMessage) => {
        ...
        if (wasOffline) {
            ...
            db.query("UPDATE users SET last_seen_at = NOW() WHERE id = $1", [userId])
                .catch((err: any) => logger.warn({ err: err.message, userId }, "last_seen_at bump failed"));
        }
        ...
        ws.on("close", () => {
            ...
            if (set) {
                ...
                if (set.size === 0) {
                    ...
                    db.query("UPDATE users SET last_seen_at = NOW() WHERE id = $1", [userId]).catch((err: any) => { logger.warn({ err: err.message, userId }, "Failed to update last_seen_at on disconnect"); });
                }
            }
            ...
            if (ws._statusSessionKey) {
                statusService.closeSession({ db, tenantId }, ws._statusSessionKey)
                    .catch((err: any) => logger.warn({ err: err.message, userId }, "statusService.closeSession failed"));
 ...
 */
function scheduleMeetingDisconnectCleanup({ db, tenantId, userId, meetingId }: MeetingCleanupArgs): void {
        ...
        try {
            ...
            if (activeParticipants.length === 0) {
                ...
                statusService.clearActivityForRef({ db, tenantId }, "in_meeting", meetingId)
                    .catch((err: any) => logger.warn({ err: err.message, meetingId }, "clearActivityForRef(in_meeting) on grace expiry failed"));
            }
        } catch (err: any) {
...
function cancelMeetingDisconnectCleanup({ tenantId, userId, meetingId }: { tenantId: number | null | undefined; userId: number; meetingId: number }): boolean {
    const key = meetingLeaveKey(tenantId, userId, meetingId);
    const existing = pendingMeetingLeaves.get(key);
    if (existing?.timer) {
...
async function handleChatMessage(db: DbLike, senderId: number, tenantId: number | null, msg: any, ws: ExtWS): Promise<void> {
    if (msg.type === "chat_message") {
        ...
        if (ws._statusSessionKey) {
            ...
            ).catch((err: any) => logger.warn({ err: err.message, meetingId }, "setSessionActivity(in_meeting) failed"));
        ...
        if (ws._statusSessionKey) {
            ...
            ).catch((err: any) => logger.warn({ err: err.message, meetingId }, "clearSessionActivity(in_meeting) on leave failed"));
        ...
        statusService.clearActivityForRef({ db, tenantId }, "in_meeting", meetingId)
            .catch((err: any) => logger.warn({ err: err.message, meetingId }, "clearActivityForRef(in_meeting) on end failed"));
        if (ws._meetingActivityRefId === meetingId) ws._meetingActivityRefId = null;
 ...
 */
function deliverLocal(tenantId: number | null | undefined, userId: number, type: WSType, data: unknown): void {
    const ck = clientKey(tenantId, userId);
    const set = clients.get(ck);
    if (!set) {
 ...
 */
function sendToUser(tenantId: number | null | undefined, userId: number, type: WSType, data: unknown): void {
 ...
 */
function broadcast(tenantId: number | null | undefined, type: WSType, data: unknown): void {
    const msg = JSON.stringify({ type, data });
 ...
 */
async function notifyUser(db: DbLike, tenantId: number | null | undefined, userId: number, type: string, title: string, body: string, linkTaskId?: number | null): Promise<void> {
    try {
...
export { setupWebSocket, sendToUser, broadcast, notifyUser, handleChatMessage, isConversationMember, isMeetingMember };