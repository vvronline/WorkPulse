...
// json2csv and pdfkit ship their own types via @types or bundled definitions.
// Use require to stay consistent with the CommonJS runtime + avoid ESM interop issues.
 ...
 */
function sendCSV(res: Response, data: Row[], fields: unknown, filename: string): void {
 ...
 */
function sendPDF(res: Response, { title, columns, rows, filename }: SendPDFOptions): void {
    ...
    function drawRow(y: number, values: string[], font: string, fontSize: number, options: DrawRowOptions = {}): number {
 ...
 */
function sendPayrollCSV(res: Response, { from, to, summaryRows, detailRows, filename }: PayrollExportOptions): void {
    const summaryFields = [
    ...
    const sanitize = (rows: Row[]): Row[] => rows.map((row) => {
        const out: Row = {};
 ...
 */
function sendPayrollPDF(res: Response, { from, to, summaryRows, detailRows, filename }: PayrollExportOptions): void {
    ...
    function computeWidths(ratios: number[]): number[] {
     ...
     */
    function drawRow(y: number, values: unknown[], widths: number[], { bold = false, fontSize = 7, bg, textColor = "#222222" }: DrawRowOptions = {}): number {
    ...
    function pageBreakNeeded(y: number, needed: number): boolean {
    ...
    for (const [empName, empRows] of Object.entries(grouped)) {
        ...
        empRows.forEach((row) => {
            ...
            const bg = STATUS_BG[String(row.status)] || "#f9f9f9";
...
export { sendCSV, sendPDF, sendPayrollCSV, sendPayrollPDF };