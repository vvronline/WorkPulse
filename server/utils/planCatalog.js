 ...
 *
 * @param {string} plan - tenants.plan
 * @param {object} rawOverrides - tenants.features JSONB (possibly malformed)
 * @returns {Object<string,boolean>} every feature key resolved to a strict boolean
 */
 ...
 *
 * @param {object|null} tenant - tenant row (or null for master-context routes)
 * @param {string} featureName
 * @returns {boolean} true if feature is on, false if off, true for null tenant
...
function isFeatureEnabled(tenant, featureName) {
    ...
    const eff = getEffectiveFeatures(tenant.plan, tenant.features);
