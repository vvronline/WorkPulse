 ...
 */
function signIngestToken({ broadcastId, meetingId, userId }: TokenParams): string {
...
function verifyIngestToken(token: string | null | undefined): IngestTokenPayload | null {
    ...
    try {
        const payload = JSON.parse(Buffer.from(body, "base64url").toString("utf8")) as IngestTokenPayload;
 ...
 */
function buildIngestUrl({ broadcastId, meetingId, userId }: TokenParams): string | null {
 ...
 */
function buildPlaybackUrl(broadcastId: string): string | null {
 ...
 */
function provisionBroadcast({ meetingId, userId }: ProvisionParams): ProvisionResult | null {
    if (!process.env.HLS_MEDIA_SERVER) return null;
    const broadcastId = crypto.randomBytes(8).toString("hex") + "-" + meetingId;
    const ingestUrl = buildIngestUrl({ broadcastId, meetingId, userId });
    const hlsUrl = buildPlaybackUrl(broadcastId);
    if (!ingestUrl || !hlsUrl) return null;
    return {
        broadcastId,
        ingestUrl,
        hlsUrl,
