...
async function verifyApprovalCode(code, hash) {
    ...
    try {
        ...
        logger.warn({ err: e.message }, 'impersonation: bcrypt compare failed');
        return false;
    }
}
 ...
 *
 * @param {{ query: Function }} db   Tenant DB pool
 * @param {{ id: number, full_name: string|null }} platformAdmin
 * @returns {Promise<{id:number, username:string, full_name:string,
 *                    email:string|null, role:string, token_version:number}>}
 */
