...
class HandlerStats {
    ...
    samples: number[] = new Array(WINDOW_SIZE).fill(0);
    sampleIdx = 0;
    ...
    recordLatency(ms: number): void {
        ...
        if (this.sampleFilled < WINDOW_SIZE) this.sampleFilled++;
    }
 ...
 */
async function recordHandler(type: string, timeoutMs: number, fn: () => Promise<unknown>): Promise<void> {
        ...
        if (timeoutMs && timeoutMs > 0) {
            await Promise.race([
                ...
                new Promise((_, reject) => setTimeout(() => {
                    stats.timeouts++;
                    const err: TimeoutError = new Error(`ws-handler timeout: ${safeType}`);
...
interface RegistrySnapshot {
    handlers: Record<string, HandlerSnapshot>;
    totals: {
        count: number;
        errors: number;
        timeouts: number;
...
function snapshot(): RegistrySnapshot {
    const handlers: Record<string, HandlerSnapshot> = {};
    let totalCount = 0;
    let totalErrors = 0;
    let totalTimeouts = 0;
