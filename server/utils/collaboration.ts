 ...
 */
function parseDocName(documentName: string): ParsedDocName | null {
 ...
 */
async function getDbForTenant(tenantId: number | null): Promise<DbForTenant | null> {
    if (!tenantId) return { query: masterQuery };
    const tenant = await getTenantById(tenantId);
    if (!tenant || tenant.status !== "active") return null;
    const poolEntry = await getTenantPool(tenant.db_name, tenant.db_host);
    return {
        query: poolEntry.query as DbForTenant["query"],
        transaction: poolEntry.transaction as DbForTenant["transaction"],
    };
}
            ...
            new Database({
                ...
                async fetch({ documentName }) {
                    ...
                    try {
                        ...
                        logger.warn({ err: (err as Error).message, documentName }, "Collab: fetch doc failed");
                        return null;
                    }
                },
                ...
                async store({ documentName, state }) {
                    ...
                    try {
                        ...
                        logger.warn({ err: (err as Error).message, documentName }, "Collab: store doc failed");
                    }
                },
            }),
        ],
