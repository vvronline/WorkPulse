 ...
 */
async function attachTenantDb(req: any, tenant: any): Promise<void> {
    req.tenant = tenant;
 ...
 */
function attachMasterDb(req: any): void {
...
async function resolveTenant(req: any, res: Response, next: NextFunction): Promise<void | Response> {
    try {
        ...
        if (jwtTenantId) {
            ...
            let tenant: any = await redis.get(cacheKey);
            if (!tenant) {
 ...
 */
function requireTenant(req: any, res: Response, next: NextFunction): void | Response {
    if (!req.tenant) {
 ...
 */
function requireMinPlan(minPlan: string) {
    return (req: any, res: Response, next: NextFunction): void | Response => {
        if (!req.tenant) return next();
 ...
 */
async function invalidateTenantCache(tenantId: number, customDomain?: string): Promise<void> {
