...
export const sql = {
    ...
    q069: "SELECT id FROM conversations WHERE id = $1 FOR UPDATE",
    ...
    q075: "DELETE FROM call_logs cl WHERE cl.status IN ('ended', 'missed', 'declined') AND EXISTS ( SELECT 1 FROM conversation_participants cp WHERE cp.conversation_id = cl.conversation_id AND cp.user_id = $1 )",
    q076: "DELETE FROM call_logs cl WHERE cl.id = ANY($1::int[]) AND cl.status IN ('ended', 'missed', 'declined') AND EXISTS ( SELECT 1 FROM conversation_participants cp WHERE cp.conversation_id = cl.conversation_id AND cp.user_id = $2 )",
    ...
    q079: "SELECT 1 FROM call_logs cl WHERE cl.conversation_id = $1 AND cl.status IN ('ringing', 'answered') LIMIT 1",
    q080: "DELETE FROM conversations WHERE id = $1 RETURNING id",
