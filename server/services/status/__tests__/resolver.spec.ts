...
import { resolveEffective, type ResolverInput, type ResolvedStatus, type SessionInput } from "../resolver";
import { IDLE_AWAY_MS, SESSION_STALE_MS } from "../constants";

const NOW = new Date("2026-01-15T12:00:00.000Z");
const ms = (delta: number): Date => new Date(NOW.getTime() + delta);

const openSession = (extra: Partial<SessionInput> = {}): SessionInput => ({
    sessionKey: "sess-1",
    disconnectedAt: null,
    lastSeenAt: NOW,
    activity: null,
...
describe("resolveEffective — validation", () => {
    ...
    test("rejects invalid manualStatus", () => {
        expect(() =>
            resolveEffective({
                sessions: [],
                manualStatus: "offline" as unknown as ResolverInput["manualStatus"], // legacy! no longer valid as a manual choice
                presencePreference: "auto",
                now: NOW,
            })
    ...
    test("rejects invalid presencePreference", () => {
        expect(() =>
            resolveEffective({
                sessions: [],
                manualStatus: null,
                presencePreference: "bogus" as unknown as ResolverInput["presencePreference"],
                now: NOW,
            })
