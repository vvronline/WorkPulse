...
type StatusCtx = Parameters<typeof statusService.openSession>[0];
...
interface FakeUser {
    ...
    status_message_expires_at: Date | null;
...
// The repository always `await`s db.query, so the fake returns a resolved
// promise. `rows` is `any[]` because the fake yields domain rows (FakeUser /
// FakeSession) that the repository reads structurally.
 ...
 */
function makeDb({ users }: { users: Partial<FakeUser>[] }): {
    ...
    async function query(sql: string, params: unknown[] = []): Promise<QueryResult> {
        ...
        if (/SELECT id, manual_status, presence_preference.*FROM users WHERE id = \$1/i.test(s)) {
            const u = state.users.get(params[0] as number);
            return { rows: u ? [u] : [] };
        }
        ...
        if (/^UPDATE users SET manual_status = \$1, status_message = \$2, status_message_expires_at = \$3 WHERE id = \$4$/i.test(s)) {
            const [status, message, expiresAt, userId] = params as [string, string | null, Date | null, number];
            const u = state.users.get(userId);
            if (u) {
                u.manual_status = status;
                u.status_message = message;
                u.status_message_expires_at = expiresAt;
            }
            return { rows: [] };
        }
        // ── set presence preference ───────────────────────────────────────
        if (/^UPDATE users SET presence_preference = \$1 WHERE id = \$2$/i.test(s)) {
            const u = state.users.get(params[1] as number);
            if (u) u.presence_preference = params[0] as string;
            return { rows: [] };
        }
        // ── touch last activity ───────────────────────────────────────────
        if (/^UPDATE users SET last_activity_at = \$1 WHERE id = \$2$/i.test(s)) {
            const u = state.users.get(params[1] as number);
            if (u) u.last_activity_at = params[0] as Date;
            return { rows: [] };
        }
        // ── open session (INSERT … ON CONFLICT) ───────────────────────────
        if (/^INSERT INTO user_presence_sessions/i.test(s)) {
            const [userId, sessionKey, deviceLabel] = params as [number, string, string | null];
        ...
        if (/^UPDATE user_presence_sessions SET last_seen_at = NOW\(\)/i.test(s)) {
            const sess = state.sessions.get(params[0] as string);
        ...
        if (/^UPDATE user_presence_sessions SET activity = \$2, activity_ref_id = \$3/i.test(s)) {
            const [sessionKey, activity, refId] = params as [string, string, number | null];
        ...
        if (/^UPDATE user_presence_sessions SET activity = NULL, activity_ref_id = NULL.*WHERE activity = \$1 AND activity_ref_id = \$2/i.test(s)) {
            const [activity, refId] = params as [string, number];
        ...
        if (/^UPDATE user_presence_sessions SET activity = NULL, activity_ref_id = NULL/i.test(s)) {
            const sessionKey = params[0] as string;
            const onlyActivity = params[1] as string | undefined;
            const sess = state.sessions.get(sessionKey);
            if (sess && !sess.disconnected_at) {
        ...
        if (/^UPDATE user_presence_sessions SET disconnected_at = NOW\(\), activity = NULL.*WHERE user_id = \$1/i.test(s)) {
            for (const sess of state.sessions.values()) {
                if (sess.user_id === (params[0] as number) && !sess.disconnected_at) {
                    sess.disconnected_at = new Date();
                    sess.activity = null;
                    sess.activity_ref_id = null;
                }
        ...
        if (/^INSERT INTO user_status_events/i.test(s)) {
            state.events.push({
                ...
                from_state: params[2] as string | null,
                to_state: params[3] as string | null,
                session_key: params[4] as string | null,
                metadata: params[5],
                created_at: new Date(),
            });
            return { rows: [] };
        ...
        if (/^SELECT id FROM users WHERE org_id = \$1 AND id <> \$2 AND is_active = TRUE$/i.test(s)) {
            const [orgId, excludeId] = params as [number, number];
            return {
        ...
        if (/^SELECT p\.id\s+FROM users a\s+JOIN users p ON p\.org_id = a\.org_id\s+WHERE a\.id = \$1/i.test(s)) {
            const actorId = params[0] as number;
...
describe("StatusService — full side-effect chain", () => {
    ...
    test("rejects invalid manual status", async () => {
        ...
        const ctx = { db: { query }, tenantId: null, actorUserId: 1 };
        await expect(
            statusService.setManualStatus(ctx, { status: "in_call" as unknown as "dnd" })
        ).rejects.toThrow(/invalid status/);
    });

    test("rejects calls without ctx.db", async () => {
        await expect(
            statusService.setManualStatus({} as unknown as StatusCtx, { status: "busy" })
...
describe("StatusService — clearActivityForRef (call/meeting end)", () => {
    test("end of group call clears in_call for every participant in one sweep", async () => {
        ...
        const map = await statusService.getEffectiveBulk(ctx, [1, 2, 3]);
        ...
        expect(results.every((r: { effective: string }) => r.effective === "available")).toBe(true);
