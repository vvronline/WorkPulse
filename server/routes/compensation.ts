...
function logAction(req: Request, action: string, entity: string, entityId: unknown, details: Record<string, unknown> = {}) {
    req.db!.query(
...
router.get("/templates", requireSameOrg, async (req: Request, res: Response) => {
    try {
...
router.post("/templates", requireRole("hr_admin"), requireSameOrg, async (req: Request, res: Response) => {
    try {
...
router.put("/templates/:id", requireRole("hr_admin"), requireSameOrg, async (req: Request, res: Response) => {
    try {
        const { name, description, components, is_default } = req.body;
...
router.delete("/templates/:id", requireRole("hr_admin"), requireSameOrg, async (req: Request, res: Response) => {
    try {
...
router.get("/employees", requireRole("hr_admin"), requireSameOrg, async (req: Request, res: Response) => {
    try {
        const result = await req.db!.query(
...
router.get("/employees/:userId", requireRole("hr_admin"), requireSameOrg, async (req: Request, res: Response) => {
    try {
        const result = await req.db!.query(
...
router.post("/employees/:userId", requireRole("hr_admin"), requireSameOrg, async (req: Request, res: Response) => {
    try {
...
router.put("/employees/:userId/:id", requireRole("hr_admin"), requireSameOrg, async (req: Request, res: Response) => {
    try {
...
router.post("/payroll-run", requireRole("hr_admin"), requireSameOrg, async (req: Request, res: Response) => {
    try {
...
router.get("/salary-slips", requireSameOrg, async (req: Request, res: Response) => {
    try {
        const { pay_period_id, status, user_id } = req.query as { pay_period_id?: string; status?: string; user_id?: string };
        ...
        let query = `SELECT ss.*, u.full_name, u.email, d.name AS department_name,
                     ...
                     WHERE ss.org_id = $1`;
        const params: unknown[] = [req.userOrgId];
        let idx = 2;
...
router.get("/salary-slips/:id", requireSameOrg, async (req: Request, res: Response) => {
    try {
...
router.put("/salary-slips/:id/publish", requireRole("hr_admin"), requireSameOrg, async (req: Request, res: Response) => {
    try {
        const result = await req.db!.query(
...
router.post("/salary-slips/bulk-publish", requireRole("hr_admin"), requireSameOrg, async (req: Request, res: Response) => {
    try {
        const { pay_period_id } = req.body;
        if (!pay_period_id) return res.status(400).json({ error: "pay_period_id is required" });
        const result = await req.db!.query(
...
router.get("/salary-slips/:id/pdf", requireSameOrg, async (req: Request, res: Response) => {
    try {
        const slip = (await req.db!.query(
...
router.get("/my-slips", async (req: Request, res: Response) => {
    try {
        const result = await req.db!.query(
...
router.get("/my-slips/:id/pdf", async (req: Request, res: Response) => {
    try {
        const slip = (await req.db!.query(
...
router.post("/disburse", requireRole("hr_admin"), requireSameOrg, async (req: Request, res: Response) => {
    try {
        const { pay_period_id } = req.body;
        if (!pay_period_id) return res.status(400).json({ error: "pay_period_id is required" });
...
router.post("/disburse/:slipId", requireRole("hr_admin"), requireSameOrg, async (req: Request, res: Response) => {
    try {
        const slip = (await req.db!.query(
            `SELECT ss.*, ebd.razorpay_fund_account_id
             FROM salary_slips ss
...
router.get("/disbursements", requireRole("hr_admin"), requireSameOrg, async (req: Request, res: Response) => {
    try {
        const { pay_period_id, status } = req.query as { pay_period_id?: string; status?: string };
...
router.post("/disburse/retry/:id", requireRole("hr_admin"), requireSameOrg, async (req: Request, res: Response) => {
    try {
...
router.get("/payment-config", requireRole("hr_admin"), requireSameOrg, async (req: Request, res: Response) => {
    try {
...
router.put("/payment-config", requireRole("hr_admin"), requireSameOrg, async (req: Request, res: Response) => {
    try {
...
router.post("/payment-config/test", requireRole("hr_admin"), requireSameOrg, async (req: Request, res: Response) => {
    try {
        const payoutService = await getPayoutService(req.db, req.userOrgId);
...
router.get("/ctc-config", requireRole("hr_admin"), requireSameOrg, async (req: Request, res: Response) => {
    try {
...
router.put("/ctc-config", requireRole("hr_admin"), requireSameOrg, async (req: Request, res: Response) => {
    try {
...
router.get("/bank-details", requireRole("hr_admin"), requireSameOrg, async (req: Request, res: Response) => {
    try {
...
router.get("/bank-details/:userId", requireSameOrg, async (req: Request, res: Response) => {
    try {
        const isAdmin = ["hr_admin", "super_admin", "platform_admin"].includes(req.userRole!);
...
router.post("/bank-details/:userId", requireRole("hr_admin"), requireSameOrg, async (req: Request, res: Response) => {
    try {
...
router.post("/bank-details/:userId/verify", requireRole("hr_admin"), requireSameOrg, async (req: Request, res: Response) => {
    try {
...
router.get("/bank-verifications", requireRole("hr_admin"), requireSameOrg, async (req: Request, res: Response) => {
    try {
        const rows = (await req.db!.query(
...
router.post("/bank-details/:userId/approve", requireRole("hr_admin"), requireSameOrg, async (req: Request, res: Response) => {
    try {
        const result = await req.db!.query(
...
router.post("/bank-details/:userId/reject", requireRole("hr_admin"), requireSameOrg, async (req: Request, res: Response) => {
    try {
        const result = await req.db!.query(
...
router.get("/my-bank-details", async (req: Request, res: Response) => {
    try {
        const row = (await req.db!.query(
            `SELECT * FROM employee_bank_details WHERE user_id = $1 AND org_id = $2`,
            [req.userId, req.userOrgId]
...
router.post("/my-bank-details", async (req: Request, res: Response) => {
    try {
        const { account_holder_name, account_number, ifsc_code, bank_name, account_type } = req.body;
        if (!account_holder_name || !account_number || !ifsc_code) {
            return res.status(400).json({ error: "account_holder_name, account_number, and ifsc_code are required" });
