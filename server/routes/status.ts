...
function asCtx(req: Request): StatusCtx {
    return {
        db: req.db,
        tenantId: (req.tenantId as number) || null,
...
router.put("/presence-preference", auth, async (req: Request, res: Response) => {
    try {
...
router.post("/activity-ping", auth, async (req: Request, res: Response) => {
    try {
