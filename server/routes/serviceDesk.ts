 ...
 */
async function resolveDefaultTenant(req: any): Promise<any> {
 ...
 */
async function isDefaultTenant(req: Request): Promise<boolean> {
 ...
 */
async function getDefaultTenantDb(req: Request): Promise<{ db: any; tenantId: number } | null> {
...
router.get("/tickets", async (req: Request, res: Response) => {
    try {
        ...
        const { status, ticket_type, priority, page = 1, per_page = 50 } = req.query as Record<string, any>;
