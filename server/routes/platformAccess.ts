...
router.use(auth, loadUserContext, (req: Request, res: Response, next: NextFunction) => {
    if (!APPROVER_ROLES.has(req.userRole as string)) {
...
router.get("/", async (req: Request, res: Response) => {
    try {
        ...
        const { status, limit: rawLimit, offset } = req.query as { status?: string; limit?: string; offset?: string };
