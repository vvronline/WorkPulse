 ...
 */
router.get("/plan-catalog", (req: Request, res: Response) => {
...
router.get("/branding", async (req: Request, res: Response) => {
    ...
    if (req.tenant && req.db && !req.isMasterRoute) {
        try {
            ...
            req.log?.warn?.({ err: (err as Error).message }, "Public branding lookup failed");
    ...
    try {
        ...
        const tenantDb: PoolLike = await getTenantPool(tenantRow.db_name, tenantRow.db_host);
        ...
        req.log?.warn?.({ err: (err as Error).message, slug }, "Public branding slug lookup failed");
        res.json(EMPTY);
    }
});
...
router.get("/notes/:token", async (req: Request, res: Response) => {
    try {
        ...
        const tenantDb: PoolLike = await getTenantPool(tenant.db_name, tenant.db_host);
        ...
        let notebook: { pages?: NotePage[] } | null;
        ...
        let branding: { logo_url: string | null; accent_color: string } = { logo_url: null, accent_color: "#6366f1" };
        ...
        logger.error({ err: (err as Error).message, token: req.params.token }, "Public note share lookup failed");
