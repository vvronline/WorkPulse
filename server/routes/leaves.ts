...
router.get("/", async (req: Request, res: Response) => {
    try {
        ...
        const { user_id, start_date, end_date, status, type } = req.query as Record<string, string>;

        const conditions: string[] = [];
        const params: unknown[] = [];
        let pi = 1;

        if (user_id && ["manager", "hr_admin", "super_admin"].includes(req.userRole as string)) {
...
router.get("/summary", async (req: Request, res: Response) => {
    try {
        let { start_date, end_date, month, year } = req.query as Record<string, string>;
...
router.get("/monthly-summary", async (req: Request, res: Response) => {
    try {
        ...
        const params: unknown[] = [`${targetYear}-01-01`, `${targetYear}-12-31`];
        let pi = 3;
...
router.get("/balance", async (req: Request, res: Response) => {
    try {
        const targetUserId = req.query.user_id && ["manager", "hr_admin", "super_admin"].includes(req.userRole as string)
        ...
        const coerced = balances.map((b: any) => ({
            ...b,
...
router.get("/pending", requireRole("manager"), async (req: Request, res: Response) => {
    try {
...
router.post("/", async (req: Request, res: Response) => {
    try {
        ...
        const validDates = rawDates.filter((d: string) => /^\d{4}-\d{2}-\d{2}$/.test(d));
        ...
        )).rows.map((r: any) => String(r.date).slice(0, 10));
        const existingSet = new Set(existingDates);
        const newDates = validDates.filter((d: string) => !existingSet.has(d));
            ...
            if (req.userOrgId) {
                ...
                if (policy) {
                    ...
                    for (const [yr, yearDates] of Object.entries(datesByYear)) {
                        ...
                        const requested = (yearDates as string[]).length * durationValue;
...
router.patch("/:id/approve", requireRole("manager"), async (req: Request, res: Response) => {
    try {
        ...
        const isHrOrAbove = (req.roleLevel as number) >= 4; // hr_admin or super_admin
...
router.patch("/:id/reject", requireRole("manager"), async (req: Request, res: Response) => {
    try {
        const leaveId = parseInt(String(req.params.id), 10);
        if (isNaN(leaveId)) return res.status(400).json({ error: "Invalid leave ID" });
...
router.patch("/:id/revoke", requireRole("manager"), async (req: Request, res: Response) => {
    try {
        const leaveId = parseInt(String(req.params.id), 10);
        if (isNaN(leaveId)) return res.status(400).json({ error: "Invalid leave ID" });
...
(router as any).updateLeaveBalance = updateLeaveBalance;
