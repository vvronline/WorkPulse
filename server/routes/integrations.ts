...
router.get("/github/oauth/callback", async (req: Request, res: Response) => {
    try {
        ...
        if (error) return res.status(400).send(`GitHub returned an error: ${error}`);
        ...
        let db: DbLike;
        if (ctx.tenantId) {
            const tenantRow = (await masterDb.masterQuery(
                "SELECT db_name, db_host FROM tenants WHERE id = $1",
                [ctx.tenantId]
            )).rows[0];
        ...
        await db.transaction(async (client) => {
            const existing = (await client.query(
                "SELECT id FROM org_integrations WHERE org_id = $1 AND provider = $2",
                [ctx.orgId, "github"]
            )).rows[0];
            let integrationId;
            const config = {
            ...
            if (existing) {
                ...
                integrationId = (await client.query(
                    `INSERT INTO org_integrations (org_id, provider, config, created_by)
                     VALUES ($1, 'github', $2, $3)
                     RETURNING id`,
                    [ctx.orgId, JSON.stringify(config), ctx.userId]
                )).rows[0].id;
            }
            // We don't issue the webhook secret yet — that happens at
            // /repos/connect time so the secret rotates per-repo install.
            await client.query(
                `INSERT INTO org_integration_secrets
                 ...
                 ON CONFLICT (integration_id) DO UPDATE SET
                    ...
                    updated_at    = NOW()`,
                [integrationId, tokenData.access_token, viewer.login, viewer.avatar_url || null, tokenData.scope || null]
            );
        });
...
router.get("/", auth, loadUserContext, requireRole("manager"), async (req: Request, res: Response) => {
    try {
        const rows = (await req.db!.query(
            `SELECT i.id, i.provider, i.config, i.is_active, i.created_at, i.updated_at,
...
router.post("/github/oauth/start", auth, loadUserContext, requireRole("manager"), async (req: Request, res: Response) => {
    try {
        if (!github.isConfigured()) {
...
router.get("/github/status", auth, loadUserContext, requireRole("manager"), async (req: Request, res: Response) => {
    try {
        const row = (await req.db!.query(
            `SELECT i.id, i.config, s.github_login, s.github_avatar,
                    (s.access_token IS NOT NULL) AS connected
               FROM org_integrations i
          LEFT JOIN org_integration_secrets s ON s.integration_id = i.id
              WHERE i.org_id = $1 AND i.provider = 'github'`,
            [req.userOrgId]
        )).rows[0];
        if (!row) return res.json({ connected: false });
        const repos = (await req.db!.query(
            `SELECT full_name, html_url, default_branch, hook_id, is_active
...
router.get("/github/repos", auth, loadUserContext, requireRole("manager"), async (req: Request, res: Response) => {
    try {
        const secret = (await req.db!.query(
...
router.post("/github/repos/connect", auth, loadUserContext, requireRole("manager"), async (req: Request, res: Response) => {
    try {
        const { repos } = req.body || {};
        if (!Array.isArray(repos) || repos.length === 0) {
            return res.status(400).json({ error: 'repos must be a non-empty array of "owner/repo" strings.' });
        }
        const integration = (await req.db!.query(
            `SELECT i.id, s.access_token, s.webhook_secret
               FROM org_integrations i
          LEFT JOIN org_integration_secrets s ON s.integration_id = i.id
              WHERE i.org_id = $1 AND i.provider = 'github'`,
            [req.userOrgId]
        ...
        if (!webhookSecret) {
            ...
            await req.db!.query(
                `UPDATE org_integration_secrets SET webhook_secret = $1, updated_at = NOW() WHERE integration_id = $2`,
                [webhookSecret, integration.id]
            );
        }
        ...
        for (const fullName of repos) {
            try {
                ...
                const hookId = await github.ensureRepoWebhook(
                    integration.access_token, fullName, hookUrl, webhookSecret
                );
                ...
                await req.db!.query(
                    `INSERT INTO github_repo_connections
                     ...
                     DO UPDATE SET hook_id = EXCLUDED.hook_id, is_active = TRUE, updated_at = NOW()`,
                    [integration.id, fullName, `https://github.com/${fullName}`, hookId]
                );
...
// here before lookup.
router.delete("/github/repos/:fullName", auth, loadUserContext, requireRole("manager"), async (req: Request, res: Response) => {
    try {
        const fullName = decodeURIComponent(String(req.params.fullName));
        const row = (await req.db!.query(
            `SELECT rc.id, rc.hook_id, s.access_token, i.id AS integration_id
               FROM github_repo_connections rc
               JOIN org_integrations i ON i.id = rc.integration_id
          LEFT JOIN org_integration_secrets s ON s.integration_id = i.id
              WHERE i.org_id = $1 AND rc.full_name = $2`,
            [req.userOrgId, fullName]
        )).rows[0];
        if (!row) return res.status(404).json({ error: "Repo connection not found" });
...
router.post("/github/disconnect", auth, loadUserContext, requireRole("manager"), async (req: Request, res: Response) => {
    try {
        const integration = (await req.db!.query(
            `SELECT i.id, s.access_token
               FROM org_integrations i
          LEFT JOIN org_integration_secrets s ON s.integration_id = i.id
              WHERE i.org_id = $1 AND i.provider = 'github'`,
            [req.userOrgId]
        ...
        if (integration.access_token) {
            const repos = (await req.db!.query(
                "SELECT full_name, hook_id FROM github_repo_connections WHERE integration_id = $1",
                [integration.id]
            )).rows;
            for (const r of repos) {
                if (r.hook_id) await github.deleteRepoWebhook(integration.access_token, r.full_name, r.hook_id);
            }
        }
...
function buildWebhookUrl(req: Request, integrationId: number): string {
    const proto = req.headers["x-forwarded-proto"] || (req.secure ? "https" : "http");
    const host = req.headers["x-forwarded-host"] || req.headers.host;
    return `${proto}://${host}/api/webhooks/github/${integrationId}`;
}

router.post("/github", auth, loadUserContext, requireRole("manager"), async (req: Request, res: Response) => {
    try {
        ...
        const config = {
            ...
            repos: Array.isArray(repos) ? repos.map((r: any) => String(r).trim()).filter(Boolean).slice(0, 100) : [],
            default_branch: default_branch ? String(default_branch).trim() : "main",
        };
        // Upsert the integration row.
        const integration = await (req.db as unknown as DbLike).transaction(async (client) => {
            const existing = (await client.query(
                "SELECT id FROM org_integrations WHERE org_id = $1 AND provider = $2",
                [req.userOrgId, "github"]
            )).rows[0];
            let row;
            if (existing) {
                ...
                row = (await client.query(
                    `INSERT INTO org_integrations (org_id, provider, config, created_by)
                     VALUES ($1, 'github', $2, $3)
                     RETURNING *`,
                    [req.userOrgId, JSON.stringify(config), req.userId]
                )).rows[0];
            }
            ...
            await client.query(
                `INSERT INTO org_integration_secrets (integration_id, webhook_secret, updated_at)
                 VALUES ($1, $2, NOW())
...
router.get("/:id/webhook", auth, loadUserContext, requireRole("manager"), async (req: Request, res: Response) => {
    try {
        const id = parseInt(String(req.params.id), 10);
        if (isNaN(id)) return res.status(400).json({ error: "Invalid id" });
        const exists = (await req.db!.query(
            "SELECT id FROM org_integrations WHERE id = $1 AND org_id = $2",
            [id, req.userOrgId]
        )).rowCount;
        if (!exists) return res.status(404).json({ error: "Integration not found" });
...
router.delete("/:id", auth, loadUserContext, requireRole("manager"), async (req: Request, res: Response) => {
    try {
        const id = parseInt(String(req.params.id), 10);
        if (isNaN(id)) return res.status(400).json({ error: "Invalid id" });
        const r = await req.db!.query(
            "DELETE FROM org_integrations WHERE id = $1 AND org_id = $2 RETURNING provider",
            [id, req.userOrgId]
        );
        if (r.rowCount === 0) return res.status(404).json({ error: "Integration not found" });
