...
router.use(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if ((req.roleLevel || 0) >= 2) return next();
    return res.status(403).json({ error: "Insufficient permissions" });
});

// ==================== TEAM ATTENDANCE ====================

router.get("/team-attendance", async (req: Request, res: Response) => {
    try {
        ...
        leaves.forEach((l: any) => { leaveMap[l.user_id] = l; });
        ...
        tasks.forEach((t: any) => { if (!taskMap[t.user_id]) taskMap[t.user_id] = t; });

        const userEntries: Record<number, any[]> = {};
        ...
        const result = users.map((u: any) => {
            ...
            const workMode = ue.find((e: any) => e.entry_type === "clock_in")?.work_mode || null;
            ...
            return {
                ...
                clockInTime: ue.find((e: any) => e.entry_type === "clock_in")?.timestamp || null,
...
router.get("/team-analytics", async (req: Request, res: Response) => {
    try {
        ...
        const byUser: Record<number, Record<string, any[]>> = {};
        entries.forEach((e: any) => {
        ...
        const taskMap: Record<number, { done: number; total: number }> = {};
        taskCounts.forEach((t: any) => { taskMap[t.user_id] = { done: parseInt(t.done || 0, 10), total: parseInt(t.total, 10) }; });
        ...
        const leaveMap: Record<number, { total: number; byType: Record<string, number> }> = {};
        leaveCounts.forEach((l: any) => {
        ...
        const todayByUser: Record<number, any[]> = {};
        ...
        const todayLeaveSet = new Set(todayLeaves.map((l: any) => l.user_id));
        ...
        const members = users.map((u: any) => {
            ...
            for (let i = 0; i <= numDays; i++) {
                ...
                if (userDays[dateStr] && userDays[dateStr].some((e: any) => e.entry_type === "clock_in")) streak++;
                else break;
            }
        ...
        members.sort((a: any, b: any) => b.hours - a.hours);
        const totalLeaves = members.reduce((s: number, m: any) => s + m.leaves, 0);
        const avgPunctuality = members.length > 0 ? Math.round(members.reduce((s: number, m: any) => s + m.punctualityPercent, 0) / members.length) : 0;
        const avgTargetMet = members.length > 0 ? Math.round(members.reduce((s: number, m: any) => s + m.targetMetPercent, 0) / members.length) : 0;
...
router.post("/approvals/bulk", async (req: Request, res: Response) => {
    try {
...
router.get("/member/:userId/hours", async (req: Request, res: Response) => {
    try {
...
router.get("/member/:userId/tasks", async (req: Request, res: Response) => {
    try {
        const targetUserId = Number(req.params.userId);
        const visibleIds = await getVisibleUserIds(req.userId, req.userRole, req.userOrgId, req.userTeamId, req.db);
        if (!visibleIds.includes(targetUserId)) return res.status(403).json({ error: "Not authorized to view this user's tasks" });

        const targetDate = (req.query as { date?: string }).date || getLocalToday(req);
        const tasks = (await req.db!.query(
...
router.get("/member/:userId/leaves", async (req: Request, res: Response) => {
    try {
        const targetUserId = Number(req.params.userId);
        const visibleIds = await getVisibleUserIds(req.userId, req.userRole, req.userOrgId, req.userTeamId, req.db);
        if (!visibleIds.includes(targetUserId)) return res.status(403).json({ error: "Not authorized to view this user's data" });
...
router.get("/member/:userId/requests", async (req: Request, res: Response) => {
    try {
        const targetUserId = Number(req.params.userId);
        const visibleIds = await getVisibleUserIds(req.userId, req.userRole, req.userOrgId, req.userTeamId, req.db);
        if (!visibleIds.includes(targetUserId)) return res.status(403).json({ error: "Not authorized to view this user's data" });
...
router.get("/member/:userId/overview", async (req: Request, res: Response) => {
    try {
        const targetUserId = Number(req.params.userId);
        const visibleIds = await getVisibleUserIds(req.userId, req.userRole, req.userOrgId, req.userTeamId, req.db);
        if (!visibleIds.includes(targetUserId)) return res.status(403).json({ error: "Not authorized to view this user's data" });
        ...
        const trendGrouped: Record<string, any[]> = {};
        trendEntries.forEach((e: any) => {
            const dateStr = getLocalDateFromTs(e.timestamp, req);
        ...
        res.json({
            ...
            recentLeaves,
            recentRequests: recentRequestsRows.map((r: any) => {
                let metadata = null;
                if (r.metadata) { try { metadata = JSON.parse(r.metadata); } catch { } }
                return { ...r, metadata };
            }),
