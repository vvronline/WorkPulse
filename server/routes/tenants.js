...
router.put('/:id', async (req, res) => {
    try {
        ...
        const result = await masterQuery(
            ...
            [
                org_name || null,
                Number.isFinite(max_users) && max_users > 0 ? max_users : (max_users === null ? null : undefined) ?? null,
                Number.isFinite(max_storage_mb) && max_storage_mb > 0 ? max_storage_mb : (max_storage_mb === null ? null : undefined) ?? null,
                tid,
            ]
        );
        ...
        for (const p of defaultPolicies) {
            ...
            if (!exists) {
                await db.query(
                    ...
                    [p.leave_type, p.annual_quota, p.carry_forward_limit]
                );
