...
function normaliseOptions(raw: unknown): Array<{ value: string; label: string }> {
...
function normaliseAppliesToTypes(raw: unknown): number[] {
    if (!Array.isArray(raw)) return [];
    return raw
...
function coerceValue(field: any, raw: unknown): unknown {
    ...
    switch (field.field_type) {
        ...
        case "select": {
            ...
            return opts.some((o: any) => String(o.value) === s) ? s : null;
        }
        case "multiselect": {
            if (!Array.isArray(raw)) return null;
            const opts = Array.isArray(field.options) ? field.options : [];
            const allowed = new Set(opts.map((o: any) => String(o.value)));
            const cleaned = raw
                .map((v: any) => String(v))
...
async function loadDefinitions(req: Request, { includeInactive = false }: { includeInactive?: boolean } = {}) {
    const sql = `
...
router.post("/", requireAdmin, async (req: Request, res: Response) => {
    try {
        ...
        if (label.length > 80) return res.status(400).json({ error: "Label must be 80 characters or less" });
        ...
        key = slugify(key);
        if (!key) return res.status(400).json({ error: "Could not derive a key from the label" });

        const exists = await req.db!.query(
            "SELECT 1 FROM custom_field_definitions WHERE org_id = $1 AND key = $2",
            [req.userOrgId, key]
        );
        ...
        const maxRow = await req.db!.query(
            "SELECT COALESCE(MAX(sort_order),0) AS m FROM custom_field_definitions WHERE org_id = $1",
            [req.userOrgId]
        );
        ...
        const ins = await req.db!.query(
            `INSERT INTO custom_field_definitions
             ...
             RETURNING *`,
            [req.userOrgId, key, label, fieldType, description, JSON.stringify(options),
...
router.put("/reorder", requireAdmin, async (req: Request, res: Response) => {
    try {
        ...
        const ids = order.map((v: any) => parseInt(v, 10)).filter(Number.isFinite);
...
router.put("/:id", requireAdmin, async (req: Request, res: Response) => {
    try {
        ...
        if (!Number.isFinite(id)) return res.status(400).json({ error: "Invalid field id" });
        const cur = (await req.db!.query(
            "SELECT * FROM custom_field_definitions WHERE id = $1 AND org_id = $2",
            [id, req.userOrgId]
        )).rows[0];
        if (!cur) return res.status(404).json({ error: "Custom field not found" });
        ...
        if (b.show_on_card !== undefined) set("show_on_card", !!b.show_on_card);
        ...
        if (updates.length === 0) return res.json(cur);
...
router.delete("/:id", requireAdmin, async (req: Request, res: Response) => {
    try {
        ...
        if (!Number.isFinite(id)) return res.status(400).json({ error: "Invalid field id" });
        const cur = (await req.db!.query(
            "SELECT id FROM custom_field_definitions WHERE id = $1 AND org_id = $2",
            [id, req.userOrgId]
        )).rows[0];
        if (!cur) return res.status(404).json({ error: "Custom field not found" });
...
async function loadTaskAccessible(req: Request, taskId: number) {
    const task = (await req.db!.query("SELECT * FROM tasks WHERE id = $1", [taskId])).rows[0];
    if (!task) return null;
    if (req.userOrgId && task.org_id && task.org_id !== req.userOrgId) return null;
    if (!req.userOrgId && task.org_id) return null;
    ...
    if (task.user_id === req.userId || task.assigned_to === req.userId) return task;
    if (req.userTeamId) {
        const r = await req.db!.query(
            "SELECT 1 FROM users WHERE id = $1 AND team_id = $2",
            [task.user_id, req.userTeamId]
        );
        if (r.rowCount > 0) return task;
    }
    return null;
}
...
router.get("/task/:taskId", async (req: Request, res: Response) => {
    try {
        ...
        for (const r of rows) values[r.field_id] = r.value;
        res.json({ values });
    } catch (err) {
...
router.put("/task/:taskId", async (req: Request, res: Response) => {
    try {
        const taskId = parseInt(String(req.params.taskId), 10);
        if (!Number.isFinite(taskId)) return res.status(400).json({ error: "Invalid task id" });
        const task = await loadTaskAccessible(req, taskId);
        ...
        await (req.db as unknown as DbLike).transaction(async (client) => {
            for (const [fidStr, raw] of Object.entries(incoming)) {
                ...
                if (coerced === null
                    ...
                    || coerced === "") {
                    await client.query(
                        "DELETE FROM task_custom_field_values WHERE task_id = $1 AND field_id = $2",
                        [taskId, fid]
                    );
                } else {
                    await client.query(
                        `INSERT INTO task_custom_field_values (task_id, field_id, value, updated_by, updated_at)
                         VALUES ($1, $2, $3::jsonb, $4, NOW())
                         ON CONFLICT (task_id, field_id)
                         DO UPDATE SET value = EXCLUDED.value, updated_by = EXCLUDED.updated_by, updated_at = NOW()`,
                        [taskId, fid, JSON.stringify(coerced), req.userId]
                    );
                }
            }
        });
        ...
        try {
            await req.db!.query(
                `INSERT INTO task_history (task_id, user_id, action, field, new_value)
                 VALUES ($1, $2, 'updated', 'custom_fields', $3)`,
                [taskId, req.userId, `${Object.keys(incoming).length} field(s)`]
            );
        } catch { /* ignore */ }
