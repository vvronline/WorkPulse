...
async function loadOrgSettings(db: any, orgId: number) {
    const r = await db.query("SELECT * FROM org_agile_settings WHERE org_id = $1", [orgId]);
    return r.rows[0] || null;
}

async function ensureSettingsRow(db: any, orgId: number) {
...
function validateColor(color: unknown, fallback = "#6366f1"): string {
    return /^#[0-9a-fA-F]{6}$/.test((color as string) || "") ? (color as string) : fallback;
}

function slugifyKey(name: unknown): string {
    return String(name || "")
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, "_")
        .replace(/^_+|_+$/g, "")
...
router.put("/settings", requireAgileEditor, async (req: Request, res: Response) => {
    try {
        if (!req.userOrgId) return res.status(400).json({ error: "Organization context required" });
        ...
        if (b.estimation_values !== undefined) {
            ...
            const cleaned = b.estimation_values.map((v: any) => (typeof v === "string" ? v : Number(v)))
                .filter((v: any) => v !== "" && (typeof v === "string" || Number.isFinite(v)))
                .slice(0, 30);
        ...
        if (b.priority_scheme !== undefined) {
            ...
            const cleaned = b.priority_scheme.slice(0, 10).map((pr: any) => ({
...
router.get("/work-item-types", async (req: Request, res: Response) => {
    try {
...
router.post("/work-item-types", requireAgileEditor, async (req: Request, res: Response) => {
    try {
        if (!req.userOrgId) return res.status(400).json({ error: "Organization context required" });
...
router.put("/work-item-types/:id", requireAgileEditor, async (req: Request, res: Response) => {
    try {
        ...
        const params: unknown[] = [];
        let p = 1;
        const set = (col: string, val: unknown) => { updates.push(`${col} = $${p++}`); params.push(val); };
...
router.delete("/work-item-types/:id", requireAgileEditor, async (req: Request, res: Response) => {
    try {
        const id = parseInt(String(req.params.id), 10);
        if (isNaN(id)) return res.status(400).json({ error: "Invalid id" });
        const existing = (await req.db!.query("SELECT * FROM work_item_types WHERE id = $1", [id])).rows[0];
...
router.put("/work-item-types/reorder", requireAgileEditor, async (req: Request, res: Response) => {
    try {
...
router.get("/workflow-states", async (req: Request, res: Response) => {
    try {
        if (!req.userOrgId) return res.json([]);
        const r = await req.db!.query(
 ...
 */
async function assertCategoriesCovered(db: any, orgId: number | null | undefined, opts: { excludeId?: number | null; makingInactiveId?: number | null } = {}) {
    ...
    const have = new Set(rows.map((r: any) => r.category));
    const missing = REQUIRED_CATEGORIES.filter(c => !have.has(c));
    if (missing.length > 0) {
        const err: any = new Error(`Workflow must keep at least one active state in each category. Missing: ${missing.join(", ")}`);
...
router.post("/workflow-states", requireAgileEditor, async (req: Request, res: Response) => {
    try {
        if (!req.userOrgId) return res.status(400).json({ error: "Organization context required" });
...
router.put("/workflow-states/:id", requireAgileEditor, async (req: Request, res: Response) => {
    try {
        const id = parseInt(String(req.params.id), 10);
        if (isNaN(id)) return res.status(400).json({ error: "Invalid id" });
...
router.delete("/workflow-states/:id", requireAgileEditor, async (req: Request, res: Response) => {
    try {
        const id = parseInt(String(req.params.id), 10);
        if (isNaN(id)) return res.status(400).json({ error: "Invalid id" });
        const existing = (await req.db!.query("SELECT * FROM workflow_states WHERE id = $1", [id])).rows[0];
...
router.put("/workflow-states/reorder", requireAgileEditor, async (req: Request, res: Response) => {
    try {
        const { order } = req.body || {};
        if (!Array.isArray(order)) return res.status(400).json({ error: "order must be an array of ids" });
        await (req.db as unknown as DbLike).transaction(async (client) => {
...
router.post("/permissions/cancel-request", async (req: Request, res: Response) => {
    try {
        if (!req.userOrgId) return res.status(400).json({ error: "Organization context required" });
        const r = await req.db!.query(
...
router.get("/permissions/requests", requireAgileReviewer, async (req: Request, res: Response) => {
    try {
        if (!req.userOrgId) return res.json([]);
        const r = await req.db!.query(
...
router.put("/permissions/requests/:id", requireAgileReviewer, async (req: Request, res: Response) => {
    try {
        const id = parseInt(String(req.params.id), 10);
        if (isNaN(id)) return res.status(400).json({ error: "Invalid id" });
...
router.get("/permissions/grants", requireAgileReviewer, async (req: Request, res: Response) => {
    try {
        if (!req.userOrgId) return res.json([]);
        const r = await req.db!.query(
...
router.delete("/permissions/grants/:id", requireAgileReviewer, async (req: Request, res: Response) => {
    try {
        const id = parseInt(String(req.params.id), 10);
        if (isNaN(id)) return res.status(400).json({ error: "Invalid id" });
