...
async function getNotebook(userId: number | undefined, db: DbLike): Promise<Notebook | null> {
 ...
 */
async function writeHistory(userId: number | undefined, page: NotePage, client: any): Promise<void> {
...
router.put("/", async (req: Request, res: Response) => {
    try {
        ...
        const old = await getNotebook(req.userId, req.db as unknown as DbLike);
        const oldMap: Record<string, NotePage> = {};
        if (old?.pages) old.pages.forEach((p) => { oldMap[p.id] = p; });

        const newPages: NotePage[] = data.pages || [];
...
router.get("/history/:pageId", async (req: Request, res: Response) => {
    try {
...
router.get("/history/snapshot/:id", async (req: Request, res: Response) => {
    try {
...
router.get("/mentionable-users", async (req: Request, res: Response) => {
    try {
...
router.get("/links/:pageId", async (req: Request, res: Response) => {
    try {
...
router.get("/daily-prefill", async (req: Request, res: Response) => {
    try {
        ...
        // Time tracked today
        const timeEntries: TimeEntryRow[] = (await req.db!.query(
        ...
        for (const e of timeEntries) {
            if (e.entry_type === "clock_in") clockIn = new Date(e.timestamp);
                ...
                if (clockIn) totalMinutes += (new Date(e.timestamp).getTime() - clockIn.getTime()) / 60000;
        ...
        if (clockIn) totalMinutes += (Date.now() - clockIn.getTime()) / 60000;
        void breakStart;
...
router.get("/oneonone-prefill/:userId", async (req: Request, res: Response) => {
    try {
        ...
        const weekEntries: TimeEntryRow[] = (await req.db!.query(
        ...
        for (const e of weekEntries) {
            ...
            else if (e.entry_type === "break_start") { if (ci) weekMinutes += (new Date(e.timestamp).getTime() - ci.getTime()) / 60000; ci = null; }
            else if (e.entry_type === "break_end") ci = new Date(e.timestamp);
            else if (e.entry_type === "clock_out") { if (ci) weekMinutes += (new Date(e.timestamp).getTime() - ci.getTime()) / 60000; ci = null; }
        }
...
router.get("/time-summary", async (req: Request, res: Response) => {
    try {
        ...
        const entries: TimeEntryRow[] = (await req.db!.query(
            `SELECT entry_type, timestamp, work_mode FROM time_entries
             WHERE user_id = $1 AND timestamp >= $2 AND timestamp <= $3
               AND (is_manual = FALSE OR approval_status = 'approved')
             ORDER BY timestamp ASC`,
        ...
        for (const e of entries) {
            if (e.entry_type === "clock_in") {
                ...
                if (breakStart) breakMinutes += (new Date(e.timestamp).getTime() - breakStart.getTime()) / 60000;
                clockIn = new Date(e.timestamp);
                breakStart = null;
            } else if (e.entry_type === "clock_out") {
                if (clockIn) totalMinutes += (new Date(e.timestamp).getTime() - clockIn.getTime()) / 60000;
...
router.post("/convert-to-task", async (req: Request, res: Response) => {
    try {
...
router.get("/sprint-embed", async (req: Request, res: Response) => {
    try {
...
router.get("/search-meetings", async (req: Request, res: Response) => {
    try {
        const { q } = req.query as { q?: string };
...
router.get("/direct-reports", async (req: Request, res: Response) => {
    try {
...
function publicShareUrlFor(req: Request, token: string): string {
...
router.get("/share/:pageId", async (req: Request, res: Response) => {
    try {
...
router.post("/share/:pageId", async (req: Request, res: Response) => {
    try {
        const pageId = String(req.params.pageId || "");
        if (!pageId) return res.status(400).json({ error: "pageId required" });
        ...
        const notebook = await getNotebook(req.userId, req.db as unknown as DbLike);
...
router.delete("/share/:pageId", async (req: Request, res: Response) => {
    try {
        const pageId = String(req.params.pageId || "");
        if (!pageId) return res.status(400).json({ error: "pageId required" });
        await masterQuery(
