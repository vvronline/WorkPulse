...
const activeBroadcasts = new Map<number, BroadcastRecord>();

/** Generate a unique meeting code: XXX-XXXX-XXX */
async function generateMeetingCode(db: any): Promise<string> {
    const chars = "ABCDEFGHJKLMNPQRSTUVWXYZ23456789";
    for (let attempt = 0; attempt < 20; attempt++) {
        const part = (n: number) => Array.from({ length: n }, () => chars[crypto.randomInt(chars.length)]).join("");
...
async function insertSystemMessage(conversationId: number, senderId: number | undefined, metadata: any, db: any, tenantId: number | string | undefined) {
...
async function getConflictsForUsers(userIds: number[], startTime: string, endTime: string, db: any): Promise<Record<number, any[]>> {
...
router.post("/check-conflicts", async (req: Request, res: Response) => {
    try {
        ...
        const ids = user_ids.map(Number).filter((n: number) => n > 0);
        ...
        const orgFilteredRes = req.userOrgId
            ...
            : { rows: [] };
        const orgIds = orgFilteredRes.rows.map((r: any) => r.id);
...
router.get("/", async (req: Request, res: Response) => {
    try {
        const { status, limit = 20, offset = 0 } = req.query as { status?: string; limit?: string | number; offset?: string | number };
        const statusFilter = status ? `AND m.status = $3` : "";
        const params: unknown[] = [req.userId, req.userOrgId || null];
...
router.get("/:code/messages", async (req: Request, res: Response) => {
    try {
        const limit = Math.min(parseInt(String((req.query as { limit?: string }).limit), 10) || 200, 500);
        ...
        const sinceRaw = parseInt(String((req.query as { since?: string }).since), 10);
...
router.post("/", async (req: Request, res: Response) => {
    try {
        ...
        const requiredIds = Array.isArray(required_participant_ids)
            ? required_participant_ids.map(Number).filter((n: number) => n > 0 && n !== req.userId)
            : [];
        const optionalIds = Array.isArray(optional_participant_ids)
            ? optional_participant_ids.map(Number).filter((n: number) => n > 0 && n !== req.userId && !requiredIds.includes(n))
            : [];
        // Legacy fallback
        const legacyIds = (!required_participant_ids && !optional_participant_ids && Array.isArray(participant_ids))
            ? participant_ids.map(Number).filter((n: number) => n > 0 && n !== req.userId)
            : [];
...
router.post("/:code/hls/start", async (req: Request, res: Response) => {
    try {
...
router.post("/:code/hls/stop", async (req: Request, res: Response) => {
    try {
        const code = req.params.code;
        const meeting = (await req.db!.query(
...
router.get("/:code/hls/status", async (req: Request, res: Response) => {
    try {
        const code = req.params.code;
        const meeting = (await req.db!.query(
