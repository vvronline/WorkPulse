...
router.get('/labels/manage', auth, loadUserContext, requireRole('manager'), async (req: Request, res: Response) => {
    try {
...
router.post('/labels', auth, loadUserContext, requireRole('manager'), async (req: Request, res: Response) => {
    try {
...
router.put('/labels/:id', auth, loadUserContext, requireRole('manager'), async (req: Request, res: Response) => {
    try {
...
router.delete('/labels/:id', auth, loadUserContext, requireRole('manager'), async (req: Request, res: Response) => {
    try {
        const labelRes = await req.db!.query('SELECT * FROM task_labels WHERE id = $1', [Number(req.params.id)]);
        const label = labelRes.rows[0];
        if (!label) return res.status(404).json({ error: 'Label not found' });
