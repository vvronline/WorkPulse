...
function validateStoryPoints(v: unknown): number | null {
 ...
 */
async function resolveWorkItemType(value: unknown, orgId: number | null | undefined, db: DbLike): Promise<any | null> {
 ...
 */
async function resolveWorkflowState(value: unknown, orgId: number | null | undefined, db: DbLike): Promise<any | null> {
    if (value === null || value === undefined || value === '') return null;
    if (!orgId) return null;
    const num = Number(value);
    let row;
 ...
 */
async function getInitialWorkflowState(orgId: number | null | undefined, db: DbLike): Promise<any | null> {
    if (!orgId) return null;
 ...
 */
async function getDefaultWorkItemType(orgId: number | null | undefined, db: DbLike): Promise<any | null> {
    if (!orgId) return null;
    let row = (await db.query(
 ...
 */
function normalizeAcceptanceCriteria(value: unknown): any[] | null {
    if (value === null || value === undefined) return null;
    if (!Array.isArray(value)) return null;
    let nextId = Math.max(0, ...value.map((c: any) => Number(c?.id) || 0)) + 1;
    return value.slice(0, 100).map((it: any) => {
