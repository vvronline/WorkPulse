...
router.get('/organizations/:id', requireRole('platform_admin'), async (req: Request, res: Response) => {
    try {
        const { id } = req.params;
...
router.post('/organizations', requireRole('platform_admin'), async (req: Request, res: Response) => {
    try {
...
router.put('/organizations/:id', requireRole('platform_admin'), async (req: Request, res: Response) => {
    try {
        const { id } = req.params;
        ...
        if (fiscal_year_start !== undefined) {
            ...
            updates.push(`fiscal_year_start = $${pi++}`); params.push(fys);
        }
...
router.delete('/organizations/:id', requireRole('platform_admin'), async (req: Request, res: Response) => {
    try {
        const { id } = req.params;
        ...
        await (req.db as DbLike).transaction(async (client: any) => {
...
router.get('/users', async (req: Request, res: Response) => {
    try {
        const { search, role, is_active, org_id } = req.query as Record<string, string>;
        const page = Math.max(parseInt(String(req.query.page)) || 1, 1);
        const perPage = Math.min(Math.max(parseInt(String(req.query.per_page)) || 50, 1), 100);
        let pi = 1;
        const where: string[] = [];
...
router.post('/role-requests/:id/approve', async (req: Request, res: Response) => {
    try {
        ...
        const approvals: Record<string, ApprovalEntry> = rc.approvals || {};
        // Check if this user's role level is required
        if (!approvals[req.userRole as string] || approvals[req.userRole as string].status !== 'pending') {
        ...
        approvals[req.userRole as string] = { status: 'approved', by: req.userId, at: new Date().toISOString() };

        // Check if all levels are now approved
        const allApproved = Object.values(approvals).every((a: ApprovalEntry) => a.status === 'approved');
        ...
        if (allApproved) {
            ...
            const remaining = Object.entries(approvals).filter(([, a]) => (a as ApprovalEntry).status === 'pending').map(([r]) => r);
...
router.post('/role-requests/:id/reject', async (req: Request, res: Response) => {
    try {
        ...
        const approvals: Record<string, ApprovalEntry> = rc.approvals || {};
        if (!approvals[req.userRole as string]) {
...
router.post('/users', requireRole('hr_admin'), async (req: Request, res: Response) => {
    try {
        ...
        const response: Record<string, unknown> = { id: result.rows[0].id, message: `User ${username} created successfully` };
        if (autoGenerated) response.initial_password = plainPw;
        res.json(response);
    } catch (err) {
...
router.get('/audit-logs', requireFeature('audit_logs'), async (req: Request, res: Response) => {
    try {
        const { actor_id, entity_type, entity_id, action, from, to, limit, offset } = req.query as Record<string, string>;
...
router.get('/stats', requireSameOrg, async (req: Request, res: Response) => {
    try {
...
router.get('/registration-settings', async (req: Request, res: Response) => {
    try {
...
router.put('/registration-settings', requireRole('platform_admin'), async (req: Request, res: Response) => {
    try {
        const { mode } = req.body;
...
router.post('/invite-codes', async (req: Request, res: Response) => {
    try {
        ...
        if (role && ROLE_LEVEL[role] >= (ROLE_LEVEL[req.userRole as string] || 0)) {
...
router.get('/task-labels', async (req: Request, res: Response) => {
    try {
...
router.post('/task-labels', async (req: Request, res: Response) => {
    try {
...
router.put('/task-labels/:id', async (req: Request, res: Response) => {
    try {
...
router.delete('/task-labels/:id', async (req: Request, res: Response) => {
    try {
        const labelRes = await req.db!.query('SELECT * FROM task_labels WHERE id = $1', [Number(req.params.id)]);
        const label = labelRes.rows[0];
        if (!label) return res.status(404).json({ error: 'Label not found' });
...
router.get('/pay-periods', requireSameOrg, async (req: Request, res: Response) => {
    try {
...
router.post('/pay-periods', requireRole('hr_admin'), requireSameOrg, async (req: Request, res: Response) => {
    try {
...
router.delete('/pay-periods/:id', requireRole('hr_admin'), async (req: Request, res: Response) => {
    try {
...
router.get('/announcements', requireRole('super_admin'), async (req: Request, res: Response) => {
    try {
...
router.post('/announcements', requireRole('super_admin'), async (req: Request, res: Response) => {
    try {
...
router.put('/announcements/:id', requireRole('super_admin'), async (req: Request, res: Response) => {
    try {
...
router.delete('/announcements/:id', requireRole('super_admin'), async (req: Request, res: Response) => {
    try {
        let sql: string, params: unknown[];
        if (req.userRole === 'platform_admin') {
