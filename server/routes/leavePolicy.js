 ...
 *
 * Real-world data is messy: the same org may already have a "holiday" row
 * created by an older build (back when built-in chips were a thing) plus a
 * custom-named "Holiday" with a slightly-different slug, leading to multiple
 * cards in the Policies tab. This helper consolidates them:
 *   1. Find every policy in the org whose leave_type matches /holiday/i.
 *   2. Keep the oldest (lowest id) row, normalise its leave_type to 'holiday'
 *      and its name to 'Holiday'.
 *   3. Re-point any balance/leave rows from the duplicates onto the survivor,
 *      then delete the duplicates.
 *   4. If no Holiday-flavoured policy exists at all, create one fresh.
 *
...
async function ensureDefaultHolidayPolicy(orgId, db) {
    ...
    return db.transaction(async (client) => {
        ...
        for (const dup of dupes) {
            // Merge balances: if both rows exist for the same (user, year),
            // sum the `used` and keep the survivor's quota; otherwise just
            // re-point the leave_type.
            await client.query(
 ...
 * in the org. Public holidays are company-wide closures, **not** personal
 * leave — so we deliberately do NOT touch `leave_balances.used`. The
 * employee's Holiday quota stays intact regardless of how many public
 * holidays are added in the year.
 *
 * We still create a balance row for each user (with `used = 0`) so the
 * Holiday card renders even for employees who have never requested a leave.
 *
 * Skips:
 *   - Users who already have any leave row for that date (avoids duplicates
 *     and respects manual entries that happened before the holiday was added).
 *   - Orgs with zero active users → 0 inserts.
 *
 * Must be called inside a transaction (`client` is the transaction's pg client).
 * Returns the number of leave rows that were inserted.
 */
 ...
 * remove the auto-booked Holiday leave rows for that date. We only touch
 * rows we recognise as auto-booked (`status='approved'`, `leave_type='holiday'`,
 * `reason LIKE 'Public holiday:%'`) so manually-entered holiday-type leaves
 * on the same date are left alone.
 *
 * We do not touch `leave_balances.used` because we never incremented it in
 * the first place — public holidays don't deplete the employee's Holiday
 * quota.
 */
 ...
 * table, counting only personal Holiday leaves (i.e. excluding rows we
 * auto-booked when admins added a public holiday — those carry the
 * 'Public holiday:' marker in their reason).
 *
 * Idempotent: running it any number of times always yields the same correct
 * value. Cheap (one UPDATE with a sub-SELECT). Cleans up any inflated counts
 * left by older code that did increment `used` on auto-booking.
 */
 ...
 * matching policy. This handles two cases:
 *   1. Tenants upgraded from before the cascade-delete fix where deleting a
 *      policy left orphan balance rows behind.
 *   2. Manual DB cleanup / partial migrations.
 * Safe to call on every balance fetch — it's idempotent and only touches rows
 * that have no policy at all.
 */
...
/**
 * pg returns NUMERIC columns as strings to preserve precision. The leave
 * balance UI does arithmetic like `quota + carried_forward`, which silently
 * becomes string concatenation if these stay as strings ('8' + '0' === '80').
 * Always coerce numeric balance fields before sending to the client.
 */
 ...
 * with the latest organisation policy. Without the UPDATE branch, an
 * existing balance row would forever keep whatever quota was current the
 * first time it was created — so changing the policy quota later would have
 * no visible effect on existing employees.
 *
 * `used` is preserved (we never overwrite it) and `carried_forward` is only
 * computed for newly-inserted rows so we don't re-credit roll-over every
 * time a quota is edited.
 */
async function initializeBalances(userId, orgId, year, db) {
    ...
    for (const policy of policies) {
        ...
        // We only set carried_forward on insert so re-running this never
        // double-credits roll-over to the user's existing balance.
        await db.query(
 ...
 * with the current policy quotas. Called after a policy is created/updated
 * so all employees immediately see the change without having to wait for
 * their next leave action to trigger initializeBalances().
 *
 * Any balance row whose used > new quota is left intact (we cap nothing —
 * the UI surfaces a negative "available" instead of silently changing
 * historical usage).
 */
...
module.exports.syncOrgBalancesToPolicy = syncOrgBalancesToPolicy;
module.exports.pruneOrphanBalances = pruneOrphanBalances;
module.exports.ensureDefaultHolidayPolicy = ensureDefaultHolidayPolicy;
module.exports.bookHolidayLeavesForOrg = bookHolidayLeavesForOrg;
module.exports.recalcHolidayUsed = recalcHolidayUsed;
