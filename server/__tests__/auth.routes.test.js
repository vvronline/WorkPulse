...
describe('POST /api/auth/login', () => {
    ...
    test('returns 200 with user data and cookie on valid login', async () => {
        ...
        tm.getTenantById.mockResolvedValueOnce({
            id: 7, slug: 'acme', db_name: 'wp_acme', db_host: null, status: 'active',
        });
        ...
        tm.getTenantPool.mockResolvedValueOnce({
            query: tenantQuery,
            transaction: jest.fn(async (fn) => fn({ query: tenantQuery })),
        });

        mockQuery
            .mockResolvedValueOnce({                                          // user_directory hit
                rows: [{ tenant_id: 7, user_id: 1 }], rowCount: 1,
            })
