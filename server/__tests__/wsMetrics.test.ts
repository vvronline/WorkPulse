...
describe("wsMetrics", () => {
    ...
    test("errors are counted and rethrown", async () => {
        ...
        await expect(
            wsMetrics.recordHandler("meeting_signal", 0, async () => { throw err; })
        ).rejects.toBe(err);
        const s = wsMetrics.snapshot();
    ...
    test("timeouts are counted and throw a WS_HANDLER_TIMEOUT error", async () => {
        await expect(
            wsMetrics.recordHandler("chat_message", 20, () => new Promise(() => { /* never resolves */ }))
        ).rejects.toMatchObject({ code: "WS_HANDLER_TIMEOUT" });
        const s = wsMetrics.snapshot();
