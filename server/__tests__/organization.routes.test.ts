...
describe("GET /api/org/roles", () => {
    ...
    test("returns canonical defaults when tenant has no rows", async () => {
        ...
        expect(res.body.roles.find((r: any) => r.role_key === "employee").label).toBe("Employee");
        expect(res.body.roles.find((r: any) => r.role_key === "manager").permission_level).toBe(3);
    });
    ...
    test("returns tenant-defined roles when present", async () => {
        ...
        const res = await request(app)
            ...
            .set(CSRF);
        expect(res.status).toBe(200);
        const eng = res.body.roles.find((r: any) => r.role_key === "engineer");
...
describe("POST /api/org/roles", () => {
    ...
    test("creates a new role on success", async () => {
        ...
        expect(res.status).toBe(200);
        expect(res.body.roles.find((r: any) => r.role_key === "principal")).toBeTruthy();
    });
...
describe("PATCH /api/org/roles/:role_key", () => {
    ...
    test("updates label/color on success", async () => {
        ...
        expect(res.status).toBe(200);
        expect(res.body.roles.find((r: any) => r.role_key === "manager").label).toBe("Engineering Lead");
    });
});
