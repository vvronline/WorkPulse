...
describe('GET /api/export/my-analytics', () => {
    ...
    test('returns CSV response for analytics with data', async () => {
        setupAuth();
        const entries = [
            { user_id: 1, entry_type: 'clock_in', timestamp: '2024-01-15T09:00:00Z', work_mode: 'office', approval_status: 'approved' },
            { user_id: 1, entry_type: 'clock_out', timestamp: '2024-01-15T17:00:00Z', work_mode: 'office', approval_status: 'approved' },
        ];
...
describe('GET /api/export/my-leaves', () => {
    ...
    test('returns CSV for personal leaves', async () => {
        setupAuth();
        const leaves = [
            { date: '2024-03-01', leave_type: 'Annual', duration: 1, status: 'approved', reason: 'Vacation', approved_by_name: 'Manager' },
        ];
...
describe('GET /api/export/my-tasks', () => {
    ...
    test('returns CSV for tasks', async () => {
        setupAuth();
        const tasks = [
            { date: '2024-03-01', title: 'Fix auth bug', status: 'done', priority: 'high', due_date: null, assigned_to_name: null },
        ];
...
describe('GET /api/export/payroll-hours', () => {
    ...
    test('returns CSV for payroll data as hr_admin', async () => {
        ...
        // user list
        mockQuery.mockResolvedValueOnce({ rows: [{ id: 2, full_name: 'Alice', department_name: 'Eng', team_name: 'Dev' }], rowCount: 1 });
...
describe('GET /api/export/team-analytics', () => {
    ...
    test('returns CSV for team analytics as manager', async () => {
        ...
        mockQuery.mockResolvedValueOnce({ rows: [{ id: 2, full_name: 'Alice', role: 'employee', team_name: 'Dev', department_name: 'Eng' }], rowCount: 1 });
        // time entries
        mockQuery.mockResolvedValueOnce({ rows: [], rowCount: 0 });
