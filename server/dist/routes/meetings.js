...
async function insertSystemMessage(conversationId, senderId, metadata, db, tenantId) {
    const result = (await db.query(`INSERT INTO messages (conversation_id, sender_id, content, format_type, metadata)
         VALUES ($1, $2, $3, 'system', $4) RETURNING id, created_at`, [conversationId, senderId, metadata.text || "", JSON.stringify(metadata)])).rows[0];
    const participants = (await db.query("SELECT user_id FROM conversation_participants WHERE conversation_id = $1", [conversationId])).rows;
    const outMsg = {
        id: result.id,
        conversationId,
        senderId,
...
async function getConflictsForUsers(userIds, startTime, endTime, db) {
    ...
    const rows = (await db.query(`SELECT user_id, id, title, start_time, end_time
         FROM calendar_events
         WHERE user_id = ANY($1)
           AND start_time < $3::timestamptz
           AND end_time > $2::timestamptz
         ORDER BY user_id, start_time
         LIMIT 100`, [userIds, startTime, endTime])).rows;
    const conflicts = {};
    for (const row of rows) {
        if (!conflicts[row.user_id])
            conflicts[row.user_id] = [];
...
router.post("/check-conflicts", async (req, res) => {
    try {
        ...
        for (const uid of ids) {
            if (conflictMap[uid]) {
                const userInfo = (await req.db.query("SELECT full_name, username FROM users WHERE id = $1", [uid])).rows[0];
                result.push({
                    userId: uid,
...
router.get("/", async (req, res) => {
    try {
        ...
        const result = await req.db.query(`SELECT m.*,
...
router.get("/:code/messages", async (req, res) => {
    try {
        ...
        const meeting = (await req.db.query("SELECT id, conversation_id, org_id, created_by FROM meetings WHERE meeting_code = $1", [req.params.code])).rows[0];
        if (!meeting)
        ...
        const access = (await req.db.query("SELECT 1 FROM meeting_participants WHERE meeting_id = $1 AND user_id = $2", [meeting.id, req.userId])).rows[0];
        ...
        // effect).
        const rows = (await req.db.query(`SELECT m.id, m.sender_id, m.content, m.metadata, m.created_at, m.format_type, m.client_msg_id,
             ...
             LIMIT $2`, [meeting.conversation_id, limit, since])).rows;
...
router.get("/:code", async (req, res) => {
    try {
        ...
        if (!participantRows) {
            participantRows = (await req.db.query(`SELECT mp.*, u.full_name, u.avatar, u.username
                 FROM meeting_participants mp JOIN users u ON u.id = mp.user_id
                 WHERE mp.meeting_id = $1 ORDER BY mp.id`, [meeting.id])).rows;
...
router.post("/", async (req, res) => {
    try {
        ...
        if (!req.userOrgId)
            return res.status(403).json({ error: "You must belong to an organization to create meetings" });
        ...
        const result = await req.db.transaction(async (client) => {
            ...
            const conv = (await client.query(`INSERT INTO conversations (org_id, name, is_group, created_at, updated_at)
                 VALUES ($1, $2, TRUE, NOW(), NOW()) RETURNING id`, [req.userOrgId, `Meeting: ${title.trim()}`])).rows[0];
            // 2. Add creator as participant of conversation
            const allParticipantIds = [req.userId, ...inviteeIds];
            for (const uid of allParticipantIds) {
                await client.query(`INSERT INTO conversation_participants (conversation_id, user_id) VALUES ($1, $2) ON CONFLICT DO NOTHING`, [conv.id, uid]);
            }
            ...
            const meeting = (await client.query(`INSERT INTO meetings (org_id, title, description, meeting_code, created_by, conversation_id, calendar_event_id, settings)
                 VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *`, [req.userOrgId, title.trim(), description || null, code, req.userId, conv.id, calendar_event_id || null, JSON.stringify(meetingSettings)])).rows[0];
            // 4. Add organizer to meeting_participants
            await client.query(`INSERT INTO meeting_participants (meeting_id, user_id, role, status) VALUES ($1, $2, 'organizer', 'invited')`, [meeting.id, req.userId]);
            ...
            for (const uid of requiredIds.length > 0 || optionalIds.length > 0 ? requiredIds : legacyIds) {
                await client.query(`INSERT INTO meeting_participants (meeting_id, user_id, role, status, participant_type) VALUES ($1, $2, 'participant', 'invited', 'required')`, [meeting.id, uid]);
            }
            for (const uid of optionalIds) {
                await client.query(`INSERT INTO meeting_participants (meeting_id, user_id, role, status, participant_type) VALUES ($1, $2, 'participant', 'invited', 'optional')`, [meeting.id, uid]);
            }
            // 6. Link calendar event if provided
            if (calendar_event_id) {
                await client.query(`UPDATE calendar_events SET meeting_id = $1 WHERE id = $2 AND user_id = $3`, [meeting.id, calendar_event_id, req.userId]);
            }
        ...
        for (const uid of inviteeIds) {
            ...
            await req.db.query(`INSERT INTO notifications (user_id, type, title, body) VALUES ($1, 'meeting_invite', $2, $3)`, [uid, `Meeting invitation`, notifBody]);
...
router.put("/:id", async (req, res) => {
    try {
        ...
        if (!meeting)
            return res.status(404).json({ error: "Meeting not found or not organizer" });
        if (meeting.org_id !== req.userOrgId)
            return res.status(403).json({ error: "Access denied" });
        if (meeting.status === "ended")
            return res.status(400).json({ error: "Cannot update ended meeting" });
        ...
        const result = await req.db.query(`UPDATE meetings SET
                title = COALESCE($1, title),
                description = COALESCE($2, description),
                settings = $3
             WHERE id = $4 RETURNING *`, [title?.trim() || null, description ?? null, JSON.stringify(newSettings), meetingId]);
        ...
        const participants = (await req.db.query(`SELECT mp.user_id, u.full_name, u.email, u.username
             FROM meeting_participants mp JOIN users u ON u.id = mp.user_id
             WHERE mp.meeting_id = $1 AND mp.user_id != $2`, [meetingId, req.userId])).rows;
        for (const p of participants) {
...
router.get("/:id/participants", async (req, res) => {
    try {
        ...
        // Must be a participant or organizer
        const access = (await req.db.query("SELECT 1 FROM meeting_participants WHERE meeting_id = $1 AND user_id = $2", [meetingId, req.userId])).rows[0];
        if (!access && meeting.created_by !== req.userId)
            return res.status(403).json({ error: "Access denied" });
        const result = await req.db.query(`SELECT mp.*, u.full_name, u.avatar, u.username, u.role
             FROM meeting_participants mp JOIN users u ON u.id = mp.user_id
             WHERE mp.meeting_id = $1 ORDER BY mp.role DESC, mp.id`, [meetingId]);
        res.json(result.rows);
    }
    catch (err) {
...
router.post("/:id/participants", async (req, res) => {
    try {
        ...
        if (meeting.org_id && targetUser.org_id !== meeting.org_id)
            return res.status(403).json({ error: "Cannot add participants from a different organization" });
        // Add to meeting_participants
        await req.db.query(`INSERT INTO meeting_participants (meeting_id, user_id, role, status)
             VALUES ($1, $2, 'participant', 'invited') ON CONFLICT (meeting_id, user_id) DO NOTHING`, [meetingId, user_id]);
        await redis.invalidateMeetingParticipants(req.tenantId, meetingId);
        // Add to conversation
        if (meeting.conversation_id) {
            await req.db.query(`INSERT INTO conversation_participants (conversation_id, user_id) VALUES ($1, $2) ON CONFLICT DO NOTHING`, [meeting.conversation_id, user_id]);
        }
        const organizer = (await req.db.query("SELECT full_name FROM users WHERE id = $1", [req.userId])).rows[0];
        // Check if new participant has a conflict with this meeting's calendar event
        let hasConflict = false;
        let conflictTitle = null;
        const calEvent = (await req.db.query("SELECT start_time, end_time FROM calendar_events WHERE meeting_id = $1 AND user_id = $2 LIMIT 1", [meetingId, meeting.created_by])).rows[0];
        if (calEvent) {
            const overlapping = (await req.db.query(`SELECT title FROM calendar_events
                 WHERE user_id = $1
                   AND start_time < $3::timestamptz
                   AND end_time > $2::timestamptz
                   AND meeting_id IS DISTINCT FROM $4
                 LIMIT 1`, [user_id, calEvent.start_time, calEvent.end_time, meetingId])).rows;
        ...
        const notifBody = hasConflict
            ? `${organizer?.full_name || "Someone"} invited you to "${meeting.title}" — ⚠️ Conflicts with "${conflictTitle}"`
            : `${organizer?.full_name || "Someone"} invited you to "${meeting.title}"`;
        await req.db.query(`INSERT INTO notifications (user_id, type, title, body) VALUES ($1, 'meeting_invite', $2, $3)`, [user_id, `Meeting invitation`, notifBody]);
...
router.post("/:code/hls/start", async (req, res) => {
    try {
        ...
        const meeting = (await req.db.query("SELECT * FROM meetings WHERE meeting_code = $1", [code])).rows[0];
        if (!meeting)
            return res.status(404).json({ error: "Meeting not found" });
        ...
        const isParticipant = (await req.db.query("SELECT 1 FROM meeting_participants WHERE meeting_id = $1 AND user_id = $2", [meeting.id, req.userId])).rows[0];
        ...
        const parts = (await req.db.query("SELECT user_id FROM meeting_participants WHERE meeting_id = $1", [meeting.id])).rows;
...
router.post("/:code/hls/stop", async (req, res) => {
    try {
        const code = req.params.code;
        const meeting = (await req.db.query("SELECT id, created_by FROM meetings WHERE meeting_code = $1", [code])).rows[0];
        if (!meeting)
            return res.status(404).json({ error: "Meeting not found" });
        const existing = activeBroadcasts.get(meeting.id);
        if (!existing)
...
router.get("/:code/hls/status", async (req, res) => {
    try {
        const code = req.params.code;
        const meeting = (await req.db.query("SELECT id FROM meetings WHERE meeting_code = $1", [code])).rows[0];
        if (!meeting)
            return res.status(404).json({ error: "Meeting not found" });
        const isParticipant = (await req.db.query("SELECT 1 FROM meeting_participants WHERE meeting_id = $1 AND user_id = $2", [meeting.id, req.userId])).rows[0];
        if (!isParticipant)
