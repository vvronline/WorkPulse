...
router.post("/", requireRole("super_admin"), async (req, res) => {
    try {
        ...
        const result = await req.db.query("INSERT INTO organizations (name, slug, created_by) VALUES ($1, $2, $3) RETURNING id", [name.trim(), slug, req.userId]);
...
router.put("/settings", requireRole("hr_admin"), requireSameOrg, async (req, res) => {
    try {
        const { name, work_hours_per_day, work_days, timezone, fiscal_year_start, min_hours_present, office_start_time, attendance_verification_enabled, office_latitude, office_longitude, office_radius_m, office_address, office_wifi_bssids, office_wifi_verification_enabled, } = req.body;
        const updates = [];
        const params = [];
        let pi = 1;
        ...
        if (timezone && timezone.length > 50)
            return res.status(400).json({ error: "Timezone must be 50 characters or less" });
        if (work_days && work_days.length > 50)
            return res.status(400).json({ error: "Work days value too long" });
        ...
        if (fiscal_year_start !== undefined) {
            ...
            params.push(Number(fiscal_year_start));
        }
        ...
        if (min_hours_present !== undefined) {
            if (min_hours_present === null || min_hours_present === "") {
                ...
                updates.push(`min_hours_present = $${pi++}`);
                params.push(mhp);
            }
        }
        ...
        if (office_start_time !== undefined) {
            if (office_start_time === null || office_start_time === "") {
                ...
                updates.push(`office_start_time = $${pi++}`);
                params.push(office_start_time);
            }
        }
        ...
        if (attendance_verification_enabled !== undefined) {
            ...
            if (flag) {
                ...
                if (incomingLat == null || incomingLng == null) {
                    const cur = (await req.db.query("SELECT office_latitude, office_longitude FROM organizations WHERE id = $1", [req.userOrgId])).rows[0];
            ...
            updates.push(`attendance_verification_enabled = $${pi++}`);
            params.push(flag);
        }
        if (office_latitude !== undefined) {
            if (office_latitude === null || office_latitude === "") {
                ...
                updates.push(`office_latitude = $${pi++}`);
                params.push(lat);
            }
        }
        if (office_longitude !== undefined) {
            if (office_longitude === null || office_longitude === "") {
                ...
                updates.push(`office_longitude = $${pi++}`);
                params.push(lng);
            }
        }
        if (office_radius_m !== undefined) {
            ...
            updates.push(`office_radius_m = $${pi++}`);
            params.push(Math.round(r));
        }
        if (office_address !== undefined) {
            if (office_address === null || office_address === "") {
                ...
                updates.push(`office_address = $${pi++}`);
                params.push(office_address.trim());
            }
        }
        ...
        if (office_wifi_verification_enabled !== undefined) {
            ...
            if (flag) {
                ...
                if (!willHaveList) {
                    const cur = (await req.db.query("SELECT office_wifi_bssids FROM organizations WHERE id = $1", [req.userOrgId])).rows[0];
            ...
            updates.push(`office_wifi_verification_enabled = $${pi++}`);
            params.push(flag);
        }
...
router.get("/members", requireRole("team_lead"), requireSameOrg, async (req, res) => {
    try {
        ...
        if (!effectiveOrgId)
            return res.json({ data: [], total: 0, page: 1, perPage });
        const where = [
            "u.org_id = $1",
        ...
        if (department_id) {
            where.push(`u.department_id = $${pi++}`);
            params.push(Number(department_id));
        }
        if (team_id) {
            where.push(`u.team_id = $${pi++}`);
...
router.post("/invite", requireRole("hr_admin"), requireSameOrg, async (req, res) => {
    try {
        ...
        const tenantRoleRows = (await req.db.query(`SELECT role_key, permission_level FROM tenant_roles WHERE org_id = $1`, [req.userOrgId])).rows;
        ...
        if (department_id) {
            ...
            if (!dept)
                return res.status(400).json({ error: "Department not found in this organization" });
        }
        if (team_id) {
        ...
        await req.db.query("UPDATE users SET org_id = $1, role = $2, department_id = $3, team_id = $4 WHERE id = $5", [req.userOrgId, assignRole, department_id || null, team_id || null, user_id]);
...
router.post("/remove-member", requireRole("hr_admin"), requireSameOrg, async (req, res) => {
    try {
        ...
        if (target.org_id !== req.userOrgId)
            return res.status(400).json({ error: "User is not in your organization" });
...
router.post("/departments", requireRole("hr_admin"), requireSameOrg, async (req, res) => {
    try {
        ...
        const result = await req.db.query("INSERT INTO departments (org_id, name, head_id) VALUES ($1, $2, $3) RETURNING id", [effectiveOrgId, name.trim(), head_id || null]);
...
router.put("/departments/:id", requireRole("hr_admin"), requireSameOrg, async (req, res) => {
    try {
        ...
        await req.db.query("UPDATE departments SET name = $1, head_id = $2 WHERE id = $3", [name?.trim() || dept.name, head_id !== undefined ? head_id : dept.head_id, id]);
...
router.post("/teams", requireRole("hr_admin"), requireSameOrg, async (req, res) => {
    try {
        ...
        if (!effectiveOrgId)
            return res.status(400).json({ error: "Organization ID is required" });
        const result = await req.db.query("INSERT INTO teams (org_id, department_id, name, lead_id) VALUES ($1, $2, $3, $4) RETURNING id", [effectiveOrgId, department_id || null, name.trim(), lead_id || null]);
...
router.put("/teams/:id", requireRole("hr_admin"), requireSameOrg, async (req, res) => {
    try {
        ...
        await req.db.query("UPDATE teams SET name = $1, department_id = $2, lead_id = $3 WHERE id = $4", [name?.trim() || team.name, department_id !== undefined ? department_id : team.department_id, lead_id !== undefined ? lead_id : team.lead_id, id]);
...
router.put("/teams/:id/sprint-config", requireRole("team_lead"), requireSameOrg, async (req, res) => {
    try {
        ...
        await req.db.query("UPDATE teams SET sprint_duration_weeks = $1, sprint_start_date = $2 WHERE id = $3", [sprint_duration_weeks !== undefined ? sprint_duration_weeks : team.sprint_duration_weeks,
...
router.get("/chart", requireSameOrg, async (req, res) => {
    try {
        const effectiveOrgId = resolveOrgId(req, "query");
        if (!effectiveOrgId)
            return res.json({ departments: [], teams: [], members: [] });
        const departments = (await req.db.query(`
...
async function invalidateOrgRoleCaches(req, orgId) {
    try {
        await redisClient.invalidateOrgRolesMap?.(req.tenantId, orgId);
    }
    catch { /* best-effort */ }
    try {
...
router.get("/roles", async (req, res) => {
    try {
        ...
        if (orgId && roles.length) {
            const counts = (await req.db.query(`SELECT role, COUNT(*)::int AS cnt
                 FROM users
                 WHERE org_id = $1 AND is_active = TRUE
                 GROUP BY role`, [orgId])).rows.reduce((acc, r) => { acc[r.role] = r.cnt; return acc; }, {});
            for (const r of roles)
                r.user_count = counts[r.role_key] || 0;
        }
        else {
            for (const r of roles)
                r.user_count = 0;
...
router.post("/roles", requireRole("super_admin"), requireSameOrg, async (req, res) => {
    try {
        ...
        const maxRow = (await req.db.query("SELECT COALESCE(MAX(sort_order), 0) AS m FROM tenant_roles WHERE org_id = $1", [orgId])).rows[0];
        const sortOrder = Number(maxRow.m) + 1;
        try {
            await req.db.query(`INSERT INTO tenant_roles (org_id, role_key, label, description, color, permission_level, is_system, sort_order)
                 VALUES ($1, $2, $3, $4, $5, $6, FALSE, $7)`, [orgId, role_key, label, description, color, permission_level, sortOrder]);
        }
        catch (e) {
            if (e.code === "23505") {
...
router.patch("/roles/:role_key", requireRole("super_admin"), requireSameOrg, async (req, res) => {
    try {
        ...
        const existing = (await req.db.query("SELECT * FROM tenant_roles WHERE org_id = $1 AND role_key = $2", [orgId, role_key])).rows[0];
        if (!existing)
        ...
        await req.db.query(`UPDATE tenant_roles SET ${setClauses.join(", ")}
             WHERE org_id = $${i++} AND role_key = $${i}`, params);
        await invalidateOrgRoleCaches(req, orgId);
...
router.delete("/roles/:role_key", requireRole("super_admin"), requireSameOrg, async (req, res) => {
    try {
        ...
        const role = (await req.db.query("SELECT * FROM tenant_roles WHERE org_id = $1 AND role_key = $2", [orgId, role_key])).rows[0];
        if (!role)
            return res.status(404).json({ error: "Role not found" });
        const inUse = parseInt((await req.db.query("SELECT COUNT(*) AS c FROM users WHERE org_id = $1 AND role = $2", [orgId, role_key])).rows[0].c, 10);
        if (inUse > 0) {
            return res.status(400).json({
                error: `Cannot delete: ${inUse} user${inUse === 1 ? "" : "s"} still hold${inUse === 1 ? "s" : ""} this role. Reassign them first.`,
            });
        }
        await req.db.query("DELETE FROM tenant_roles WHERE org_id = $1 AND role_key = $2", [orgId, role_key]);
        await invalidateOrgRoleCaches(req, orgId);
