...
async function getOrgWorkConfig(orgId, db, tenantId) {
    if (!orgId)
        return { work_hours_per_day: 8, work_days: "1,2,3,4,5" };
...
router.get("/status", auth, async (req, res) => {
    try {
        ...
        const orgRow = (await req.db.query(`SELECT u.org_id FROM users u WHERE u.id = $1`, [req.userId])).rows[0];
        ...
        if (isLiveSession && status.state !== "logged_out" && status.floorMinutes >= targetMinutes) {
            const didClockOut = await req.db.transaction(async (client) => {
                const latest = (await client.query(`SELECT entry_type FROM time_entries WHERE user_id = $1 AND ${pgDateInTz("timestamp", tzMod)} = $2::date ORDER BY timestamp DESC, id DESC LIMIT 1 FOR UPDATE`, [req.userId, today])).rows[0];
                if (!latest || latest.entry_type === "clock_out")
                    return false;
                await client.query("INSERT INTO time_entries (user_id, entry_type) VALUES ($1, $2)", [req.userId, "clock_out"]);
                return true;
            });
            if (didClockOut) {
                ...
                const refreshed = await req.db.query(`SELECT * FROM time_entries
                     WHERE user_id = $1 AND ${pgDateInTz("timestamp", tzMod)} = $2::date
                       AND (approval_status IS NULL OR approval_status != 'rejected')
                     ORDER BY timestamp ASC, id ASC`, [req.userId, today]);
...
router.post("/clock-in", auth, loadUserContext, async (req, res) => {
    try {
        ...
        if (req.userOrgId) {
            ...
            const todayEntries = (await req.db.query(`SELECT * FROM time_entries
                 WHERE user_id = $1 AND ${pgDateInTz("timestamp", tzMod)} = $2::date
                   AND (approval_status IS NULL OR approval_status != 'rejected')
                 ORDER BY timestamp ASC, id ASC`, [req.userId, today])).rows;
            const todayStatus = computeStatus(todayEntries);
            if (todayStatus.state === "logged_out" && todayStatus.floorMinutes >= targetMin) {
                const approvedOT = (await req.db.query(`SELECT id FROM approval_requests
                     WHERE requester_id = $1
                       AND type = 'overtime'
                       AND status = 'approved'
                       AND (metadata->>'date') = $2`, [req.userId, today])).rows[0];
                if (!approvedOT) {
        ...
        if (req.userOrgId) {
            const orgVerify = (await req.db.query(`SELECT attendance_verification_enabled, office_latitude, office_longitude, office_radius_m,
                        office_wifi_bssids, office_wifi_verification_enabled
                 FROM organizations WHERE id = $1`, [req.userOrgId])).rows[0];
            if (orgVerify?.attendance_verification_enabled) {
                ...
                const formattedBssid = normalisedBssid && normalisedBssid.length === 12
                    ...
                    : null;
                const wifiMatch = !!(orgVerify.office_wifi_verification_enabled &&
                    formattedBssid &&
                ...
                if (needsLocation) {
                    ...
                    if (inside)
                        verifyMeta.verified_via = "geofence";
                }
                const enrolledRow = (await req.db.query("SELECT face_descriptor FROM users WHERE id = $1", [req.userId])).rows[0];
        ...
        const txResult = await req.db.transaction(async (client) => {
            const lastRes = await client.query(`SELECT * FROM time_entries
                 WHERE user_id = $1 AND ${pgDateInTz("timestamp", tzMod)} = $2::date
                   AND (is_manual IS NOT TRUE)
                 ORDER BY timestamp DESC, id DESC LIMIT 1`, [req.userId, today]);
            const lastEntry = lastRes.rows[0];
            if (lastEntry && lastEntry.entry_type !== "clock_out") {
                return { error: "Already logged in. Logout first." };
            }
            await client.query(`INSERT INTO time_entries
...
router.post("/break-start", auth, async (req, res) => {
    try {
        ...
        const txResult = await req.db.transaction(async (client) => {
            ...
            await client.query("INSERT INTO time_entries (user_id, entry_type) VALUES ($1, $2)", [req.userId, "break_start"]);
            return { ok: true };
        });
        if (txResult.error)
            return res.status(400).json({ error: txResult.error });
...
router.post("/break-end", auth, async (req, res) => {
    try {
        ...
        const txResult = await req.db.transaction(async (client) => {
            ...
            await client.query("INSERT INTO time_entries (user_id, entry_type) VALUES ($1, $2)", [req.userId, "break_end"]);
            return { ok: true };
        });
        if (txResult.error)
            return res.status(400).json({ error: txResult.error });
    ...
    try {
        ...
        const result = await req.db.query(`SELECT * FROM time_entries
             WHERE user_id = $1
               ...
               AND (approval_status IS NULL OR approval_status != 'rejected')
             ORDER BY timestamp ASC`, [req.userId, fromDate, toDate]);
        const grouped = {};
...
router.get("/manual-entries", auth, loadUserContext, async (req, res) => {
    try {
        const result = await req.db.query(`SELECT ar.id as request_id, ar.status as approval_status, ar.metadata, ar.created_at,
...
router.post("/manual-entry", auth, loadUserContext, async (req, res) => {
    try {
        ...
        const tzMod = getTzModifier(req);
        const existingRes = await req.db.query(`SELECT COUNT(*) AS count FROM time_entries
             WHERE user_id = $1 AND ${pgDateInTz("timestamp", tzMod)} = $2::date`, [req.userId, date]);
        ...
        const leaveRes = await req.db.query("SELECT id, leave_type FROM leaves WHERE user_id = $1 AND date = $2", [req.userId, date]);
        if (leaveRes.rows[0]) {
        ...
        if (req.userOrgId) {
            const lockedPeriod = (await req.db.query(`SELECT label FROM pay_periods WHERE org_id = $1 AND start_date <= $2 AND end_date >= $2`, [req.userOrgId, date])).rows[0];
            if (lockedPeriod) {
        ...
        await req.db.transaction(async (client) => {
            const ins = (uid, type, ts, wm) => client.query("INSERT INTO time_entries (user_id, entry_type, timestamp, work_mode, is_manual, approval_status) VALUES ($1,$2,$3,$4,TRUE,$5)", [uid, type, ts, wm || null, approvalStatus]);
            ...
            if (clockOutTs)
                await ins(req.userId, "clock_out", clockOutTs, null);
            if (needsApproval) {
                const approver = await findApprover(req.db, req.userId, req.userOrgId);
                txApprover = approver;
                await client.query(`INSERT INTO approval_requests (org_id, requester_id, approver_id, type, reference_id, reason, metadata)
                     VALUES ($1,$2,$3,'manual_entry',NULL,$4,$5)`, [req.userOrgId || null, req.userId, approver?.id || null, "Manual time entry",
        ...
        if (needsApproval && txApprover?.id) {
            void (async () => {
                try {
                    ...
                    await req.db.query("INSERT INTO notifications (user_id, type, title, body) VALUES ($1, $2, $3, $4)", [txApprover.id, "approval", "New Manual Entry Request", `${requesterName} submitted a manual time entry for ${date}.`]);
...
router.put("/manual-entry/:date", auth, loadUserContext, async (req, res) => {
    try {
        ...
        if (breaks && Array.isArray(breaks)) {
            if (breaks.length > 20)
                return res.status(400).json({ error: "Maximum 20 breaks allowed" });
            const sorted = [...breaks].sort((a, b) => (a.start || "").localeCompare(b.start || ""));
            for (let i = 0; i < sorted.length; i++) {
                const brk = sorted[i];
                if (!brk.start || !brk.end || !isValidTime(brk.start) || !isValidTime(brk.end)) {
        ...
        await req.db.transaction(async (client) => {
            await client.query(`UPDATE approval_requests
                 ...
                 WHERE requester_id = $1 AND type = 'manual_entry' AND status = 'pending'
                   AND metadata::jsonb->>'date' = $2`, [req.userId, date]);
            await client.query(`DELETE FROM time_entries WHERE user_id = $1 AND ${pgDateInTz("timestamp", tzMod)} = $2::date`, [req.userId, date]);
            const ins = (uid, type, ts, wm) => client.query("INSERT INTO time_entries (user_id, entry_type, timestamp, work_mode, is_manual, approval_status) VALUES ($1,$2,$3,$4,TRUE,$5)", [uid, type, ts, wm || null, approvalStatus]);
            await ins(req.userId, "clock_in", clockInTs, VALID_WORK_MODES.includes(work_mode) ? work_mode : "office");
            if (breaks && Array.isArray(breaks)) {
                const sorted = [...breaks].sort((a, b) => a.start.localeCompare(b.start));
            ...
            if (needsApproval) {
                ...
                await client.query(`INSERT INTO approval_requests (org_id, requester_id, approver_id, type, reference_id, reason, metadata)
                     VALUES ($1,$2,$3,'manual_entry',NULL,$4,$5)`, [req.userOrgId || null, req.userId, approver?.id || null, "Manual time entry (edited)",
                    JSON.stringify({ date, clock_in, clock_out: clock_out || null, work_mode: VALID_WORK_MODES.includes(work_mode) ? work_mode : "office" })]);
            }
        });
        ...
        if (needsApproval && txApproverEdit?.id) {
            void (async () => {
                try {
                    const requesterName = (await req.db.query("SELECT full_name FROM users WHERE id = $1", [req.userId])).rows[0]?.full_name || "A team member";
                    await req.db.query("INSERT INTO notifications (user_id, type, title, body) VALUES ($1, $2, $3, $4)", [txApproverEdit.id, "approval", "Manual Entry Updated", `${requesterName} updated a manual time entry for ${date}.`]);
...
router.delete("/entries/:date", auth, loadUserContext, async (req, res) => {
    try {
        ...
        const tzMod = getTzModifier(req);
        const protectedRes = await req.db.query(`SELECT 1 FROM time_entries
             WHERE user_id = $1 AND ${pgDateInTz("timestamp", tzMod)} = $2::date
               AND approval_status IN ('pending','approved')
             LIMIT 1`, [req.userId, date]);
        ...
        const result = await req.db.query(`DELETE FROM time_entries WHERE user_id = $1 AND ${pgDateInTz("timestamp", tzMod)} = $2::date`, [req.userId, date]);
...
router.get("/entries/:date", auth, async (req, res) => {
    try {
        ...
        const result = await req.db.query(`SELECT * FROM time_entries
             WHERE user_id = $1 AND ${pgDateInTz("timestamp", tzMod)} = $2::date
             ORDER BY timestamp ASC`, [req.userId, date]);
...
router.get("/widgets", auth, async (req, res) => {
    try {
        ...
        const tzMod = getTzModifier(req);
        const offsetMin = getOffsetMin(req);
        const entriesRes = await req.db.query(`SELECT * FROM time_entries
             WHERE user_id = $1 AND ${pgDateInTz("timestamp", tzMod)} >= ($2::date - INTERVAL '30 days')
             ORDER BY timestamp ASC`, [req.userId, today]);
        const grouped = {};
        ...
        try {
            const leaveRes = await req.db.query(`SELECT date FROM leaves WHERE user_id = $1 AND status = 'approved' AND date::date >= $2::date - INTERVAL '60 days' AND date::date <= $3::date`, [req.userId, today, today]);
            leaveRes.rows.forEach((r) => leaveDatesSet.add(r.date));
            leaveRes.rows.forEach((r) => { if (r.date >= monthStart)
                leaveCount++; });
        }
        catch (_) { /* ignore */ }
        let totalFloorMin = 0;
        ...
        if (req.userOrgId) {
            const orgConfig = await getOrgWorkConfig(req.userOrgId, req.db, req.tenantId);
            if (orgConfig.work_hours_per_day)
                orgWhpd = orgConfig.work_hours_per_day;
        }
        ...
        Object.keys(grouped).forEach((date) => {
            ...
            if (summary.floorMinutes >= TARGET)
                targetMetDays++;
            if (summary.workMode === "remote")
                remoteDays++;
            else
                officeDays++;
        });
...
router.get("/weekly", auth, async (req, res) => {
    try {
        ...
        const result = await req.db.query(`SELECT * FROM time_entries
             ...
             ORDER BY timestamp ASC`, [req.userId, mondayStr, sundayStr]);
        const grouped = {};
        result.rows.forEach((e) => {
            const date = getLocalDateFromTs(e.timestamp, req);
            if (!grouped[date])
...
router.get("/task-summary", auth, async (req, res) => {
    try {
        const today = getLocalToday(req);
        const result = await req.db.query("SELECT * FROM tasks WHERE (user_id = $1 OR assigned_to = $1) AND date = $2 ORDER BY priority DESC, created_at ASC", [req.userId, today]);
...
router.post("/overtime-request", auth, loadUserContext, async (req, res) => {
    try {
        ...
        const existingRes = await req.db.query(`SELECT id FROM approval_requests
        ...
        await req.db.query(`INSERT INTO approval_requests (org_id, requester_id, approver_id, type, reference_id, reason, metadata)
             VALUES ($1,$2,$3,'overtime',NULL,$4,$5)`, [req.userOrgId || null, req.userId, approver?.id || null, reason,
            JSON.stringify({ date, hours: numHours })]);
        try {
            if (approver?.id) {
                const requesterName = (await req.db.query("SELECT full_name FROM users WHERE id = $1", [req.userId])).rows[0]?.full_name || "A team member";
                await req.db.query("INSERT INTO notifications (user_id, type, title, body) VALUES ($1, $2, $3, $4)", [approver.id, "approval", "Overtime Request", `${requesterName} requested ${numHours}h overtime for ${date}.`]);
...
router.get("/overtime-requests", auth, async (req, res) => {
    try {
        const result = await req.db.query(`SELECT ar.id, ar.status, ar.reason, ar.metadata, ar.created_at, ar.reject_reason,
