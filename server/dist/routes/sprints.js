...
async function snapshotBurndown(db, sprintId) {
    const tasks = (await db.query(`SELECT t.story_points, ws.is_terminal, t.is_blocked
           FROM tasks t
           LEFT JOIN workflow_states ws ON ws.id = t.workflow_state_id
          WHERE t.sprint_id = $1`, [sprintId])).rows;
    ...
    await db.query(`INSERT INTO sprint_burndown_snapshots (sprint_id, snapshot_date, total_points, done_points, remaining_points, blocked_points, open_tasks)
         ...
         DO UPDATE SET total_points = EXCLUDED.total_points,
                       ...
                       open_tasks = EXCLUDED.open_tasks`, [sprintId, total, done, remaining, blocked, openCount]);
}
...
router.get("/active", auth, loadUserContext, async (req, res) => {
    try {
        ...
        if (cached !== null)
            return res.json({ sprint: cached || null });
...
router.post("/", auth, loadUserContext, requireRole("team_lead"), async (req, res) => {
    try {
        ...
        if (name.trim().length > 100)
            return res.status(400).json({ error: "Sprint name must be 100 characters or less" });
        if (goal && goal.length > 1000)
            return res.status(400).json({ error: "Sprint goal must be 1000 characters or less" });
        ...
        const result = await req.db.query("INSERT INTO sprints (team_id, name, start_date, end_date, goal, status) VALUES ($1, $2, $3, $4, $5, 'planned') RETURNING id", [req.userTeamId, name, start_date, end_date, goal || null]);
...
router.put("/:id", auth, loadUserContext, requireRole("team_lead"), async (req, res) => {
    try {
        ...
        const sprint = (await req.db.query("SELECT s.* FROM sprints s JOIN teams t ON t.id = s.team_id WHERE s.id = $1 AND t.org_id = $2", [id, req.userOrgId])).rows[0];
        if (!sprint)
...
router.get("/:id/stats", auth, loadUserContext, async (req, res) => {
    try {
        ...
        const sprint = (await req.db.query(`SELECT s.*, t.org_id AS team_org_id
               FROM sprints s
               JOIN teams t ON t.id = s.team_id
              WHERE s.id = $1`, [id])).rows[0];
        if (!sprint)
            return res.status(404).json({ error: "Sprint not found" });
        if (sprint.team_org_id !== req.userOrgId)
            return res.status(404).json({ error: "Sprint not found" });
        if (!isOrgAdmin && sprint.team_id !== req.userTeamId)
            return res.status(403).json({ error: "Access denied" });
        const tasks = (await req.db.query(`SELECT t.id, t.title, t.status, t.workflow_state_id, t.work_item_type_id,
                    t.story_points, t.assigned_to, t.is_blocked, t.completed_at, t.priority
               FROM tasks t
              WHERE t.sprint_id = $1`, [id])).rows;
        // Pull workflow state metadata (category, terminal flag) and type metadata
        const stateIds = [...new Set(tasks.map((t) => t.workflow_state_id).filter(Boolean))];
        const typeIds = [...new Set(tasks.map((t) => t.work_item_type_id).filter(Boolean))];
        const assigneeIds = [...new Set(tasks.map((t) => t.assigned_to).filter(Boolean))];
        const [states, types, users] = await Promise.all([
            stateIds.length
                ? req.db.query("SELECT id, key, name, category, color, is_terminal FROM workflow_states WHERE id = ANY($1)", [stateIds]).then((r) => r.rows)
                : [],
            typeIds.length
                ? req.db.query("SELECT id, key, name, color, icon FROM work_item_types WHERE id = ANY($1)", [typeIds]).then((r) => r.rows)
                : [],
            assigneeIds.length
                ? req.db.query("SELECT id, username, full_name, avatar FROM users WHERE id = ANY($1)", [assigneeIds]).then((r) => r.rows)
                : [],
        ]);
        ...
        const totalPoints = tasks.reduce((sum, t) => sum + num(t.story_points), 0);
        const unestimatedTasks = tasks.filter((t) => t.story_points == null).length;
        ...
        const doneTasks = Object.values(byState)
            .filter((b) => b.state.is_terminal)
            .reduce((sum, b) => sum + b.taskCount, 0);
        // By work item type
        const byType = {};
        for (const t of tasks) {
            ...
            if (!byType[k])
                byType[k] = { type: wit, taskCount: 0, points: 0, done: 0 };
        ...
        res.json({
            ...
            byState: Object.values(byState).sort((a, b) => (a.state.sort_order ?? 0) - (b.state.sort_order ?? 0)),
...
router.post("/:id/start", auth, loadUserContext, requireRole("team_lead"), async (req, res) => {
    try {
        const { id } = req.params;
        const sprint = (await req.db.query("SELECT s.* FROM sprints s JOIN teams t ON t.id = s.team_id WHERE s.id = $1 AND t.org_id = $2", [id, req.userOrgId])).rows[0];
        if (!sprint)
        ...
        if (sprint.status === "completed")
            return res.status(400).json({ error: "Cannot restart a completed sprint" });
        // Auto-complete any other active sprint on the same team
        await req.db.query(`UPDATE sprints SET status = 'completed', completed_at = NOW()
              WHERE team_id = $1 AND status = 'active' AND id != $2`, [sprint.team_id, id]);
        await req.db.query("UPDATE sprints SET status = 'active', started_at = COALESCE(started_at, NOW()) WHERE id = $1", [id]);
...
router.post("/:id/complete", auth, loadUserContext, requireRole("team_lead"), async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const velocityRow = (await req.db.query(`SELECT COALESCE(SUM(t.story_points), 0)::float AS velocity,
                    COUNT(*) FILTER (WHERE ws.is_terminal) AS done_tasks,
                    COUNT(*) AS total_tasks
               FROM tasks t
               LEFT JOIN workflow_states ws ON ws.id = t.workflow_state_id
              WHERE t.sprint_id = $1`, [id])).rows[0];
        let rolledOver = 0;
        if (rolloverTo && rolloverTo !== "backlog") {
            const targetId = parseInt(rolloverTo, 10);
            if (isNaN(targetId))
                return res.status(400).json({ error: "Invalid rolloverTo sprint id" });
            const target = (await req.db.query("SELECT id, status FROM sprints WHERE id = $1 AND team_id = $2", [targetId, sprint.team_id])).rows[0];
            if (!target)
                return res.status(400).json({ error: "Target sprint not found" });
            if (target.status === "completed")
                return res.status(400).json({ error: "Cannot roll over to a completed sprint" });
            const r = await req.db.query(`UPDATE tasks SET sprint_id = $1
                  WHERE sprint_id = $2
                    AND COALESCE((SELECT is_terminal FROM workflow_states WHERE id = tasks.workflow_state_id), FALSE) = FALSE`, [targetId, id]);
            rolledOver = r.rowCount;
        }
        else {
            const r = await req.db.query(`UPDATE tasks SET sprint_id = NULL
                  WHERE sprint_id = $1
                    AND COALESCE((SELECT is_terminal FROM workflow_states WHERE id = tasks.workflow_state_id), FALSE) = FALSE`, [id]);
            rolledOver = r.rowCount;
        }
        await req.db.query(`UPDATE sprints
                SET status = 'completed',
                    completed_at = NOW(),
                    velocity_points = $1
              WHERE id = $2`, [velocityRow.velocity, id]);
        await redis.invalidateActiveSprint(req.tenantId, sprint.team_id);
        const updated = (await req.db.query("SELECT * FROM sprints WHERE id = $1", [id])).rows[0];
        res.json({
            sprint: updated,
...
router.get("/:id/burndown", auth, loadUserContext, async (req, res) => {
    try {
        const id = parseInt(String(req.params.id), 10);
        if (isNaN(id))
            return res.status(400).json({ error: "Invalid sprint id" });
        ...
        const sprint = (await req.db.query(`SELECT s.*, t.org_id AS team_org_id
               FROM sprints s JOIN teams t ON t.id = s.team_id WHERE s.id = $1`, [id])).rows[0];
        if (!sprint || sprint.team_org_id !== req.userOrgId)
            return res.status(404).json({ error: "Sprint not found" });
        if (!isOrgAdmin && sprint.team_id !== req.userTeamId)
            return res.status(403).json({ error: "Access denied" });
        ...
        const snapshots = (await req.db.query(`SELECT snapshot_date, total_points, done_points, remaining_points, blocked_points, open_tasks
               FROM sprint_burndown_snapshots
              WHERE sprint_id = $1
              ORDER BY snapshot_date ASC`, [id])).rows;
...
router.get("/velocity/recent", auth, loadUserContext, async (req, res) => {
    try {
        if (!req.userTeamId)
            return res.json({ sprints: [], average: 0 });
        const limit = Math.min(parseInt(String(req.query.limit), 10) || 6, 20);
        const rows = (await req.db.query(`SELECT id, name, start_date, end_date, completed_at,
                    COALESCE(velocity_points, 0)::float AS velocity_points
               FROM sprints
              WHERE team_id = $1 AND status = 'completed'
              ORDER BY completed_at DESC NULLS LAST
              LIMIT $2`, [req.userTeamId, limit])).rows;
        const series = rows.reverse(); // oldest → newest for charting
        const avg = series.length
            ? series.reduce((a, r) => a + Number(r.velocity_points), 0) / series.length
            : 0;
 ...
 */
async function loadAccessibleSprint(req, res, sprintId) {
    const sprint = (await req.db.query(`SELECT s.*, t.org_id, t.name AS team_name
           FROM sprints s JOIN teams t ON t.id = s.team_id
          WHERE s.id = $1`, [sprintId])).rows[0];
...
router.get("/:id/cumulative-flow", auth, loadUserContext, async (req, res) => {
    try {
        const id = parseInt(String(req.params.id), 10);
        if (isNaN(id))
            return res.status(400).json({ error: "Invalid sprint id" });
        ...
        const tasks = (await req.db.query(`SELECT t.id, t.created_at, t.completed_at,
        ...
        const history = (await req.db.query(`SELECT th.task_id, th.created_at, th.old_value, th.new_value
               FROM task_history th
              WHERE th.task_id = ANY($1) AND th.action = 'status_change'
              ORDER BY th.task_id, th.created_at ASC`, [taskIds])).rows;
        ...
        const stateRows = (await req.db.query("SELECT key, category FROM workflow_states WHERE org_id = $1", [req.userOrgId])).rows;
        const keyToCategory = {};
        for (const s of stateRows)
            keyToCategory[s.key] = s.category;
        // Default mappings for legacy tenants without seeded states.
        const fallback = { pending: "open", in_progress: "in_progress", in_review: "in_review", done: "done" };
        /** Reconstruct category for `task` on a given date (end of day). */
        const categoryOnDate = (task, date) => {
            const dateMs = new Date(date + "T23:59:59Z").getTime();
            // Task didn't exist yet
            if (new Date(task.created_at).getTime() > dateMs)
                return null;
            ...
            for (const ev of events) {
                if (new Date(ev.created_at).getTime() <= dateMs)
                    key = ev.new_value;
                else
                    break;
            }
        ...
        const series = days.map(date => {
            const counts = { date, open: 0, in_progress: 0, in_review: 0, done: 0 };
            for (const t of tasks) {
...
router.get("/:id/cycle-time", auth, loadUserContext, async (req, res) => {
    try {
        const id = parseInt(String(req.params.id), 10);
        if (isNaN(id))
            return res.status(400).json({ error: "Invalid sprint id" });
        ...
        if (!sprint)
            return;
        const rows = (await req.db.query(`SELECT t.id, t.title, t.story_points,
          ...
          LEFT JOIN work_item_types wit ON wit.id = t.work_item_type_id
              ...
              ORDER BY t.completed_at ASC`, [id])).rows.map((r) => ({
            id: r.id,
            title: r.title,
        ...
        const stats = (key) => {
            const vals = rows.map((r) => r[key]).filter((v) => v != null && v >= 0).sort((a, b) => a - b);
            if (vals.length === 0)
                return { avg: null, median: null, p90: null, n: 0 };
...
function normalizeActionItems(value) {
    if (!Array.isArray(value))
        return [];
    return value.slice(0, 50).map((it, i) => ({
        id: Number(it?.id) || (i + 1),
...
router.get("/:id/retrospective", auth, loadUserContext, async (req, res) => {
    try {
        const id = parseInt(String(req.params.id), 10);
        if (isNaN(id))
            return res.status(400).json({ error: "Invalid sprint id" });
        ...
        if (!sprint)
            return;
        const row = (await req.db.query(`SELECT r.*, c.full_name AS created_by_name, u.full_name AS updated_by_name
               FROM sprint_retrospectives r
          LEFT JOIN users c ON c.id = r.created_by
          LEFT JOIN users u ON u.id = r.updated_by
              WHERE r.sprint_id = $1`, [id])).rows[0];
...
router.put("/:id/retrospective", auth, loadUserContext, async (req, res) => {
    try {
        const id = parseInt(String(req.params.id), 10);
        if (isNaN(id))
            return res.status(400).json({ error: "Invalid sprint id" });
        ...
        await req.db.query(`INSERT INTO sprint_retrospectives
             ...
             ON CONFLICT (sprint_id) DO UPDATE SET
                ...
                updated_at  = NOW()`, [id, wentWell, toImprove, JSON.stringify(actionItems), teamMood, summary, req.userId]);
        const row = (await req.db.query(`SELECT r.*, c.full_name AS created_by_name, u.full_name AS updated_by_name
               FROM sprint_retrospectives r
          LEFT JOIN users c ON c.id = r.created_by
          LEFT JOIN users u ON u.id = r.updated_by
