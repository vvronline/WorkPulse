...
router.get("/branding", async (req, res) => {
    ...
    if (!slug)
        return res.json(EMPTY);
    try {
        const tenantRow = (await masterQuery(`SELECT id, db_name, db_host FROM tenants WHERE slug = $1 AND status = 'active'`, [slug])).rows[0];
        if (!tenantRow)
            return res.json(EMPTY);
...
router.get("/notes/:token", async (req, res) => {
    try {
        ...
        const tokenRow = (await masterQuery(`SELECT tenant_id, user_id, page_id, page_title, created_at
               FROM note_share_tokens WHERE token = $1`, [token])).rows[0];
        if (!tokenRow)
        ...
        const notebookRow = (await tenantDb.query(`SELECT data, updated_at FROM notebooks WHERE user_id = $1`, [tokenRow.user_id])).rows[0];
        if (!notebookRow)
            return res.status(404).json({ error: "This share link is no longer active." });
        let notebook;
        try {
            notebook = JSON.parse(notebookRow.data);
        }
        catch {
            notebook = null;
        }
        ...
        if (!page)
            return res.status(404).json({ error: "This page is no longer shared or has been archived." });
        ...
        let orgName = null;
        try {
            const userRow = (await tenantDb.query(`SELECT u.org_id, o.name AS org_name FROM users u
                  LEFT JOIN organizations o ON o.id = u.org_id
                  WHERE u.id = $1`, [tokenRow.user_id])).rows[0];
            orgName = userRow?.org_name || tenant.org_name || null;
            if (userRow?.org_id) {
                const b = (await tenantDb.query(`SELECT logo_url, accent_color FROM org_branding WHERE org_id = $1`, [userRow.org_id])).rows[0];
                if (b)
                    branding = { logo_url: b.logo_url || null, accent_color: b.accent_color || "#6366f1" };
            }
        }
