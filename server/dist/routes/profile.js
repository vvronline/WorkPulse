...
router.delete("/avatar", auth, async (req, res) => {
    ...
    if (user?.avatar) {
        try {
            await fsPromises.unlink(safeAvatarPath(user.avatar));
        }
        catch { /* ignore */ }
    }
...
router.put("/", auth, async (req, res) => {
    try {
        ...
        if (req.tenantId) {
            const dirCheck = await masterQuery("SELECT 1 FROM user_directory WHERE username = $1 AND user_id != $2", [username.toLowerCase(), req.userId]);
            if (dirCheck.rows[0])
                return res.status(400).json({ error: "Username already taken" });
        }
        ...
        if (req.tenantId) {
            await masterQuery("UPDATE user_directory SET username = $1 WHERE tenant_id = $2 AND user_id = $3", [username.toLowerCase(), req.tenantId, req.userId]);
        }
...
router.put("/email", auth, async (req, res) => {
    try {
        ...
        if (req.tenantId) {
            const dirCheck = await masterQuery("SELECT 1 FROM user_directory WHERE email = $1 AND user_id != $2", [email.toLowerCase(), req.userId]);
            if (dirCheck.rows[0])
                return res.status(400).json({ error: "Email already in use" });
        }
        ...
        if (req.tenantId && oldUser?.email) {
            await masterQuery("UPDATE user_directory SET email = $1 WHERE tenant_id = $2 AND user_id = $3", [email.toLowerCase(), req.tenantId, req.userId]);
        }
        res.json({ email });
    }
    catch (err) {
...
router.put("/password", auth, loadUserContext, async (req, res) => {
    try {
        ...
        if (req.tenantId)
            tokenPayload.tenant_id = req.tenantId;
        if (req.isPlatformUser)
            tokenPayload.platform = true;
...
async function ensureNotificationPrefsColumn(req) {
    if (req.db && _notificationPrefsColumnReady.has(req.db))
        return;
    try {
        await req.db.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS notification_prefs JSONB NOT NULL DEFAULT '{}'::jsonb`);
        if (req.db)
...
router.get("/notification-prefs", auth, async (req, res) => {
    try {
        ...
        const row = (await req.db.query("SELECT notification_prefs FROM users WHERE id = $1", [req.userId])).rows[0];
...
router.put("/notification-prefs", auth, async (req, res) => {
    try {
        ...
        const existing = (await req.db.query("SELECT notification_prefs FROM users WHERE id = $1", [req.userId])).rows[0]?.notification_prefs || {};
        const merged = { ...existing, ...prefs, v: 1 };
        await req.db.query("UPDATE users SET notification_prefs = $1::jsonb WHERE id = $2", [JSON.stringify(merged), req.userId]);
        res.json(merged);
    }
    catch (err) {
...
router.delete("/", auth, async (req, res) => {
    try {
        ...
        if (["super_admin", "hr_admin"].includes(user.role) && user.org_id) {
            const adminCount = (await req.db.query("SELECT COUNT(*) FROM users WHERE org_id = $1 AND role IN ('super_admin', 'hr_admin') AND id != $2 AND is_active = TRUE", [user.org_id, req.userId])).rows[0].count;
        ...
        await req.db.transaction(async (client) => {
            ...
            try {
                await client.query("DELETE FROM audit_logs WHERE actor_id = $1", [req.userId]);
            }
            catch { /* ignore */ }
...
router.get("/face-status", auth, async (req, res) => {
    try {
        const row = (await req.db.query("SELECT face_enrolled_at, (face_descriptor IS NOT NULL) AS enrolled FROM users WHERE id = $1", [req.userId])).rows[0];
        res.json({
...
router.post("/face-enroll", auth, async (req, res) => {
    try {
        ...
        await req.db.query(`UPDATE users SET face_descriptor = $1::jsonb, face_enrolled_at = NOW() WHERE id = $2`, [json, req.userId]);
...
router.delete("/face-enroll", auth, async (req, res) => {
    try {
        await req.db.query(`UPDATE users SET face_descriptor = NULL, face_enrolled_at = NULL WHERE id = $1`, [req.userId]);
