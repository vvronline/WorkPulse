...
async function resolveDefaultDomainUser(identifier) {
    ...
    const dirRes = await masterQuery("SELECT tenant_id, user_id FROM user_directory WHERE email = $1 OR username = $1", [identifier.toLowerCase()]);
    if (dirRes.rows[0]) {
        const { tenant_id, user_id } = dirRes.rows[0];
        const tdb = await getTenantDb(tenant_id);
        if (!tdb)
            return { error: "Organization is not available." };
        const userRes = await tdb.query("SELECT * FROM users WHERE id = $1", [user_id]);
        if (userRes.rows[0]) {
            // Check if this user is also a platform admin
            const platCheck = await masterQuery("SELECT 1 FROM platform_users WHERE LOWER(username) = $1 OR LOWER(email) = $1", [identifier.toLowerCase()]);
    ...
    const platRes = await masterQuery("SELECT * FROM platform_users WHERE username = $1 OR email = $1", [identifier]);
    if (platRes.rows[0]) {
...
async function createSession(userId, deviceInfo, db, tenantId) {
    ...
    const sessRes = await db.query("SELECT id FROM user_sessions WHERE user_id = $1 ORDER BY created_at ASC", [userId]);
...
router.post("/register", async (req, res) => {
    try {
            ...
            if (tenant_slug) {
                ...
                const bootstrapResult = await masterTransaction(async (client) => {
                    ...
                    if (parseInt(platCount) === 0 && parseInt(tenantCount) === 0) {
                        ...
                        const result = await client.query("INSERT INTO platform_users (username, password, full_name, email) VALUES ($1,$2,$3,$4) RETURNING id", [username, hash, full_name, email]);
                ...
                if (bootstrapResult) {
                    ...
                    const token = jwt.sign({ id: bootstrapResult.id, username, tv: 0, sid, tenant_id: null, platform: true }, process.env.JWT_SECRET, { expiresIn: "8h" });
        ...
        if (mode === "invite_only") {
            ...
            const invRes = await db.query("SELECT * FROM invite_codes WHERE code = $1 AND is_active = TRUE", [invite_code]);
        ...
        const result = await db.transaction(async (client) => {
            if (inviteRow) {
                const fresh = await client.query("SELECT used_count, max_uses FROM invite_codes WHERE id = $1 AND is_active = TRUE FOR UPDATE", [inviteRow.id]);
                const f = fresh.rows[0];
                if (!f || (f.max_uses > 0 && f.used_count >= f.max_uses)) {
                    throw new Error("INVITE_EXHAUSTED");
                }
                await client.query("UPDATE invite_codes SET used_count = used_count + 1 WHERE id = $1", [inviteRow.id]);
            }
            ...
            const ins = await client.query("INSERT INTO users (username, password, full_name, email, org_id, role) VALUES ($1,$2,$3,$4,$5,$6) RETURNING id, role", [username, hash, full_name, email, assignedOrgId, finalRole]);
            return ins.rows[0];
        });
        // Add to user_directory in master DB (retry once on failure — cross-DB so can't share transaction)
        try {
            await masterQuery("INSERT INTO user_directory (email, username, tenant_id, user_id) VALUES ($1, $2, $3, $4) ON CONFLICT DO NOTHING", [email.toLowerCase(), username.toLowerCase(), tenantId, result.id]);
        }
        catch (dirErr) {
        ...
        const token = jwt.sign({ id: result.id, username, tv: 0, sid, tenant_id: tenantId }, process.env.JWT_SECRET, { expiresIn: "8h" });
        res.cookie("token", token, cookieOptions(req));
 ...
 */
async function finishLogin(req, res, { user, db, tenantId, isPlatformUser }) {
    const sid = await createSession(user.id, req.headers["user-agent"], db, tenantId);
    const token = jwt.sign({ id: user.id, username: user.username, tv: user.token_version || 0, sid, tenant_id: tenantId, platform: isPlatformUser || undefined }, process.env.JWT_SECRET, { expiresIn: "8h" });
    res.cookie("token", token, cookieOptions(req));
    if (isPlatformUser) {
        ...
        if (tenantId) {
            ...
            const reportsRes = await db.query("SELECT 1 FROM users WHERE manager_id = $1 AND is_active = TRUE LIMIT 1", [user.id]);
            return res.json({
                user: {
                    id: user.id,
        ...
        const tenantsRes = await masterQuery("SELECT * FROM tenants WHERE status = $1 ORDER BY id LIMIT 1", ["active"]);
        ...
        if (primaryTenant) {
            ...
            let tenantUser = (await tenantDb.query("SELECT * FROM users WHERE username = $1 OR email = $2", [user.username, user.email || ""])).rows[0];
            if (!tenantUser) {
                // Create a platform_admin user in the tenant DB
                tenantUser = (await tenantDb.query(`INSERT INTO users (username, password, full_name, email, org_id, role)
                     VALUES ($1, $2, $3, $4, 1, 'platform_admin') RETURNING *`, [user.username, user.password, user.full_name, user.email || `${user.username}@platform.local`])).rows[0];
                // Register in user_directory for future logins
                if (user.email) {
                    await masterQuery("INSERT INTO user_directory (email, username, tenant_id, user_id) VALUES ($1, $2, $3, $4) ON CONFLICT DO NOTHING", [user.email.toLowerCase(), user.username.toLowerCase(), primaryTenant.id, tenantUser.id]);
                }
            }
            ...
            const tenantToken = jwt.sign({ id: tenantUser.id, username: tenantUser.username, tv: tenantUser.token_version || 0, sid: tenantSid, tenant_id: primaryTenant.id, platform: true }, process.env.JWT_SECRET, { expiresIn: "8h" });
            res.cookie("token", tenantToken, cookieOptions(req));
            const reportsRes = await tenantDb.query("SELECT 1 FROM users WHERE manager_id = $1 AND is_active = TRUE LIMIT 1", [tenantUser.id]);
            return res.json({
                user: {
                    id: tenantUser.id,
        ...
        try {
            ...
            try {
                await masterQuery(`UPDATE tenants SET is_default = TRUE WHERE id = $1`, [newTenant.id]);
        ...
        const tenantUser = (await tenantDb.query(`INSERT INTO users (username, password, full_name, email, org_id, role)
             VALUES ($1, $2, $3, $4, 1, 'platform_admin') RETURNING *`, [user.username, user.password, user.full_name, user.email || `${user.username}@platform.local`])).rows[0];
        if (user.email) {
            await masterQuery("INSERT INTO user_directory (email, username, tenant_id, user_id) VALUES ($1, $2, $3, $4) ON CONFLICT DO NOTHING", [user.email.toLowerCase(), user.username.toLowerCase(), newTenant.id, tenantUser.id]);
        }
        const newSid = await createSession(tenantUser.id, req.headers["user-agent"], tenantDb, newTenant.id);
        const newToken = jwt.sign({ id: tenantUser.id, username: tenantUser.username, tv: 0, sid: newSid, tenant_id: newTenant.id, platform: true }, process.env.JWT_SECRET, { expiresIn: "8h" });
...
router.post("/login", async (req, res) => {
    try {
        ...
        if (req.tenant) {
            ...
            const userRes = await db.query("SELECT * FROM users WHERE username = $1 OR email = $1", [username]);
            user = userRes.rows[0];
        }
        else {
...
router.post("/forgot-password", async (req, res) => {
    try {
        ...
        if (!req.tenant) {
            ...
            const dirRes = await masterQuery("SELECT ud.tenant_id, ud.user_id, t.slug, t.db_name, t.db_host FROM user_directory ud JOIN tenants t ON t.id = ud.tenant_id WHERE ud.email = $1", [email.toLowerCase()]);
            if (dirRes.rows[0]) {
        ...
        await db.query("INSERT INTO password_reset_tokens (user_id, token, expires_at) VALUES ($1, $2, $3)", [user.id, tokenHash, expiresAt]);
...
router.post("/reset-password", async (req, res) => {
    try {
        ...
        const rowRes = await db.query(`SELECT prt.id, prt.user_id, prt.expires_at, prt.used, u.username
             FROM password_reset_tokens prt
             JOIN users u ON u.id = prt.user_id
             WHERE prt.token = $1`, [tokenHash]);
        ...
        const hash = await bcrypt.hash(password, BCRYPT_ROUNDS);
        await db.query("UPDATE users SET password = $1, token_version = COALESCE(token_version, 0) + 1 WHERE id = $2", [hash, row.user_id]);
...
router.post("/refresh", auth, async (req, res) => {
    try {
        ...
        if (req.isImpersonated) {
            ...
            if (req.impersonatedTenantName)
                claims.impersonated_tenant_name = req.impersonatedTenantName;
        }
    ...
    try {
        ...
        if (token) {
            ...
            if (decoded.id) {
                ...
                await statusService.closeAllSessions({ db: req.db, tenantId: decoded.tenant_id || null }, decoded.id, { source: "logout" }).catch((err) => logger.warn({ err: err.message, userId: decoded.id }, "logout: statusService.closeAllSessions failed"));
            }
        }
    }
...
router.post("/device-token", auth, async (req, res) => {
    try {
        ...
        await db.query(`INSERT INTO device_tokens (user_id, tenant_id, device_token, platform, last_seen_at, created_at)
             VALUES ($1, $2, $3, $4, NOW(), NOW())
             ON CONFLICT (user_id, device_token) DO UPDATE
             SET platform = EXCLUDED.platform, last_seen_at = NOW()`, [userId, tenantId || null, deviceToken, platform]);
