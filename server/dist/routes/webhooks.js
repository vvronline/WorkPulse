...
async function getDbForIntegration(integrationId) {
    ...
    const tenants = (await masterQuery("SELECT id, slug, db_name, db_host FROM tenants WHERE status != 'deleted'")).rows;
    for (const tenant of tenants) {
        try {
            const pool = await getTenantPool(tenant.db_name, tenant.db_host);
            const exists = (await pool.query("SELECT id, org_id FROM org_integrations WHERE id = $1", [integrationId])).rows[0];
            if (exists)
                return { db: pool, orgId: exists.org_id, tenantId: tenant.id };
        }
        catch (e) {
    ...
    const db = require("../db");
    const exists = (await db.masterQuery("SELECT id, org_id FROM org_integrations WHERE id = $1", [integrationId])).rows[0];
    if (exists)
        return { db: { query: db.masterQuery, transaction: db.masterTransaction }, orgId: exists.org_id, tenantId: null };
    return null;
}
router.post("/github/:integrationId", express_1.default.raw({ type: "*/*", limit: "5mb" }), async (req, res) => {
    try {
        const integrationId = parseInt(String(req.params.integrationId), 10);
        if (isNaN(integrationId))
            return res.status(400).send("Invalid integration id");
        ...
        if (tenantId) {
            const tenantRow = (await masterQuery("SELECT plan, features, status FROM tenants WHERE id = $1", [tenantId])).rows[0];
        ...
        const secretRow = (await db.query("SELECT webhook_secret FROM org_integration_secrets WHERE integration_id = $1", [integrationId])).rows[0];
        ...
        if (refsToUpsert.length === 0)
            return res.json({ ok: true, linked: 0 });
        ...
        const keyToTaskId = new Map(resolvedKeys.map((r) => [r.key, r.task_id]));
        let linked = 0;
        for (const r of refsToUpsert) {
            const taskId = keyToTaskId.get(r.raw);
            if (!taskId)
                continue;
            await db.query(`INSERT INTO task_git_refs
                 ...
                 DO UPDATE SET status = EXCLUDED.status, title = EXCLUDED.title, url = EXCLUDED.url,
                               ...
                               event_at = NOW(), updated_at = NOW()`, [taskId, integrationId, r.ref_type, r.status, r.external_id, r.title, r.url,
