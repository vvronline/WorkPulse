...
router.get("/config", async (req, res) => {
    try {
        ...
        const types = (await req.db.query(`SELECT id, key, name, icon, color, description, is_default, is_epic, is_active, sort_order
               FROM work_item_types
              WHERE org_id = $1 AND is_active = TRUE
              ORDER BY sort_order ASC, id ASC`, [req.userOrgId])).rows;
        const states = (await req.db.query(`SELECT id, key, name, category, color, icon, wip_limit, is_initial, is_terminal, is_active, sort_order
...
router.put("/settings", requireAgileEditor, async (req, res) => {
    try {
        ...
        if (b.priority_scheme !== undefined) {
            ...
            const cleaned = b.priority_scheme.slice(0, 10).map((pr) => ({
                key: String(pr.key || slugifyKey(pr.label)).slice(0, 30),
                label: String(pr.label || pr.key || "Priority").slice(0, 30),
                color: validateColor(pr.color),
            }));
        ...
        await req.db.query(`UPDATE org_agile_settings SET ${updates.join(", ")} WHERE org_id = $${p}`, params);
        const row = await loadOrgSettings(req.db, req.userOrgId);
...
router.get("/work-item-types", async (req, res) => {
    try {
        if (!req.userOrgId)
            return res.json([]);
        const r = await req.db.query(`SELECT id, key, name, icon, color, description, is_default, is_epic, is_active, sort_order, created_at
               FROM work_item_types
              WHERE org_id = $1
              ORDER BY sort_order ASC, id ASC`, [req.userOrgId]);
        res.json(r.rows);
    }
    catch (err) {
...
router.post("/work-item-types", requireAgileEditor, async (req, res) => {
    try {
        ...
        if (exists.rows[0])
            return res.status(409).json({ error: "A work item type with this key already exists" });
        ...
        await req.db.transaction(async (client) => {
            ...
            await client.query(`INSERT INTO work_item_types (org_id, key, name, icon, color, description, is_epic, is_default, sort_order)
...
router.put("/work-item-types/:id", requireAgileEditor, async (req, res) => {
    try {
        ...
        if (!existing || existing.org_id !== req.userOrgId)
        ...
        if (b.name !== undefined)
            set("name", String(b.name).trim().slice(0, 50));
        if (b.icon !== undefined)
            set("icon", b.icon ? String(b.icon).slice(0, 40) : null);
        if (b.color !== undefined)
            set("color", validateColor(b.color, existing.color));
        if (b.description !== undefined)
            set("description", b.description ? String(b.description).slice(0, 500) : null);
        if (b.is_epic !== undefined)
            set("is_epic", !!b.is_epic);
        if (b.is_active !== undefined)
            set("is_active", !!b.is_active);
        if (b.sort_order !== undefined)
            set("sort_order", parseInt(b.sort_order, 10) || 0);
        await req.db.transaction(async (client) => {
...
router.delete("/work-item-types/:id", requireAgileEditor, async (req, res) => {
    try {
        ...
        if (existing.is_default) {
            const otherDefault = (await req.db.query("SELECT id FROM work_item_types WHERE org_id = $1 AND id != $2 AND is_active = TRUE LIMIT 1", [req.userOrgId, id])).rows[0];
            if (!otherDefault)
                return res.status(409).json({ error: "Cannot delete the only active work item type. Add another first." });
        }
...
router.get("/workflow-states", async (req, res) => {
    try {
        if (!req.userOrgId)
            return res.json([]);
        const r = await req.db.query(`SELECT id, key, name, category, color, icon, wip_limit, is_initial, is_terminal, is_active, sort_order, created_at
               FROM workflow_states
              WHERE org_id = $1
              ORDER BY sort_order ASC, id ASC`, [req.userOrgId]);
        res.json(r.rows);
...
async function assertCategoriesCovered(db, orgId, opts = {}) {
    ...
    const rows = (await db.query(`SELECT category FROM workflow_states
          WHERE org_id = $1 AND is_active = TRUE AND id != COALESCE($2, -1) AND id != COALESCE($3, -1)`, [orgId, excludeId, makingInactiveId])).rows;
...
router.post("/workflow-states", requireAgileEditor, async (req, res) => {
    try {
        ...
        if (exists.rows[0])
            return res.status(409).json({ error: "A workflow state with this key already exists" });
        ...
        await req.db.transaction(async (client) => {
            ...
            await client.query(`INSERT INTO workflow_states (org_id, key, name, category, color, icon, wip_limit, is_initial, is_terminal, sort_order)
...
router.put("/workflow-states/:id", requireAgileEditor, async (req, res) => {
    try {
        ...
        const set = (col, val) => { updates.push(`${col} = $${p++}`); params.push(val); };
        if (b.name !== undefined)
            set("name", String(b.name).trim().slice(0, 40));
        ...
        if (b.icon !== undefined)
            set("icon", b.icon ? String(b.icon).slice(0, 40) : null);
        if (b.wip_limit !== undefined)
            set("wip_limit", b.wip_limit != null ? (parseInt(b.wip_limit, 10) || null) : null);
        if (b.is_terminal !== undefined)
            set("is_terminal", !!b.is_terminal);
        if (b.sort_order !== undefined)
            set("sort_order", parseInt(b.sort_order, 10) || 0);
        ...
        if (b.is_active === false) {
            ...
            try {
                ...
                if (e.code === "CATEGORY_MISSING")
                    return res.status(409).json({ error: e.message, missing: e.missing });
                throw e;
            }
            set("is_active", false);
        }
        ...
        if (b.category !== undefined && b.category !== existing.category) {
            try {
                ...
                const others = (await req.db.query(`SELECT 1 FROM workflow_states
                      WHERE org_id = $1 AND is_active = TRUE AND category = $2 AND id != $3
                      LIMIT 1`, [req.userOrgId, existing.category, id])).rows;
                if (others.length === 0) {
                    return res.status(409).json({
...
router.get("/permissions/me", async (req, res) => {
    try {
        ...
        if (!canEdit && req.userOrgId) {
            const r = await req.db.query(`SELECT status FROM agile_editor_requests
                  WHERE org_id = $1 AND user_id = $2
                  ORDER BY created_at DESC LIMIT 1`, [req.userOrgId, req.userId]);
            if (r.rows[0])
                requestStatus = r.rows[0].status;
        }
...
router.post("/permissions/request", async (req, res) => {
    try {
        ...
        if (await isAgileEditor(req))
            return res.status(400).json({ error: "You already have edit access" });
        // Block if there's already a pending request
        const pending = await req.db.query(`SELECT id FROM agile_editor_requests
              WHERE org_id = $1 AND user_id = $2 AND status = 'pending' LIMIT 1`, [req.userOrgId, req.userId]);
        if (pending.rows[0])
        ...
        const ins = await req.db.query(`INSERT INTO agile_editor_requests (org_id, user_id, reason)
             VALUES ($1, $2, $3) RETURNING *`, [req.userOrgId, req.userId, reason]);
        // Notify all super_admins in the org
        try {
            const admins = (await req.db.query(`SELECT id FROM users WHERE org_id = $1 AND role = 'super_admin' AND is_active = TRUE`, [req.userOrgId])).rows;
            ...
            for (const a of admins) {
                await req.db.query(`INSERT INTO notifications (user_id, type, title, body) VALUES ($1, 'agile_request', $2, $3)`, [a.id, "Agile editor access requested", `${name} is requesting Agile settings edit access`]);
            }
        }
        catch (e) {
            req.log.warn({ err: e }, "Failed to notify admins of agile request");
        }
...
router.post("/permissions/cancel-request", async (req, res) => {
    try {
        if (!req.userOrgId)
            return res.status(400).json({ error: "Organization context required" });
        const r = await req.db.query(`UPDATE agile_editor_requests
                SET status = 'cancelled', reviewed_at = NOW()
              WHERE org_id = $1 AND user_id = $2 AND status = 'pending'`, [req.userOrgId, req.userId]);
...
router.put("/permissions/requests/:id", requireAgileReviewer, async (req, res) => {
    try {
        ...
        if (!reqRow || reqRow.org_id !== req.userOrgId)
            return res.status(404).json({ error: "Request not found" });
        if (reqRow.status !== "pending")
            return res.status(400).json({ error: "Request already reviewed" });
        if (action === "approve") {
            await req.db.transaction(async (client) => {
                await client.query(`UPDATE agile_editor_requests
                        SET status = 'approved', reviewed_by = $1, reviewed_at = NOW()
                      WHERE id = $2`, [req.userId, id]);
                // Insert (or reactivate) grant
                await client.query(`INSERT INTO agile_editor_grants (org_id, user_id, granted_by)
                     VALUES ($1, $2, $3)
                     ON CONFLICT (org_id, user_id) DO UPDATE
                        SET revoked_at = NULL, granted_by = EXCLUDED.granted_by, granted_at = NOW()`, [reqRow.org_id, reqRow.user_id, req.userId]);
            });
            // Notify requester
            await req.db.query(`INSERT INTO notifications (user_id, type, title, body)
                 VALUES ($1, 'agile_grant', 'Agile editor access granted', 'You can now edit Agile settings.')`, [reqRow.user_id]);
            logAction(req, "approve", "agile_editor_request", id, { user_id: reqRow.user_id });
        }
        else {
            await req.db.query(`UPDATE agile_editor_requests
                    SET status = 'rejected', reviewed_by = $1, reviewed_at = NOW(), reject_reason = $2
                  WHERE id = $3`, [req.userId, reject_reason ? String(reject_reason).slice(0, 500) : null, id]);
            await req.db.query(`INSERT INTO notifications (user_id, type, title, body)
                 VALUES ($1, 'agile_grant', 'Agile editor request rejected', $2)`, [reqRow.user_id, reject_reason ? `Reason: ${reject_reason}` : "Your request was rejected."]);
...
router.get("/permissions/grants", requireAgileReviewer, async (req, res) => {
    try {
        if (!req.userOrgId)
            return res.json([]);
        const r = await req.db.query(`SELECT g.*, u.username, u.full_name, u.avatar, u.email,
          ...
          LEFT JOIN users gb ON gb.id = g.granted_by
              ...
              ORDER BY g.granted_at DESC`, [req.userOrgId]);
        res.json(r.rows);
    }
    catch (err) {
...
router.delete("/permissions/grants/:id", requireAgileReviewer, async (req, res) => {
    try {
        ...
        if (!grant || grant.org_id !== req.userOrgId)
            return res.status(404).json({ error: "Grant not found" });
        await req.db.query("UPDATE agile_editor_grants SET revoked_at = NOW() WHERE id = $1", [id]);
        await req.db.query(`INSERT INTO notifications (user_id, type, title, body)
             VALUES ($1, 'agile_grant', 'Agile editor access revoked', 'Your edit access for Agile settings was revoked.')`, [grant.user_id]);
