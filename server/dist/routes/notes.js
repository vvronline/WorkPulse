...
async function writeHistory(userId, page, client) {
    ...
    await q("INSERT INTO notebook_history (user_id, page_id, page_title, content) VALUES ($1, $2, $3, $4)", [userId, page.id, page.title || "Untitled", page.content || ""]);
    const oldest = (await q("SELECT id FROM notebook_history WHERE user_id = $1 AND page_id = $2 ORDER BY saved_at DESC LIMIT ALL OFFSET $3", [userId, page.id, MAX_HISTORY])).rows;
    if (oldest.length > 0) {
...
router.put("/", async (req, res) => {
    try {
        ...
        const oldMap = {};
        if (old?.pages)
            old.pages.forEach((p) => { oldMap[p.id] = p; });
        ...
        await req.db.transaction(async (client) => {
            ...
            await client.query(`INSERT INTO notebooks (user_id, data, updated_at) VALUES ($1, $2, NOW())
                 ON CONFLICT(user_id) DO UPDATE SET data = EXCLUDED.data, updated_at = NOW()`, [req.userId, JSON.stringify(data)]);
        });
        res.json({ ok: true });
    }
    catch (e) {
...
router.get("/history/:pageId", async (req, res) => {
    try {
        const rows = (await req.db.query("SELECT id, page_title, saved_at FROM notebook_history WHERE user_id = $1 AND page_id = $2 ORDER BY saved_at DESC LIMIT 50", [req.userId, req.params.pageId])).rows;
...
router.get("/history/snapshot/:id", async (req, res) => {
    try {
        const row = (await req.db.query("SELECT id, page_id, page_title, content, saved_at FROM notebook_history WHERE id = $1 AND user_id = $2", [req.params.id, req.userId])).rows[0];
        if (!row)
...
router.get("/mentionable-users", async (req, res) => {
    try {
        ...
        const rows = (await req.db.query(`SELECT id, full_name, avatar, username FROM users
             WHERE org_id = $1 AND is_active = TRUE AND id != $2
             ORDER BY full_name`, [user.org_id, req.userId])).rows;
        res.json({ users: rows });
    }
    catch (e) {
...
router.post("/links", async (req, res) => {
    try {
        ...
        await req.db.query(`INSERT INTO note_links (user_id, page_id, entity_type, entity_id)
             VALUES ($1, $2, $3, $4)
             ON CONFLICT (page_id, entity_type, entity_id) DO NOTHING`, [req.userId, String(pageId), entityType, eid]);
        res.json({ ok: true });
    }
    catch (e) {
...
router.delete("/links", async (req, res) => {
    try {
        ...
        if (!eid || eid <= 0)
            return res.status(400).json({ error: "Invalid entityId" });
        await req.db.query(`DELETE FROM note_links WHERE page_id = $1 AND entity_type = $2 AND entity_id = $3 AND user_id = $4`, [String(pageId), entityType, eid, req.userId]);
        res.json({ ok: true });
    }
    catch (e) {
...
router.get("/links/:pageId", async (req, res) => {
    try {
        ...
        const links = (await req.db.query(`SELECT id, entity_type, entity_id, created_at FROM note_links
             WHERE page_id = $1 AND user_id = $2 ORDER BY created_at DESC`, [pageId, req.userId])).rows;
        ...
        for (const link of links) {
            ...
            try {
                if (link.entity_type === "task") {
                    const r = (await req.db.query(`SELECT id, title, status, priority, date, due_date, sprint_id FROM tasks WHERE id = $1`, [link.entity_id])).rows[0];
                    detail = r || null;
                }
                else if (link.entity_type === "calendar_event") {
                    const r = (await req.db.query(`SELECT id, title, description, start_time, end_time, all_day, color FROM calendar_events WHERE id = $1`, [link.entity_id])).rows[0];
                    detail = r || null;
                }
                else if (link.entity_type === "meeting") {
                    const r = (await req.db.query(`SELECT id, title, scheduled_start, scheduled_end, meeting_code, status FROM meetings WHERE id = $1`, [link.entity_id])).rows[0];
                    detail = r || null;
                }
            }
...
router.get("/daily-prefill", async (req, res) => {
    try {
        ...
        const tasks = (await req.db.query(`SELECT id, title, status, priority FROM tasks
             WHERE (user_id = $1 OR assigned_to = $1)
               AND (date = $2 OR (completed_at >= $3 AND completed_at <= $4))
             ORDER BY completed_at DESC NULLS LAST, created_at DESC
             LIMIT 30`, [req.userId, dateStr, startOfDay.toISOString(), endOfDay.toISOString()])).rows;
        // Time tracked today
        const timeEntries = (await req.db.query(`SELECT entry_type, timestamp, work_mode FROM time_entries
             WHERE user_id = $1 AND timestamp >= $2 AND timestamp <= $3
               AND (is_manual = FALSE OR approval_status = 'approved')
             ORDER BY timestamp ASC`, [req.userId, startOfDay.toISOString(), endOfDay.toISOString()])).rows;
        ...
        for (const e of timeEntries) {
            if (e.entry_type === "clock_in")
                ...
                if (clockIn)
                    totalMinutes += (new Date(e.timestamp).getTime() - clockIn.getTime()) / 60000;
        ...
        if (clockIn)
            totalMinutes += (Date.now() - clockIn.getTime()) / 60000;
        void breakStart;
        const hoursWorked = Math.round(totalMinutes / 6) / 10; // round to 1 decimal
        // Meetings attended today
        const meetings = (await req.db.query(`SELECT m.id, m.title, m.scheduled_start, m.scheduled_end, m.meeting_code
             FROM meetings m
             JOIN meeting_participants mp ON mp.meeting_id = m.id
             WHERE mp.user_id = $1
               AND m.scheduled_start >= $2 AND m.scheduled_start <= $3
             ORDER BY m.scheduled_start ASC`, [req.userId, startOfDay.toISOString(), endOfDay.toISOString()])).rows;
        // Calendar events today
        const events = (await req.db.query(`SELECT id, title, start_time, end_time, all_day FROM calendar_events
             WHERE user_id = $1 AND start_time >= $2 AND start_time <= $3
             ORDER BY start_time ASC`, [req.userId, startOfDay.toISOString(), endOfDay.toISOString()])).rows;
...
router.get("/oneonone-prefill/:userId", async (req, res) => {
    try {
        const reportId = parseInt(String(req.params.userId), 10);
        if (!reportId || reportId <= 0)
            return res.status(400).json({ error: "Invalid userId" });
        // Verify the requesting user is the report's manager (or higher)
        const report = (await req.db.query(`SELECT id, full_name, team_id, manager_id FROM users WHERE id = $1`, [reportId])).rows[0];
        if (!report)
            return res.status(404).json({ error: "User not found" });
        // Allow if requester is the manager, or has a management role in same org
        const requester = (await req.db.query(`SELECT role, org_id FROM users WHERE id = $1`, [req.userId])).rows[0];
        ...
        const tasks = (await req.db.query(`SELECT id, title, status, priority, date, completed_at FROM tasks
             WHERE (user_id = $1 OR assigned_to = $1)
               AND created_at >= $2
             ORDER BY created_at DESC LIMIT 20`, [reportId, twoWeeksAgo.toISOString()])).rows;
        // Recent leaves
        const leaves = (await req.db.query(`SELECT date, leave_type, duration, status FROM leaves
             WHERE user_id = $1 AND date >= $2
             ORDER BY date DESC LIMIT 10`, [reportId, twoWeeksAgo.toISOString().split("T")[0]])).rows;
        // Sprint progress (if in a team with active sprint)
        let sprint = null;
        if (report.team_id) {
            const activeSprint = (await req.db.query(`SELECT id, name, start_date, end_date, goal FROM sprints
                 WHERE team_id = $1 AND status = 'active' LIMIT 1`, [report.team_id])).rows[0];
            if (activeSprint) {
                const sprintTasks = (await req.db.query(`SELECT status, COUNT(*)::int AS count FROM tasks
                     WHERE sprint_id = $1 AND (user_id = $2 OR assigned_to = $2)
                     GROUP BY status`, [activeSprint.id, reportId])).rows;
        ...
        const weekEntries = (await req.db.query(`SELECT entry_type, timestamp FROM time_entries
             WHERE user_id = $1 AND timestamp >= $2
               AND (is_manual = FALSE OR approval_status = 'approved')
             ORDER BY timestamp ASC`, [reportId, weekStart.toISOString()])).rows;
        let weekMinutes = 0;
        let ci = null;
        for (const e of weekEntries) {
            if (e.entry_type === "clock_in")
                ci = new Date(e.timestamp);
            else if (e.entry_type === "break_start") {
                if (ci)
                    weekMinutes += (new Date(e.timestamp).getTime() - ci.getTime()) / 60000;
                ci = null;
            }
            else if (e.entry_type === "break_end")
                ci = new Date(e.timestamp);
...
router.get("/time-summary", async (req, res) => {
    try {
        ...
        const endOfDay = new Date(today);
        endOfDay.setHours(23, 59, 59, 999);
        const entries = (await req.db.query(`SELECT entry_type, timestamp, work_mode FROM time_entries
             WHERE user_id = $1 AND timestamp >= $2 AND timestamp <= $3
               AND (is_manual = FALSE OR approval_status = 'approved')
             ORDER BY timestamp ASC`, [req.userId, startOfDay.toISOString(), endOfDay.toISOString()])).rows;
        let totalMinutes = 0;
        ...
        for (const e of entries) {
            if (e.entry_type === "clock_in") {
                ...
                if (breakStart)
                    breakMinutes += (new Date(e.timestamp).getTime() - breakStart.getTime()) / 60000;
                clockIn = new Date(e.timestamp);
                breakStart = null;
            }
            else if (e.entry_type === "clock_out") {
...
router.post("/convert-to-task", async (req, res) => {
    try {
        ...
        let sprintId = null;
        if (userRow?.team_id) {
            const sprint = (await req.db.query(`SELECT id FROM sprints WHERE team_id = $1 AND status = 'active' LIMIT 1`, [userRow.team_id])).rows[0];
            if (sprint)
                sprintId = sprint.id;
        }
        const result = await req.db.query(`INSERT INTO tasks (user_id, org_id, title, date, status, priority, sprint_id)
             VALUES ($1, $2, $3, $4, 'pending', 'medium', $5)
             RETURNING id, title, date, status, priority, sprint_id`, [req.userId, userRow?.org_id || null, title.trim(), dateStr, sprintId]);
        const task = result.rows[0];
        // Also link the task to the note page if pageId is provided
        if (pageId) {
            await req.db.query(`INSERT INTO note_links (user_id, page_id, entity_type, entity_id)
                 VALUES ($1, $2, 'task', $3)
                 ON CONFLICT (page_id, entity_type, entity_id) DO NOTHING`, [req.userId, String(pageId), task.id]);
        }
        res.json({ task });
    }
    catch (e) {
...
router.get("/sprint-embed", async (req, res) => {
    try {
        ...
        if (!userRow?.team_id)
            return res.json({ sprint: null, tasks: [] });
        const sprint = (await req.db.query(`SELECT id, name, goal, status, start_date, end_date FROM sprints
             WHERE team_id = $1 AND status = 'active' LIMIT 1`, [userRow.team_id])).rows[0];
        if (!sprint)
            return res.json({ sprint: null, tasks: [] });
        ...
        const tasks = (await req.db.query(`SELECT t.id, t.title, t.status, t.priority, t.assigned_to,
             ...
             ORDER BY CASE t.priority
                          ...
                          ELSE 4
                      END,
                      t.created_at ASC`, [sprint.id])).rows;
...
router.get("/search-tasks", async (req, res) => {
    try {
        ...
        if (!q || !q.trim()) {
            ...
            rows = (await req.db.query(`SELECT id, title, status, priority, date FROM tasks
                 WHERE (user_id = $1 OR assigned_to = $1${orgId ? " OR org_id = $2" : ""})
                 ORDER BY created_at DESC LIMIT 20`, orgId ? [req.userId, orgId] : [req.userId])).rows;
        }
        else {
            ...
            rows = (await req.db.query(`SELECT id, title, status, priority, date FROM tasks
                 WHERE (user_id = $1 OR assigned_to = $1${orgId ? " OR org_id = $3" : ""})
                   AND title ILIKE $2
                 ORDER BY created_at DESC LIMIT 20`, orgId ? [req.userId, ilikePat, orgId] : [req.userId, ilikePat])).rows;
        }
        res.json({ tasks: rows });
    }
    catch (e) {
...
router.get("/search-meetings", async (req, res) => {
    try {
        ...
        const rows = (await req.db.query(`SELECT m.id, m.title, m.scheduled_start, m.scheduled_end, m.meeting_code, m.status
             FROM meetings m
             JOIN meeting_participants mp ON mp.meeting_id = m.id
             WHERE mp.user_id = $1 AND m.title ILIKE $2
             ORDER BY m.scheduled_start DESC LIMIT 15`, [req.userId, ilikePat])).rows;
...
router.get("/search-events", async (req, res) => {
    try {
        const { q } = req.query;
        const ilikePat = `%${(q || "").replace(/[\\%_]/g, (c) => `\\${c}`)}%`;
        const rows = (await req.db.query(`SELECT id, title, start_time, end_time, all_day FROM calendar_events
             WHERE user_id = $1 AND title ILIKE $2
             ORDER BY start_time DESC LIMIT 15`, [req.userId, ilikePat])).rows;
...
router.get("/direct-reports", async (req, res) => {
    try {
        const reports = (await req.db.query(`SELECT id, full_name, avatar, username FROM users
             WHERE manager_id = $1 AND is_active = TRUE
             ORDER BY full_name`, [req.userId])).rows;
        res.json({ reports });
    }
    catch (e) {
...
router.get("/share/:pageId", async (req, res) => {
    try {
        ...
        const row = (await masterQuery(`SELECT token, page_title, created_at FROM note_share_tokens
              WHERE tenant_id = $1 AND user_id = $2 AND page_id = $3`, [req.tenantId, req.userId, pageId])).rows[0];
        if (!row)
...
router.post("/share/:pageId", async (req, res) => {
    try {
        ...
        const existing = (await masterQuery(`SELECT token FROM note_share_tokens
              WHERE tenant_id = $1 AND user_id = $2 AND page_id = $3`, [req.tenantId, req.userId, pageId])).rows[0];
        let token = existing?.token;
        if (!token) {
            // Unguessable: 32 bytes → ~43 url-safe chars.
            token = crypto.randomBytes(32).toString("base64url");
            await masterQuery(`INSERT INTO note_share_tokens
                    (token, tenant_id, user_id, page_id, page_title, created_by)
                 VALUES ($1, $2, $3, $4, $5, $6)`, [token, req.tenantId, req.userId, pageId, page.title || "Untitled", req.userId]);
        }
        else {
            // Refresh the cached title so revisitors see the current name.
            await masterQuery(`UPDATE note_share_tokens SET page_title = $1
                  WHERE tenant_id = $2 AND user_id = $3 AND page_id = $4`, [page.title || "Untitled", req.tenantId, req.userId, pageId]);
        }
...
router.delete("/share/:pageId", async (req, res) => {
    try {
        ...
        if (!pageId)
            return res.status(400).json({ error: "pageId required" });
        await masterQuery(`DELETE FROM note_share_tokens
              WHERE tenant_id = $1 AND user_id = $2 AND page_id = $3`, [req.tenantId, req.userId, pageId]);
        res.json({ ok: true });
    }
    catch (err) {
