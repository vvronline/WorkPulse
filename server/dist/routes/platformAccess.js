...
const { generateApprovalCode, hashApprovalCode, getImpersonationPolicy, computeEffectiveStatus, expireStaleRequests, getActiveSession, } = require("../utils/impersonationApproval");
...
router.post("/:id/approve", async (req, res) => {
    try {
        ...
        const row = (await masterQuery("SELECT * FROM tenant_access_requests WHERE id = $1 AND tenant_id = $2", [rid, req.tenantId])).rows[0];
        if (!row)
