...
const { validateStoryPoints, resolveWorkItemType, resolveWorkflowState, getInitialWorkflowState, getDefaultWorkItemType, normalizeAcceptanceCriteria, } = require('./_helpers/agile');
...
router.post('/', auth, loadUserContext, async (req, res) => {
    try {
        const { title, description, priority, date, assigned_to, due_date, label_ids, sprint_id, story_points, work_item_type_id, workflow_state_id, parent_task_id, acceptance_criteria, is_blocked, blocked_reason, project_id } = req.body;
        if (!title || !title.trim())
        ...
        if (project_id !== undefined && project_id !== null && project_id !== '') {
            ...
            if (!isNaN(projNum) && req.userOrgId) {
                const proj = (await req.db.query('SELECT id FROM projects WHERE id = $1 AND org_id = $2 AND is_archived = FALSE', [projNum, req.userOrgId])).rows[0];
                if (!proj)
                    return res.status(400).json({ error: 'Invalid project or project is archived' });
        ...
        if (sprint_id) {
            const sprint = (await req.db.query('SELECT s.id, s.team_id, s.end_date FROM sprints s JOIN teams t ON t.id = s.team_id WHERE s.id = $1 AND t.org_id = $2', [sprint_id, req.userOrgId])).rows[0];
        ...
        const taskId = await req.db.transaction(async (client) => {
            let taskNumber = null;
            if (validProjectId) {
                const numRes = await client.query(`UPDATE projects
                        SET next_task_number = next_task_number + 1,
                            updated_at = NOW()
                      WHERE id = $1
                      RETURNING next_task_number - 1 AS task_number`, [validProjectId]);
                taskNumber = numRes.rows[0]?.task_number || null;
            }
            const insertRes = await client.query(`INSERT INTO tasks
                 ...
                 RETURNING id`, [req.userId, targetDate, title.trim(), description?.trim() || null, validPriority,
        ...
        if (assignedTo && assignedTo !== req.userId) {
            ...
            if (assignee) {
                await req.db.query('INSERT INTO notifications (user_id, type, title, body, link_task_id) VALUES ($1, $2, $3, $4, $5)', [assignedTo, 'task', `Task Assigned: ${task.title}`, `${assigner?.full_name || 'Someone'} assigned you a task`, task.id]);
...
router.patch('/:id/status', auth, loadUserContext, async (req, res) => {
    try {
        ...
        if (!target && status)
            target = await resolveWorkflowState(status, req.userOrgId, req.db);
        ...
        if (!target)
            return res.status(400).json({ error: 'Invalid status / workflow state' });
        ...
        if (target.id) {
            const settingsRow = (await req.db.query('SELECT enable_wip_limits FROM org_agile_settings WHERE org_id = $1', [req.userOrgId])).rows[0];
            const stateRow = (await req.db.query('SELECT wip_limit, name FROM workflow_states WHERE id = $1', [target.id])).rows[0];
            if (settingsRow?.enable_wip_limits && stateRow?.wip_limit) {
                const occupancy = (await req.db.query(`SELECT COUNT(*)::int AS c FROM tasks
                      WHERE workflow_state_id = $1 AND id != $2 AND org_id = $3`, [target.id, id, req.userOrgId])).rows[0].c;
        ...
        if (startsCycle)
            cycleStartedAtClause = ', cycle_started_at = NOW()';
        await req.db.query(`UPDATE tasks SET status = $1, workflow_state_id = $2, completed_at = $3${cycleStartedAtClause}
              WHERE id = $4`, [target.key, target.id, completedAt, id]);
...
router.put('/:id', auth, loadUserContext, async (req, res) => {
    try {
        ...
        if (isNaN(id))
            return res.status(400).json({ error: 'Invalid task ID' });
        const { title, description, priority, assigned_to, due_date, label_ids, sprint_id, story_points, work_item_type_id, workflow_state_id, parent_task_id, acceptance_criteria, is_blocked, blocked_reason, project_id } = req.body;
        const task = (await req.db.query('SELECT * FROM tasks WHERE id = $1', [id])).rows[0];
        if (!await canAccessTask(task, req.userId, req.userOrgId, req.db, req.userRole))
            return res.status(404).json({ error: 'Task not found' });
        ...
        if (wantsProjectAssign) {
            ...
            const numRes = await req.db.query(`UPDATE projects
                    SET next_task_number = next_task_number + 1,
                        updated_at = NOW()
                  WHERE id = $1
                  RETURNING next_task_number - 1 AS task_number`, [proj.id]);
        ...
        if (work_item_type_id !== undefined) {
            if (work_item_type_id === null || work_item_type_id === '') {
                ...
                if (!newWitRow)
                    return res.status(400).json({ error: 'Invalid work_item_type' });
        ...
        if (parent_task_id !== undefined) {
            if (parent_task_id === null || parent_task_id === '') {
                ...
                const parentNum = parseInt(parent_task_id, 10);
                if (isNaN(parentNum) || parentNum === id)
        ...
        await req.db.query(`UPDATE tasks SET title = $1, description = $2, priority = $3, assigned_to = $4,
              ...
              WHERE id = $18`, [newTitle, newDesc, newPriority, newAssignedTo, newDueDate, newSprintId,
        ...
        if (newTitle !== task.title)
            await logHistory(id, req.userId, 'updated', 'title', task.title, newTitle, null, req.db);
        if (newDesc !== task.description)
            await logHistory(id, req.userId, 'updated', 'description', task.description ? task.description.slice(0, 100) : null, newDesc ? newDesc.slice(0, 100) : null, null, req.db);
        if (newPriority !== task.priority)
            await logHistory(id, req.userId, 'updated', 'priority', task.priority, newPriority, null, req.db);
        ...
        if (newDueDate !== task.due_date)
            await logHistory(id, req.userId, 'updated', 'due_date', task.due_date, newDueDate, null, req.db);
        ...
        if (newAssignedTo && String(newAssignedTo) !== String(task.assigned_to) && newAssignedTo !== req.userId) {
            ...
            const assigner = (await req.db.query('SELECT full_name FROM users WHERE id = $1', [req.userId])).rows[0];
            if (assignee) {
                await req.db.query('INSERT INTO notifications (user_id, type, title, body, link_task_id) VALUES ($1, $2, $3, $4, $5)', [newAssignedTo, 'task', `Task Assigned: ${updated.title}`, `${assigner?.full_name || 'Someone'} assigned you a task`, updated.id]);
