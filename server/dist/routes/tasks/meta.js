...
router.get('/assignable-users', auth, loadUserContext, async (req, res) => {
    try {
        ...
        if (req.userOrgId) {
            ...
            users = (await req.db.query('SELECT id, username, full_name, avatar FROM users WHERE org_id = $1 AND is_active = TRUE AND hidden_from_directory = FALSE ORDER BY full_name ASC', [req.userOrgId])).rows;
        }
        else {
