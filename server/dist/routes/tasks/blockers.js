...
router.patch('/:id/block', auth, loadUserContext, async (req, res) => {
    try {
        ...
        await req.db.query('UPDATE tasks SET is_blocked = $1, blocked_reason = $2 WHERE id = $3', [flag, reason, id]);
        await req.db.query(`INSERT INTO task_history (task_id, action, field, new_value, user_id)
             VALUES ($1, $2, 'blocker', $3, $4)`, [id, flag ? 'blocked' : 'unblocked', reason || (flag ? 'No reason given' : null), req.userId]).catch(() => { });
