...
async function materialiseTeamSprints(team, req) {
    if (!team?.sprint_start_date || !team.sprint_duration_weeks)
        return [];
    ...
    return await req.db.transaction(async (client) => {
        ...
        for (let i = 0; i < 2; i++) {
            ...
            const existing = (await client.query('SELECT id, name, start_date, end_date, status, goal FROM sprints WHERE team_id = $1 AND name = $2', [team.id, name])).rows[0];
            if (existing) {
                out.push(existing);
            }
            else {
                const inserted = await client.query('INSERT INTO sprints (team_id, name, start_date, end_date, status) VALUES ($1, $2, $3, $4, $5) RETURNING id, name, start_date, end_date, status, goal', [team.id, name, fmt(sMs), fmt(eMs), i === 0 ? 'active' : 'planned']);
...
module.exports = { materialiseTeamSprints, getTodayStr, fmt };
//# sourceMappingURL=sprintMaterialise.js.map