...
async function canAccessTask(task, userId, requesterOrgId, db, requesterRole) {
    ...
    if (!user || user.org_id !== requesterOrgId)
        return false;
    if (!owner || owner.org_id !== requesterOrgId)
        return false;
...
async function loadAccessibleTask(req, res, taskId) {
    ...
    if (!isOrgAdmin && task.user_id !== req.userId && task.assigned_to !== req.userId) {
        ...
        const sameTeam = (await req.db.query(`SELECT 1 FROM users u WHERE u.id = $1 AND u.team_id = (SELECT team_id FROM users WHERE id = $2)`, [req.userId, task.user_id])).rowCount > 0;
        if (!sameTeam) {
...
module.exports = { canAccessTask, loadAccessibleTask };
