...
router.get('/:id/dependencies', auth, loadUserContext, async (req, res) => {
    try {
        ...
        if (!task)
            return;
        const blocking = (await req.db.query(`SELECT d.id AS link_id, d.type, t.id, t.title, t.status, t.workflow_state_id, t.is_blocked
               FROM task_dependencies d JOIN tasks t ON t.id = d.depends_on_id
              WHERE d.task_id = $1
              ORDER BY t.id`, [id])).rows;
        const blockedBy = (await req.db.query(`SELECT d.id AS link_id, d.type, t.id, t.title, t.status, t.workflow_state_id, t.is_blocked
...
router.post('/:id/dependencies', auth, loadUserContext, async (req, res) => {
    try {
        ...
        if (linkType === 'blocks') {
            const reverse = (await req.db.query("SELECT 1 FROM task_dependencies WHERE task_id = $1 AND depends_on_id = $2 AND type = 'blocks'", [otherId, id])).rowCount;
            if (reverse > 0)
                return res.status(400).json({ error: 'Would create a circular blocks dependency' });
        }
        const r = await req.db.query(`INSERT INTO task_dependencies (task_id, depends_on_id, type, created_by)
             VALUES ($1, $2, $3, $4)
             ON CONFLICT (task_id, depends_on_id, type) DO NOTHING
             RETURNING id`, [id, otherId, linkType, req.userId]);
        if (r.rowCount === 0)
            return res.status(409).json({ error: 'Dependency already exists' });
        // History trace on both ends
        await req.db.query(`INSERT INTO task_history (task_id, action, field, new_value, user_id)
             VALUES ($1, 'dependency_added', $2, $3, $4),
                    ($5, 'dependency_added', $6, $7, $4)`, [id, linkType, `→ #${otherId}`, req.userId, otherId, linkType, `← #${id}`]).catch(() => { });
...
router.delete('/:id/dependencies/:depId', auth, loadUserContext, async (req, res) => {
    try {
        ...
        if (!task)
            return;
        const r = await req.db.query('DELETE FROM task_dependencies WHERE id = $1 AND task_id = $2 RETURNING depends_on_id, type', [linkId, id]);
        if (r.rowCount === 0)
            return res.status(404).json({ error: 'Dependency not found' });
        await req.db.query(`INSERT INTO task_history (task_id, action, field, new_value, user_id)
             VALUES ($1, 'dependency_removed', $2, $3, $4)`, [id, r.rows[0].type, `→ #${r.rows[0].depends_on_id}`, req.userId]).catch(() => { });
        res.json({ ok: true });
    }
    catch (err) {
