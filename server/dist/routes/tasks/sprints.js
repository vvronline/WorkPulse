    ...
    try {
        ...
        if (isOrgAdmin) {
            ...
            for (const team of teamsNeedingSprints) {
                try {
                    await materialiseTeamSprints(team, req);
                }
                catch (e) {
                    req.log.warn({ err: e, teamId: team.id }, 'Failed to auto-materialise sprints for team');
                }
            }
        ...
        `, [req.userTeamId])).rows;
        if (sprints.length === 0) {
            const team = (await req.db.query('SELECT id, sprint_start_date, sprint_duration_weeks FROM teams WHERE id = $1', [req.userTeamId])).rows[0];
...
router.patch('/:id/assign-sprint', auth, loadUserContext, async (req, res) => {
    try {
        ...
        if (sprint_id === null || sprint_id === undefined || sprint_id === '') {
            ...
            const sprint = (await req.db.query('SELECT s.id, s.team_id, s.name, s.end_date FROM sprints s JOIN teams t ON t.id = s.team_id WHERE s.id = $1 AND t.org_id = $2', [sprint_id, req.userOrgId])).rows[0];
