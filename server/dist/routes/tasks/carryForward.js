    ...
    try {
        ...
        const carried = await req.db.transaction(async (client) => {
            ...
            for (const t of incomplete) {
                ...
                const exists = (await client.query(`SELECT id FROM tasks
                      WHERE (user_id = $1 OR assigned_to = $1)
                        AND date = $2
                        AND LOWER(TRIM(title)) = LOWER(TRIM($3))
                        AND ((org_id = $4) OR (org_id IS NULL AND $4::integer IS NULL))`, [req.userId, today, t.title, req.userOrgId || null])).rows[0];
                if (!exists) {
                    ...
                    const insertRes = await client.query(`INSERT INTO tasks
                         ...
                         RETURNING id`, [req.userId, today, t.title, t.description, t.priority, t.assigned_to, dueDate, req.userOrgId || null,
                    ...
                    for (const lbl of origLabels) {
                        await client.query('INSERT INTO task_label_map (task_id, label_id) VALUES ($1, $2) ON CONFLICT DO NOTHING', [newTaskId, lbl.label_id]);
                    }
