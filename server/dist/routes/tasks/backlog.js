...
router.get('/backlog', auth, loadUserContext, async (req, res) => {
    try {
        ...
        const summary = { total, byStatus: {}, byPriority: { high: 0, medium: 0, low: 0 } };
        for (const col of ['pending', 'in_progress', 'in_review', 'done'])
            summary.byStatus[col] = 0;
        for (const t of enriched) {
            if (summary.byStatus[t.status] !== undefined)
    ...
    try {
        ...
        if (sprint_id) {
            ...
            const sprint = isOrgAdmin
                ? (await req.db.query('SELECT s.id, s.team_id, s.end_date FROM sprints s JOIN teams t ON t.id = s.team_id WHERE s.id = $1 AND t.org_id = $2', [sprint_id, req.userOrgId])).rows[0]
        ...
        const taskId = await req.db.transaction(async (client) => {
            ...
            const insertRes = await client.query(`INSERT INTO tasks
                 ...
                 RETURNING id`, [req.userId, title.trim(), description?.trim() || null, validPriority, statusKey,
