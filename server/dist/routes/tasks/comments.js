...
router.post('/:id/comments', auth, loadUserContext, handleUpload, async (req, res) => {
    ...
    try {
        ...
        const result = await req.db.query(`INSERT INTO task_comments (task_id, user_id, content, file_url, file_name, file_type, file_size)
             VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id`, [req.params.id, req.userId, content || null, fileUrl, fileName, fileType, fileSize]);
        ...
        try {
            ...
            if (mentionedIds.size > 0) {
                ...
                const orgMentionRows = req.userOrgId
                    ? (await req.db.query('SELECT id FROM users WHERE id = ANY($1) AND org_id = $2 AND is_active = TRUE', [[...mentionedIds], req.userOrgId])).rows
                    : [];
                for (const row of orgMentionRows) {
                    ...
                    if (!await canAccessTask(task, uid, req.userOrgId, req.db, mentionRole))
                        continue;
                    await req.db.query('INSERT INTO notifications (user_id, type, title, body, link_task_id) VALUES ($1, $2, $3, $4, $5)', [uid, 'mention', `${commenterName} mentioned you`, `In task: ${task.title}`, task.id]);
...
router.put('/:id/comments/:commentId', auth, loadUserContext, async (req, res) => {
    try {
        ...
        if (content.length > 2000)
            return res.status(400).json({ error: 'Comment must be 2000 characters or less' });
        await req.db.query('UPDATE task_comments SET content = $1, updated_at = $2 WHERE id = $3', [content.trim(), new Date().toISOString(), req.params.commentId]);
