...
router.put('/:id/acceptance-criteria', auth, loadUserContext, async (req, res) => {
    try {
        ...
        await req.db.query('UPDATE tasks SET acceptance_criteria = $1::jsonb WHERE id = $2', [JSON.stringify(cleaned), id]);
        await req.db.query(`INSERT INTO task_history (task_id, action, field, new_value, user_id)
             VALUES ($1, 'updated', 'acceptance_criteria', $2, $3)`, [id, `${cleaned.length} item(s)`, req.userId]).catch(() => { });
