...
router.get("/", auth, loadUserContext, async (req, res) => {
    try {
        ...
        if (!paginate) {
            ...
            const rows = (await req.db.query(`SELECT p.*, u.full_name AS lead_name, u.username AS lead_username,
                        (SELECT COUNT(*)::int FROM tasks WHERE project_id = p.id) AS task_count
                   FROM projects p
              LEFT JOIN users u ON u.id = p.lead_user_id
                   ${baseWhere}
                   ${orderBy}`, [req.userOrgId])).rows;
            return res.json(rows);
        }
        const [pageRes, countRes] = await Promise.all([
            req.db.query(`SELECT p.*, u.full_name AS lead_name, u.username AS lead_username,
                        (SELECT COUNT(*)::int FROM tasks WHERE project_id = p.id) AS task_count
                   FROM projects p
              LEFT JOIN users u ON u.id = p.lead_user_id
                   ${baseWhere}
                   ${orderBy}
                   LIMIT $2 OFFSET $3`, [req.userOrgId, limit, offset]),
...
// ─── Create project ──────────────────────────────────────────────────────
router.post("/", auth, loadUserContext, requireRole("manager"), async (req, res) => {
    try {
        if (!req.userOrgId)
        ...
        const inserted = (await req.db.query(`INSERT INTO projects (org_id, key, name, description, color, lead_user_id, created_by)
             VALUES ($1, $2, $3, $4, $5, $6, $7)
             RETURNING *`, [req.userOrgId, upperKey, name.trim().slice(0, 200), description?.slice(0, 2000) || null,
            validColor, validLead, req.userId])).rows[0];
...
// ─── Get one project ─────────────────────────────────────────────────────
router.get("/:id", auth, loadUserContext, async (req, res) => {
    try {
        const id = parseInt(String(req.params.id), 10);
        if (isNaN(id))
            return res.status(400).json({ error: "Invalid project id" });
        const row = (await req.db.query(`SELECT p.*, u.full_name AS lead_name, u.username AS lead_username,
                    (SELECT COUNT(*)::int FROM tasks WHERE project_id = p.id) AS task_count
               FROM projects p
          LEFT JOIN users u ON u.id = p.lead_user_id
              WHERE p.id = $1 AND p.org_id = $2`, [id, req.userOrgId])).rows[0];
        if (!row)
...
// ─── Update project ──────────────────────────────────────────────────────
router.put("/:id", auth, loadUserContext, requireRole("manager"), async (req, res) => {
    try {
        const id = parseInt(String(req.params.id), 10);
        if (isNaN(id))
            return res.status(400).json({ error: "Invalid project id" });
        ...
        const updated = (await req.db.query(`UPDATE projects
                SET name = $1, description = $2, color = $3, lead_user_id = $4, updated_at = NOW()
              WHERE id = $5
              RETURNING *`, [newName, newDesc, newColor, newLead, id])).rows[0];
...
// ─── Archive / unarchive ─────────────────────────────────────────────────
router.patch("/:id/archive", auth, loadUserContext, requireRole("manager"), async (req, res) => {
    try {
        const id = parseInt(String(req.params.id), 10);
        if (isNaN(id))
            return res.status(400).json({ error: "Invalid project id" });
        const archived = !!req.body?.is_archived;
        const r = await req.db.query("UPDATE projects SET is_archived = $1, updated_at = NOW() WHERE id = $2 AND org_id = $3 RETURNING *", [archived, id, req.userOrgId]);
        if (r.rowCount === 0)
            return res.status(404).json({ error: "Project not found" });
...
router.delete("/:id", auth, loadUserContext, requireRole("super_admin"), async (req, res) => {
    try {
        const id = parseInt(String(req.params.id), 10);
        if (isNaN(id))
            return res.status(400).json({ error: "Invalid project id" });
        ...
        await req.db.transaction(async (client) => {
            if (taskCount > 0) {
                await client.query("UPDATE tasks SET project_id = NULL, task_number = NULL WHERE project_id = $1", [id]);
            }
...
router.get("/:id/tasks", auth, loadUserContext, async (req, res) => {
    try {
        ...
        const [tasksRes, countRes] = await Promise.all([
            req.db.query(`SELECT t.* FROM tasks t
