...
router.get("/team-attendance", async (req, res) => {
    try {
        ...
        "SELECT id, full_name, avatar, role, team_id, department_id FROM users WHERE id = ANY($1) AND is_active = TRUE AND hidden_from_directory = FALSE", [visibleIds])).rows;
        const entries = (await req.db.query(`SELECT * FROM time_entries WHERE user_id = ANY($1) AND (timestamp + $2::interval)::date = $3::date ORDER BY timestamp ASC`, [visibleIds, tzMod, targetDate])).rows;
        const leaves = (await req.db.query(`SELECT user_id, leave_type, status as leave_status FROM leaves WHERE user_id = ANY($1) AND date = $2`, [visibleIds, targetDate])).rows;
        const leaveMap = {};
        leaves.forEach((l) => { leaveMap[l.user_id] = l; });
        const tasks = (await req.db.query(`SELECT user_id, title, status FROM tasks WHERE user_id = ANY($1) AND date = $2 AND status IN ('in_progress', 'in_review') ORDER BY priority DESC`, [visibleIds, targetDate])).rows;
        const taskMap = {};
        tasks.forEach((t) => { if (!taskMap[t.user_id])
            taskMap[t.user_id] = t; });
        const userEntries = {};
        entries.forEach((e) => {
            if (!userEntries[e.user_id])
                userEntries[e.user_id] = [];
...
router.get("/team-analytics", async (req, res) => {
    try {
        ...
        const visibleIds = await getVisibleUserIds(req.userId, req.userRole, req.userOrgId, req.userTeamId, req.db);
        if (visibleIds.length === 0)
            return res.json({ members: [], totalMembers: 0 });
        ...
        if (from && to) {
            ...
            const numDays = days === "month" ? 30 : days === "quarter" ? 90 : parseInt(String(days)) || 7;
        ...
        const tzMod = getTzModifier(req);
        const entries = (await req.db.query(`SELECT * FROM time_entries WHERE user_id = ANY($1) AND (timestamp + $2::interval)::date BETWEEN $3::date AND $4::date ORDER BY timestamp ASC`, [visibleIds, tzMod, fromDate, toDate])).rows;
        const byUser = {};
        entries.forEach((e) => {
            if (!byUser[e.user_id])
                byUser[e.user_id] = {};
            const dateStr = getLocalDateFromTs(e.timestamp, req);
            if (!byUser[e.user_id][dateStr])
                byUser[e.user_id][dateStr] = [];
            byUser[e.user_id][dateStr].push(e);
        });
        const users = (await req.db.query(`SELECT u.id, u.full_name, u.email, u.avatar, u.role, u.department_id, u.team_id,
                    d.name as department_name, t.name as team_name
             FROM users u
             LEFT JOIN departments d ON d.id = u.department_id
             LEFT JOIN teams t ON t.id = u.team_id
             WHERE u.id = ANY($1) AND u.is_active = TRUE AND u.hidden_from_directory = FALSE`, [visibleIds])).rows;
        const taskCounts = (await req.db.query(`SELECT user_id, SUM(CASE WHEN status = 'done' THEN 1 ELSE 0 END) as done, COUNT(*) as total
             FROM tasks WHERE user_id = ANY($1) AND date BETWEEN $2 AND $3 GROUP BY user_id`, [visibleIds, fromDate, toDate])).rows;
        const taskMap = {};
        taskCounts.forEach((t) => { taskMap[t.user_id] = { done: parseInt(t.done || 0, 10), total: parseInt(t.total, 10) }; });
        const leaveCounts = (await req.db.query(`SELECT user_id, leave_type, COUNT(*) as count FROM leaves
             WHERE user_id = ANY($1) AND date BETWEEN $2 AND $3 AND status != 'rejected'
             GROUP BY user_id, leave_type`, [visibleIds, fromDate, toDate])).rows;
        const leaveMap = {};
        leaveCounts.forEach((l) => {
            if (!leaveMap[l.user_id])
                leaveMap[l.user_id] = { total: 0, byType: {} };
            const cnt = parseInt(l.count, 10);
            leaveMap[l.user_id].total += cnt;
            leaveMap[l.user_id].byType[l.leave_type] = cnt;
        });
        const pendingCount = (await req.db.query("SELECT COUNT(*) as count FROM approval_requests WHERE approver_id = $1 AND status = 'pending'", [req.userId])).rows[0];
        const todayEntries = (await req.db.query(`SELECT * FROM time_entries WHERE user_id = ANY($1) AND (timestamp + $2::interval)::date = $3::date ORDER BY timestamp ASC`, [visibleIds, tzMod, toDate])).rows;
        const todayByUser = {};
        todayEntries.forEach((e) => {
            if (!todayByUser[e.user_id])
                todayByUser[e.user_id] = [];
            todayByUser[e.user_id].push(e);
        });
        const todayLeaves = (await req.db.query(`SELECT user_id FROM leaves WHERE user_id = ANY($1) AND date = $2 AND status != 'rejected'`, [visibleIds, toDate])).rows;
        ...
        if (req.userOrgId) {
            ...
            if (org?.work_hours_per_day)
                orgWhpd = org.work_hours_per_day;
        ...
        for (let i = 0; i < numDays; i++) {
            ...
            if (isJsDowWorkDay(d.getUTCDay(), workDaySet))
                expectedWeekdays++;
        }
        ...
        const members = users.map((u) => {
            ...
            sortedDates.forEach(date => {
                ...
                if (!dayEntries.some((e) => e.entry_type === "clock_in"))
                    return;
                daysWorked++;
                ...
                if (ci) {
                    ...
                    if (localDate.getUTCHours() < 10)
                        earlyDays++;
                    if (ci.work_mode === "remote")
                        remoteDays++;
                    else
                        officeDays++;
                }
            ...
            const trend = trendDates.map(date => {
                const dayEntries = userDays[date];
                if (!dayEntries || !dayEntries.some((e) => e.entry_type === "clock_in"))
                    return 0;
            ...
            for (let i = 0; i <= numDays; i++) {
                ...
                if (userDays[dateStr] && userDays[dateStr].some((e) => e.entry_type === "clock_in"))
                    streak++;
                else
                    break;
            }
...
router.get("/approvals", async (req, res) => {
    try {
        ...
        const isHrOrAbove = (req.roleLevel || 0) >= ROLE_LEVEL.hr_admin;
        const conditions = [];
        const params = [];
        let pi = 1;
        ...
        if (filterStatus !== "all") {
            conditions.push(`ar.status = $${pi++}`);
            params.push(filterStatus);
        }
        if (type) {
            conditions.push(`ar.type = $${pi++}`);
            params.push(type);
        }
...
router.post("/approvals/:id/approve", async (req, res) => {
    try {
        ...
        const txResult = await req.db.transaction(async (client) => {
            ...
            if (approval.type === "leave" && approval.reference_id) {
                ...
                if (metadata.date) {
                    ...
                    await client.query(`UPDATE time_entries SET approval_status = 'approved', approved_by = $1 WHERE user_id = $2 AND (timestamp + $3::interval)::date = $4::date AND is_manual = TRUE`, [req.userId, approval.requester_id, tzMod, metadata.date]);
                }
            }
        ...
        // Respond immediately after the transaction commits. The notification /
        // email / WebSocket fan-out below runs in the background so a slow
        // notification insert (or a cold tenant DB pool) never delays the HTTP
        // response — mobile clients enforce a request timeout and were
        // surfacing false "failed to approve" errors even though the approval
        // was already committed (the web client has no axios timeout).
            ...
            try {
                ...
                if (requester) {
                    if (txResult.type === "leave" || txResult.type === "leave_withdraw") {
                        ...
                        await req.db.query("INSERT INTO notifications (user_id, type, title, body) VALUES ($1, $2, $3, $4)", [txResult.requesterId, "leave", "Leave Approved \u2705", `Your ${leaveInfo.leave_type} leave on ${leaveInfo.date} has been approved.`]);
                        ...
                        if (txResult.metadata) {
                            try {
                                meta = JSON.parse(txResult.metadata);
                            }
                            catch { }
                        }
                        const entryDate = meta.date || "";
                        await req.db.query("INSERT INTO notifications (user_id, type, title, body) VALUES ($1, $2, $3, $4)", [txResult.requesterId, "approval", "Manual Entry Approved \u2705", `Your manual time entry for ${entryDate} has been approved.`]);
                        ...
                        await req.db.query("INSERT INTO notifications (user_id, type, title, body) VALUES ($1, $2, $3, $4)", [txResult.requesterId, "approval", "Overtime Approved \u2705", `Your overtime request for ${overtimeDate} has been approved. Comp-off has been credited.`]);
...
router.post("/approvals/:id/reject", async (req, res) => {
    try {
        ...
        const txResult = await req.db.transaction(async (client) => {
            ...
            if (approval.type === "leave" && approval.reference_id) {
                ...
                if (metadata.date) {
                    ...
                    const reqOffset = requesterRow?.timezone_offset || 0;
                    const tzMod = `${-reqOffset} minutes`;
                    await client.query(`UPDATE time_entries SET approval_status = 'rejected', approved_by = $1 WHERE user_id = $2 AND (timestamp + $3::interval)::date = $4::date AND is_manual = TRUE`, [req.userId, approval.requester_id, tzMod, metadata.date]);
                }
            }
            return { ok: true, type: approval.type, requesterId: approval.requester_id, referenceId: approval.reference_id, metadata: approval.metadata };
        });
        if (txResult.error)
            return res.status(txResult.status).json({ error: txResult.error });
        // Respond immediately after the transaction commits — the notification
        // fan-out below runs in the background (see the approve handler comment).
            ...
            try {
                ...
                if (requester) {
                    if (txResult.type === "leave") {
                        const leave = txResult.referenceId ? (await req.db.query("SELECT * FROM leaves WHERE id = $1", [txResult.referenceId])).rows[0] : null;
                        const leaveInfo = leave || { leave_type: "leave", date: "" };
                        await req.db.query("INSERT INTO notifications (user_id, type, title, body) VALUES ($1, $2, $3, $4)", [txResult.requesterId, "leave", "Leave Rejected", `Your ${leaveInfo.leave_type} leave on ${leaveInfo.date} has been rejected.${reject_reason ? " Reason: " + reject_reason : ""}`]);
                        ...
                        const entryDate = meta.date || "";
                        await req.db.query("INSERT INTO notifications (user_id, type, title, body) VALUES ($1, $2, $3, $4)", [txResult.requesterId, "approval", "Manual Entry Rejected", `Your manual time entry for ${entryDate} has been rejected.${reject_reason ? " Reason: " + reject_reason : ""}`]);
    ...
    try {
        ...
        await req.db.transaction(async (client) => {
            for (const id of ids) {
                ...
                await client.query("UPDATE approval_requests SET status = $1, reject_reason = $2, reviewed_at = NOW(), approver_id = $3 WHERE id = $4", [status, action === "reject" ? (reject_reason || null) : null, req.userId, id]);
                if (approval.type === "leave" && approval.reference_id) {
                    await client.query("UPDATE leaves SET status = $1, approved_by = $2, reviewed_at = NOW(), reject_reason = $3 WHERE id = $4", [status, req.userId, action === "reject" ? (reject_reason || null) : null, approval.reference_id]);
                    if (action === "approve") {
                        const leave = (await client.query("SELECT * FROM leaves WHERE id = $1", [approval.reference_id])).rows[0];
                        if (leave)
                            await updateLeaveBalance(leave.user_id, leave.leave_type, leave.date, leave.duration || "full", "add", client);
                    ...
                    if (metadata.date) {
                        const tzMod = getTzModifier(req);
                        await client.query(`UPDATE time_entries SET approval_status = $1, approved_by = $2 WHERE user_id = $3 AND (timestamp + $4::interval)::date = $5::date AND is_manual = TRUE`, [action === "approve" ? "approved" : "rejected", req.userId, approval.requester_id, tzMod, metadata.date]);
                    }
                }
...
router.get("/member/:userId/hours", async (req, res) => {
    try {
        ...
        const tzMod = getTzModifier(req);
        const entries = (await req.db.query(`SELECT * FROM time_entries WHERE user_id = $1 AND (timestamp + $2::interval)::date BETWEEN $3::date AND $4::date ORDER BY timestamp ASC`, [targetUserId, tzMod, fromDate, toDate])).rows;
        const grouped = {};
        entries.forEach((e) => {
...
router.get("/member/:userId/tasks", async (req, res) => {
    try {
        ...
        const tasks = (await req.db.query("SELECT * FROM tasks WHERE (user_id = $1 OR assigned_to = $1) AND date = $2 ORDER BY priority DESC, created_at ASC", [targetUserId, targetDate])).rows;
        res.json(tasks);
    }
    catch (err) {
...
router.get("/member/:userId/leaves", async (req, res) => {
    try {
        ...
        if (!visibleIds.includes(targetUserId))
            return res.status(403).json({ error: "Not authorized to view this user's data" });
        const { from, to } = req.query;
        const offsetMin = parseInt(String(req.headers["x-timezone-offset"])) || 0;
        ...
        const leaves = (await req.db.query(`SELECT l.*, u.full_name as approved_by_name FROM leaves l
             LEFT JOIN users u ON u.id = l.approved_by
             WHERE l.user_id = $1 AND l.date BETWEEN $2 AND $3 ORDER BY l.date DESC`, [targetUserId, fromDate, toDate])).rows;
        res.json(leaves);
    }
    catch (err) {
...
router.get("/member/:userId/requests", async (req, res) => {
    try {
        ...
        if (!visibleIds.includes(targetUserId))
            return res.status(403).json({ error: "Not authorized to view this user's data" });
        const requests = (await req.db.query(`SELECT ar.*, u.full_name as approver_name FROM approval_requests ar
             LEFT JOIN users u ON u.id = ar.approver_id
             WHERE ar.requester_id = $1 ORDER BY ar.created_at DESC LIMIT 100`, [targetUserId])).rows;
        res.json(requests.map((r) => {
            let metadata = null;
            if (r.metadata) {
                try {
...
router.get("/member/:userId/overview", async (req, res) => {
    try {
        ...
        if (!visibleIds.includes(targetUserId))
            return res.status(403).json({ error: "Not authorized to view this user's data" });
        const user = (await req.db.query(`SELECT u.id, u.full_name, u.email, u.avatar, u.role, u.team_id, u.department_id, u.created_at,
                    d.name as department_name, t.name as team_name
             FROM users u
             LEFT JOIN departments d ON d.id = u.department_id
             LEFT JOIN teams t ON t.id = u.team_id
             WHERE u.id = $1`, [targetUserId])).rows[0];
        if (!user)
        ...
        const todayEntries = (await req.db.query(`SELECT * FROM time_entries WHERE user_id = $1 AND (timestamp + $2::interval)::date = $3::date ORDER BY timestamp ASC`, [targetUserId, tzMod, today])).rows;
        const todayFloorMs = computeFloorMs(todayEntries, true);
        const todayBreakMs = computeBreakMs(todayEntries, true);
        const pendingRequests = (await req.db.query("SELECT COUNT(*) as count FROM approval_requests WHERE requester_id = $1 AND status = 'pending'", [targetUserId])).rows[0];
        const monthLeaves = (await req.db.query("SELECT COUNT(*) as count FROM leaves WHERE user_id = $1 AND date >= $2 AND date <= $3 AND status != 'rejected'", [targetUserId, monthStart, today])).rows[0];
        const todayTasks = (await req.db.query("SELECT * FROM tasks WHERE (user_id = $1 OR assigned_to = $1) AND date = $2 ORDER BY priority DESC, created_at ASC", [targetUserId, today])).rows;
        const recentLeaves = (await req.db.query("SELECT * FROM leaves WHERE user_id = $1 AND date >= $2 ORDER BY date DESC LIMIT 10", [targetUserId, thirtyDaysAgo])).rows;
        const recentRequestsRows = (await req.db.query(`SELECT ar.*, u.full_name as approver_name FROM approval_requests ar
             LEFT JOIN users u ON u.id = ar.approver_id
             WHERE ar.requester_id = $1 ORDER BY ar.created_at DESC LIMIT 10`, [targetUserId])).rows;
        ...
        const trendEntries = (await req.db.query(`SELECT * FROM time_entries WHERE user_id = $1 AND (timestamp + $2::interval)::date BETWEEN $3::date AND $4::date ORDER BY timestamp ASC`, [targetUserId, tzMod, trendStartStr, today])).rows;
        const trendGrouped = {};
        trendEntries.forEach((e) => {
            const dateStr = getLocalDateFromTs(e.timestamp, req);
            if (!trendGrouped[dateStr])
        ...
        const last30Entries = (await req.db.query(`SELECT * FROM time_entries WHERE user_id = $1 AND (timestamp + $2::interval)::date BETWEEN $3::date AND $4::date ORDER BY timestamp ASC`, [targetUserId, tzMod, thirtyDaysAgo, today])).rows;
        const grouped30 = {};
        last30Entries.forEach((e) => {
            const dateStr = getLocalDateFromTs(e.timestamp, req);
            if (!grouped30[dateStr])
                grouped30[dateStr] = [];
        ...
        Object.values(grouped30).forEach((dayEntries) => {
            ...
            if (Math.round(fMs / 60000) >= targetMin)
                targetMet30++;
            const ci = dayEntries.find((e) => e.entry_type === "clock_in");
            if (ci) {
                const utcMs = ci.timestamp instanceof Date ? ci.timestamp.getTime() : new Date(ci.timestamp.replace(" ", "T") + "Z").getTime();
        ...
        const monthTaskStats = (await req.db.query(`SELECT COUNT(*) as total, SUM(CASE WHEN status = 'done' THEN 1 ELSE 0 END) as done,
                    SUM(CASE WHEN status = 'in_progress' THEN 1 ELSE 0 END) as in_progress
             FROM tasks WHERE (user_id = $1 OR assigned_to = $1) AND date >= $2 AND date <= $3`, [targetUserId, monthStart, monthEnd])).rows[0];
        ...
        try {
            ...
            const rawBalances = targetOrgId
                ? (await req.db.query(`SELECT lb.*, lp.name as policy_name, lp.color
                     FROM leave_balances lb
                     JOIN leave_policies lp ON lp.leave_type = lb.leave_type AND lp.org_id = $3
                     WHERE lb.user_id = $1 AND lb.year = $2
                     ORDER BY lb.leave_type ASC`, [targetUserId, year, targetOrgId])).rows
                : (await req.db.query(`SELECT lb.*
                     FROM leave_balances lb
                     WHERE lb.user_id = $1 AND lb.year = $2
                     ORDER BY lb.leave_type ASC`, [targetUserId, year])).rows;
            leaveBalances = rawBalances.map((b) => {
