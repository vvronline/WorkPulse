...
router.get("/policies", requireSameOrg, async (req, res) => {
    try {
        ...
        await ensureDefaultHolidayPolicy(req.userOrgId, req.db);
...
router.post("/policies", requireRole("hr_admin"), requireSameOrg, async (req, res) => {
    try {
        ...
        if (quota < 0 || quota > 365)
            return res.status(400).json({ error: "Annual quota must be between 0 and 365" });
        if (cfLimit < 0 || cfLimit > 365)
            return res.status(400).json({ error: "Carry forward limit must be between 0 and 365" });
        ...
        if (existing) {
            await req.db.query(`UPDATE leave_policies SET annual_quota = $1, accrual_type = $2, carry_forward_limit = $3,
                 half_day_allowed = $4, quarter_day_allowed = $5 WHERE id = $6`, [quota, accrual_type || "annual", cfLimit, !!half_day_allowed, !!quarter_day_allowed, existing.id]);
            ...
            await syncOrgBalancesToPolicy(req.userOrgId, leave_type, req.db);
            (0, audit_1.logAction)(req, "update", "leave_policy", existing.id, { leave_type });
            res.json({ message: `Leave policy for ${leave_type} updated` });
        }
        else {
            const result = await req.db.query(`INSERT INTO leave_policies (org_id, leave_type, annual_quota, accrual_type, carry_forward_limit, half_day_allowed, quarter_day_allowed)
                 VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id`, [req.userOrgId, leave_type, quota, accrual_type || "annual", cfLimit, !!half_day_allowed, !!quarter_day_allowed]);
            // Sync balances for any pre-existing rows of this leave_type as well
            await syncOrgBalancesToPolicy(req.userOrgId, leave_type, req.db);
            (0, audit_1.logAction)(req, "create", "leave_policy", result.rows[0].id, { leave_type, annual_quota: quota });
...
router.delete("/policies/:id", requireRole("hr_admin"), requireSameOrg, async (req, res) => {
    try {
        ...
        const result = await req.db.transaction(async (client) => {
            ...
            const balDel = await client.query(`DELETE FROM leave_balances lb
                 USING users u
                 WHERE lb.user_id = u.id
                   AND u.org_id = $1
                   AND lb.leave_type = $2`, [req.userOrgId, policy.leave_type]);
            // Drop any leave records (pending / approved / etc.) for this type
            const leaveDel = await client.query(`DELETE FROM leaves l
                 USING users u
                 WHERE l.user_id = u.id
                   AND u.org_id = $1
                   AND l.leave_type = $2`, [req.userOrgId, policy.leave_type]);
        ...
        (0, audit_1.logAction)(req, "delete", "leave_policy", Number(id), {
...
router.get("/balances", async (req, res) => {
    try {
        ...
        if (req.userOrgId) {
            ...
            await pruneOrphanBalances(req.userOrgId, req.db);
            ...
            await recalcHolidayUsed(req.userId, year, req.db);
        }
        ...
        const balances = req.userOrgId
            ? (await req.db.query(`SELECT lb.*
                 ...
                 ORDER BY lb.leave_type ASC`, [req.userId, year, req.userOrgId])).rows
            : (await req.db.query("SELECT * FROM leave_balances WHERE user_id = $1 AND year = $2", [req.userId, year])).rows;
...
router.get("/balances/:userId", requireRole("team_lead"), requireSameOrg, async (req, res) => {
    try {
        ...
        await recalcHolidayUsed(targetUserId, year, req.db);
        const balances = (await req.db.query(`SELECT lb.*
             FROM leave_balances lb
             JOIN leave_policies lp
               ON lp.leave_type = lb.leave_type AND lp.org_id = $3
             WHERE lb.user_id = $1 AND lb.year = $2
             ORDER BY lb.leave_type ASC`, [targetUserId, year, req.userOrgId])).rows;
        res.json(balances.map(numericBalance));
    }
    catch (err) {
...
router.put("/balances/:userId", requireRole("hr_admin"), requireSameOrg, async (req, res) => {
    try {
        ...
        if (!leave_type || !year)
            return res.status(400).json({ error: "Leave type and year are required" });
        const targetUser = (await req.db.query("SELECT org_id FROM users WHERE id = $1", [targetUserId])).rows[0];
        if (!targetUser || targetUser.org_id !== req.userOrgId) {
            return res.status(403).json({ error: "Cannot modify balances for users outside your organization" });
        }
        const existing = (await req.db.query("SELECT id FROM leave_balances WHERE user_id = $1 AND leave_type = $2 AND year = $3", [targetUserId, leave_type, year])).rows[0];
        if (existing) {
            const updates = [];
            const params = [];
            let pi = 1;
            if (quota !== undefined) {
                updates.push(`quota = $${pi++}`);
                params.push(quota);
            }
            if (carried_forward !== undefined) {
                updates.push(`carried_forward = $${pi++}`);
                params.push(carried_forward);
            }
            if (updates.length === 0)
            ...
            await req.db.query("INSERT INTO leave_balances (user_id, leave_type, year, quota, carried_forward) VALUES ($1, $2, $3, $4, $5)", [targetUserId, leave_type, year, quota || 0, carried_forward || 0]);
        }
        (0, audit_1.logAction)(req, "update_balance", "leave_balance", targetUserId, { leave_type, year, quota, carried_forward });
...
router.get("/holidays", requireSameOrg, async (req, res) => {
    try {
        ...
        const holidays = (await req.db.query(`SELECT * FROM holidays WHERE org_id = $1 AND date LIKE $2 ORDER BY date ASC`, [req.userOrgId, `${y}-%`])).rows;
        res.json(holidays);
    }
    catch (err) {
...
router.post("/holidays", requireRole("hr_admin"), requireSameOrg, async (req, res) => {
    try {
        ...
        await ensureDefaultHolidayPolicy(req.userOrgId, req.db);
        const inserted = await req.db.transaction(async (client) => {
            const r = await client.query("INSERT INTO holidays (org_id, date, name, is_optional) VALUES ($1, $2, $3, $4) RETURNING id", [req.userOrgId, date, name.trim(), !!is_optional]);
        ...
        (0, audit_1.logAction)(req, "create", "holiday", inserted.id, {
    ...
    try {
        ...
        const result = await req.db.transaction(async (client) => {
            ...
            for (const h of holidays) {
                if (h.date && h.name) {
                    const r = await client.query("INSERT INTO holidays (org_id, date, name, is_optional) VALUES ($1, $2, $3, $4) ON CONFLICT DO NOTHING RETURNING id", [req.userOrgId, h.date, h.name.trim(), !!h.is_optional]);
                    if (r.rowCount > 0) {
                        added++;
                        if (!h.is_optional) {
                            leavesBooked += await bookHolidayLeavesForOrg(req.userOrgId, h.date, h.name.trim(), req.userId, client);
                        }
                    }
                }
            }
        ...
        (0, audit_1.logAction)(req, "batch_create", "holiday", null, {
...
router.delete("/holidays/:id", requireRole("hr_admin"), requireSameOrg, async (req, res) => {
    try {
        ...
        // was added.
        const removed = await req.db.transaction(async (client) => {
        ...
        (0, audit_1.logAction)(req, "delete", "holiday", Number(id), {
 ...
 */
async function ensureDefaultHolidayPolicy(orgId, db) {
    if (!orgId)
        return null;
    return db.transaction(async (client) => {
        // All policies in the org that *look like* the holiday policy.
        const matches = (await client.query(`SELECT * FROM leave_policies
              WHERE org_id = $1 AND LOWER(leave_type) LIKE 'holiday%'
              ORDER BY id ASC`, [orgId])).rows;
        if (matches.length === 0) {
            const inserted = (await client.query(`INSERT INTO leave_policies
        ...
        for (const dup of dupes) {
            await client.query(`INSERT INTO leave_balances (user_id, leave_type, year, quota, used, carried_forward)
                 ...
                 ON CONFLICT (user_id, leave_type, year) DO UPDATE
                    SET used = leave_balances.used + EXCLUDED.used`, [orgId, dup.leave_type]);
            await client.query(`DELETE FROM leave_balances lb
                 USING users u
                 WHERE lb.user_id = u.id AND u.org_id = $1 AND lb.leave_type = $2`, [orgId, dup.leave_type]);
            // Re-point leave records to the canonical 'holiday' slug.
            await client.query(`UPDATE leaves l
                    SET leave_type = 'holiday'
                  FROM users u
                 WHERE l.user_id = u.id AND u.org_id = $1 AND l.leave_type = $2`, [orgId, dup.leave_type]);
            await client.query("DELETE FROM leave_policies WHERE id = $1", [dup.id]);
        }
        const final = (await client.query("SELECT * FROM leave_policies WHERE id = $1", [survivor.id])).rows[0];
        return final;
    });
}
/**
 ...
 */
async function bookHolidayLeavesForOrg(orgId, date, name, actorId, client) {
    if (!orgId || !date)
        return 0;
    const inserted = (await client.query(`INSERT INTO leaves
         ...
         RETURNING user_id`, [orgId, date, `Public holiday: ${name}`, actorId || null])).rows;
    ...
    await client.query(`INSERT INTO leave_balances (user_id, leave_type, year, quota, used, carried_forward)
         ...
         ON CONFLICT (user_id, leave_type, year) DO NOTHING`, [userIds, year]);
 ...
 */
async function unbookHolidayLeavesForOrg(orgId, date, client) {
    if (!orgId || !date)
        return 0;
    const removed = (await client.query(`DELETE FROM leaves l
         USING users u
         WHERE l.user_id = u.id
           AND u.org_id = $1
           AND l.date = $2
         ...
         RETURNING l.user_id`, [orgId, date])).rows;
 ...
 */
async function recalcHolidayUsed(userId, year, db) {
    if (!userId || !year)
        return;
    await db.query(`UPDATE leave_balances lb
          ...
          WHERE lb.user_id = $1
            ...
            AND lb.year = $3`, [userId, `${year}-%`, year]);
}
/**
 * Delete any leave_balances rows in the org whose leave_type no longer has a
 * matching policy.
 */
async function pruneOrphanBalances(orgId, db) {
    if (!orgId)
        return;
    await db.query(`DELETE FROM leave_balances lb
         USING users u
         WHERE lb.user_id = u.id
           AND u.org_id = $1
           AND NOT EXISTS (
...
async function initializeBalances(userId, orgId, year, db) {
    ...
    for (const policy of policies) {
        ...
        let carryForward = 0;
        if (cfLimit > 0) {
            const prevBalance = (await db.query("SELECT quota, used, carried_forward FROM leave_balances WHERE user_id = $1 AND leave_type = $2 AND year = $3", [userId, policy.leave_type, year - 1])).rows[0];
            if (prevBalance) {
        ...
        await db.query(`INSERT INTO leave_balances (user_id, leave_type, year, quota, carried_forward)
             VALUES ($1, $2, $3, $4, $5)
             ON CONFLICT (user_id, leave_type, year)
             DO UPDATE SET quota = EXCLUDED.quota`, [userId, policy.leave_type, year, policyQuota, carryForward]);
    }
}
/**
 ...
 */
async function syncOrgBalancesToPolicy(orgId, leaveType, db) {
    if (!orgId || !leaveType)
        return;
    const policy = (await db.query("SELECT annual_quota FROM leave_policies WHERE org_id = $1 AND leave_type = $2", [orgId, leaveType])).rows[0];
    if (!policy)
        return;
    await db.query(`UPDATE leave_balances lb SET quota = $1
         FROM users u
         WHERE u.id = lb.user_id AND u.org_id = $2 AND lb.leave_type = $3`, [policy.annual_quota, orgId, leaveType]);
}
...
const routerWithHelpers = router;
...
module.exports = routerWithHelpers;
