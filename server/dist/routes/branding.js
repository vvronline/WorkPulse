...
const { templates, TEMPLATE_KEYS, TEMPLATE_PREVIEW_ARGS, applyBranding, invalidateBrandingCache, loadOrgBranding, } = require("../utils/mailer");
...
router.get("/", requireSameOrg, async (req, res) => {
    try {
        const row = (await req.db.query(`SELECT b.logo_url, b.accent_color, b.updated_at, o.name AS org_name
             FROM organizations o
             LEFT JOIN org_branding b ON b.org_id = o.id
             WHERE o.id = $1`, [req.userOrgId])).rows[0];
        res.json({
...
router.put("/", requireRole("hr_admin"), requireSameOrg, async (req, res) => {
    try {
        ...
        await req.db.query(`INSERT INTO org_branding (org_id, accent_color, updated_by, updated_at)
             VALUES ($1, $2, $3, NOW())
             ON CONFLICT (org_id) DO UPDATE SET
                 accent_color = EXCLUDED.accent_color,
                 updated_by   = EXCLUDED.updated_by,
                 updated_at   = NOW()`, [req.userOrgId, accent_color.toLowerCase(), req.userId]);
        invalidateBrandingCache(req.tenantId, req.userOrgId);
        logAction(req, "update", "org_branding", req.userOrgId, { accent_color });
        const row = (await req.db.query(`SELECT logo_url, accent_color, updated_at FROM org_branding WHERE org_id = $1`, [req.userOrgId])).rows[0];
...
router.post("/logo", requireRole("hr_admin"), requireSameOrg, upload.single("logo"), async (req, res) => {
    if (!req.file)
        return res.status(400).json({ error: "No file uploaded" });
    const newUrl = getUploadUrl(req.tenantId, req.userOrgId, "branding", req.file.filename);
    let oldLogo = null;
    try {
        const existing = (await req.db.query(`SELECT logo_url FROM org_branding WHERE org_id = $1`, [req.userOrgId])).rows[0];
        oldLogo = existing?.logo_url || null;
        await req.db.query(`INSERT INTO org_branding (org_id, logo_url, updated_by, updated_at)
             VALUES ($1, $2, $3, NOW())
             ON CONFLICT (org_id) DO UPDATE SET
                 logo_url   = EXCLUDED.logo_url,
                 updated_by = EXCLUDED.updated_by,
                 updated_at = NOW()`, [req.userOrgId, newUrl, req.userId]);
        invalidateBrandingCache(req.tenantId, req.userOrgId);
...
router.delete("/logo", requireRole("hr_admin"), requireSameOrg, async (req, res) => {
    try {
        const existing = (await req.db.query(`SELECT logo_url FROM org_branding WHERE org_id = $1`, [req.userOrgId])).rows[0];
        await req.db.query(`UPDATE org_branding SET logo_url = NULL, updated_by = $2, updated_at = NOW()
              WHERE org_id = $1`, [req.userOrgId, req.userId]);
        invalidateBrandingCache(req.tenantId, req.userOrgId);
 ...
 * the built-in).
 */
router.get("/email-templates", requireRole("hr_admin"), requireSameOrg, async (req, res) => {
    try {
        const rows = (await req.db.query(`SELECT template_key, subject, body_html, enabled, updated_at
               FROM org_email_templates WHERE org_id = $1`, [req.userOrgId])).rows;
        const overrideMap = new Map(rows.map((r) => [r.template_key, r]));
        const list = TEMPLATE_KEYS.map((key) => {
 ...
 */
router.put("/email-templates/:templateKey", requireRole("hr_admin"), requireSameOrg, async (req, res) => {
    try {
        ...
        await req.db.query(`INSERT INTO org_email_templates (org_id, template_key, subject, body_html, enabled, updated_by, updated_at)
             ...
             ON CONFLICT (org_id, template_key) DO UPDATE SET
                 ...
                 updated_by = EXCLUDED.updated_by,
                 updated_at = NOW()`, [req.userOrgId, key, subject, body_html, enabledFlag, req.userId]);
        invalidateBrandingCache(req.tenantId, req.userOrgId);
...
router.delete("/email-templates/:templateKey", requireRole("hr_admin"), requireSameOrg, async (req, res) => {
    try {
        const key = String(req.params.templateKey || "");
        if (!TEMPLATE_KEYS.includes(key))
            return res.status(400).json({ error: "Unknown template key" });
        await req.db.query(`DELETE FROM org_email_templates WHERE org_id = $1 AND template_key = $2`, [req.userOrgId, key]);
        invalidateBrandingCache(req.tenantId, req.userOrgId);
 ...
 */
router.post("/email-templates/:templateKey/preview", requireRole("hr_admin"), requireSameOrg, async (req, res) => {
    try {
        const key = String(req.params.templateKey || "");
        if (!TEMPLATE_KEYS.includes(key))
            return res.status(400).json({ error: "Unknown template key" });
        ...
        if (!subject || !body) {
            const saved = (await req.db.query(`SELECT subject, body_html FROM org_email_templates WHERE org_id = $1 AND template_key = $2`, [req.userOrgId, key])).rows[0];
