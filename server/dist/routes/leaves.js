...
async function updateLeaveBalance(userId, leaveType, date, duration, operation, client) {
    if (!client)
        throw new Error("updateLeaveBalance must be called within a transaction (client is required)");
    const q = client.query.bind(client);
    const year = parseInt(date.slice(0, 4));
    const durationValue = duration === "half" ? 0.5 : duration === "quarter" ? 0.25 : 1;
    const balRes = await q("SELECT id, used FROM leave_balances WHERE user_id = $1 AND leave_type = $2 AND year = $3 FOR UPDATE", [userId, leaveType, year]);
    ...
    if (!balance) {
        ...
        const ins = await q("INSERT INTO leave_balances (user_id, leave_type, year, quota, used, carried_forward) VALUES ($1, $2, $3, 0, 0, 0) RETURNING id, used", [userId, leaveType, year]);
        balance = ins.rows[0];
    }
...
router.get("/", async (req, res) => {
    try {
        ...
        await req.db.query(`UPDATE leaves
                SET status = 'approved'
              WHERE user_id = $1
                AND leave_type = 'holiday'
                AND status = 'withdraw_pending'
                AND reason LIKE 'Public holiday:%'`, [req.userId]);
        await req.db.query(`UPDATE approval_requests
        ...
        if (type) {
            conditions.push(`l.leave_type = $${pi++}`);
            params.push(type);
        }
    ...
    try {
        ...
        if (targetUserId !== req.userId && req.userOrgId) {
            ...
            if (!targetUser || targetUser.org_id !== req.userOrgId)
                return res.status(403).json({ error: "Cannot view balance for users outside your organization" });
        }
...
router.post("/", async (req, res) => {
    try {
        ...
        if (!rawDates || rawDates.length === 0)
            return res.status(400).json({ error: "Date(s) required" });
        if (rawDates.length > 60)
            return res.status(400).json({ error: "Cannot apply for more than 60 days at once" });
        if (!leave_type)
            return res.status(400).json({ error: "Leave type required" });
        ...
        const existingDates = (await req.db.query("SELECT date FROM leaves WHERE user_id = $1 AND date = ANY($2::text[])", [req.userId, validDates])).rows.map((r) => String(r.date).slice(0, 10));
        const existingSet = new Set(existingDates);
        const newDates = validDates.filter((d) => !existingSet.has(d));
        if (newDates.length === 0)
            return res.json({ message: "0 leave(s) submitted", ids: [] });
        // ── Policy enforcement ────────────────────────────────────────────────
        if (req.userOrgId) {
            const policy = (await req.db.query("SELECT * FROM leave_policies WHERE org_id = $1 AND leave_type = $2", [req.userOrgId, leave_type])).rows[0];
            // If org has configured any policies, the requested type must be covered
            if (!policy) {
                const orgHasPolicy = (await req.db.query("SELECT 1 FROM leave_policies WHERE org_id = $1 LIMIT 1", [req.userOrgId])).rows[0];
                if (orgHasPolicy) {
        ...
        const created = await req.db.transaction(async (client) => {
            ...
            if (req.userOrgId) {
                const policy = (await q("SELECT * FROM leave_policies WHERE org_id = $1 AND leave_type = $2", [req.userOrgId, leave_type])).rows[0];
                if (policy) {
                    // Fetch org fiscal year start for accrual calculation
                    const org = (await q("SELECT fiscal_year_start FROM organizations WHERE id = $1", [req.userOrgId])).rows[0];
                    const datesByYear = {};
                    ...
                    for (const [yr, yearDates] of Object.entries(datesByYear)) {
                        ...
                        const balance = (await q("SELECT quota, used, carried_forward FROM leave_balances WHERE user_id = $1 AND leave_type = $2 AND year = $3 FOR UPDATE", [req.userId, leave_type, year])).rows[0];
                        ...
                        const pendingRow = (await q(`SELECT COALESCE(SUM(
                                CASE duration WHEN 'half' THEN 0.5 WHEN 'quarter' THEN 0.25 ELSE 1 END
                             ), 0) AS pending_days
                             FROM leaves
                             WHERE user_id = $1 AND leave_type = $2 AND status = 'pending' AND date LIKE $3`, [req.userId, leave_type, `${year}-%`])).rows[0];
                        ...
                        if (requested > available) {
                            throw Object.assign(new Error(`Insufficient ${leave_type} leave balance for ${year}. Available: ${Math.max(0, available)} day(s), Requested: ${requested}`), { isValidation: true });
                        }
                    }
                }
            }
            const ids = [];
            for (const d of newDates) {
                const leaveResult = await q(`INSERT INTO leaves (user_id, leave_type, date, duration, reason, status, approved_by)
                     VALUES ($1, $2, $3, $4, $5, 'pending', $6)
                     RETURNING id`, [req.userId, leave_type, d, leaveDuration, reason || null, approver?.id || null]);
                const newLeave = leaveResult.rows[0];
                if (!newLeave)
                    continue;
                ids.push(newLeave.id);
                await q(`INSERT INTO approval_requests (org_id, requester_id, approver_id, type, reference_id, reason, metadata)
        ...
        try {
            if (approver?.id) {
                ...
                await req.db.query("INSERT INTO notifications (user_id, type, title, body) VALUES ($1, $2, $3, $4)", [approver.id, "approval", "New Leave Request", `${requesterName} submitted ${created.length} ${leave_type} leave request(s).`]);
...
router.patch("/:id/approve", requireRole("manager"), async (req, res) => {
    try {
        ...
        const leave = (await req.db.query("SELECT l.*, u.org_id AS leave_org_id, u.manager_id AS leave_manager_id FROM leaves l JOIN users u ON u.id = l.user_id WHERE l.id = $1", [leaveId])).rows[0];
        if (!leave)
        ...
        const approved = await req.db.transaction(async (client) => {
            ...
            const freshLeave = (await client.query("SELECT id, status, user_id, leave_type, date, duration FROM leaves WHERE id = $1 FOR UPDATE", [leave.id])).rows[0];
            if (!freshLeave || freshLeave.status !== "pending")
                return false;
            await client.query("UPDATE leaves SET status = 'approved', approved_by = $1, reviewed_at = NOW() WHERE id = $2", [req.userId, leave.id]);
            await client.query("UPDATE approval_requests SET status = 'approved', approver_id = $1, reviewed_at = NOW() WHERE type = 'leave' AND reference_id = $2 AND status = 'pending'", [req.userId, leave.id]);
            await updateLeaveBalance(freshLeave.user_id, freshLeave.leave_type, freshLeave.date, freshLeave.duration || "full", "add", client);
            return true;
        });
        if (!approved)
            return res.status(400).json({ error: "Leave is no longer pending" });
        ...
        if (leaveUser) {
            await req.db.query("INSERT INTO notifications (user_id, type, title, body) VALUES ($1, $2, $3, $4)", [leave.user_id, "leave", "Leave Approved ✅", `Your ${leave.leave_type} leave on ${leave.date} has been approved.`]);
...
router.patch("/:id/reject", requireRole("manager"), async (req, res) => {
    try {
        ...
        await req.db.transaction(async (client) => {
            await client.query("UPDATE leaves SET status = 'rejected', reject_reason = $1, approved_by = $2, reviewed_at = NOW() WHERE id = $3", [reason || null, req.userId, leave.id]);
            await client.query("UPDATE approval_requests SET status = 'rejected', approver_id = $1, reviewed_at = NOW(), reject_reason = $2 WHERE type = 'leave' AND reference_id = $3 AND status = 'pending'", [req.userId, reason || null, leave.id]);
        });
        // Notify the leave requester
        const leaveUser = (await req.db.query("SELECT email, full_name FROM users WHERE id = $1", [leave.user_id])).rows[0];
        if (leaveUser) {
            await req.db.query("INSERT INTO notifications (user_id, type, title, body) VALUES ($1, $2, $3, $4)", [leave.user_id, "leave", "Leave Rejected", `Your ${leave.leave_type} leave on ${leave.date} has been rejected.${reason ? " Reason: " + reason : ""}`]);
...
router.delete("/:id", async (req, res) => {
    try {
        ...
        if (leave.status === "approved")
            return res.status(400).json({ error: "Cannot cancel an approved leave. Ask your manager to revoke it." });
...
router.post("/:id/withdraw", async (req, res) => {
    try {
        ...
        if (leave.status === "pending") {
            ...
            await req.db.query(`INSERT INTO approval_requests (org_id, requester_id, approver_id, type, reference_id, reason, metadata)
            ...
            try {
                if (approver?.id) {
                    const requesterName = (await req.db.query("SELECT full_name FROM users WHERE id = $1", [req.userId])).rows[0]?.full_name || "A team member";
                    await req.db.query("INSERT INTO notifications (user_id, type, title, body) VALUES ($1, $2, $3, $4)", [approver.id, "approval", "Leave Withdrawal Request", `${requesterName} requested withdrawal of ${leave.leave_type} leave on ${leave.date}.`]);
...
router.patch("/:id/revoke", requireRole("manager"), async (req, res) => {
    try {
        ...
        if (!leave)
            return res.status(404).json({ error: "Leave not found" });
        if (req.userOrgId && leave.leave_org_id !== req.userOrgId)
            return res.status(403).json({ error: "Cannot revoke leaves from another organization" });
        if (leave.status !== "approved")
            return res.status(400).json({ error: "Leave is not approved" });
        const revoked = await req.db.transaction(async (client) => {
            ...
            const freshLeave = (await client.query("SELECT id, status, user_id, leave_type, date, duration FROM leaves WHERE id = $1 FOR UPDATE", [leave.id])).rows[0];
            if (!freshLeave || freshLeave.status !== "approved")
                return false;
            await client.query("UPDATE leaves SET status = 'revoked', approved_by = $1, reviewed_at = NOW() WHERE id = $2", [req.userId, leave.id]);
        ...
        if (!revoked)
            return res.status(400).json({ error: "Leave is no longer approved" });
        // Notify the leave requester
        const leaveUser = (await req.db.query("SELECT email, full_name FROM users WHERE id = $1", [leave.user_id])).rows[0];
        if (leaveUser) {
            await req.db.query("INSERT INTO notifications (user_id, type, title, body) VALUES ($1, $2, $3, $4)", [leave.user_id, "leave", "Leave Revoked", `Your ${leave.leave_type} leave on ${leave.date} has been revoked by management.`]);
...
// Attach the helper to the router so other modules can import it via
// `require('./leaves').updateLeaveBalance` (RouterWithHelpers pattern).
router.updateLeaveBalance = updateLeaveBalance;
