...
async function verifyParticipant(convId, userId, db) {
    const r = await db.query("SELECT 1 FROM conversation_participants WHERE conversation_id = $1 AND user_id = $2", [convId, userId]);
...
router.get("/ice-config", auth, async (req, res) => {
    try {
        ...
        if (expiresAt)
            ...
            payload.expiresAt = Math.floor(Date.now() / 1000) + ttl;
...
router.get("/presence", auth, async (req, res) => {
    try {
        ...
        if (!orgId)
            return res.json({});
        const orgMembers = (await req.db.query("SELECT id FROM users WHERE id = ANY($1) AND org_id = $2", [ids, orgId])).rows.map((r) => r.id);
        const orgMemberSet = new Set(orgMembers);
        const allowedIds = ids.filter((id) => orgMemberSet.has(id));
        if (allowedIds.length === 0)
            return res.json({});
        const statusService = require("../services/status");
        const payloads = await statusService.getEffectiveBulk({ db: req.db, tenantId: req.tenantId || null }, allowedIds);
        const result = {};
...
router.post("/conversations", auth, async (req, res) => {
    try {
        ...
        if (isSelfChat) {
            ...
            const selfUser = (await req.db.query("SELECT id, org_id FROM users WHERE id = $1 AND is_active = TRUE", [req.userId])).rows[0];
            if (!selfUser)
            ...
            const conv = await req.db.transaction(async (client) => {
                ...
                const c = (await client.query("INSERT INTO conversations (org_id) VALUES ($1) RETURNING id", [orgId])).rows[0];
                await client.query("INSERT INTO conversation_participants (conversation_id, user_id) VALUES ($1, $2)", [c.id, req.userId]);
                return c;
            });
            return res.status(conv.existed ? 200 : 201).json({ conversationId: conv.id });
        }
        const users = (await req.db.query("SELECT id, org_id FROM users WHERE id = ANY($1) AND is_active = TRUE", [[req.userId, otherUserId]])).rows;
        if (users.length !== 2)
            return res.status(400).json({ error: "User not found" });
        ...
        const conv = await req.db.transaction(async (client) => {
            ...
            if (existing) {
                ...
                await client.query(`DELETE FROM conversation_participants
                     WHERE conversation_id = $1 AND user_id NOT IN ($2, $3)`, [existing.conversation_id, req.userId, otherUserId]);
                return { id: existing.conversation_id, existed: true };
            }
            const c = (await client.query("INSERT INTO conversations (org_id) VALUES ($1) RETURNING id", [orgId])).rows[0];
            await client.query("INSERT INTO conversation_participants (conversation_id, user_id) VALUES ($1, $2), ($1, $3)", [c.id, req.userId, otherUserId]);
            return c;
        });
        if (conv.existed) {
...
router.post("/conversations/group", auth, async (req, res) => {
    try {
        ...
        const users = (await req.db.query("SELECT id FROM users WHERE id = ANY($1) AND org_id = $2 AND is_active = TRUE", [allIds, orgId])).rows;
        if (users.length !== allIds.length) {
            return res.status(400).json({ error: "Some users not found in your organization" });
        }
        const conv = await req.db.transaction(async (client) => {
            const c = (await client.query("INSERT INTO conversations (org_id, name, is_group, created_by) VALUES ($1, $2, TRUE, $3) RETURNING id", [orgId, name.trim().slice(0, 100), req.userId])).rows[0];
            const values = allIds.map((_uid, i) => `($1, $${i + 2})`).join(", ");
            await client.query(`INSERT INTO conversation_participants (conversation_id, user_id) VALUES ${values}`, [c.id, ...allIds]);
            return c;
        });
...
router.put("/conversations/:id/group", auth, async (req, res) => {
    try {
        ...
        if (Array.isArray(addUserIds) && addUserIds.length > 0) {
            const valid = (await req.db.query("SELECT id FROM users WHERE id = ANY($1) AND org_id = $2 AND is_active = TRUE", [addUserIds, conv.org_id])).rows.map((r) => r.id);
            for (const uid of valid) {
                await req.db.query("INSERT INTO conversation_participants (conversation_id, user_id) VALUES ($1, $2) ON CONFLICT DO NOTHING", [convId, uid]);
        ...
        if (Array.isArray(removeUserIds) && removeUserIds.length > 0) {
            ...
            const validRemoveIds = (await req.db.query(`SELECT u.id FROM users u
                 JOIN conversation_participants cp ON cp.user_id = u.id AND cp.conversation_id = $3
                 WHERE u.id = ANY($1) AND u.org_id = $2 AND u.is_active = TRUE`, [removeUserIds, conv.org_id, convId])).rows.map((r) => r.id);
            for (const uid of validRemoveIds) {
                if (uid === req.userId)
                    continue;
                await req.db.query("DELETE FROM conversation_participants WHERE conversation_id = $1 AND user_id = $2", [convId, uid]);
...
router.post("/conversations/:id/read", auth, async (req, res) => {
    try {
        ...
        await req.db.query(`INSERT INTO message_reads (conversation_id, user_id, last_read_at)
             VALUES ($1, $2, NOW())
             ON CONFLICT (conversation_id, user_id) DO UPDATE SET last_read_at = NOW()`, [convId, req.userId]);
        redis.resetUnread(req.tenantId, req.userId, convId);
        // Notify others about read receipt
        const participants = (await req.db.query("SELECT user_id FROM conversation_participants WHERE conversation_id = $1 AND user_id != $2", [convId, req.userId])).rows;
...
router.post("/conversations/:id/files", auth, loadUserContext, chatUpload.single("file"), async (req, res) => {
    try {
        ...
        const result = (await req.db.query(`INSERT INTO messages (conversation_id, sender_id, content, file_url, file_name, file_type, file_size, reply_to_id)
             VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id, created_at`, [convId, req.userId, content, fileUrl, safeName, req.file.mimetype, req.file.size, replyToId])).rows[0];
        await req.db.query("UPDATE conversations SET updated_at = NOW() WHERE id = $1", [convId]);
        await req.db.query(`INSERT INTO message_reads (conversation_id, user_id, last_read_at)
             VALUES ($1, $2, $3)
             ON CONFLICT (conversation_id, user_id) DO UPDATE SET last_read_at = $3`, [convId, req.userId, result.created_at]);
        const sender = (await req.db.query("SELECT full_name, avatar, username FROM users WHERE id = $1", [req.userId])).rows[0];
        const participants = (await req.db.query("SELECT user_id FROM conversation_participants WHERE conversation_id = $1", [convId])).rows;
        const outMsg = {
            id: result.id,
            conversationId: convId,
            senderId: req.userId,
...
router.post("/messages/:id/reactions", auth, async (req, res) => {
    try {
        ...
        const existing = (await req.db.query("SELECT id FROM message_reactions WHERE message_id = $1 AND user_id = $2 AND emoji = $3", [msgId, req.userId, emoji])).rows[0];
        let action;
        if (existing) {
            ...
            await req.db.query("INSERT INTO message_reactions (message_id, user_id, emoji) VALUES ($1, $2, $3) ON CONFLICT (message_id, user_id, emoji) DO NOTHING", [msgId, req.userId, emoji]);
            action = "added";
        }
        const sender = (await req.db.query("SELECT full_name FROM users WHERE id = $1", [req.userId])).rows[0];
        const participants = (await req.db.query("SELECT user_id FROM conversation_participants WHERE conversation_id = $1", [msg.conversation_id])).rows;
        for (const p of participants) {
...
router.post("/messages/:id/forward", auth, async (req, res) => {
    try {
        ...
        const original = (await req.db.query("SELECT * FROM messages WHERE id = $1 AND deleted_at IS NULL", [msgId])).rows[0];
        if (!original)
            return res.status(404).json({ error: "Message not found" });
        ...
        for (const cId of conversationIds) {
            ...
            if (isNaN(convIdNum))
                continue;
            if (!(await verifyParticipant(convIdNum, req.userId, req.db)))
                continue;
            const result = (await req.db.query(`INSERT INTO messages (conversation_id, sender_id, content, file_url, file_name, file_type, file_size, forwarded_from_id)
                 VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id, created_at`, [convIdNum, req.userId, original.content, original.file_url, original.file_name, original.file_type, original.file_size, msgId])).rows[0];
            await req.db.query("UPDATE conversations SET updated_at = NOW() WHERE id = $1", [convIdNum]);
            await req.db.query(`INSERT INTO message_reads (conversation_id, user_id, last_read_at)
                 VALUES ($1, $2, $3)
                 ON CONFLICT (conversation_id, user_id) DO UPDATE SET last_read_at = $3`, [convIdNum, req.userId, result.created_at]);
            const participants = (await req.db.query("SELECT user_id FROM conversation_participants WHERE conversation_id = $1", [convIdNum])).rows;
            const outMsg = {
                id: result.id,
...
router.post("/messages/:id/star", auth, async (req, res) => {
    try {
        ...
        const existing = (await req.db.query("SELECT 1 FROM starred_messages WHERE user_id = $1 AND message_id = $2", [req.userId, msgId])).rows[0];
        if (existing) {
...
router.post("/conversations/:id/polls", auth, async (req, res) => {
    try {
        ...
        const poll = (await req.db.query(`INSERT INTO polls (conversation_id, creator_id, question, options, multi_select)
             VALUES ($1, $2, $3, $4, $5) RETURNING *`, [convId, req.userId, question.trim().slice(0, 500), JSON.stringify(cleanOpts), !!multiSelect])).rows[0];
        // Insert a message of type 'poll' referencing this poll
        const result = (await req.db.query(`INSERT INTO messages (conversation_id, sender_id, content, format_type, metadata)
             VALUES ($1, $2, $3, 'poll', $4) RETURNING id, created_at`, [convId, req.userId, question.trim().slice(0, 500), JSON.stringify({ pollId: poll.id })])).rows[0];
        await req.db.query("UPDATE conversations SET updated_at = NOW() WHERE id = $1", [convId]);
        const sender = (await req.db.query("SELECT full_name, avatar, username FROM users WHERE id = $1", [req.userId])).rows[0];
        const participants = (await req.db.query("SELECT user_id FROM conversation_participants WHERE conversation_id = $1", [convId])).rows;
        const outMsg = {
...
router.post("/polls/:id/vote", auth, async (req, res) => {
    try {
        ...
        // Toggle vote
        const existing = (await req.db.query("SELECT id FROM poll_votes WHERE poll_id = $1 AND user_id = $2 AND option_idx = $3", [pollId, req.userId, optionIdx])).rows[0];
        if (existing) {
        ...
        const votes = (await req.db.query("SELECT option_idx, array_agg(user_id) AS user_ids FROM poll_votes WHERE poll_id = $1 GROUP BY option_idx", [pollId])).rows;
        const voteMap = {};
        for (const v of votes)
            voteMap[v.option_idx] = v.user_ids;
        // Broadcast poll update
        const participants = (await req.db.query("SELECT user_id FROM conversation_participants WHERE conversation_id = $1", [poll.conversation_id])).rows;
        for (const p of participants) {
...
router.get("/polls/:id", auth, async (req, res) => {
    try {
        ...
        const votes = (await req.db.query(`SELECT pv.option_idx, pv.user_id, u.full_name
             FROM poll_votes pv JOIN users u ON u.id = pv.user_id
             WHERE pv.poll_id = $1`, [pollId])).rows;
        const voteMap = {};
        for (const v of votes) {
            if (!voteMap[v.option_idx])
                voteMap[v.option_idx] = [];
...
router.post("/conversations/:id/pin", auth, async (req, res) => {
    try {
        ...
        const result = (await req.db.query(`UPDATE conversation_participants SET is_pinned = NOT is_pinned
             WHERE conversation_id = $1 AND user_id = $2
             RETURNING is_pinned`, [convId, req.userId])).rows[0];
...
router.post("/conversations/:id/favourite", auth, async (req, res) => {
    try {
        ...
        const result = (await req.db.query(`UPDATE conversation_participants SET is_favourite = NOT is_favourite
             WHERE conversation_id = $1 AND user_id = $2
             RETURNING is_favourite`, [convId, req.userId])).rows[0];
...
router.post("/messages/:id/delivered", auth, async (req, res) => {
    try {
        ...
        const msg = (await req.db.query(`SELECT m.conversation_id FROM messages m
             JOIN conversation_participants cp ON cp.conversation_id = m.conversation_id AND cp.user_id = $1
             WHERE m.id = $2`, [req.userId, msgId])).rows[0];
        if (!msg)
            return res.status(403).json({ error: "Not a participant" });
        await req.db.query(`UPDATE messages SET delivered_to = delivered_to || $1::jsonb
             WHERE id = $2 AND NOT delivered_to @> $1::jsonb`, [JSON.stringify([req.userId]), msgId]);
        res.json({ ok: true });
    }
    catch (err) {
