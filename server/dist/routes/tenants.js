...
const { createTenant, deleteTenant, suspendTenant, reactivateTenant, getTenantById, getTenantPool, getPoolStats, listActiveTenants, } = require("../utils/tenantManager");
...
const { PLANS, PLAN_KEYS, FEATURE_LABELS, getEffectiveFeatures, getPlanLimits, sanitizeFeatureOverrides, planFeatureDiff, loadPlanCatalog, savePlanCatalog, getPlans, DEFAULT_PLANS, FEATURE_KEYS, } = require("../utils/planCatalog");
const { generateApprovalCode, hashApprovalCode, verifyApprovalCode, getImpersonationPolicy, updateImpersonationPolicy, computeEffectiveStatus, expireStaleRequests, getActiveSession, getOrCreateInspectorUser, } = require("../utils/impersonationApproval");
const { getPlatformConfig, updatePlatformConfig, } = require("../utils/platformConfig");
...
async function verifyActorPassword(req, password) {
    ...
    let actor;
    if (req.tenantId) {
        actor = (await req.db.query("SELECT id, password, full_name, is_active FROM users WHERE id = $1", [req.userId])).rows[0];
    }
    else {
        actor = (await masterQuery("SELECT id, password, full_name, is_active FROM platform_users WHERE id = $1", [req.userId])).rows[0];
    }
...
function buildSessionSummary(session) {
    if (!session)
        return { session_start: new Date(), total: 0, reads: 0, writes: 0, actions: [] };
    ...
    try {
        ...
        if (req.isPlatformUser) {
            ...
            if (platUser) {
                ...
                if (!existing) {
                    const newUser = (await db.query(`INSERT INTO users (username, password, full_name, email, org_id, role)
                         VALUES ($1, $2, $3, $4, 1, 'platform_admin') RETURNING id`, [platUser.username, platUser.password, platUser.full_name, platUser.email || `${platUser.username}@platform.local`])).rows[0];
                    if (platUser.email) {
                        await masterQuery("INSERT INTO user_directory (email, username, tenant_id, user_id) VALUES ($1, $2, $3, $4) ON CONFLICT DO NOTHING", [platUser.email.toLowerCase(), platUser.username.toLowerCase(), tenant.id, newUser.id]);
                    }
                }
            }
        }
...
router.get("/", async (req, res) => {
    try {
        ...
        if (search) {
            where.push(`(t.org_name ILIKE $${p} OR t.slug ILIKE $${p})`);
            params.push(`%${search}%`);
            p++;
        }
...
router.put("/plan-catalog", async (req, res) => {
    try {
...
router.post("/plan-catalog/reset", async (req, res) => {
    try {
...
router.get("/platform-users", async (req, res) => {
    try {
        const result = await masterQuery("SELECT id, username, full_name, email, avatar, is_active, created_at FROM platform_users ORDER BY created_at DESC");
...
router.post("/platform-users", async (req, res) => {
    try {
        ...
        const result = await masterQuery("INSERT INTO platform_users (username, password, full_name, email) VALUES ($1, $2, $3, $4) RETURNING id, username, full_name, email, is_active, created_at", [username.toLowerCase(), hash, full_name, email.toLowerCase()]);
...
router.get("/impersonation-policy", async (req, res) => {
    try {
...
router.put("/impersonation-policy", async (req, res) => {
    try {
        const { requires_consent, break_glass_allowed, max_session_minutes, code_ttl_minutes, } = req.body || {};
        const patch = {};
        if (typeof requires_consent === "boolean")
            patch.impersonation_requires_consent = requires_consent;
        if (typeof break_glass_allowed === "boolean")
            patch.impersonation_break_glass_allowed = break_glass_allowed;
...
router.post("/:id/access-requests", async (req, res) => {
    try {
        ...
        const me = (await masterQuery("SELECT id, full_name, email FROM platform_users WHERE id = $1", [req.userId])).rows[0];
        ...
        try {
            ...
            const admins = (await db.query(`SELECT id FROM users WHERE is_active = TRUE AND role IN ('super_admin','platform_admin')`)).rows;
            const { sendToUser } = require("../utils/ws");
            for (const a of admins) {
                try {
                    await db.query(`INSERT INTO notifications (user_id, type, title, body)
                         VALUES ($1, 'platform_access_request', $2, $3)`, [a.id,
...
router.get("/access-requests", async (req, res) => {
    try {
        await expireStaleRequests();
        const { status, tenant_id, limit: rawLimit, offset } = req.query;
        const limit = Math.min(Math.max(Number(rawLimit) || 50, 1), 200);
        const off = Math.max(Number(offset) || 0, 0);
        ...
        let p = 2;
        if (status) {
            where.push(`ar.status = $${p++}`);
            params.push(status);
        }
        if (tenant_id) {
            where.push(`ar.tenant_id = $${p++}`);
...
router.delete("/access-requests/:reqId", async (req, res) => {
    try {
        const rid = Number(req.params.reqId);
        const row = (await masterQuery("SELECT * FROM tenant_access_requests WHERE id = $1 AND requested_by = $2", [rid, req.userId])).rows[0];
        if (!row)
        ...
        try {
            ...
            for (const t of activeTenants.rows) {
                try {
                    ...
                    const r = await pool.query(`SELECT COUNT(*)::int AS c
                           FROM users
...
router.get("/:id", async (req, res) => {
    try {
        ...
        const userCount = (await masterQuery("SELECT COUNT(*) FROM user_directory WHERE tenant_id = $1", [tenant.id])).rows[0].count;
...
router.put("/:id", async (req, res) => {
    try {
        ...
        const result = await masterQuery(`UPDATE tenants SET
...
router.put("/:id/domain", async (req, res) => {
    try {
        ...
        const result = await masterQuery("UPDATE tenants SET custom_domain = $1, updated_at = NOW() WHERE id = $2 RETURNING *", [custom_domain || null, tid]);
        await redis.del(`tenant:id:${tid}`);
...
router.put("/:id/features", async (req, res) => {
    try {
        ...
        const result = await masterQuery("UPDATE tenants SET features = $1, updated_at = NOW() WHERE id = $2 RETURNING *", [JSON.stringify(merged), tid]);
        await redis.del(`tenant:id:${tid}`);
        if (tenant.custom_domain)
            await redis.del(`tenant:domain:${tenant.custom_domain}`);
...
router.put("/:id/plan", async (req, res) => {
    try {
        ...
        params.push(tid);
        const result = await masterQuery(`UPDATE tenants SET ${updates.join(", ")} WHERE id = $${p} RETURNING *`, params);
        await redis.del(`tenant:id:${tid}`);
        if (tenant.custom_domain)
            await redis.del(`tenant:domain:${tenant.custom_domain}`);
...
router.put("/:id/limits", async (req, res) => {
    try {
        ...
        if (!tenant)
            return res.status(404).json({ error: "Tenant not found" });
        const result = await masterQuery("UPDATE tenants SET max_users = $1, max_storage_mb = $2, updated_at = NOW() WHERE id = $3 RETURNING *", [max_users ?? tenant.max_users, max_storage_mb ?? tenant.max_storage_mb, tid]);
        await redis.del(`tenant:id:${tid}`);
...
router.post("/:id/impersonate", async (req, res) => {
    try {
        ...
        const me = (await masterQuery("SELECT id, password, full_name, is_active FROM platform_users WHERE id = $1", [req.userId])).rows[0];
        if (!me || !me.is_active) {
        ...
        const ttlMinutes = Math.min(policy.maxSessionMinutes, request?.duration_minutes || policy.maxSessionMinutes);
        ...
        }, process.env.JWT_SECRET, { expiresIn: `${ttlMinutes}m` });
...
const NON_DEFAULT_USER_DATA_MSG = "User data for non-default tenants is only accessible via approved impersonation.";
...
router.get("/:id/users", async (req, res) => {
    try {
        ...
        if (!tenant)
            return res.status(404).json({ error: "Tenant not found" });
        if (!ensureDefaultTenant(tenant, res))
            return;
        const db = await getTenantPool(tenant.db_name, tenant.db_host);
        ...
        const usersRes = await db.query(`SELECT u.id, u.username, u.full_name, u.email, u.role, u.is_active, u.org_id, u.created_at
             FROM users u ${whereClause}
             ORDER BY u.created_at DESC
             LIMIT $${params.length + 1} OFFSET $${params.length + 2}`, [...params, limit, off]);
...
router.post("/:id/users", async (req, res) => {
    try {
        ...
        const result = await db.query("INSERT INTO users (username, password, full_name, email, org_id, role) VALUES ($1,$2,$3,$4,1,$5) RETURNING id, username, full_name, email, role", [username, hash, full_name, email, validRole]);
        // Add to user_directory
        await masterQuery("INSERT INTO user_directory (email, username, tenant_id, user_id) VALUES ($1, $2, $3, $4)", [email.toLowerCase(), username.toLowerCase(), tid, result.rows[0].id]);
...
router.put("/:tenantId/users/:userId/deactivate", async (req, res) => {
    try {
        ...
        if (!ensureDefaultTenant(tenant, res))
            return;
        const db = await getTenantPool(tenant.db_name, tenant.db_host);
        const result = await db.query("UPDATE users SET is_active = FALSE WHERE id = $1 RETURNING id, username", [uid]);
        if (!result.rows[0])
...
router.post("/:id/seed", async (req, res) => {
    try {
        ...
        for (const pol of defaultPolicies) {
            ...
            if (!exists) {
                await db.query("INSERT INTO leave_policies (org_id, leave_type, annual_quota, carry_forward_limit) VALUES (1, $1, $2, $3)", [pol.leave_type, pol.annual_quota, pol.carry_forward_limit]);
