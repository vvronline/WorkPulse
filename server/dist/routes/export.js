...
async function getUserTargetMs(userId, db) {
    ...
    if (orgRow?.org_id) {
        ...
        if (orgConfig?.work_hours_per_day)
            return orgConfig.work_hours_per_day * 3600000;
    }
    ...
    try {
        ...
        for (const member of members) {
            ...
            if (memberOrgRow?.org_id) {
                ...
                if (mOrgCfg?.work_hours_per_day)
                    memberTargetMs = mOrgCfg.work_hours_per_day * 3600000;
            }
            ...
            const tasksDone = (await req.db.query("SELECT COUNT(*) AS c FROM tasks WHERE (user_id = $1 OR assigned_to = $1) AND status = 'done' AND date BETWEEN $2 AND $3", [member.id, from, to])).rows[0].c;
            const leaves = (await req.db.query("SELECT COUNT(*) AS c FROM leaves WHERE user_id = $1 AND status = 'approved' AND date BETWEEN $2 AND $3", [member.id, from, to])).rows[0].c;
            rows.push({
...
router.get("/payroll-hours", requireRole("team_lead"), async (req, res) => {
    try {
        ...
        const userIds = await getVisibleUserIds(req.userId, req.userRole, req.userOrgId, req.userTeamId, req.db);
        if (userIds.length === 0)
            return sendCSV(res, [], [], "payroll.csv");
        // 1. Org work configuration
        const orgRow = (await req.db.query(`SELECT work_days, work_hours_per_day FROM organizations WHERE id = $1`, [req.userOrgId])).rows[0] || {};
        const workDaySet = new Set((orgRow.work_days || "1,2,3,4,5")
        ...
        const holidaySet = new Set((await req.db.query(`SELECT date FROM holidays
                 WHERE org_id = $1 AND date BETWEEN $2 AND $3 AND is_optional = FALSE`, [req.userOrgId, from, to])).rows.map((h) => h.date));
        // 3. Member info
        const members = (await req.db.query(`SELECT u.id, u.full_name, u.email, u.role, u.timezone_offset,
                    d.name AS department_name, tm.name AS team_name
             FROM users u
             LEFT JOIN departments d  ON d.id  = u.department_id
             LEFT JOIN teams     tm  ON tm.id  = u.team_id
             WHERE u.id = ANY($1) AND u.is_active = TRUE
             ORDER BY u.full_name`, [userIds])).rows;
        ...
        for (const member of members) {
            ...
            const entries = (await req.db.query(`SELECT * FROM time_entries
                 WHERE user_id = $1 AND approval_status = 'approved'
                   AND (timestamp + $4::interval)::date BETWEEN $2::date AND $3::date
                 ORDER BY timestamp ASC`, [member.id, from, to, memberInterval])).rows;
            ...
            const leaveMap = {};
            for (const l of (await req.db.query(`SELECT date, leave_type, duration FROM leaves
                 WHERE user_id = $1 AND status = 'approved' AND date BETWEEN $2 AND $3`, [member.id, from, to])).rows) {
                leaveMap[l.date] = l;
            }
            // Period accumulators
            ...
            for (const date of allDays) {
                ...
                if (!isWorkDay) {
                    if (!hasWork)
                        continue; // off day, didn't work — skip
                    dayTypeKey = "weekend_overtime";
                }
                else if (isHoliday) {
                    if (!hasWork)
                        continue; // holiday, didn't work — skip
...
router.get("/team-leaves", requireRole("team_lead"), async (req, res) => {
    try {
        ...
        const userIds = await getVisibleUserIds(req.userId, req.userRole, req.userOrgId, req.userTeamId, req.db);
        if (userIds.length === 0)
            return sendCSV(res, [], [], "team_leaves.csv");
        const leaves = (await req.db.query(`
