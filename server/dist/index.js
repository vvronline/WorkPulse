...
if (!process.env.JWT_SECRET) {
    logger_1.logger.fatal("JWT_SECRET environment variable is not set. Server cannot start.");
    process.exit(1);
}
...
const collaboration_1 = require("./utils/collaboration");
...
app.use("/api/notes", express_1.default.json({ limit: "5mb" }));
app.use("/api/profile/avatar", express_1.default.json({ limit: "10mb" }));
...
app.use("/uploads", authMiddleware, async (req, res, next) => {
    const resolved = path_1.default.resolve(__dirname, "uploads", req.path.replace(/^\//, ""));
    if (!resolved.startsWith(path_1.default.resolve(__dirname, "uploads"))) {
...
app.get("/api/health", async (req, res) => {
    try {
        await (0, db_1.masterQuery)("SELECT 1");
        const detail = req.query.detail === "true";
        if (detail) {
            const { expectedMigrationCount } = require("./utils/migrationRunner");
            const migResult = await (0, db_1.masterQuery)("SELECT COUNT(*)::int AS count FROM _migrations");
        ...
        logger_1.logger.error({ err }, "Health check DB ping failed");
        res.status(503).json({ status: "error", time: new Date().toISOString(), error: "Database unreachable" });
    }
});
// ============= AUTO CLOCK-OUT (multi-tenant) =============
async function autoClockOut() {
    const result = await (0, tenantManager_1.forEachTenant)(async (db) => { await autoClockOutForDb(db); }, { label: "autoClockOut" });
    if (result.failed > 0) {
        logger_1.logger.warn({ ok: result.ok, failed: result.failed }, "autoClockOut completed with failures");
    }
}
async function autoClockOutForDb(db) {
    ...
    for (let i = 0; i < activeUsers.length; i += BATCH) {
        ...
        await Promise.allSettled(batch.map((user) => autoClockOutUser(db, user).catch((e) => logger_1.logger.error({ userId: user.id, err: e }, "Auto clock-out failed"))));
    }
}
async function autoClockOutUser(db, user) {
    ...
    await db.transaction(async (client) => {
        ...
        if (!lastEntryRow || lastEntryRow.entry_type === "clock_out")
            return;
        ...
        if (lastEntryRow.entry_type === "break_start") {
            await client.query("INSERT INTO time_entries (user_id, entry_type, timestamp) VALUES ($1, $2, $3)", [user.id, "break_end", autoTs]);
        }
        await client.query("INSERT INTO time_entries (user_id, entry_type, timestamp) VALUES ($1, $2, $3)", [user.id, "clock_out", autoTs]);
        logger_1.logger.info({ userId: user.id, date: yesterdayStr }, "Auto clock-out applied");
    });
}
...
async function cleanupTokens() {
    ...
    if (result.failed > 0) {
        logger_1.logger.warn({ ok: result.ok, failed: result.failed }, "cleanupTokens completed with failures");
    }
}
...
async function bootstrap() {
    ...
    bootstrapPromise = (async () => {
        ...
        try {
            ...
            logger_1.logger.info("Migration sweep complete on bootstrap");
        }
        catch (err) {
            logger_1.logger.error({ err: err.message }, "Migration sweep FAILED on bootstrap — schema may be incomplete");
        }
    })();
    return bootstrapPromise;
}
...
if (process.env.NODE_ENV !== "test") {
    bootstrap().catch((err) => {
        logger_1.logger.error({ err }, "Bootstrap failed at module load — server may run with stale schema");
    });
}
...
if (require.main === module) {
    ...
    (async () => {
        ...
        (0, collaboration_1.createCollaborationServer)(httpServer);
        httpServer.listen(PORT, () => {
            logger_1.logger.info({ port: PORT }, "Server running");
        });
        (0, jobs_1.initJobs)({ autoClockOut, cleanupTokens });
        async function shutdown() {
            logger_1.logger.info("Shutting down gracefully...");
            httpServer.close(async () => {
                await (0, jobs_1.shutdownJobs)();
                await redis.shutdown();
                await (0, tenantManager_1.destroyAllPools)();
                await db_1.pool.end();
                process.exit(0);
            });
            setTimeout(async () => { await (0, jobs_1.shutdownJobs)(); await redis.shutdown(); await (0, tenantManager_1.destroyAllPools)(); await db_1.pool.end(); process.exit(1); }, 5000);
        }
