...
exports.transaction = exports.query = exports.masterTransaction = exports.masterQuery = exports.pool = void 0;
exports.makePoolQuery = makePoolQuery;
exports.makePoolTransaction = makePoolTransaction;
exports.initDB = initDB;
exports.initMasterDB = initMasterDB;
exports.initTenantSchema = initTenantSchema;
exports.seedAgileDefaults = seedAgileDefaults;
/**
...
if (!process.env.DATABASE_URL) {
    logger_1.logger.fatal('DATABASE_URL environment variable is not set. Server cannot start.');
    process.exit(1);
}
...
exports.pool = pool;
pool.on('error', (err) => {
    logger_1.logger.error({ err }, 'Unexpected master DB pool error');
});
// ────────────────────────────────────────────────────────────────────────────
...
exports.masterTransaction = masterTransaction;
...
async function initDB() {
    ...
    try {
        ...
        logger_1.logger.warn({ err: err.message }, 'Agile defaults seeding failed in initDB (non-fatal)');
    }
    logger_1.logger.info('Database schema initialised (master + legacy tenant tables)');
}
// ────────────────────────────────────────────────────────────────────────────
...
async function initMasterDB() {
    ...
    `);
    logger_1.logger.info('Master DB schema initialised');
}
async function initTenantSchema(q) {
    ...
    logger_1.logger.info('Tenant schema initialised');
}
// ────────────────────────────────────────────────────────────────────────────
...
async function seedAgileDefaults(q) {
    const orgs = (await q(`SELECT o.id FROM organizations o
         LEFT JOIN org_agile_settings s ON s.org_id = o.id
         WHERE s.org_id IS NULL`)).rows;
    for (const { id: orgId } of orgs) {
        // 1. Settings row
        await q(`INSERT INTO org_agile_settings (org_id) VALUES ($1)
        ...
        for (const t of defaultTypes) {
            await q(`INSERT INTO work_item_types (org_id, key, name, icon, color, description, is_default, is_epic, sort_order)
                 VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
                 ON CONFLICT (org_id, key) DO NOTHING`, [orgId, t.key, t.name, t.icon, t.color, t.description, t.is_default, t.is_epic, t.sort_order]);
        }
        ...
        for (const st of defaultStates) {
            await q(`INSERT INTO workflow_states (org_id, key, name, category, color, sort_order, is_initial, is_terminal)
                 VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
                 ON CONFLICT (org_id, key) DO NOTHING`, [orgId, st.key, st.name, st.category, st.color, st.sort_order, st.is_initial, st.is_terminal]);
        }
    }
