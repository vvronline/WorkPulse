...
const { PLANS, PLAN_KEYS, PLAN_RANK, FEATURE_LABELS, FEATURE_KEYS, getEffectiveFeatures, getPlanLimits, isFeatureEnabled, isAtLeastPlan, planFeatureDiff, sanitizeFeatureOverrides, coerceFeatureValue, } = require("../utils/planCatalog");
...
describe("Feature → endpoint coverage map", () => {
    ...
    test("every gateable feature has at least one requireFeature call in routes/", () => {
        ...
        for (const f of fs.readdirSync(routesDir)) {
            ...
            try {
                if (fs.statSync(full).isFile() && (f.endsWith(".js") || f.endsWith(".ts"))) {
        ...
        for (const f of FEATURE_KEYS) {
            ...
            if (!blob.includes(`requireFeature('${f}')`) && !blob.includes(`isFeatureEnabled(${"tenantRow"}`)) {
                ...
                if (f === "webhooks" && blob.includes(`isFeatureEnabled(`))
                    continue;
                missing.push(f);
            }
        }
...
//# sourceMappingURL=planGating.test.js.map