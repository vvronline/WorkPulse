...
describe("GET /api/calendar", () => {
    ...
    test("returns 400 when from/to params are missing", async () => {
        setupAuth();
        const res = await request(app).get("/api/calendar").set(CSRF).set("Cookie", authCookie());
...
describe("PUT /api/calendar/:id", () => {
    ...
    test("returns 401 without auth", async () => {
        const res = await request(app).put("/api/calendar/1").set(CSRF).send({ title: "Updated" });
        expect(res.status).toBe(401);
    });
...
describe("DELETE /api/calendar/:id", () => {
    ...
    test("returns 401 without auth", async () => {
        const res = await request(app).delete("/api/calendar/1").set(CSRF);
        expect(res.status).toBe(401);
    });
    test("returns 404 when event does not exist or belongs to another user", async () => {
        setupAuth();
...
//# sourceMappingURL=calendar.routes.test.js.map