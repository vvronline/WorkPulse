...
function makeHappyPathDb({ persistedId = 999 } = {}) {
    const queries = [];
    const responses = [
        // 1. participant check
        { rows: [{ "?column?": 1 }] },
    ...
    const query = jest.fn((..._args) => Promise.resolve(responses.shift() || { rows: [] }));
...
describe("chatMessage handler — happy path", () => {
    ...
    test("inserts, broadcasts to every participant, bumps unread for non-senders", async () => {
        ...
        const broadcasts = send.mock.calls.filter((c) => c[2] === "chat_message");
    ...
    test("mentions broadcast a chat_mention ONLY to participants who are mentioned (and not the sender)", async () => {
        ...
        const mentionCalls = send.mock.calls.filter((c) => c[2] === "chat_mention");
...
//# sourceMappingURL=wsHandlers.chatMessage.test.js.map