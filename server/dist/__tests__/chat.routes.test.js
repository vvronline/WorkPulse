...
describe("PUT /api/chat/conversations/:id/group", () => {
    ...
    test("removes only users validated in the same org", async () => {
        ...
        const deleteCalls = mockQuery.mock.calls.filter(([sql]) => typeof sql === "string" &&
            sql.includes("DELETE FROM conversation_participants WHERE conversation_id = $1 AND user_id = $2"));
