...
describe("wsIdempotency", () => {
    ...
    describe("withIdempotency wrapper", () => {
        test("runs the fn the first time and returns its result", async () => {
            ...
            const out = await withIdempotency({ tenantId: 1, senderId: 7, type: "meeting_raise_hand", clientMsgId: "a" }, fn);
            expect(out).toBe("ok-1");
            expect(fn).toHaveBeenCalledTimes(1);
        });
        test("replays the cached result on a second call with the same id", async () => {
            const fn = jest.fn().mockResolvedValue("ok-1");
            await withIdempotency({ tenantId: 1, senderId: 7, type: "meeting_raise_hand", clientMsgId: "a" }, fn);
            const out = await withIdempotency({ tenantId: 1, senderId: 7, type: "meeting_raise_hand", clientMsgId: "a" }, fn);
            expect(out).toBe("ok-1");
        ...
        test("throws from the wrapped fn are NOT cached — retry gets a fresh world", async () => {
            ...
            await expect(withIdempotency({ tenantId: 1, senderId: 1, type: "x", clientMsgId: "r" }, fn)).rejects.toThrow("first transient");
            // Second call same id should re-run the fn (the first error wasn't cached).
            const out = await withIdempotency({ tenantId: 1, senderId: 1, type: "x", clientMsgId: "r" }, fn);
        ...
        test("throws TypeError if fn is not a function", async () => {
            await expect(withIdempotency({ tenantId: 1, senderId: 1, type: "x", clientMsgId: "a" }, null)).rejects.toThrow(TypeError);
        });
    });
...
//# sourceMappingURL=wsIdempotency.test.js.map