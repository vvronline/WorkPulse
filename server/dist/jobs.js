...
exports.pruneStaleInspectorUsers = pruneStaleInspectorUsers;
...
const tenantManager_1 = require("./utils/tenantManager");
...
async function pruneStaleInspectorUsers() {
    let pruned = 0;
    const result = await (0, tenantManager_1.forEachTenant)(async (db) => {
    ...
    if (pruned > 0) {
        logger_1.logger.info({ pruned, tenantsOk: result.ok, tenantsFailed: result.failed }, "Pruned stale Platform Inspector users");
    }
    return pruned;
}
/**
...
async function runRetentionCleanup() {
    ...
    if (policy.auditLogRetentionDays > 0) {
        const r = await (0, db_1.masterQuery)(`DELETE FROM platform_audit_logs WHERE created_at < NOW() - ($1 || ' days')::interval`, [String(policy.auditLogRetentionDays)]);
        stats.audit_logs = r.rowCount || 0;
    }
    // Purge old impersonation session requests
    if (policy.sessionLogRetentionDays > 0) {
        const r = await (0, db_1.masterQuery)(`DELETE FROM tenant_access_requests
             WHERE status IN ('expired', 'consumed', 'denied', 'cancelled', 'revoked')
               AND created_at < NOW() - ($1 || ' days')::interval`, [String(policy.sessionLogRetentionDays)]);
    ...
    if (policy.deletedTenantCleanupDays > 0) {
        const r = await (0, db_1.masterQuery)(`DELETE FROM tenants
             WHERE status = 'deleted'
               AND updated_at < NOW() - ($1 || ' days')::interval`, [String(policy.deletedTenantCleanupDays)]);
        stats.tenants = r.rowCount || 0;
    }
    // Purge per-tenant audit logs
    if (policy.auditLogRetentionDays > 0) {
        await (0, tenantManager_1.forEachTenant)(async (db) => {
            await db.query(`DELETE FROM audit_logs WHERE created_at < NOW() - ($1 || ' days')::interval`, [String(policy.auditLogRetentionDays)]);
        }, { label: "retentionCleanup" });
    }
    const total = stats.audit_logs + stats.session_logs + stats.tenants;
    if (total > 0) {
        logger_1.logger.info(stats, "Data retention cleanup completed");
    }
    return stats;
}
/**
...
function initJobs({ autoClockOut, cleanupTokens }) {
    ...
    if (!Queue || !redisClient) {
        ...
        logger_1.logger.info("BullMQ unavailable — using setInterval for background jobs");
        autoClockOut();
        fallbackIntervals.push(setInterval(autoClockOut, 5 * 60 * 1000));
        fallbackIntervals.push(setInterval(cleanupTokens, 60 * 60 * 1000));
        fallbackIntervals.push(setInterval(() => pruneStaleInspectorUsers().catch((err) => logger_1.logger.error({ err }, "Inspector prune (interval) failed")), 24 * 60 * 60 * 1000));
        fallbackIntervals.push(setInterval(() => runRetentionCleanup().catch((err) => logger_1.logger.error({ err }, "Retention cleanup (interval) failed")), 24 * 60 * 60 * 1000));
        return;
    }
    ...
    autoClockOutQueue.upsertJobScheduler("auto-clock-out-schedule", {
        ...
        name: "auto-clock-out",
    }).catch((err) => logger_1.logger.warn({ err: err.message }, "Failed to set auto-clock-out schedule"));
    const clockOutWorker = new Worker("auto-clock-out", async () => {
        await autoClockOut();
    }, { connection, concurrency: 1 });
    clockOutWorker.on("failed", (job, err) => {
        logger_1.logger.error({ err, jobId: job?.id }, "Auto clock-out job failed");
    });
    ...
    tokenCleanupQueue.upsertJobScheduler("token-cleanup-schedule", {
        ...
        name: "token-cleanup",
    }).catch((err) => logger_1.logger.warn({ err: err.message }, "Failed to set token-cleanup schedule"));
    const cleanupWorker = new Worker("token-cleanup", async () => {
        await cleanupTokens();
    }, { connection, concurrency: 1 });
    cleanupWorker.on("failed", (job, err) => {
        logger_1.logger.error({ err, jobId: job?.id }, "Token cleanup job failed");
    });
    ...
    }).catch((err) => logger_1.logger.warn({ err: err.message }, "Failed to set inspector-prune schedule"));
    ...
    inspectorPruneWorker.on("failed", (job, err) => {
        logger_1.logger.error({ err, jobId: job?.id }, "Inspector prune job failed");
    });
    ...
    }).catch((err) => logger_1.logger.warn({ err: err.message }, "Failed to set retention-cleanup schedule"));
    const retentionWorker = new Worker("retention-cleanup", async () => {
        await runRetentionCleanup();
    }, { connection, concurrency: 1 });
    retentionWorker.on("failed", (job, err) => {
        logger_1.logger.error({ err, jobId: job?.id }, "Retention cleanup job failed");
    });
    workers.push(retentionWorker);
    // Run auto clock-out immediately on startup
    autoClockOut();
    logger_1.logger.info("BullMQ job queues initialized (auto-clock-out: 5m, token-cleanup: 1h, inspector-prune: 24h, retention-cleanup: 24h)");
}
async function shutdownJobs() {
    for (const id of fallbackIntervals)
        clearInterval(id);
    fallbackIntervals = [];
