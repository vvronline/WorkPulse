...
exports.requireMinPlan = requireMinPlan;
...
async function resolveFromDomain(host) {
    ...
    // Query master DB
    const res = await (0, db_1.masterQuery)(`SELECT id, slug, db_name, db_host, status, features, max_users, max_storage_mb
         FROM tenants WHERE custom_domain = $1 AND status != 'deleted'`, [domain]);
...
async function attachTenantDb(req, tenant) {
    ...
    const db = await (0, tenantManager_1.getTenantPool)(tenant.db_name, tenant.db_host);
    req.db = {
        query: db.query,
    ...
    if (typeof req.enrichLogger === "function")
        req.enrichLogger();
}
/**
...
async function resolveTenant(req, res, next) {
    try {
        ...
        if (jwtTenantId) {
            ...
            if (!tenant) {
                tenant = await (0, tenantManager_1.getTenantById)(jwtTenantId);
                if (tenant) {
        ...
        logger_1.logger.error({ err, host: req.headers.host }, "Tenant resolution error");
...
async function checkUserLimit(req, res, next) {
    if (!req.tenant || !req.tenant.max_users)
