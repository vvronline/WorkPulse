...
exports.EFFECTIVE_STATUSES = exports.ACTIVITIES = exports.PRESENCE_PREFERENCES = exports.MANUAL_STATUSES = void 0;
exports.setManualStatus = setManualStatus;
exports.setPresencePreference = setPresencePreference;
exports.recordActivityPing = recordActivityPing;
...
exports.clearActivityForRef = clearActivityForRef;
...
const broadcaster = __importStar(require("./broadcaster"));
...
async function applyChange(ctx, { userId, source, sessionKey = null, previousEffective = null, metadata = null }) {
    assertCtx(ctx);
    if (!userId)
        throw new TypeError("applyChange: userId is required");
    if (!constants.isSource(source))
        throw new TypeError(`applyChange: invalid source "${source}"`);
...
async function setManualStatus(ctx, { status, message = null, messageExpiresAt = null }) {
    ...
    if (!userId)
        throw new TypeError("setManualStatus: ctx.actorUserId required");
    ...
    try {
        ...
        ctx.logger?.warn?.({ err: err?.message, userId }, "setManualStatus: applyChange failed (status saved) — falling back to fresh resolve");
...
async function setPresencePreference(ctx, preference) {
    ...
    const userId = ctx.actorUserId;
    if (!userId)
        throw new TypeError("setPresencePreference: ctx.actorUserId required");
    const previous = await peekEffective(ctx, userId);
    ...
    try {
        ...
        ctx.logger?.warn?.({ err: err?.message, userId }, "setPresencePreference: applyChange failed (preference saved) — falling back to fresh resolve");
        return resolveAndCache(ctx, userId);
    }
}
...
async function openSession(ctx, { userId, sessionKey, deviceLabel = null }) {
    ...
    if (!userId || !sessionKey)
        throw new TypeError("openSession: userId and sessionKey required");
    const previous = await peekEffective(ctx, userId);
...
async function closeAllSessions(ctx, userId, { source = "logout" } = {}) {
    assertCtx(ctx);
    if (!userId)
        throw new TypeError("closeAllSessions: userId required");
    if (!constants.isSource(source))
        throw new TypeError(`closeAllSessions: invalid source "${source}"`);
    const previous = await peekEffective(ctx, userId);
...
exports.MANUAL_STATUSES = constants.MANUAL_STATUSES;
exports.PRESENCE_PREFERENCES = constants.PRESENCE_PREFERENCES;
exports.ACTIVITIES = constants.ACTIVITIES;
exports.EFFECTIVE_STATUSES = constants.EFFECTIVE_STATUSES;
