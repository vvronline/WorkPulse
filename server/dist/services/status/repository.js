...
exports.getUserPrefsBulk = getUserPrefsBulk;
exports.getOpenSessions = getOpenSessions;
exports.getOpenSessionsBulk = getOpenSessionsBulk;
exports.setManualStatus = setManualStatus;
exports.setPresencePreference = setPresencePreference;
exports.touchLastActivity = touchLastActivity;
exports.openSession = openSession;
exports.touchSession = touchSession;
exports.closeSession = closeSession;
exports.closeAllSessionsForUser = closeAllSessionsForUser;
exports.setSessionActivity = setSessionActivity;
exports.clearSessionActivity = clearSessionActivity;
exports.clearActivityByRef = clearActivityByRef;
exports.recordEvent = recordEvent;
const constants_1 = require("./constants");
// ─── Reads ───────────────────────────────────────────────────────────────────
/** Fetch the preferences row for a user. */
async function getUserPrefs(db, userId) {
    const row = (await db.query(`SELECT id, manual_status, presence_preference, status_message,
                status_message_expires_at, last_activity_at
           FROM users
          WHERE id = $1`, [userId])).rows[0];
    if (!row)
        return null;
    return {
        userId: row.id,
...
async function getUserPrefsBulk(db, userIds) {
    if (!Array.isArray(userIds) || userIds.length === 0)
        return new Map();
    const rows = (await db.query(`SELECT id, manual_status, presence_preference, status_message,
                status_message_expires_at, last_activity_at
           FROM users
          WHERE id = ANY($1::int[])`, [userIds])).rows;
    const map = new Map();
...
async function getOpenSessions(db, userId) {
    return (await db.query(`SELECT id, session_key, device_label, connected_at, last_seen_at,
                disconnected_at, activity, activity_ref_id
           FROM user_presence_sessions
          WHERE user_id = $1 AND disconnected_at IS NULL`, [userId])).rows.map(mapSessionRow);
}
/** Open sessions for many users. Returns Map<userId, SessionInput[]>. */
async function getOpenSessionsBulk(db, userIds) {
    if (!Array.isArray(userIds) || userIds.length === 0)
        return new Map();
    const rows = (await db.query(`SELECT user_id, session_key, device_label, connected_at, last_seen_at,
                disconnected_at, activity, activity_ref_id
           FROM user_presence_sessions
          WHERE user_id = ANY($1::int[]) AND disconnected_at IS NULL`, [userIds])).rows;
    const map = new Map();
...
async function setManualStatus(db, userId, { status, message, messageExpiresAt }) {
    if (!(0, constants_1.isManualStatus)(status))
        throw new TypeError(`setManualStatus: invalid status "${status}"`);
    await db.query(`UPDATE users
            SET manual_status = $1,
                status_message = $2,
                status_message_expires_at = $3
          WHERE id = $4`, [status, message ?? null, messageExpiresAt ?? null, userId]);
}
async function setPresencePreference(db, userId, preference) {
    if (!(0, constants_1.isPresencePreference)(preference)) {
...
async function openSession(db, userId, { sessionKey, deviceLabel = null }) {
    if (!sessionKey)
        throw new TypeError("openSession: sessionKey is required");
    // ON CONFLICT re-opens a session that had been marked disconnected
    // (e.g. brief network blip producing a duplicate WS connect).
    await db.query(`INSERT INTO user_presence_sessions (user_id, session_key, device_label, connected_at, last_seen_at)
              VALUES ($1, $2, $3, NOW(), NOW())
         ON CONFLICT (session_key) DO UPDATE
                SET disconnected_at = NULL,
                    last_seen_at = NOW(),
                    user_id = EXCLUDED.user_id,
                    device_label = COALESCE(EXCLUDED.device_label, user_presence_sessions.device_label)`, [userId, sessionKey, deviceLabel]);
}
async function touchSession(db, sessionKey) {
    if (!sessionKey)
        return;
    await db.query(`UPDATE user_presence_sessions SET last_seen_at = NOW()
          WHERE session_key = $1 AND disconnected_at IS NULL`, [sessionKey]);
}
async function closeSession(db, sessionKey) {
    if (!sessionKey)
        return null;
    const row = (await db.query(`UPDATE user_presence_sessions
            SET disconnected_at = NOW(), activity = NULL, activity_ref_id = NULL
          WHERE session_key = $1 AND disconnected_at IS NULL
          RETURNING user_id`, [sessionKey])).rows[0];
    return row?.user_id ?? null;
}
async function closeAllSessionsForUser(db, userId) {
    await db.query(`UPDATE user_presence_sessions
            SET disconnected_at = NOW(), activity = NULL, activity_ref_id = NULL
          WHERE user_id = $1 AND disconnected_at IS NULL`, [userId]);
}
async function setSessionActivity(db, sessionKey, activity, refId = null) {
    if (!sessionKey)
        throw new TypeError("setSessionActivity: sessionKey is required");
...
async function clearActivityByRef(db, activity, refId) {
    if (!activity || refId == null)
        return [];
    if (!(0, constants_1.isActivity)(activity)) {
        throw new TypeError(`clearActivityByRef: invalid activity "${activity}"`);
    }
    const rows = (await db.query(`UPDATE user_presence_sessions
            SET activity = NULL, activity_ref_id = NULL, last_seen_at = NOW()
          WHERE activity = $1 AND activity_ref_id = $2 AND disconnected_at IS NULL
          RETURNING user_id`, [activity, refId])).rows;
...
async function recordEvent(db, { userId, source, fromState, toState, sessionKey, metadata }) {
    if (!(0, constants_1.isSource)(source))
        throw new TypeError(`recordEvent: invalid source "${source}"`);
    await db.query(`INSERT INTO user_status_events (user_id, source, from_state, to_state, session_key, metadata)
              VALUES ($1, $2, $3, $4, $5, $6)`, [userId, source, fromState ?? null, toState ?? null, sessionKey ?? null, metadata ? JSON.stringify(metadata) : null]);
}
