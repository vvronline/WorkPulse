...
describe("resolveEffective — happy path table", () => {
    const cases = [
        ...
        },
        {
            ...
            input: {
                sessions: [openSession({ lastSeenAt: ms(-constants_1.SESSION_STALE_MS - 1) })],
                manualStatus: "available",
                presencePreference: "auto",
                now: NOW,
            },
        ...
        // ── hard manual states ─────────────────────────────────────────────
        {
            ...
            input: {
                ...
                manualStatus: "dnd",
                presencePreference: "auto",
                lastActivityAt: ms(-constants_1.IDLE_AWAY_MS - 1),
                now: NOW,
            },
    ...
    test.each(cases)("$name", ({ input, expected }) => {
        expect((0, resolver_1.resolveEffective)(input)).toEqual(expected);
    });
});
describe("resolveEffective — validation", () => {
    test("rejects missing now", () => {
        expect(() => (0, resolver_1.resolveEffective)({
            sessions: [],
            manualStatus: null,
            presencePreference: "auto",
        })).toThrow(/now/);
...
describe("resolveEffective — multi-session scenarios", () => {
    test("one device offline, one device on a call → effective in_call", () => {
        const result = (0, resolver_1.resolveEffective)({
            sessions: [
    ...
    test("activity-only session goes stale → effective falls back to available", () => {
        const result = (0, resolver_1.resolveEffective)({
            sessions: [
                openSession({ sessionKey: "old", activity: "in_call", lastSeenAt: ms(-constants_1.SESSION_STALE_MS - 1) }),
