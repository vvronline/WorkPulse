...
exports.FEATURE_KEYS = exports.FEATURE_LABELS = exports.PLAN_RANK = exports.PLAN_KEYS = exports.DEFAULT_PLANS = exports.PLANS = void 0;
exports.getPlans = getPlans;
exports.loadPlanCatalog = loadPlanCatalog;
exports.savePlanCatalog = savePlanCatalog;
exports.getEffectiveFeatures = getEffectiveFeatures;
exports.getPlanLimits = getPlanLimits;
exports.isFeatureEnabled = isFeatureEnabled;
exports.isAtLeastPlan = isAtLeastPlan;
exports.planFeatureDiff = planFeatureDiff;
exports.sanitizeFeatureOverrides = sanitizeFeatureOverrides;
exports.coerceFeatureValue = coerceFeatureValue;
const DEFAULT_PLANS = {
    standard: {
        label: "Standard",
...
async function loadPlanCatalog() {
    const now = Date.now();
    if (now - planCacheTime < PLAN_CACHE_TTL)
        return PLANS;
    try {
        const { masterQuery } = require("../db");
        const res = await masterQuery(`SELECT value FROM app_settings WHERE key = 'plans_catalog'`);
        if (res.rows[0]?.value) {
            const parsed = JSON.parse(res.rows[0].value);
            if (parsed && typeof parsed === "object" && Object.keys(parsed).length > 0) {
                exports.PLANS = PLANS = parsed;
            }
        }
    }
    catch {
 ...
 */
const PLAN_RANK = { standard: 1, pro: 2, enterprise: 3 };
exports.PLAN_RANK = PLAN_RANK;
 ...
 */
function sanitizeFeatureOverrides(raw) {
 ...
 */
function getEffectiveFeatures(plan, rawOverrides = {}) {
    ...
    const out = { ...planDef.features };
    for (const k of FEATURE_KEYS) {
        if (k in overrides)
            out[k] = overrides[k];
        else if (!(k in out))
            out[k] = false; // fail closed for any feature missing in plan def
    }
    return out;
}
/**
 ...
 */
function isAtLeastPlan(current, required) {
 ...
 */
function planFeatureDiff(fromPlan, toPlan) {
