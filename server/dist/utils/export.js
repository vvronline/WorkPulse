...
exports.sendPayrollCSV = sendPayrollCSV;
exports.sendPayrollPDF = sendPayrollPDF;
...
function sendPDF(res, { title, columns, rows, filename }) {
    ...
    rows.forEach((row, idx) => {
        ...
        const heights = values.map((val, i) => doc.heightOfString(val, { width: colWidths[i] - cellPadding * 2 }));
...
function sendPayrollPDF(res, { from, to, summaryRows, detailRows, filename }) {
    ...
    function drawRow(y, values, widths, { bold = false, fontSize = 7, bg, textColor = "#222222" } = {}) {
        ...
        doc.font(font).fontSize(fontSize);
        const cellHeights = values.map((v, i) => doc.heightOfString(String(v ?? ""), { width: widths[i] - pad * 2 }));
    ...
    summaryRows.forEach((row, i) => {
        ...
        doc.font("Helvetica").fontSize(7);
        const needed = Math.max(...vals.map((v, j) => doc.heightOfString(v, { width: sumW[j] - pad * 2 }))) + pad * 2 + 2;
    ...
    for (const [empName, empRows] of Object.entries(grouped)) {
        ...
        empRows.forEach((row) => {
            ...
            doc.font("Helvetica").fontSize(7);
            const needed = Math.max(...vals.map((v, i) => doc.heightOfString(v, { width: detW[i] - pad * 2 }))) + pad * 2 + 2;
            if (pageBreakNeeded(y, needed)) {
                doc.addPage();
                y = doc.y;
                y += drawRow(y, detCols.map((c) => c.header), detW, { bold: true, fontSize: 7, bg: "#2471a3", textColor: "#ffffff" });
