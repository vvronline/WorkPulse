...
exports.destroyTenantPool = destroyTenantPool;
exports.destroyAllPools = destroyAllPools;
...
exports.listActiveTenants = listActiveTenants;
exports.forEachTenant = forEachTenant;
/**
...
function evictIfNeeded() {
    ...
    for (const [dbName, entry] of poolCache) {
        if (now - entry.lastUsed > IDLE_TIMEOUT_MS) {
            logger_1.logger.info({ dbName }, "Evicting idle tenant pool");
    ...
    while (poolCache.size >= MAX_POOLS) {
        ...
        if (oldestKey) {
            logger_1.logger.info({ dbName: oldestKey }, "Evicting LRU tenant pool");
...
async function getTenantPool(dbName, dbHost) {
    ...
    const promise = (async () => {
        ...
        tenantPool.on("error", (err) => {
            logger_1.logger.error({ err, dbName }, "Tenant pool error");
        });
        // Verify connectivity
        ...
        const entry = {
            pool: tenantPool,
            query: (0, db_1.makePoolQuery)(tenantPool),
            transaction: (0, db_1.makePoolTransaction)(tenantPool),
            lastUsed: Date.now(),
        };
        ...
        if (!migratedDbs.has(dbName)) {
            ...
            try {
                ...
                logger_1.logger.error({ err: err.message, dbName }, "Per-pool migrations failed (non-fatal)");
            }
        }
        logger_1.logger.info({ dbName, poolCount: poolCache.size }, "Tenant pool created");
        return entry;
    })();
...
async function destroyTenantPool(dbName) {
    ...
    if (entry) {
        ...
        poolCache.delete(dbName);
        logger_1.logger.info({ dbName }, "Tenant pool destroyed");
    }
}
/**
...
async function destroyAllPools() {
    ...
    for (const [dbName, entry] of poolCache) {
        ...
        logger_1.logger.info({ dbName }, "Shutting down tenant pool");
    }
    poolCache.clear();
...
async function createTenant({ orgName, slug, plan, features, maxUsers, maxStorageMb }) {
    ...
    const result = await (0, db_1.masterQuery)(`INSERT INTO tenants (org_name, slug, db_name, plan, features, max_users, max_storage_mb)
         VALUES ($1, $2, $3, $4, $5, $6, $7)
         RETURNING *`, [orgName, slug, dbName, effectivePlan, JSON.stringify(features || {}), maxUsers ?? planLimits.max_users, maxStorageMb ?? planLimits.max_storage_mb]);
    ...
    try {
        ...
        await (0, db_1.masterQuery)(`CREATE DATABASE "${dbName}"`);
    }
    catch (err) {
        if (err.code === "42P04") {
            // Database already exists — fine (idempotent)
            logger_1.logger.warn({ dbName }, "Tenant database already exists");
        }
        else {
            // Roll back the tenants row
            await (0, db_1.masterQuery)("DELETE FROM tenants WHERE id = $1", [tenant.id]);
            throw err;
        }
    }
    // 3. Connect to the new database and initialise schema
    const db = await getTenantPool(dbName);
    await (0, db_1.initTenantSchema)(db.query);
    // 4. Create the organizations row inside the tenant DB (1 org per tenant DB)
    await db.query(`INSERT INTO organizations (id, name, slug, created_by)
         VALUES (1, $1, $2, NULL)
         ON CONFLICT (id) DO NOTHING`, [orgName, slug]);
    logger_1.logger.info({ tenantId: tenant.id, dbName, slug }, "Tenant created");
    return { tenant, db };
}
/**
...
async function deleteTenant(tenantId, hardDelete = false) {
    const tenantRes = await (0, db_1.masterQuery)("SELECT * FROM tenants WHERE id = $1", [tenantId]);
    ...
    if (hardDelete) {
        ...
        await (0, db_1.masterQuery)(`SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = $1 AND pid <> pg_backend_pid()`, [tenant.db_name]);
        await (0, db_1.masterQuery)(`DROP DATABASE IF EXISTS "${tenant.db_name}"`);
        await (0, db_1.masterQuery)("DELETE FROM user_directory WHERE tenant_id = $1", [tenantId]);
        await (0, db_1.masterQuery)("DELETE FROM tenants WHERE id = $1", [tenantId]);
        logger_1.logger.info({ tenantId, dbName: tenant.db_name }, "Tenant hard-deleted");
    }
    else {
        await (0, db_1.masterQuery)(`UPDATE tenants SET status = 'deleted', updated_at = NOW() WHERE id = $1`, [tenantId]);
        logger_1.logger.info({ tenantId, dbName: tenant.db_name }, "Tenant soft-deleted");
    }
}
/**
...
async function suspendTenant(tenantId, reason) {
    await (0, db_1.masterQuery)(`UPDATE tenants SET status = 'suspended', suspended_at = NOW(), suspended_reason = $2, updated_at = NOW()
         WHERE id = $1`, [tenantId, reason || null]);
    logger_1.logger.info({ tenantId, reason }, "Tenant suspended");
}
/**
 * Reactivate a suspended tenant.
 */
async function reactivateTenant(tenantId) {
    await (0, db_1.masterQuery)(`UPDATE tenants SET status = 'active', suspended_at = NULL, suspended_reason = NULL, updated_at = NOW()
         WHERE id = $1`, [tenantId]);
    logger_1.logger.info({ tenantId }, "Tenant reactivated");
}
/**
 * Get tenant record from master DB by ID.
 */
async function getTenantById(tenantId) {
    const res = await (0, db_1.masterQuery)("SELECT * FROM tenants WHERE id = $1", [tenantId]);
...
async function getTenantBySlug(slug) {
    const res = await (0, db_1.masterQuery)("SELECT * FROM tenants WHERE slug = $1", [slug]);
    return res.rows[0] || null;
}
/**
...
async function getTenantByDomain(domain) {
    const res = await (0, db_1.masterQuery)(`SELECT * FROM tenants WHERE custom_domain = $1 AND status != 'deleted'`, [domain.toLowerCase()]);
    return res.rows[0] || null;
}
/**
...
async function listActiveTenants() {
    const res = await (0, db_1.masterQuery)(`SELECT * FROM tenants WHERE status = 'active' ORDER BY id`);
    return res.rows;
}
/**
...
async function forEachTenant(fn, opts = {}) {
    ...
    try {
        ...
        logger_1.logger.error({ err: err.message, label }, "forEachTenant: failed to list tenants");
        return { ok, failed };
    }
    ...
    if (includeLegacyMaster) {
        try {
            ...
            if (!masterCovered) {
                ...
                const hasUsers = (await (0, db_1.masterQuery)(`
                    SELECT 1 FROM information_schema.tables
                    WHERE table_schema = 'public' AND table_name = 'users' LIMIT 1
                `)).rows.length > 0;
                if (hasUsers) {
                    try {
                        await fn({ query: db_1.masterQuery, transaction: db_1.masterTransaction }, { id: null, slug: "master", db_name: masterDbName });
                        ok++;
                    }
                    catch (err) {
                        failed++;
