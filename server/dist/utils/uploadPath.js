...
function getUploadDir(tenantId, orgId, kind) {
    if (!tenantId)
        throw new Error("uploadPath: tenantId is required");
    if (!orgId)
        throw new Error("uploadPath: orgId is required");
    if (!kind || typeof kind !== "string" || /[/\\.]/.test(kind)) {
        throw new Error("uploadPath: invalid kind");
    }
    const dir = path_1.default.join(UPLOADS_ROOT, `tenant_${Number(tenantId)}`, `org_${Number(orgId)}`, kind);
...
function getUploadUrl(tenantId, orgId, kind, filename) {
    ...
    if (!orgId)
        throw new Error("uploadPath: orgId is required");
    if (!filename)
        throw new Error("uploadPath: filename is required");
