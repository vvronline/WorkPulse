...
exports.isMeetingMember = isMeetingMember;
/**
...
const chatMessage_1 = require("./wsHandlers/chatMessage");
const wsIdempotency_1 = require("./wsIdempotency");
...
function recordCallTransitionFailure(data) {
...
const meetingLeaveKey = (tenantId, userId, meetingId) => `${tenantId || 0}:${userId}:${meetingId}`;
...
async function _checkMembership(db, key, sql, params) {
    ...
    try {
        ...
        logger_1.logger.warn({ err: err.message, key }, "ws membership check failed");
...
async function isConversationMember(db, conversationId, userId) {
    if (!conversationId || !userId)
        return false;
    return _checkMembership(db, `conv:${conversationId}:${userId}`, "SELECT 1 FROM conversation_participants WHERE conversation_id = $1 AND user_id = $2", [conversationId, userId]);
}
/** Is `userId` a participant (joined or invited) of the given meeting? (cached) */
async function isMeetingMember(db, meetingId, userId) {
    if (!meetingId || !userId)
        return false;
    return _checkMembership(db, `meet:${meetingId}:${userId}`, `SELECT 1 FROM meeting_participants WHERE meeting_id = $1 AND user_id = $2 AND status IN ('joined','invited')`, [meetingId, userId]);
}
function setupWebSocket(server) {
    const wss = new WebSocketServer({
        ...
        verifyClient: ({ req }, done) => {
            ...
            if (!origin)
                return done(true); // non-browser clients (Electron, curl) have no Origin
            ...
            logger_1.logger.warn({ origin }, "WebSocket connection rejected: invalid origin");
    ...
    if (sub) {
        sub.subscribe("ws:broadcast", (err) => {
            if (err)
                logger_1.logger.warn({ err: err.message }, "Redis subscribe failed for ws:broadcast");
        });
        sub.on("message", (channel, raw) => {
            try {
                const envelope = JSON.parse(raw);
                if (envelope._from === INSTANCE_ID)
    ...
    wss.on("connection", async (ws, req) => {
        ...
        if (tenantId) {
            try {
                ...
                logger_1.logger.warn({ err: e.message, tenantId }, "WS tenant pool failed");
                ws.close(4003, "Tenant unavailable");
                return;
            }
        }
        ...
        ws._statusSessionKey = (0, crypto_1.randomUUID)();
        ...
        statusService.openSession({ db, tenantId }, { userId, sessionKey: ws._statusSessionKey, deviceLabel }).catch((err) => {
            logger_1.logger.warn({ err: err.message, userId }, "statusService.openSession failed");
        });
        logger_1.logger.debug({ userId, tenantId, sessionKey: ws._statusSessionKey }, "WS client connected");
        ...
        if (wasOffline) {
            ...
            db.query("UPDATE users SET last_seen_at = NOW() WHERE id = $1", [userId])
                .catch((err) => logger_1.logger.warn({ err: err.message, userId }, "last_seen_at bump failed"));
        }
        ws.on("message", (raw) => {
            ...
            if (!ws._authCheckInFlight && now - (ws._lastAuthCheckAt || 0) > WS_AUTH_RECHECK_MS) {
                ...
                Promise.resolve()
                    ...
                    if ((ws._tokenVersion ?? 0) !== dbTv) {
                        logger_1.logger.debug({ userId, tenantId }, "WS session revoked — closing socket");
            ...
            if (++ws._rlCount > 60) {
                logger_1.logger.warn({ userId, tenantId, count: ws._rlCount }, "WS rate limit exceeded, dropping message");
                return;
            }
            try {
                ...
                wsMetrics.recordHandler(msg?.type || "unknown", WS_HANDLER_DEFAULT_TIMEOUT_MS, () => handleChatMessage(db, userId, tenantId, msg, ws)).catch((err) => {
                    logger_1.logger.error({ err: err?.message, stack: err?.stack, userId, tenantId, type: msg?.type }, "WS message handler error");
                });
            }
        ...
        ws.on("close", () => {
            ...
            if (set) {
                ...
                if (set.size === 0) {
                    ...
                    db.query("UPDATE users SET last_seen_at = NOW() WHERE id = $1", [userId]).catch((err) => { logger_1.logger.warn({ err: err.message, userId }, "Failed to update last_seen_at on disconnect"); });
                }
            }
            ...
            if (ws._statusSessionKey) {
                statusService.closeSession({ db, tenantId }, ws._statusSessionKey)
                    .catch((err) => logger_1.logger.warn({ err: err.message, userId }, "statusService.closeSession failed"));
            ...
            logger_1.logger.debug({ userId, tenantId }, "WS client disconnected");
        });
        ws.on("error", (err) => {
            logger_1.logger.warn({ err: err?.message, userId }, "WebSocket error");
            ws.close();
        });
    ...
    const heartbeat = setInterval(() => {
        wss.clients.forEach((ws) => {
            ...
            if (ws._missedPongs > MAX_MISSED_PONGS) {
                logger_1.logger.debug({ userId: ws.userId, missed: ws._missedPongs }, "WS terminating after missed pongs");
                return ws.terminate();
            }
            ws.isAlive = false;
            try {
...
function scheduleMeetingDisconnectCleanup({ db, tenantId, userId, meetingId }) {
    ...
    const timer = setTimeout(async () => {
        ...
        try {
            ...
            const isJoined = (await db.query(`SELECT 1 FROM meeting_participants WHERE meeting_id = $1 AND user_id = $2 AND status = 'joined'`, [meetingId, userId])).rows[0];
            if (!isJoined)
                return;
            ...
            const hasOtherSessionForMeeting = (() => {
                ...
                if (!set)
                    return false;
                for (const ws of set) {
                    if (ws._activeMeetingId === meetingId)
                        return true;
                }
                return false;
            })();
            if (hasOtherSessionForMeeting)
                return;
            await db.query(`UPDATE meeting_participants SET status = 'left', left_at = NOW() WHERE meeting_id = $1 AND user_id = $2`, [meetingId, userId]);
            const activeParticipants = (await db.query(`SELECT user_id FROM meeting_participants WHERE meeting_id = $1 AND status = 'joined'`, [meetingId])).rows;
            ...
            if (activeParticipants.length === 0) {
                ...
                statusService.clearActivityForRef({ db, tenantId }, "in_meeting", meetingId)
                    .catch((err) => logger_1.logger.warn({ err: err.message, meetingId }, "clearActivityForRef(in_meeting) on grace expiry failed"));
            }
        }
        catch (err) {
            logger_1.logger.warn({ err: err.message, userId, meetingId }, "Delayed meeting cleanup failed");
        }
...
async function handleChatMessage(db, senderId, tenantId, msg, ws) {
    if (msg.type === "chat_message") {
        ...
        await (0, chatMessage_1.chatMessage)({
        ...
        const participant = (await db.query("SELECT 1 FROM conversation_participants WHERE conversation_id = $1 AND user_id = $2", [conversationId, senderId])).rows[0];
        if (!participant)
            return;
        // Notify other participants
        const participants = (await db.query("SELECT user_id FROM conversation_participants WHERE conversation_id = $1 AND user_id != $2", [conversationId, senderId])).rows;
        ...
        if (!participant)
            return;
        await db.query(`INSERT INTO message_reads (conversation_id, user_id, last_read_at)
             VALUES ($1, $2, NOW())
             ON CONFLICT (conversation_id, user_id) DO UPDATE SET last_read_at = NOW()`, [conversationId, senderId]);
        ...
        const { conversationId, callType, clientMsgId: rawCallInitiateId, } = msg.data || {};
        if (!conversationId || !["voice", "video"].includes(callType))
            return;
        await (0, wsIdempotency_1.withIdempotency)({
            tenantId,
            senderId,
            type: "call_initiate",
            ...
            const participant = (await db.query("SELECT 1 FROM conversation_participants WHERE conversation_id = $1 AND user_id = $2", [conversationId, senderId])).rows[0];
            if (!participant) {
                logger_1.logger.warn({ senderId, conversationId }, "call_initiate: sender not a participant");
            ...
            const [callLogResult, callerResult, convResult] = await Promise.all([
                db.query(`INSERT INTO call_logs (conversation_id, caller_id, call_type, status)
                         VALUES ($1, $2, $3, 'ringing') RETURNING id, created_at`, [conversationId, senderId, callType]),
            ...
            logger_1.logger.info({ senderId, callId: callLog.id, conversationId, callType, participantCount: participants.length, tenantId }, "call_initiate: notifying participants");
            for (const p of participants) {
                ...
                pushNotifications_1.pushNotifications.sendCallNotification(db.query, p.user_id, tenantId, {
                    callId: callLog.id,
                    conversationId,
                    callerId: senderId,
                    ...
                    (0, logger_1.logPushCallLifecycle)({
                    ...
                    logger_1.logger.warn({ err: err.message, userId: p.user_id, callId: callLog.id }, "Failed to send call push notification");
                    (0, logger_1.logPushCallLifecycle)({
                        event: "push_send_result",
                        tenantId,
                        userId: p.user_id,
            ...
            if (ws._statusSessionKey) {
                statusService.setSessionActivity({ db, tenantId }, ws._statusSessionKey, "in_call", callLog.id).catch((err) => logger_1.logger.warn({ err: err.message, callId: callLog.id }, "setSessionActivity(in_call) failed"));
        ...
        if (!callId || !conversationId)
            return;
        await (0, wsIdempotency_1.withIdempotentCallAction)({ tenantId, senderId, callId, action: "answer", clientMsgId: rawIdAccept }, async () => {
            const [callLogResult, participantResult] = await Promise.all([
                db.query(`SELECT * FROM call_logs WHERE id = $1 AND conversation_id = $2`, [callId, conversationId]),
                db.query("SELECT 1 FROM conversation_participants WHERE conversation_id = $1 AND user_id = $2", [conversationId, senderId]),
            ]);
            const callLog = callLogResult.rows[0];
            if (!callLog) {
                logger_1.logger.warn({ senderId, callId, conversationId }, "call_accept: call log not found");
                recordCallTransitionFailure({
                    event: "call_transition_failed",
                    action: "answer",
                    tenantId,
            ...
            if (callLog.status !== "ringing") {
                logger_1.logger.info({ senderId, callId, conversationId, status: callLog.status }, "call_accept: terminal/invalid state; ignoring");
                recordCallTransitionFailure({
                    event: "call_transition_failed",
                    action: "answer",
                    tenantId,
            ...
            if (!participantResult.rows[0]) {
                logger_1.logger.warn({ senderId, callId, conversationId }, "call_accept: sender not a participant");
                recordCallTransitionFailure({
                    event: "call_transition_failed",
                    action: "answer",
                    tenantId,
            ...
            if (!updatedCall.rows[0]) {
                logger_1.logger.info({ senderId, callId, conversationId }, "call_accept: transition already applied by another action");
                recordCallTransitionFailure({
                    event: "call_transition_failed",
                    action: "answer",
                    tenantId,
            ...
            logger_1.logger.info({ senderId, callerId: callLog.caller_id, callId, conversationId, tenantId }, "call_accept: notifying caller");
            ...
            if (ws._statusSessionKey) {
                statusService.setSessionActivity({ db, tenantId }, ws._statusSessionKey, "in_call", callId).catch((err) => logger_1.logger.warn({ err: err.message, callId }, "setSessionActivity(in_call) failed"));
        ...
        if (!conversationId)
            return;
        const callLog = (await db.query(`SELECT id FROM call_logs WHERE conversation_id = $1 AND caller_id = $2 AND status = 'ringing' ORDER BY created_at DESC LIMIT 1`, [conversationId, senderId])).rows[0];
        if (!callLog)
            return;
        ...
        statusService.clearActivityForRef({ db, tenantId }, "in_call", callLog.id)
            .catch((err) => logger_1.logger.warn({ err: err.message, callId: callLog.id }, "clearActivityForRef(in_call) on cancel failed"));
        ws._callActivityRefId = null;
    }
        ...
        if (!callId || !conversationId)
            return;
        await (0, wsIdempotency_1.withIdempotentCallAction)({ tenantId, senderId, callId, action: "reject", clientMsgId: rawIdReject }, async () => {
            // Verify sender is a participant in this conversation
            const isParticipant = (await db.query("SELECT 1 FROM conversation_participants WHERE conversation_id = $1 AND user_id = $2", [conversationId, senderId])).rows[0];
            if (!isParticipant)
                return;
            const callLog = (await db.query(`SELECT * FROM call_logs WHERE id = $1 AND conversation_id = $2`, [callId, conversationId])).rows[0];
            if (!callLog) {
                recordCallTransitionFailure({
                    event: "call_transition_failed",
                    action: "reject",
            ...
            if (callLog.status !== "ringing") {
                logger_1.logger.info({ senderId, callId, conversationId, status: callLog.status }, "call_reject: terminal/invalid state; ignoring");
                recordCallTransitionFailure({
                    event: "call_transition_failed",
                    action: "reject",
                    tenantId,
            ...
            const updated = await db.query(`UPDATE call_logs
            ...
            statusService.clearActivityForRef({ db, tenantId }, "in_call", callId)
                .catch((err) => logger_1.logger.warn({ err: err.message, callId }, "clearActivityForRef(in_call) on reject failed"));
        });
    }
        ...
        if (!callId || !conversationId)
            return;
        await (0, wsIdempotency_1.withIdempotentCallAction)({ tenantId, senderId, callId, action: "end", clientMsgId: rawIdEnd }, async () => {
            // Verify sender is a participant in this conversation
            const isParticipant = (await db.query("SELECT 1 FROM conversation_participants WHERE conversation_id = $1 AND user_id = $2", [conversationId, senderId])).rows[0];
            if (!isParticipant) {
                recordCallTransitionFailure({
            ...
            if (["ended", "missed", "declined"].includes(callLog.status)) {
                logger_1.logger.info({ senderId, callId, conversationId, status: callLog.status }, "call_end: terminal state; ignoring duplicate");
                recordCallTransitionFailure({
                    event: "call_transition_failed",
                    action: "end",
                    tenantId,
            ...
            const allParticipants = (await db.query("SELECT user_id FROM conversation_participants WHERE conversation_id = $1", [conversationId])).rows;
            ...
            statusService.clearActivityForRef({ db, tenantId }, "in_call", callId)
                .catch((err) => logger_1.logger.warn({ err: err.message, callId }, "clearActivityForRef(in_call) on end failed"));
            if (ws._callActivityRefId === callId)
                ws._callActivityRefId = null;
        });
    }
        ...
        if (!conversationId || !targetUserId || !signal)
            return;
        ...
        if (!signal.type || !VALID_SIGNAL_TYPES.includes(signal.type)) {
            logger_1.logger.warn({ senderId, signalType: signal?.type }, "call_signal: rejected unknown signal type");
            return;
        }
        // Validate signal payload per type
        if (signal.type === "offer" || signal.type === "answer") {
            if (typeof signal.sdp !== "string" || signal.sdp.length === 0 || signal.sdp.length > 100000) {
                logger_1.logger.warn({ senderId, signalType: signal.type, sdpLen: signal.sdp?.length }, "call_signal: invalid SDP");
                return;
            }
        }
        else if (signal.type === "ice-candidate") {
            if (signal.candidate != null && (typeof signal.candidate !== "object" || typeof signal.candidate.candidate !== "string")) {
                logger_1.logger.warn({ senderId }, "call_signal: invalid ICE candidate structure");
                return;
            }
        }
        else if (signal.type === "video-state") {
            if (typeof signal.videoOff !== "boolean")
                return;
        }
        ...
        if (!senderOk || !targetOk) {
            logger_1.logger.warn({ senderId, targetUserId, conversationId, senderOk, targetOk, signalType: signal.type }, "call_signal: participant check failed");
            return;
        }
        logger_1.logger.debug({ senderId, targetUserId, conversationId, signalType: signal.type, tenantId }, "call_signal: relaying");
        ...
        if (!callId || !conversationId)
            return;
        const callLog = (await db.query(`SELECT * FROM call_logs WHERE id = $1 AND conversation_id = $2 AND status = 'answered'`, [callId, conversationId])).rows[0];
        if (!callLog)
            return;
        ...
        const others = (await db.query("SELECT user_id FROM conversation_participants WHERE conversation_id = $1 AND user_id != $2", [conversationId, senderId])).rows;
        ...
        if (!conversationId || !targetUserId || !emoji)
            return;
        ...
        if (cancelledPending) {
            logger_1.logger.debug({ userId: senderId, meetingId }, "Cancelled pending meeting cleanup on rejoin");
        }
        ...
        const mp = (await db.query("SELECT status FROM meeting_participants WHERE meeting_id = $1 AND user_id = $2", [meetingId, senderId])).rows[0];
        ...
        // Upsert participant
        await db.query(`INSERT INTO meeting_participants (meeting_id, user_id, role, status, joined_at)
             VALUES ($1, $2, 'participant', 'joined', NOW())
             ON CONFLICT (meeting_id, user_id) DO UPDATE SET status = 'joined', joined_at = NOW(), left_at = NULL`, [meetingId, senderId]);
        ...
        if (meeting.status === "active" && !isFirstStart) {
            const currentlyJoined = (await db.query(`SELECT COUNT(*) as cnt FROM meeting_participants WHERE meeting_id = $1 AND status = 'joined' AND user_id != $2`, [meetingId, senderId])).rows[0];
        ...
        if (isFirstStart || isRestart || isFirstJoinActive) {
            ...
            const allInvited = (await db.query(`SELECT mp.user_id FROM meeting_participants mp
                 WHERE mp.meeting_id = $1 AND mp.user_id != $2`, [meetingId, senderId])).rows;
            ...
            for (const p of allInvited) {
                try {
                    await db.query(`INSERT INTO notifications (user_id, type, title, body) VALUES ($1, $2, $3, $4)`, [p.user_id, notifType, notifTitle, notifBody]);
                }
        ...
        const [joinerResult, allParticipantsResult] = await Promise.all([
            ...
            db.query(`SELECT mp.user_id, u.full_name, u.avatar, u.username
                 FROM meeting_participants mp JOIN users u ON u.id = mp.user_id
                 WHERE mp.meeting_id = $1 AND mp.status = $2`, [meetingId, "joined"]),
        ]);
        ...
        if (meeting.conversation_id && !wasAlreadyJoined) {
            const sysMsg = (await db.query(`INSERT INTO messages (conversation_id, sender_id, content, format_type, metadata)
                 VALUES ($1, $2, '', 'system', $3) RETURNING id, created_at`, [meeting.conversation_id, senderId, JSON.stringify({
                    type: "meeting_joined", meetingId, name: joiner?.full_name,
                })])).rows[0];
            const convParticipants = (await db.query("SELECT user_id FROM conversation_participants WHERE conversation_id = $1", [meeting.conversation_id])).rows;
        ...
        if (ws._statusSessionKey) {
            statusService.setSessionActivity({ db, tenantId }, ws._statusSessionKey, "in_meeting", meetingId).catch((err) => logger_1.logger.warn({ err: err.message, meetingId }, "setSessionActivity(in_meeting) failed"));
        ...
        const isJoined = (await db.query(`SELECT 1 FROM meeting_participants WHERE meeting_id = $1 AND user_id = $2 AND status = 'joined'`, [meetingId, senderId])).rows[0];
        if (!isJoined)
            return;
        await db.query(`UPDATE meeting_participants SET status = 'left', left_at = NOW() WHERE meeting_id = $1 AND user_id = $2`, [meetingId, senderId]);
        const activeParticipants = (await db.query(`SELECT mp.user_id FROM meeting_participants mp WHERE mp.meeting_id = $1 AND mp.status = 'joined'`, [meetingId])).rows;
        for (const p of activeParticipants) {
        ...
        if (ws._statusSessionKey) {
            statusService.clearSessionActivity({ db, tenantId }, ws._statusSessionKey, "in_meeting").catch((err) => logger_1.logger.warn({ err: err.message, meetingId }, "clearSessionActivity(in_meeting) on leave failed"));
        ...
        await db.query(`UPDATE meeting_participants SET status = 'left', left_at = NOW() WHERE meeting_id = $1`, [meetingId]);
        const activeParticipants = (await db.query("SELECT user_id FROM meeting_participants WHERE meeting_id = $1", [meetingId])).rows;
        for (const p of activeParticipants) {
        ...
        statusService.clearActivityForRef({ db, tenantId }, "in_meeting", meetingId)
            .catch((err) => logger_1.logger.warn({ err: err.message, meetingId }, "clearActivityForRef(in_meeting) on end failed"));
        if (ws._meetingActivityRefId === meetingId)
            ws._meetingActivityRefId = null;
    }
        ...
        if (!meetingId || !targetUserId || !signal)
            return;
        ...
        if (!senderOk || !targetOk) {
            logger_1.logger.warn({ senderId, targetUserId, meetingId, senderOk, targetOk, signalType: signal?.type }, "meeting_signal: participant check failed");
            return;
        }
        logger_1.logger.debug({ senderId, targetUserId, meetingId, signalType: signal.type, tenantId }, "meeting_signal: relaying");
        ...
        if (!targetUser)
            return;
        await db.query(`INSERT INTO meeting_participants (meeting_id, user_id, role, status)
             VALUES ($1, $2, 'participant', 'invited') ON CONFLICT (meeting_id, user_id) DO NOTHING`, [meetingId, targetUserId]);
        if (meeting.conversation_id) {
            await db.query(`INSERT INTO conversation_participants (conversation_id, user_id) VALUES ($1, $2) ON CONFLICT DO NOTHING`, [meeting.conversation_id, targetUserId]);
        }
        ...
        if (!meetingId || !targetUserId)
            return;
        await (0, wsIdempotency_1.withIdempotency)({ tenantId, senderId, type: "meeting_mute_participant", clientMsgId: rawIdMute }, async () => {
            const meeting = (await db.query("SELECT * FROM meetings WHERE id = $1", [meetingId])).rows[0];
            if (!meeting)
                return;
            const meetingPerms = require("./meetingPermissions");
            if (!meetingPerms.can({ userId: senderId }, meeting, meetingPerms.ACTIONS.MUTE_OTHERS))
                return;
            ...
            const participants = (await db.query(`SELECT user_id FROM meeting_participants WHERE meeting_id = $1 AND status = 'joined'`, [meetingId])).rows;
            for (const p of participants) {
        ...
        if (!meetingId)
            return;
        await (0, wsIdempotency_1.withIdempotency)({ tenantId, senderId, type: "meeting_raise_hand", clientMsgId: rawIdHand }, async () => {
            // Verify sender is an active participant
            const senderOk = (await db.query(`SELECT 1 FROM meeting_participants WHERE meeting_id = $1 AND user_id = $2 AND status = 'joined'`, [meetingId, senderId])).rows[0];
            if (!senderOk)
                return;
            const participants = (await db.query(`SELECT user_id FROM meeting_participants WHERE meeting_id = $1 AND status = 'joined'`, [meetingId])).rows;
        ...
        if (videoOff !== undefined)
            trackState.videoOff = !!videoOff;
        if (screenSharing !== undefined)
            trackState.screenSharing = !!screenSharing;
        for (const p of participants) {
            if (p.user_id !== senderId) {
        ...
        if (!(await isMeetingMember(db, meetingId, senderId)))
            return;
        if (!(await isMeetingMember(db, meetingId, targetUserId)))
            return;
        ...
        if (!file_url && (!text || typeof text !== "string" || !text.trim() || text.length > 5000)) {
            if (clientMsgId)
                sendToUser(tenantId, senderId, "meeting_message_error", { clientMsgId, reason: "invalid-payload" });
            return;
        }
        ...
        if (!senderOk) {
            if (safeClientMsgId)
                sendToUser(tenantId, senderId, "meeting_message_error", { clientMsgId: safeClientMsgId, reason: "not-a-participant" });
            return;
        }
        ...
        const [senderResult, meetingResult, participantsResult] = await Promise.all([
            ...
            db.query(`SELECT user_id FROM meeting_participants WHERE meeting_id = $1 AND status IN ('joined','invited')`, [meetingId]),
        ]);
        ...
        if (conversationId) {
            try {
                ...
                const inserted = await db.query(`INSERT INTO messages (conversation_id, sender_id, content, format_type, metadata, client_msg_id)
                     VALUES ($1, $2, $3, 'text', $4, $5)
                     ON CONFLICT (conversation_id, sender_id, client_msg_id)
                     WHERE client_msg_id IS NOT NULL
                     DO NOTHING
                     RETURNING id, created_at`, [conversationId, senderId, content, JSON.stringify(metadata), safeClientMsgId]);
                if (inserted.rows[0]) {
                    ...
                    const existing = (await db.query(`SELECT id, created_at FROM messages
                          WHERE conversation_id = $1 AND sender_id = $2 AND client_msg_id = $3
                          LIMIT 1`, [conversationId, senderId, safeClientMsgId])).rows[0];
                    if (existing) {
                ...
                persistError = err;
                logger_1.logger.warn({ err: err.message, meetingId, conversationId }, "meeting_chat: persist failed");
            }
        }
        if (persistError) {
        ...
        const allowed = (await db.query(`SELECT 1 FROM meeting_participants
              WHERE meeting_id = $1 AND user_id = $2`, [meetingId, senderId])).rows[0];
        if (!allowed)
            return;
        const meetingRow = (await db.query("SELECT conversation_id FROM meetings WHERE id = $1", [meetingId])).rows[0];
        ...
        const rows = (await db.query(`SELECT m.id, m.sender_id, m.content, m.metadata, m.created_at, m.client_msg_id,
              ...
              ORDER BY m.id ASC
              LIMIT 200`, [meetingRow.conversation_id, since])).rows;
        for (const r of rows) {
        ...
        if (!callId || !conversationId || !targetUserId)
            return;
        ...
        const senderOk = (await db.query("SELECT 1 FROM conversation_participants WHERE conversation_id = $1 AND user_id = $2", [conversationId, senderId])).rows[0];
        if (!senderOk)
            return;
        // Refuse to upgrade a 1:1 conversation into a group via this path.
        const conv = (await db.query("SELECT is_group FROM conversations WHERE id = $1", [conversationId])).rows[0];
        if (!conv || !conv.is_group) {
            logger_1.logger.warn({ senderId, callId, conversationId, targetUserId }, "call_add_participant: rejected — conversation is not a group");
            return;
        }
        // Add target to conversation (no-op if they were already a member)
        await db.query(`INSERT INTO conversation_participants (conversation_id, user_id) VALUES ($1, $2) ON CONFLICT DO NOTHING`, [conversationId, targetUserId]);
        ...
        pushNotifications_1.pushNotifications.sendCallNotification(db.query, targetUserId, tenantId, {
            callId,
            conversationId,
            callerId: senderId,
            callerName: caller?.full_name || "Unknown",
            ...
            logger_1.logger.warn({ err: err.message, userId: targetUserId, callId }, "Failed to send group call push notification");
        });
    }
}
/**
...
function deliverLocal(tenantId, userId, type, data) {
    ...
    if (!set) {
        if (type === "call_signal" || type === "call_accepted" || type === "call_incoming" || type === "meeting_signal" || type === "meeting_participant_joined" || type === "meeting_started") {
            logger_1.logger.warn({ tenantId, userId, type, clientKey: ck, totalKeys: clients.size }, "deliverLocal: no connections found for user");
        }
        return;
    }
    ...
    if (delivered === 0 && (type === "call_signal" || type === "call_accepted" || type === "call_incoming" || type === "meeting_signal" || type === "meeting_participant_joined" || type === "meeting_started")) {
        logger_1.logger.warn({ tenantId, userId, type, clientKey: ck, connections: set.size }, "deliverLocal: user has connections but none are open");
    }
}
/**
...
function broadcast(tenantId, type, data) {
    ...
    for (const [key, set] of clients) {
        ...
        if (String(tenantId || 0) !== keyTenant)
            continue;
        for (const ws of set) {
            if (ws.readyState === 1)
                ws.send(msg);
...
async function notifyUser(db, tenantId, userId, type, title, body, linkTaskId) {
    try {
        ...
        if (row) {
            ...
            pushNotifications_1.pushNotifications.sendNotificationAlert(db.query, userId, tenantId || null, {
                notificationId: row.id,
                title,
                body,
                type,
            }).catch((err) => {
                logger_1.logger.warn({ err: err.message, userId }, "Failed to send push notification alert");
            });
        }
    }
