...
const timeCalc_1 = require("./timeCalc");
async function calculateAttendance(db, userId, orgId, startDate, endDate, timezoneOffset = 0) {
    const orgRow = (await db.query(`SELECT work_days, work_hours_per_day, min_hours_present FROM organizations WHERE id = $1`, [orgId])).rows[0] || {};
    ...
    const minHoursPresent = (orgRow.min_hours_present != null && Number(orgRow.min_hours_present) >= 0)
        ...
        : workHpd / 2;
    const holidaySet = new Set((await db.query(`SELECT date FROM holidays
             WHERE org_id = $1 AND date BETWEEN $2 AND $3 AND is_optional = FALSE`, [orgId, startDate, endDate])).rows.map((h) => h.date));
    ...
    const entries = (await db.query(`SELECT * FROM time_entries
         ...
         ORDER BY timestamp ASC`, [userId, startDate, endDate, memberInterval])).rows;
    const byDate = {};
    ...
    const leaveMap = {};
    for (const l of (await db.query(`SELECT date, leave_type, duration FROM leaves
         WHERE user_id = $1 AND status = 'approved' AND date BETWEEN $2 AND $3`, [userId, startDate, endDate])).rows) {
        leaveMap[l.date] = l;
    }
    const allDays = [];
    ...
    for (const date of allDays) {
        ...
        const floorMs = hasWork ? (0, timeCalc_1.computeFloorMs)(dayEntries) : 0;
