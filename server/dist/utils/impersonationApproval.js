...
exports.generateApprovalCode = generateApprovalCode;
exports.hashApprovalCode = hashApprovalCode;
exports.verifyApprovalCode = verifyApprovalCode;
exports.getImpersonationPolicy = getImpersonationPolicy;
exports.updateImpersonationPolicy = updateImpersonationPolicy;
exports.computeEffectiveStatus = computeEffectiveStatus;
exports.expireStaleRequests = expireStaleRequests;
exports.getActiveSession = getActiveSession;
exports.getOrCreateInspectorUser = getOrCreateInspectorUser;
/**
...
async function hashApprovalCode(code) {
    return bcryptjs_1.default.hash(code, APPROVAL_CODE_BCRYPT_ROUNDS);
}
async function verifyApprovalCode(code, hash) {
    if (!code || !hash)
        return false;
    try {
        ...
        logger_1.logger.warn({ err: e.message }, "impersonation: bcrypt compare failed");
        return false;
    }
}
/**
 ...
 * the seed step).
 */
async function getImpersonationPolicy() {
    const res = await (0, db_1.masterQuery)(`SELECT key, value FROM app_settings WHERE key = ANY($1::text[])`, [[
    ...
    for (const r of res.rows)
        map[r.key] = r.value;
    const parseBool = (v, dflt) => v == null ? dflt : v === "true" || v === "1";
...
async function updateImpersonationPolicy(patch) {
 ...
 */
async function getOrCreateInspectorUser(db, platformAdmin) {
    ...
    const existingRes = await db.query(`SELECT id, username, full_name, email, role, is_active, hidden_from_directory,
    ...
    if (existing) {
        ...
        if (needsRename || needsReactivate || needsRehide) {
            await db.query(`UPDATE users
                  ...
                  WHERE id = $2`, [fullName, existing.id]);
    ...
    let inserted;
    try {
        inserted = await db.query(`INSERT INTO users
             ...
             RETURNING id, username, full_name, email, role,
                       COALESCE(token_version, 0) AS token_version`, [username, passwordHash, fullName]);
    }
    catch (err) {
        ...
        if (err && err.code === "23505") {
            const retry = await db.query(`SELECT id, username, full_name, email, role,
                        COALESCE(token_version, 0) AS token_version
    ...
    logger_1.logger.info({ inspectorUserId: inserted.rows[0].id, platformAdminId: platformAdmin.id }, "impersonation: created synthetic platform inspector user");
 ...
 */
async function getActiveSession(tenantId) {
    const res = await (0, db_1.masterQuery)(`
...
//# sourceMappingURL=impersonationApproval.js.map