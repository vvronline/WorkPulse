...
exports.generateEphemeralCreds = generateEphemeralCreds;
exports.buildTurnUrlList = buildTurnUrlList;
exports.buildStunServers = buildStunServers;
exports.fetchCloudflareCreds = fetchCloudflareCreds;
...
const cfCredCache = new Map(); // userId → { iceServers, expiresAt }
/**
 ...
 */
async function fetchCloudflareCreds(userId) {
    ...
    if (cached && cached.expiresAt - now > CF_REFRESH_BEFORE)
        return cached;
    try {
        const resp = await fetch(`https://rtc.live.cloudflare.com/v1/turn/keys/${encodeURIComponent(tokenId)}/credentials/generate`, {
            method: "POST",
            headers: {
        ...
        console.warn("[coturn] Cloudflare TURN credential fetch failed:", err?.message || err);
        return null;
    }
}
/**
...
function buildStunServers() {
    ...
    if (!raw)
        return DEFAULT_STUN.map(u => ({ urls: u }));
    return raw
        .split(",")
        .map(s => s.trim())
        .filter(Boolean)
 ...
 */
function generateEphemeralCreds(userId) {
    ...
    const credential = crypto_1.default.createHmac("sha1", secret).update(username).digest("base64");
 ...
 * aren't set.
 */
async function buildIceServers(userId) {
    ...
    if (String(process.env.DISABLE_PUBLIC_TURN || "false").toLowerCase() !== "true") {
        iceServers.push({ urls: "turn:openrelay.metered.ca:80", username: "openrelayproject", credential: "openrelayproject" }, { urls: "turn:openrelay.metered.ca:443", username: "openrelayproject", credential: "openrelayproject" }, { urls: "turn:openrelay.metered.ca:443?transport=tcp", username: "openrelayproject", credential: "openrelayproject" });
        return { iceServers, ttl: null, mode: "public-fallback" };
    }
    return { iceServers, ttl: null, mode: "stun-only" };
}
//# sourceMappingURL=coturn.js.map