...
exports._sendErrorAck = sendErrorAck;
const logger_1 = require("../logger");
const wsValidate_1 = require("../wsValidate");
...
const chatMessageSchema = {
    conversationId: wsValidate_1.schema.posInt(),
    content: wsValidate_1.schema.str({ min: 1, max: 5_000 }),
    replyToId: wsValidate_1.schema.posInt({ optional: true }),
    // 'text' (default), 'markdown', 'code'
    formatType: wsValidate_1.schema.str({ max: 16, optional: true }),
    clientMsgId: wsValidate_1.schema.str({ max: 64, optional: true }),
 ...
 */
function sendErrorAck(ws, clientMsgId, reason) {
...
async function chatMessage({ db, senderId, tenantId, data, ws, sendToUser }) {
    // ── 1. Schema check ────────────────────────────────────────────────
    const parsed = (0, wsValidate_1.validate)(chatMessageSchema, data);
    if (!parsed.ok) {
    ...
    const { conversationId, content, replyToId, formatType, clientMsgId } = parsed.value;
    ...
    if (INJECTION_RE.test(content)) {
        logger_1.logger.warn({ senderId, conversationId }, "chat_message: rejected content with script-like pattern");
    ...
    const result = (await db.query(`INSERT INTO messages (conversation_id, sender_id, content, reply_to_id, format_type)
         VALUES ($1, $2, $3, $4, $5) RETURNING id, created_at`, [conversationId, senderId, content.trim(), replyToId || null, fmtType])).rows[0];
    await db.query("UPDATE conversations SET updated_at = NOW() WHERE id = $1", [conversationId]);
    await db.query(`INSERT INTO message_reads (conversation_id, user_id, last_read_at)
         VALUES ($1, $2, $3)
         ON CONFLICT (conversation_id, user_id) DO UPDATE SET last_read_at = $3`, [conversationId, senderId, result.created_at]);
    // ── 4. Gather fan-out audience + sender meta + reply context ───────
    const [participantsResult, senderResult] = await Promise.all([
        db.query("SELECT user_id FROM conversation_participants WHERE conversation_id = $1", [conversationId]),
    ...
    if (replyToId) {
        const replyMsg = (await db.query(`SELECT m.content, u.full_name AS sender_name
             FROM messages m JOIN users u ON u.id = m.sender_id
             WHERE m.id = $1 AND m.conversation_id = $2`, [replyToId, conversationId])).rows[0];
        if (replyMsg) {
    ...
    for (const p of participants) {
        ...
        if (p.user_id !== senderId) {
            ...
            pushNotifications_1.pushNotifications.sendMessageNotification(db.query, p.user_id, tenantId, {
                conversationId,
                messageId: result.id,
                senderId,
                ...
                logger_1.logger.warn({ err: err.message, userId: p.user_id, messageId: result.id }, "Failed to send message push notification");
            });
        }
    }
