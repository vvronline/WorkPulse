...
exports.TEMPLATE_PREVIEW_ARGS = exports.TEMPLATE_KEYS = exports.templates = void 0;
exports.getTransporter = getTransporter;
exports.sendMail = sendMail;
exports.notifyByEmail = notifyByEmail;
exports.getEffectiveTemplate = getEffectiveTemplate;
exports.loadOrgBranding = loadOrgBranding;
...
async function getGmailAccessToken() {
    ...
    const res = await httpsPostJson("oauth2.googleapis.com", "/token", { "Content-Type": "application/x-www-form-urlencoded" }, form);
    gmailAccessToken = {
...
async function sendViaGmailApi({ from, to, subject, html }) {
    ...
    try {
        await httpsPostJson("gmail.googleapis.com", "/gmail/v1/users/me/messages/send", { "Content-Type": "application/json", Authorization: `Bearer ${accessToken}` }, JSON.stringify({ raw }));
    }
    catch (err) {
        // Log the raw Gmail response body so the exact rejection reason is visible.
        const e = err;
        logger_1.logger.error({ statusCode: e.statusCode, body: e.body, from, to }, "Gmail API: messages.send rejected");
        throw err;
    }
}
/**
...
function makeLookup(family) {
    if (!family)
        return undefined; // 0/undefined → use default OS resolver
    return (hostname, options, callback) => {
        ...
        dns_1.default.lookup(hostname, { ...options, family }, callback);
    };
}
function getTransporter() {
    ...
    if (lookup)
        TIMEOUTS.dnsTimeout = Number(process.env.SMTP_DNS_TIMEOUT_MS) || 10000;
    // ── Mode 1: Gmail OAuth2 ────────────────────────────────────────
    if (process.env.GMAIL_CLIENT_ID && process.env.GMAIL_CLIENT_SECRET && process.env.GMAIL_REFRESH_TOKEN) {
        ...
        transporter.verify().then(() => {
            logger_1.logger.info({ port }, "Email transport: Gmail OAuth2 — verified");
        }).catch((err) => {
            ...
            logger_1.logger.error({ err: err.message, port }, `Email transport: Gmail OAuth2 — verification FAILED${hint}`);
            transporter = null;
        });
        return transporter;
    }
    ...
    if (process.env.SMTP_USER && process.env.SMTP_PASS) {
        ...
        logger_1.logger.info({ port }, "Email transport: SMTP (password)");
        return transporter;
    }
    return null;
}
...
function sendMail({ to, subject, html }) {
    if (!to || !to.includes("@")) {
        logger_1.logger.warn({ to, subject }, "Email skipped — invalid recipient");
    ...
    if (forceApi && canUseGmailApi()) {
        return sendViaGmailApi({ from: FROM(), to, subject, html })
            .then(() => { logger_1.logger.info({ to, subject }, "Email sent successfully (Gmail API)"); return true; })
            .catch((err) => { logger_1.logger.error({ err: err.message, to, subject }, "Gmail API send failed"); return false; });
    }
    ...
    if (!mailer) {
        ...
        logger_1.logger.debug({ to, subject }, "Email skipped (SMTP not configured)");
        return Promise.resolve(false);
    }
    let attempt = 0;
    return new Promise((resolve) => {
        const trySend = () => {
            ...
            mailer.sendMail({ from: FROM(), to, subject, html }).then(() => {
                logger_1.logger.info({ to, subject }, "Email sent successfully");
                resolve(true);
            }).catch((err) => {
                ...
                if (isConnError(err) && canUseGmailApi()) {
                    logger_1.logger.warn({ err: err.message, to, subject }, "SMTP unreachable — falling back to Gmail API");
                    sendViaGmailApi({ from: FROM(), to, subject, html })
                        .then(() => { logger_1.logger.info({ to, subject }, "Email sent successfully (Gmail API)"); resolve(true); })
                        .catch((apiErr) => { logger_1.logger.error({ err: apiErr.message, to, subject }, "Gmail API fallback failed"); resolve(false); });
                    return;
                }
                if (attempt <= MAX_RETRIES) {
                    logger_1.logger.warn({ err: err.message, to, subject, attempt }, "Email send failed — retrying");
                    setTimeout(trySend, RETRY_DELAY_MS * attempt);
                }
                else {
                    logger_1.logger.error({ err, to, subject, attempts: attempt }, "Failed to send email after retries");
                    resolve(false);
                }
            });
        };
...
exports.TEMPLATE_KEYS = TEMPLATE_KEYS;
const templates = {
...
exports.TEMPLATE_PREVIEW_ARGS = TEMPLATE_PREVIEW_ARGS;
/**
...
async function loadOrgBranding(db, tenantId, orgId) {
    ...
    if (cached && (Date.now() - cached.cachedAt) < BRANDING_TTL_MS)
        return cached.value;
    try {
        const branding = (await db.query(`SELECT logo_url, accent_color FROM org_branding WHERE org_id = $1`, [orgId])).rows[0] || { logo_url: null, accent_color: "#6366f1" };
        const overrides = {};
        const overrideRows = (await db.query(`SELECT template_key, subject, body_html, enabled
               FROM org_email_templates
              WHERE org_id = $1`, [orgId])).rows;
        ...
        logger_1.logger.debug({ err: err.message, orgId }, "loadOrgBranding: falling back to defaults");
        return { logo_url: null, accent_color: "#6366f1", overrides: {} };
    }
}
/**
...
function notifyByEmail(templateName, ...args) {
    ...
    getEffectiveTemplate(templateName, args, opts).then(({ to, subject, html, enabled }) => {
        if (!enabled) {
            logger_1.logger.debug({ templateName, to }, "Email skipped (template disabled by org override)");
            return;
        }
        if (to)
            sendMail({ to, subject, html });
    }).catch((err) => {
        logger_1.logger.error({ err: err.message, templateName }, "Failed to render email template");
    });
}
