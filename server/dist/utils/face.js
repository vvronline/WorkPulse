...
exports.DEFAULT_MATCH_THRESHOLD = exports.FACE_DESCRIPTOR_LENGTH = void 0;
...
exports.FACE_DESCRIPTOR_LENGTH = FACE_DESCRIPTOR_LENGTH;
...
function isValidDescriptor(d) {
    if (!Array.isArray(d) || d.length !== FACE_DESCRIPTOR_LENGTH)
        return false;
...
function parseDescriptor(input) {
    ...
    if (!Array.isArray(arr) || arr.length !== FACE_DESCRIPTOR_LENGTH)
        return null;
    const out = new Array(FACE_DESCRIPTOR_LENGTH);
 ...
 */
function compareDescriptors(enrolled, candidate, threshold = DEFAULT_MATCH_THRESHOLD) {
