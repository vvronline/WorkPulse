...
exports.expectedMigrationCount = exports.MIGRATIONS = void 0;
exports.runTenantMigrations = runTenantMigrations;
exports.sweepAllTenants = sweepAllTenants;
/**
    ...
    },
    {
        ...
        async up(query) {
            ...
            if (corruptedIds.length > 0) {
                ...
                `, [corruptedIds]);
                logger_1.logger.info({ conversationCount: corruptedIds.length }, 'Cleaned up legacy DM conversations with extra participants');
            }
...
async function runTenantMigrations(query, opts = {}) {
    ...
    try {
        ...
        logger_1.logger.error({ err: err.message, label }, 'Migration runner: failed to ensure _migrations table');
    ...
    try {
        ...
        logger_1.logger.error({ err: err.message, label }, 'Migration runner: failed to read _migrations');
        return { applied, skipped, failed };
    }
    for (const mig of MIGRATIONS) {
        ...
        try {
            await mig.up(query);
            await query('INSERT INTO _migrations (name) VALUES ($1) ON CONFLICT (name) DO NOTHING', [mig.name]);
            applied.push(mig.name);
            logger_1.logger.info({ migration: mig.name, label }, 'Migration applied');
        }
        catch (err) {
    ...
    try {
        ...
        logger_1.logger.error({ err: err.message, label }, 'Agile defaults seeding failed (non-fatal)');
    }
    return { applied, skipped, failed };
}
async function sweepAllTenants() {
    ...
    logger_1.logger.info(totals, 'Migration sweep complete');
    return totals;
}
const expectedMigrationCount = MIGRATIONS.length;
exports.expectedMigrationCount = expectedMigrationCount;
