...
/**
 * Built-in / "starter" leave-type defaults used as fallback when an org has
 * not yet configured its own custom leave policies. Mirrors
 * client/src/constants/leaves.ts.
 */
...
export const LEAVE_TYPE_MAP: Record<string, LeaveTypeMeta> = Object.fromEntries(
...
export function getLeaveType(val: string): LeaveTypeMeta {
  return LEAVE_TYPE_MAP[val] ?? LEAVE_TYPES[4];
}
 ...
 * Known types keep their built-in icon/colour (with overrides allowed from the
 * policy's name/color). Custom org types render with the FileEdit icon and the
 * policy's configured colour.
 */
export function buildLeaveTypeMeta(
  policies: LeavePolicyLike[] = [],
): Record<string, LeaveTypeMeta> {
  const out: Record<string, LeaveTypeMeta> = { ...LEAVE_TYPE_MAP };
  for (const p of policies) {
    const base: LeaveTypeMeta = LEAVE_TYPE_MAP[p.leave_type] || {
      value: p.leave_type,
      label: prettify(p.leave_type),
      Icon: FileEdit,
...
/**
 * Build an array of leave-type options (suitable for chip pickers) from the
 * org's policies. Returns an empty array when the org has not configured any
 * leave policies yet.
 */
export function buildLeaveTypeOptions(
  policies: LeavePolicyLike[] = [],
): LeaveTypeMeta[] {
