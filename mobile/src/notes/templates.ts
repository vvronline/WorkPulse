/* ─────────────────────────────────────────────────────────
   Notes templates — ported from the web client. Used by the
   Home screen quick-action tiles and the empty-state picker.
   Each template renders directly into Quill's HTML content.
   ───────────────────────────────────────────────────────── */
...
export function buildJournalPrefillHtml(prefill: DailyPrefill): string {
  const parts: string[] = [];
  parts.push(`<h2>${todayLabel()}</h2>`);

  if (prefill.hoursWorked != null) {
    parts.push(
      `<div class="ql-callout" data-callout="info"><p>⏱ <strong>${prefill.hoursWorked}h</strong> tracked today</p></div>`,
    );
  }
  ...
  if (prefill.tasks && prefill.tasks.length > 0) {
    ...
    if (done.length > 0 || other.length > 0) {
      ...
      other.forEach((t) =>
        parts.push(`<li data-list="unchecked">${escHtml(t.title)} <em>(${escHtml(t.status)})</em></li>`),
      );
      parts.push(`</ul>`);
    }
  } else {
  ...
  if (prefill.meetings && prefill.meetings.length > 0) {
    parts.push(`<h3>Meetings</h3>`);
    prefill.meetings.forEach((m) => {
      const time = m.scheduled_start
        ? new Date(m.scheduled_start).toLocaleTimeString(undefined, {
            hour: "2-digit",
            minute: "2-digit",
          })
        : "";
  ...
  if (
    prefill.events &&
    prefill.events.length > 0 &&
    (!prefill.meetings || prefill.meetings.length === 0)
  ) {
...
export function buildOneOnOnePrefillHtml(prefill: OneOnOnePrefill): string {
  const parts: string[] = [];
  ...
  if (prefill.hoursThisWeek != null) {
    parts.push(
      `<div class="ql-callout" data-callout="info"><p>📊 ${escHtml(reportName)} logged <strong>${prefill.hoursThisWeek}h</strong> this week</p></div>`,
    );
  }
  ...
  if (prefill.sprint) {
    ...
    if (sp.taskBreakdown && sp.taskBreakdown.length > 0) {
      const bd: Record<string, number> = {};
      sp.taskBreakdown.forEach((r) => {
        bd[r.status] = r.count;
      });
      const total = Object.values(bd).reduce((a, b) => a + b, 0);
      const done = bd.done || 0;
      parts.push(
        `<p>Progress: <strong>${done}/${total}</strong> tasks done (${total > 0 ? Math.round((done / total) * 100) : 0}%)</p>`,
      );
  ...
  if (prefill.leaves && prefill.leaves.length > 0) {
    parts.push(`<h3>Recent leaves</h3>`);
    prefill.leaves.forEach((l) => {
      parts.push(
        `<ul><li>${escHtml(l.date)} — ${escHtml(l.leave_type)} (${l.duration}, ${l.status})</li></ul>`,
      );
    });
  }
