...
export default function AuditLogsScreen() {
  ...
  const {
    ...
    queryKey: ["admin", "audit", range, page],
    queryFn: async () => {
      ...
      return { logs: r.data.logs || [], total: r.data.total || 0 };
    },
  });
  ...
  const errorMessage = isError
    ? (error as any)?.response?.data?.error || "Failed to load audit logs"
    : null;
