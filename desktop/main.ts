    ...
    if (!apiKey) {
        ...
        for (const f of candidates) {
            try {
                ...
                console.warn(`[WorkPulse] Error reading ${f}:`, (err as Error)?.message);
            }
        }
    }
    if (apiKey) {
...
async function openRemoteUpload(pathWithQuery: string, suggestedName?: string): Promise<void> {
    ...
    ipcMain.handle("get-wifi-info", async () => {
        ...
        try {
            if (process.platform === "win32") {
                ...
                if (!bssidM) {
                    const error =
                        stateM && /disconnected/i.test(stateM[1]) ? "wifi_disconnected" : "bssid_unavailable";
            ...
            console.warn("[WorkPulse] get-wifi-info failed:", (err as Error)?.message);
            return { ok: false, error: (err as Error)?.message || "wifi_lookup_failed" };
        }
    });
...
`.trim();
            ...
            console.warn("[WorkPulse] get-native-location failed:", (err as Error)?.message);
            return { ok: false, error: (err as Error)?.message || "native_location_failed" };
        }
    });
    ...
    ipcMain.handle("open-location-settings", async () => {
        try {
            ...
            console.warn("[WorkPulse] open-location-settings failed:", (err as Error)?.message);
    ...
    ipcMain.handle("get-ip-location", async () => {
        ...
        const providers: {
            url: string;
            parse: (j: any) => { latitude: number; longitude: number; accuracy: number } | null;
        }[] = [
            {
        ...
        for (const p of providers) {
            try {
                ...
                console.warn(`[WorkPulse] IP geolocation provider failed (${p.url}):`, (err as Error)?.message);
            }
        }
    ...
    protocol.handle("workpulse", async (request) => {
        ...
        if (pathname.startsWith("/api/") || pathname.startsWith("/api\\")) {
            ...
            try {
                ...
                const fetchOpts: RequestInit & { bypassCustomProtocolHandlers?: boolean } = {
                    method: request.method,
                    headers,
                    credentials: "include",
                    cache: "no-store",
        ...
        try {
            ...
            if (isUpload && isOwnHost) {
                openRemoteUpload(pathname + (u.search || "")).catch((err) => {
                    console.error("[WorkPulse] Failed to open uploaded file:", (err as Error)?.message);
                });
                return { action: "deny" };
            }
        } catch {
    ...
    if (LOADED_GOOGLE_API_KEY) {
        (async () => {
            try {
                ...
                let parsed: { location?: { lat: number; lng: number }; accuracy?: number } | null;
                try {
