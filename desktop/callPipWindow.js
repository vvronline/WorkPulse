const { BrowserWindow, ipcMain, app, screen, nativeImage } = require('electron');
...
const STATE_FILE = () => path.join(app.getPath('userData'), 'call-pip-window-state.json');
...
function savePipState(bounds) {
    ...
    saveTimer = setTimeout(() => {
        try { fs.writeFileSync(STATE_FILE(), JSON.stringify(bounds)); } catch { /* ignore */ }
    }, 500);
}

function setupCallPipWindow(mainWindow) {
    ...
    const openPip = (state) => {
        ...
        if (pipWindow && !pipWindow.isDestroyed()) {
            ...
            try { pipWindow.webContents.send('call:pip-state', lastState); } catch { /* ignore */ }
            try { pipWindow.show(); pipWindow.focus(); } catch { /* ignore */ }
            return;
        }
        ...
        const bounds = saved && Number.isFinite(saved.x) && Number.isFinite(saved.y)
            ? { ...defaultBounds, ...saved }
            : defaultBounds;
        ...
        pipWindow.on('closed', () => {
            ...
            if (!wasSilent && mainWindow && !mainWindow.isDestroyed()) {
                try { mainWindow.webContents.send('call:pip-window-closed'); } catch { /* ignore */ }
            }
        });
    };

    // ── IPC: from main window ──
    ipcMain.on('call:pip-open', (event, state) => {
        // Only the main window may open a pip window
        if (event.sender !== mainWindow.webContents) return;
        openPip(state);
    });

    ipcMain.on('call:pip-close', (event) => {
        if (event.sender !== mainWindow.webContents) return;
        closePip(true);
    });

    ipcMain.on('call:pip-update-state', (event, partial) => {
        if (event.sender !== mainWindow.webContents) return;
    ...
    // ── IPC: from pip window ──
    ipcMain.on('call:pip-ready', (event) => {
    ...
    ipcMain.on('call:pip-action', (event, payload) => {
        if (!pipWindow || event.sender !== pipWindow.webContents) return;
        ...
        try { mainWindow.webContents.send('call:pip-action', payload || {}); } catch { /* ignore */ }
    });
...
module.exports = { setupCallPipWindow };