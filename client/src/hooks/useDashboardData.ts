...
export const QUOTES = [
    ...
    {
        text: "Do the hard jobs first. The easy ones will take care of themselves.",
    ...
    {
        text: "Your work is going to fill a large part of your life. Love what you do.",
        author: "Steve Jobs",
    },
    {
...
export function useDashboardData() {
    ...
    const [widgets, setWidgets] = useState<Record<string, unknown> | null>(
        null,
    );
    const [weeklyData, setWeeklyData] = useState<unknown>(null);
    const [taskSummary, setTaskSummary] = useState<unknown>(null);
    ...
    const { reminders, dismiss: dismissReminder } =
        useEventReminder(todayEvents);
    ...
    const fetchStatus = useCallback(async () => {
        try {
            ...
            const [statusRes, widgetsRes, weeklyRes, taskRes, eventsRes] =
            ...
            if (taskRes.status === "fulfilled")
                setTaskSummary(taskRes.value.data);
            if (eventsRes.status === "fulfilled")
                setTodayEvents(eventsRes.value.data || []);
        } catch (err) {
    ...
    const clockInEntry = status?.entries?.find(
    ...
    const clockInTime = useMemo(
        () =>
            clockInEntry
                ? new Date(
                      clockInEntry.timestamp as string,
                  ).toLocaleTimeString([], {
                      hour: "2-digit",
                      minute: "2-digit",
                  })
    ...
    const handleClockIn = useCallback(
        () => handleAction(() => clockIn(workMode), "clockIn"),
