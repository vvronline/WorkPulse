...
function tone(ctx, freq, startOffset, duration, volume, type = 'sine') {
...
function pluck(ctx, freq, startOffset, duration, volume) {
...
function pulseOutgoing(ctx, vol) {
    // Slow single soft pulse
    tone(ctx, 523, 0, 0.45, vol * 0.45, 'sine');
    return 1500;
}

function softOutgoing(ctx, vol) {
...
function urgentMention(ctx, vol) {
    // Quick double alert
 ...
 */
export function getPresetLabel(category, id) {
 ...
 */
export function playSound(category, id, { volume = 0.5, loop = false } = {}) {
    ...
    try {
        ...
        const tick = () => {
            if (cancelled) return;
            try {
                const d = preset.synth(ctx, volume) || 800;
...
export function previewSound(category, id, volume = 0.5) {
