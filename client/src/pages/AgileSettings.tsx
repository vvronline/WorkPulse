...
import {
    ...
    getWorkItemTypes, createWorkItemType, updateWorkItemType, deleteWorkItemType,
    getWorkflowStates, createWorkflowState, updateWorkflowState, deleteWorkflowState,
    getAgilePermissions,
} from "../api";
...
export default function AgileSettings() {
    ...
    const [perms, setPerms] = useState<Perms>({ canEdit: false, isSuperAdmin: false, role: "" });
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState("");

    useEffect(() => {
        getAgilePermissions().then(r => setPerms(r.data as Perms)).catch(() => { }).finally(() => setLoading(false));
    }, []);
...
function GeneralTab({ canEdit, onSaved, setError }: GeneralTabProps) {
    ...
    const updateField = (key: string, val: any) => setSettings((s: any) => ({ ...s, [key]: val }));
...
function TypesTab({ canEdit, onChanged, setError }: TypesTabProps) {
    ...
    const [form, setForm] = useState<any>({ name: "", color: "#6366f1", icon: "", is_epic: false, is_default: false, description: "" });

    const reload = () => getWorkItemTypes().then(r => setList(r.data as any[]));
    useEffect(() => { reload(); }, []);
...
function WorkflowTab({ canEdit, onChanged, setError }: WorkflowTabProps) {
    const [list, setList] = useState<any[]>([]);
    const [adding, setAdding] = useState(false);
    const [form, setForm] = useState<any>({ name: "", category: "open", color: "#6b7280", wip_limit: "", is_initial: false, is_terminal: false });

    const reload = () => getWorkflowStates().then(r => setList(r.data as any[]));
    useEffect(() => { reload(); }, []);
    ...
    const grouped = useMemo(() => {
        const g: Record<string, any[]> = { open: [], in_progress: [], in_review: [], done: [] };
