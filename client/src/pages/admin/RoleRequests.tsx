...
export default function RoleRequests({ userRole }: RoleRequestsProps) {
  ...
  const { data: requests = EMPTY_REQUESTS } = useQuery({
    queryKey: ["admin", "role-requests", filter],
    queryFn: async (): Promise<any[]> =>
      ((await getRoleChangeRequests({ status: filter || undefined } as any))
        .data as any) || [],
  });
  ...
  const renderApprovalChain = (approvals: any) => {
    if (!approvals || typeof approvals !== "object") return null;
    return Object.entries(approvals).map(([role, info]: [string, any]) => (
      <span
        key={role}
        style={{
      ...
      >
        ...
        {info.status === "approved" ? <Check size={12} /> : <Clock size={12} />}
      </span>
    ));
  };
          ...
          {requests.map((r) => (
            <tr key={r.id}>
              ...
              <td>
                ...
                {r.status === "approved" && (
                  <span className={s.badgeActive}>Approved</span>
                )}
                {r.status === "rejected" && (
                  <span className={s.badgeInactive}>Rejected</span>
                )}
                {r.status === "cancelled" && (
                  <span className={s.badgeInactive}>Cancelled</span>
                )}
              </td>
              <td>
                {r.status === "pending" && (
                  <div className={s.actions}>
                    {(r.approvals?.[userRole as string]?.status === "pending" ||
                    ...
                    {(r.approvals?.[userRole as string] ||
