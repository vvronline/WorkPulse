...
export default function RoleRequests({ userRole }: RoleRequestsProps) {
    ...
    const fetchRequests = useCallback(() => {
        getRoleChangeRequests({ status: filter || undefined } as any).then(r => setRequests((r.data as any) || [])).catch(() => {});
    }, [filter]);
                            ...
                            <td>
                                {r.status === "pending" && (
                                    <div className={s.actions}>
                                        {(r.approvals?.[userRole as string]?.status === "pending" || userRole === "platform_admin") && (
                                        ...
                                        {(r.approvals?.[userRole as string] || userRole === "super_admin" || userRole === "platform_admin") && (
