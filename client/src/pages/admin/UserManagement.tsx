...
interface SavedView {
  key: string;
  label: string;
  test: (u: any, ctx?: { pendingByUser: Record<string, any> }) => boolean;
}
...
const SAVED_VIEWS: SavedView[] = [
  ...
  { key: "no-team", label: "No team", test: (u) => !u.team_id },
  {
    key: "no-manager",
    label: "No manager",
    test: (u) => !u.manager_id && u.role !== "platform_admin",
  },
  {
    key: "pending",
    label: "Role pending",
    test: (u, ctx) => !!ctx?.pendingByUser[u.id],
  },
  {
    key: "admins",
    label: "Admins",
    test: (u) => ["hr_admin", "super_admin", "platform_admin"].includes(u.role),
  },
];
...
const EMPTY_PENDING: Record<string, any> = {};

export default function UserManagement({ userRole }: UserManagementProps) {
  const { user: currentUser } = useAuth() as any;
  const { labelFor: roleLabel, roles: tenantRoles } = useRoleLabels() as any;
  ...
  const { data: users = EMPTY_USERS, isLoading: loading } = useQuery({
    queryKey: ["admin", "users", debouncedSearch, filterRole, filterActive],
    queryFn: async () => {
  ...
  const { data: pendingByUser = EMPTY_PENDING } = useQuery({
    queryKey: ["admin", "users", "pending"],
    queryFn: async () => {
      const r = await getRoleChangeRequests({ status: "pending" });
      const map: Record<string, any> = {};
      ((r.data as any[]) || []).forEach((rr) => {
        map[rr.target_user_id] = rr;
      });
      return map;
    },
  });
  ...
  useEffect(() => {
    if (userRole === "platform_admin") {
      getAdminOrganizations()
        .then((r) => setOrganizations((r.data as any)?.data || r.data || []))
  ...
  const allVisibleSelected =
    visibleUsers.length > 0 && visibleUsers.every((u) => selected.has(u.id));
  ...
  const selectableCount = selectedUsers.filter(
    (u) => u.id !== currentUser?.id && u.role !== "platform_admin",
  ).length;
  ...
  const openDrawer = (u: any) => setDrawerUser(u);
  const closeDrawer = () => setDrawerUser(null);

  const onUserSaved = (msg?: string) => {
    setToast(msg || "");
  ...
  // ─── Bulk actions ────────────────────────────────────────────────────
  const runBulk = async (label: string, fn: (u: any) => Promise<any>) => {
  ...
  const bulkActivate = () => {
    setConfirmOpts({
      ...
      onConfirm: () =>
        runBulk("Activate", (u) =>
          u.is_active ? Promise.resolve() : toggleUserActive(u.id),
        ),
    });
  };
  ...
  const bulkClearTeam = () => {
    setConfirmOpts({
      ...
      onConfirm: () =>
        runBulk("Clear team", (u) =>
          updateUserAssignment(u.id, {
    ...
    <div className={s.wrap}>
      ...
      <div className={s.toolbar}>
        ...
        >
          ...
          {(tenantRoles?.length
            ? tenantRoles.map((r: any) => r.role_key)
            : ROLES
          ).map((r: string) => (
      ...
      <div className={s.viewsRow}>
        ...
        {SAVED_VIEWS.map((v) => (
          ...
          >
            {v.label}
            {viewCounts[v.key] > 0 && (
              <span className={s.count}>{viewCounts[v.key]}</span>
            )}
          </button>
        ))}
      </div>
        ...
        <div className={s.bulkBar}>
          <span className={s.bulkLabel}>
            ...
            {selectableCount !== selected.size &&
              ` (${selectableCount} actionable)`}
          </span>
          <button
            className={`${s.bulkAction} ${s.success}`}
          ...
          <button
            className={`${s.bulkAction} ${s.danger}`}
          ...
          <button
            className={`${s.bulkAction} ${s.secondary}`}
            onClick={bulkClearTeam}
            disabled={selectableCount === 0}
          >
            Clear team
          </button>
            ...
            return (
              ...
              >
                ...
                <div className={s.userCell}>
                  ...
                  <div className={s.userMeta}>
                    <div className={s.userName}>
                      {u.full_name}
                      {pending && (
                        <span className={s.pendingPill}>role pending</span>
                      )}
                    </div>
                ...
                <div className={s.cellText}>
                  {u.department_name || (
                    <em className={s.cellMuted}>no dept</em>
                  )}
                  {u.team_name && (
                    <>
                      {" "}
                      · <span style={{ opacity: 0.85 }}>{u.team_name}</span>
                    </>
                  )}
                </div>
                <div className={s.cellText}>
                ...
                <div>
                  <span
                    className={`${s.statusDot} ${u.is_active ? "" : s.inactive}`}
                  >
      ...
      {drawerUser && (
        <UserDrawer
          ...
          onClose={closeDrawer}
          onSaved={(msg) => {
            onUserSaved(msg); /* keep drawer open so user can keep editing */
          }}
      ...
      {confirmOpts && (
        <TypedConfirm {...confirmOpts} onCancel={() => setConfirmOpts(null)} />
      )}
    </div>
  );
}
