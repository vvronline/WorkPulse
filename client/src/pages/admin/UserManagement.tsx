...
export default function UserManagement({ userRole }: UserManagementProps) {
    ...
    const [pendingByUser, setPendingByUser] = useState<Record<string, any>>({});
    ...
    const fetchUsers = useCallback(() => {
        ...
        getAdminUsers(params)
            .then(r => setUsers((r.data as any)?.data ?? r.data ?? []))
    ...
    const fetchPending = useCallback(() => {
        getRoleChangeRequests({ status: "pending" }).then(r => {
            ...
            ((r.data as any[]) || []).forEach(rr => { map[rr.target_user_id] = rr; });
        ...
        <div className={s.wrap}>
            ...
            <div className={s.toolbar}>
                ...
                <select value={filterRole} onChange={e => setFilterRole(e.target.value)} aria-label="Role filter">
                    ...
                    {(tenantRoles?.length ? tenantRoles.map((r: any) => r.role_key) : ROLES).map((r: string) => (
