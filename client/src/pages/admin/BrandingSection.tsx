...
export default function BrandingSection({ canEdit }: BrandingSectionProps) {
  ...
  const { data: branding, isLoading: loading } = useQuery({
    ...
    queryFn: async () => (await getBranding()).data as BrandingData,
  });

  // Populate the editable form from the fetched record.
  useEffect(() => {
    if (branding) {
      setData(branding);
      setDraftAccent(branding.accent_color || DEFAULT_ACCENT);
  ...
  useEffect(() => {
    lastBlobRef.current = draftLogoPreview;
  }, [draftLogoPreview]);
  useEffect(
    () => () => {
  ...
  const accentDirty =
    draftAccent.toLowerCase() !== (data.accent_color || "").toLowerCase();
  ...
  const save = async () => {
    ...
    try {
      ...
      if (accentDirty) {
        ...
        nextRow = {
          ...nextRow,
          accent_color: (row as BrandingData).accent_color,
        };
      }
      setData(nextRow);
  ...
  const effectiveLogoSrc = (() => {
    ...
    if (data.logo_url) {
      return data.logo_url.startsWith("http")
        ? data.logo_url
        : `${serverURL}${data.logo_url}`;
    }
    return null;
  })();
  ...
  return (
    <div className={s.wrap}>
          ...
          <div className={s.logoBtns}>
            ...
            {(draftLogoFile || removeLogo) && (
              <span className={s.pendingTag}>
                {removeLogo
                  ? "Removal pending — click Save to apply"
                  : "New logo selected — click Save to apply"}
              </span>
            )}
            <p className={s.hint}>
              PNG, JPG, SVG, GIF or WebP — max 2 MB. Recommended height
              40&nbsp;px.
            </p>
          </div>
        </div>
      ...
      {/* Preview */}
      <div
        className={s.previewBlock}
        style={{ "--preview-accent": draftAccent } as React.CSSProperties}
      >
        <div className={s.previewLabel}>Live preview</div>
        <div className={s.previewCard}>
          {effectiveLogoSrc && (
            <img src={effectiveLogoSrc} alt="" className={s.previewLogo} />
          )}
          ...
          <button type="button" className={s.previewBtn}>
            Primary action
          </button>
        </div>
      </div>
      ...
      {!canEdit && (
        <p className={s.readOnly}>
          You don't have permission to edit branding. Contact your HR admin or
          super admin.
        </p>
      )}
    </div>
