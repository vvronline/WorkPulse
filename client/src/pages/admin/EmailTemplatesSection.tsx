...
export default function EmailTemplatesSection({
  ...
  const { data: list = EMPTY_TEMPLATES, isLoading: loading } = useQuery({
    queryKey: ["admin", "emailTemplates"],
    queryFn: async () =>
      (((await getEmailTemplates()).data as any).templates ||
        []) as EmailTemplate[],
  });
  ...
  useEffect(() => {
    if (list.length && !selected) {
      setSelected(list[0].template_key);
    }
  }, [list, selected]);
  ...
  if (!list.length)
    return <div className={s.loading}>No email templates available.</div>;
    ...
    <div className={s.editorInner}>
      ...
      <div className={s.fieldGroup}>
        ...
        <div className={s.tokenHint}>
          Tokens: <code>{"{{accent}}"}</code> = your accent color. Recipient
          name, dates, task titles and other dynamic values are inserted
          automatically — see the preview below.
        </div>
      </div>
      ...
      <div className={s.actions}>
        ...
        {template.is_overridden && (
          ...
          >
            <RotateCcw size={14} />{" "}
            {reverting ? "Reverting…" : "Revert to built-in"}
          </button>
        )}
        {dirty && <span className={s.dirtyHint}>Unsaved changes</span>}
      </div>
      ...
      <div className={s.previewWrap}>
        <div className={s.previewHeader}>
          ...
          {previewLoading && (
            <span className={s.previewLoading}>updating…</span>
          )}
        </div>
        ...
        <p className={s.previewNote}>
          Sample data is used to fill template variables. The actual emails sent
          to your team will use the real recipient name, dates, task titles,
          etc.
        </p>
      </div>
