...
export default function CompensationSetup() {
  ...
  const { data, isLoading: loading } = useQuery({
    ...
    queryFn: async () => {
      const [tmpl, emp, members, bank, ctc] = await Promise.allSettled([
        getCompensationTemplates(),
        getEmployeeCompensations(),
      ...
      return {
        templates:
          tmpl.status === "fulfilled"
            ? (tmpl.value.data as any[])
            : EMPTY_TEMPLATES,
        employees:
          emp.status === "fulfilled"
            ? (emp.value.data as any[])
            : EMPTY_EMPLOYEES,
        members:
          members.status === "fulfilled"
            ? (members.value.data as any).data || EMPTY_MEMBERS
            : EMPTY_MEMBERS,
        bankVerifications:
          bank.status === "fulfilled"
            ? (bank.value.data as any[]) || EMPTY_BANK
            : EMPTY_BANK,
        ctcConfig:
          ctc.status === "fulfilled" ? (ctc.value.data as CtcConfig) : null,
      };
    },
  });

  const templates: any[] = data?.templates ?? EMPTY_TEMPLATES;
  const employees: any[] = data?.employees ?? EMPTY_EMPLOYEES;
  const allMembers: any[] = data?.members ?? EMPTY_MEMBERS;
  const bankVerifications: any[] = data?.bankVerifications ?? EMPTY_BANK;
  const ctcConfig = data?.ctcConfig ?? CTC_DEFAULTS;

  // Load-into-form: populate the editable CTC form from the saved config.
  useEffect(() => {
    if (data?.ctcConfig) setCtcForm(data.ctcConfig);
  }, [data?.ctcConfig]);
  ...
  const handleSaveTemplate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ["admin", "compensation"] });
    } catch (err: any) {
  ...
  const handleAssign = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await assignCompensation(userId, {
        ...assignForm,
        ctc_annual: assignForm.ctc_annual || 0,
      });
      setAssignModal(null);
      setSelectedUserId("");
      queryClient.invalidateQueries({ queryKey: ["admin", "compensation"] });
  ...
  return (
    <div>
      ...
      <div className={s.tabBar}>
        <button
          className={`${s.tabBtn} ${tab === "templates" ? s.active : ""}`}
          onClick={() => setTab("templates")}
        >
          <LayoutTemplate size={14} /> Templates
        </button>
        <button
          className={`${s.tabBtn} ${tab === "employees" ? s.active : ""}`}
          onClick={() => setTab("employees")}
        >
          <Users size={14} /> Employees
        </button>
        <button
          className={`${s.tabBtn} ${tab === "ctc" ? s.active : ""}`}
          onClick={() => setTab("ctc")}
        >
          <Settings size={14} /> CTC Settings
        </button>
        <button
          className={`${s.tabBtn} ${tab === "bank" ? s.active : ""}`}
          onClick={() => setTab("bank")}
        >
              ...
              {templateForm.components.map((comp, idx) => (
                <div key={idx} className={s.componentRow}>
                  <input
                    ...
                    value={comp.key}
                    onChange={(e) =>
                      updateComponent(idx, "key", e.target.value)
                    }
                    className={s.inputSm}
                    required
                  />
                  <input
                    ...
                    value={comp.label}
                    onChange={(e) =>
                      updateComponent(idx, "label", e.target.value)
                    }
                    className={s.inputSm}
                    required
                  />
              ...
              <tbody>
                {employees.map((emp) => (
                  <tr key={emp.id}>
                    ...
                    <td>
                      {emp.ctc_annual > 0
                        ? `₹${Number(emp.ctc_annual).toLocaleString("en-IN")}`
                        : "-"}
                    </td>
                        ...
                        onClick={() => {
                          ...
                          setAssignForm({
                            effective_from: new Date()
                              ...
                              .slice(0, 10),
                            ctc_annual:
                              emp.ctc_annual > 0 ? emp.ctc_annual : "",
      ...
      {tab === "ctc" && (
        <form onSubmit={handleSaveCtcConfig} className={s.formCard}>
          ...
          >
            These percentages are used to auto-calculate monthly component
            amounts when an Annual CTC is entered during compensation
            assignment.
          </p>
          ...
          <div className={s.formRow}>
            <div className={s.formGroup}>
              ...
              <input
                ...
                onChange={(e) =>
                  setCtcForm((f) => ({
                    ...f,
                    basic_pct: e.target.value as any,
                  }))
                }
                className={s.input}
                required
            ...
            <div className={s.formGroup}>
              ...
              <input
                ...
                onChange={(e) =>
                  setCtcForm((f) => ({ ...f, hra_pct: e.target.value as any }))
                }
                className={s.input}
                required
              />
            ...
            <div className={s.formGroup}>
              ...
              <input
                ...
                onChange={(e) =>
                  setCtcForm((f) => ({
                    ...f,
                    conveyance_pct: e.target.value as any,
                  }))
                }
                className={s.input}
                required
          ...
          <div className={s.formRow}>
            <div className={s.formGroup}>
              ...
              <input
                ...
                onChange={(e) =>
                  setCtcForm((f) => ({ ...f, pf_pct: e.target.value as any }))
                }
                className={s.input}
                required
              />
            ...
            <div className={s.formGroup}>
              ...
              <input
                ...
                onChange={(e) =>
                  setCtcForm((f) => ({ ...f, pf_max: e.target.value as any }))
                }
                className={s.input}
                required
              />
            ...
            <div className={s.formGroup}>
              ...
              <input
                ...
                onChange={(e) =>
                  setCtcForm((f) => ({ ...f, pt_fixed: e.target.value as any }))
                }
                className={s.input}
                required
              />
          ...
          >
            Special Allowance is automatically the remaining balance after
            Basic, HRA, and Conveyance. TDS defaults to ₹0 and can be set
            manually per employee.
          </p>

          <div className={s.formActions}>
            <button type="submit" className={s.btnPrimary} disabled={ctcSaving}>
              <Save size={14} />{" "}
              {ctcSaved ? "Saved!" : ctcSaving ? "Saving..." : "Save Settings"}
            </button>
          </div>
        </form>
      )}
            ...
            <table className={s.table}>
              ...
              <tbody>
                      ...
                      {!b.is_verified ? (
                        <div style={{ display: "flex", gap: 6 }}>
                          ...
                          <button
                            ...
                            onClick={async () => {
                              if (!window.confirm("Reject this bank detail?"))
                                return;
                              try {
                        ...
                        <small style={{ color: "var(--text-muted)" }}>
                          {b.verified_at
                            ? `Verified ${new Date(b.verified_at).toLocaleDateString()}`
                            : "—"}
                        </small>
                      )}
                    </td>
                ...
                {bankVerifications.length === 0 && (
                  <tr>
                    <td colSpan={8}>No bank details submitted yet.</td>
                  </tr>
                )}
              </tbody>
            </table>
        ...
        <div className={s.modalOverlay} onClick={() => setAssignModal(null)}>
          <div className={s.modal} onClick={(e) => e.stopPropagation()}>
            <h3>
              {assignModal._isNew
                ? "Assign Compensation"
                : `Edit Compensation — ${assignModal.full_name || "Employee"}`}
            </h3>
                  ...
                  onChange={(e) => {
                    ...
                    if (tid) {
                      ...
                      if (tmpl) {
                        ...
                        tmpl.components.forEach((c: Component) => {
                          comps[c.key] = 0;
                        });
                        setAssignForm((f) => {
                          const updated = {
                          ...
                          if (Number(f.ctc_annual) > 0) {
                            const { base_salary, components } = calcFromCtc(
                              parseFloat(String(f.ctc_annual)),
                              ctcConfig,
                              comps,
                            );
