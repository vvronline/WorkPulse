...
import {
    Shield, Loader2, X, CheckCircle2, XCircle, Copy, AlertTriangle,
    Clock, Eye, Pencil, History, BadgeAlert,
} from 'lucide-react';
import {
    listIncomingAccessRequests, approveAccessRequest,
    denyAccessRequest, revokeAccessSession,
} from '../../api';
...
export default function PlatformAccessInbox() {
    ...
    const [revealedCode, setRevealedCode] = useState(null);   // { id, code, expires_at }
    const [denyTarget, setDenyTarget] = useState(null);       // { id }
    const [denyReason, setDenyReason] = useState('');
    const [revokeTarget, setRevokeTarget] = useState(null);   // request row
    ...
    const load = useCallback(async () => {
        try {
            ...
            setActive(r.data?.active_session || null);
        } catch (e) {
    ...
    // Cheap polling so new requests appear without manual refresh; the page
    // is typically idle so a 15s interval is plenty.
    useEffect(() => {
    ...
    const handleApprove = async (req) => {
        try {
            ...
            setRevealedCode({
                id: req.id,
                code: r.data?.approval_code,
                expires_at: r.data?.code_expires_at,
    ...
    return (
        <div className="platform-access-inbox" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            <header>
            ...
            {error && (
                <div role="alert" style={alertStyle('danger')}>
                    <span>{error}</span>
                    <button onClick={() => setError('')} aria-label="Close" style={closeBtn}><X size={14} /></button>
                </div>
            )}
            ...
            {active && (
                <section style={{ ...cardStyle, borderColor: 'var(--warning)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                        ...
                        <strong style={{ flex: 1 }}>Inspector currently active in your workspace</strong>
                        <button
                            style={{ ...btn, background: 'var(--danger)', color: '#fff', borderColor: 'var(--danger)' }}
                    ...
                    <div style={metaGrid}>
                        <div><strong>Inspector:</strong> {active.requested_by_name || `User #${active.requested_by}`}</div>
                        <div><strong>Reason:</strong> {active.reason}</div>
                        <div><strong>Scope:</strong> {active.scope}</div>
                        <div><strong>Started:</strong> {active.consumed_at ? new Date(active.consumed_at).toLocaleTimeString() : '—'}</div>
                        <div><strong>Ends:</strong> {active.session_ends_at ? new Date(active.session_ends_at).toLocaleTimeString() : '—'}</div>
                    </div>
                </section>
            )}
            ...
            {revealedCode && (
                <section style={{ ...cardStyle, borderColor: 'var(--success)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                        ...
                        <button onClick={() => setRevealedCode(null)} style={closeBtn} aria-label="Dismiss"><X size={14} /></button>
                    </div>
                    <p style={{ color: 'var(--text-secondary)', fontSize: 12, margin: '0 0 10px' }}>
                        Share this code with <strong>{revealedCode.inspector || 'the inspector'}</strong> over a trusted
                        channel (phone / support ticket). It expires at{' '}
                        <strong>{revealedCode.expires_at ? new Date(revealedCode.expires_at).toLocaleTimeString() : '—'}</strong>
            ...
            <section style={cardStyle}>
                <h3 style={sectionH3}>
                    <Clock size={14} /> Pending requests{pending.length > 0 && (
                        <span style={badgeWarn}>{pending.length}</span>
                    )}
                </h3>
                {loading ? (
                    <div style={emptyMsg}><Loader2 size={14} className="spin" /> Loading…</div>
                ) : pending.length === 0 ? (
                            ...
                            <li key={req.id} style={listItem}>
                                <div style={{ flex: 1 }}>
                                    ...
                                    <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 4, display: 'flex', gap: 12 }}>
                                        <span style={pillStyle(req.scope === 'read' ? 'info' : 'warn')}>
                                            {req.scope === 'read' ? <Eye size={11} /> : <Pencil size={11} />}
                                            {req.scope}
                                        </span>
                                ...
                                <div style={{ display: 'flex', gap: 6 }}>
                                    <button style={{ ...btn, background: 'var(--success)', color: '#fff', borderColor: 'var(--success)' }} onClick={() => handleApprove(req)}>
                                        <CheckCircle2 size={13} /> Approve
                                    </button>
                                    <button style={{ ...btn, color: 'var(--danger)', borderColor: 'var(--danger)' }} onClick={() => setDenyTarget(req)}>
                    ...
                    <table style={{ width: '100%', fontSize: 12, borderCollapse: 'collapse' }}>
                        ...
                        <tbody>
                            {history.slice(0, 20).map(req => (
                                <tr key={req.id} style={{ borderTop: '1px solid var(--border, #2a2d33)' }}>
                                    <td style={td}>{new Date(req.requested_at).toLocaleString()}</td>
                                    <td style={td}>{req.requested_by_name || `#${req.requested_by}`}</td>
                                    <td style={{ ...td, color: 'var(--text-secondary)' }}>{req.reason}</td>
                                    <td style={td}><StatusBadge status={req.status} /></td>
                                    <td style={td}>
                                        {req.status === 'consumed' && !req.revoked_at && (
                                            <button style={miniBtn} onClick={() => setRevokeTarget(req)}>Revoke</button>
                                        )}
                                    </td>
                                </tr>
                            ))}
...
const miniBtn = { ...btn, padding: '3px 8px', fontSize: 11 };
const closeBtn = { background: 'none', border: 0, cursor: 'pointer', color: 'inherit' };
...
const emptyMsg = { fontSize: 12, color: 'var(--text-muted)', padding: '8px 0' };
const codeBox = {
    flex: 1,
    fontFamily: 'SF Mono, Cascadia Mono, monospace',
    fontSize: 28, letterSpacing: 8, padding: '12px 18px',
    textAlign: 'center', borderRadius: 8,
    background: 'var(--bg-input, #14161a)',
    border: '1px dashed var(--success)',
    color: 'var(--success)',
};
const th = { padding: '8px 6px', fontWeight: 500, fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.04em' };
...
function pillStyle(kind) {
    const palette = {
        info: { bg: 'color-mix(in srgb, var(--accent) 14%, transparent)', fg: 'var(--accent)' },
        warn: { bg: 'color-mix(in srgb, var(--warning) 14%, transparent)', fg: 'var(--warning)' },
    };
    const c = palette[kind] || palette.info;
    return {
        display: 'inline-flex', alignItems: 'center', gap: 3,
        padding: '1px 8px', borderRadius: 999,
        background: c.bg, color: c.fg, fontSize: 11,
    };
}

function StatusBadge({ status }) {
    const palette = {
        approved: 'var(--accent)', consumed: 'var(--accent)',
        denied: 'var(--danger)', revoked: 'var(--danger)',
        expired: 'var(--text-muted)', cancelled: 'var(--text-muted)',
        pending: 'var(--warning)',
    };
    return (
        <span style={{
