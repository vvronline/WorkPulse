...
export default function OrganizationsManagement({ onOrgChange, onManageOrg }: OrganizationsManagementProps) {
    ...
    const fetchOrgs = useCallback(() => {
        getAdminOrganizations().then(r => setOrgs((r.data as any).data || r.data)).catch(e => console.error(e));
    }, []);
    ...
    const handleCreate = async (data: any) => {
        try {
            ...
            fetchOrgs();
            onOrgChange?.();
        } catch (e: any) { setMsg(e.response?.data?.error || "Failed to create organization"); }
    };
    ...
    const handleUpdate = async (id: any, data: any) => {
        try {
            ...
            fetchOrgs();
            onOrgChange?.();
        } catch (e: any) { setMsg(e.response?.data?.error || "Failed to update organization"); }
    };
    ...
    const handleDelete = async (id: any) => {
        try {
            ...
            fetchOrgs();
            onOrgChange?.();
        } catch (e: any) { setMsg(e.response?.data?.error || "Failed to delete organization"); }
    };
    ...
    return (
        <>
            ...
            {editing && <OrgModal org={editing} onClose={() => setEditing(null)} onSave={(data: any) => handleUpdate(editing.id, data)} />}
            {deleting && (
                <TypedConfirm
