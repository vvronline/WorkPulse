...
interface AddPeopleWizardProps {
  userRole?: string;
  onCompleted?: (message: string) => void;
}

const EMPTY_ORGS: any[] = [];
const EMPTY_DEPTS: any[] = [];
const EMPTY_TEAMS: any[] = [];

export default function AddPeopleWizard({
  userRole,
  onCompleted,
}: AddPeopleWizardProps) {
  ...
  const { data: organizations = EMPTY_ORGS } = useQuery({
    queryKey: ["admin", "wizard", "organizations", userRole],
    queryFn: async () => {
      if (userRole === "platform_admin") {
        const r = await getAdminOrganizations();
        return ((r.data as any)?.data || r.data || []) as any[];
      }
      const r = await getCurrentOrg();
      return ((r.data as any)?.id ? [r.data] : []) as any[];
    },
  });

  // Auto-select the single org for non-platform admins (preserves prior behavior).
  useEffect(() => {
    if (
      ...
      setDefaults((d) => ({
        ...d,
        org_id: String((organizations[0] as any).id),
      }));
    }
  }, [userRole, organizations, defaults.org_id]);

  const refDataEnabled = !(userRole === "platform_admin" && !defaults.org_id);
  const { data: departments = EMPTY_DEPTS } = useQuery({
    queryKey: ["admin", "wizard", "departments", userRole, defaults.org_id],
    enabled: refDataEnabled,
    queryFn: async () => {
      const orgParam =
        userRole === "platform_admin" ? { org_id: defaults.org_id } : {};
      const r = await getOrgDepartments(orgParam as any);
      return ((r.data as any) || []) as any[];
    },
  });
  const { data: teams = EMPTY_TEAMS } = useQuery({
    queryKey: ["admin", "wizard", "teams", userRole, defaults.org_id],
    enabled: refDataEnabled,
    queryFn: async () => {
      const orgParam =
        userRole === "platform_admin" ? { org_id: defaults.org_id } : {};
      const r = await getOrgTeams(orgParam as any);
      return ((r.data as any) || []) as any[];
    },
  });
  ...
  const filteredTeams = useMemo(
    () =>
      defaults.department_id
        ? teams.filter(
            (t) => t.department_id === Number(defaults.department_id),
          )
        : teams,
      ...
      switch (method) {
        case "single": {
          ...
          if (defaults.department_id)
            payload.department_id = Number(defaults.department_id);
          ...
          setResult({
            ...
            initial_password: (r.data as any).initial_password,
          });
          onCompleted?.((r.data as any).message);
          break;
        }
        case "paste": {
                  ...
                  .map((c) => {
                    ...
                    if (
                      ...
                      const d = departments.find(
                        (d) => d.id === Number(defaults.department_id),
                      );
                      v = d?.name || "";
                    }
                    if (c === "team_name" && !v && defaults.team_id) {
                      const t = teams.find(
                        (t) => t.id === Number(defaults.team_id),
                      );
                      v = t?.name || "";
                    }
          ...
          fd.append(
            "file",
            new File([blob], "paste.csv", { type: "text/csv" }),
          );
          const r = await importUsers(fd, true);
          setResult({ kind: "bulk", ...(r.data as any) });
    ...
    <div className={w.steps}>
      {[1, 2, 3].map((n) => {
        ...
        return (
          <div
            key={n}
            className={`${w.step} ${step === n ? w.current : ""} ${step > n ? w.done : ""}`}
          >
            <div className={w.stepDot}>
      ...
      <div className={w.fieldGrid}>
        <div className={w.field}>
          ...
          <select
            value={defaults.role}
            onChange={(e) =>
              setDefaults((d) => ({ ...d, role: e.target.value }))
            }
          >
            {ROLES.map((r) => (
          ...
          <div className={w.fieldGrid}>
            <div className={w.field}>
              ...
              <input
                ...
                onChange={(e) =>
                  setSingleForm({ ...singleForm, full_name: e.target.value })
                }
            ...
            <div className={w.field}>
              <label>
                Initial password (min 8 chars, auto-generated if empty)
              </label>
              <input
                type="password"
      ...
      return (
        <>
          ...
          <p className={w.helpText}>
            ...
            may be a header (e.g. <code>full_name, email, username, role</code>
            ).
          </p>
          <textarea
      ...
      return (
        <>
          ...
          <p className={w.helpText}>
            ...
            <code> username, full_name, email</code>. Optional:{" "}
            <code>
              password, role, department_name, team_name, manager_username
            </code>
            .
          </p>
          <div
        ...
        <div className={w.confirmReady}>
          ...
          {method === "single" && (
            <p className={w.helpText}>
              Create{" "}
              <strong>
                {singleForm.full_name || singleForm.username || "this user"}
              </strong>{" "}
          ...
          {method === "paste" && (
            <p className={w.helpText}>
              Import <strong>{parsedRows.length}</strong> user
            ...
            <div className={w.credBox}>
              ...
              <div className={w.codeRow}>
                ...
                <button
                  ...
                  onClick={() => copy(result.initial_password, "pw")}
                >
      ...
      return (
        <div>
          <div className={w.bulkSummary}>
            {result.imported > 0 && (
              <span className={w.bulkOk}>
                <Check size={16} />
                {result.imported} imported
              </span>
            )}
            {result.failed?.length > 0 && (
              <span className={w.bulkFail}>
...
function downloadCSV(rows: any[]) {
  const header =
    "Username,Full Name,Email,Role,Initial Password,Auto-Generated";
