...
export default function AddPeopleWizard({ userRole, onCompleted }: AddPeopleWizardProps) {
    ...
    useEffect(() => {
        if (userRole === "platform_admin") {
            getAdminOrganizations().then(r => {
                setOrganizations((r.data as any)?.data || r.data || []);
            }).catch(() => {});
        } else {
            getCurrentOrg().then(r => {
                if ((r.data as any)?.id) {
                    setOrganizations([r.data]);
                    setDefaults(d => ({ ...d, org_id: String((r.data as any).id) }));
                }
            }).catch(() => {});
        }
    }, [userRole]);
    ...
    useEffect(() => {
        ...
        getOrgDepartments(orgParam as any).then(r => setDepartments((r.data as any) || [])).catch(() => setDepartments([]));
        getOrgTeams(orgParam as any).then(r => setTeams((r.data as any) || [])).catch(() => setTeams([]));
        ...
        try {
            switch (method) {
                case "single": {
                    ...
                    setResult({ kind: "single", message: (r.data as any).message, initial_password: (r.data as any).initial_password });
                ...
                case "paste": {
                    ...
                    if ((r.data as any).imported > 0) onCompleted?.(`Added ${(r.data as any).imported} user(s)`);
                    break;
                }
                case "file": {
    ...
    const renderStepIndicator = () => (
        <div className={w.steps}>
            {[1, 2, 3].map(n => {
                const labels: Record<number, string> = { 1: "Method", 2: "Details", 3: "Send" };
                return (
