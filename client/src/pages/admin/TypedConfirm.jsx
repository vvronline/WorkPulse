...
export default function TypedConfirm({
    title, message, hint, confirmLabel = 'Confirm', requireText, danger,
    onConfirm, onCancel,
}) {
    const [typed, setTyped] = useState('');
    const [busy, setBusy] = useState(false);

    useEffect(() => { setTyped(''); setBusy(false); }, [requireText, message]);
        ...
        <div className={s.confirmOverlay} onClick={onCancel}>
            ...
            >
                ...
                <div className={s.confirmActions}>
                    <button className={`${s.btn} ${s.secondary}`} onClick={onCancel} disabled={busy}>
                        Cancel
                    </button>
                    <button
