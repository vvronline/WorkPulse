...
export default function PayPeriods() {
  ...
  const error = isError ? "Failed to load pay periods." : "";
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setForm({
        label: "",
        start_date: getMonthRange(-1).start,
        end_date: getMonthRange(-1).end,
      });
      queryClient.invalidateQueries({ queryKey: ["admin", "pay-periods"] });
    } catch (err: any) {
  ...
  const handleExport = async (format: string) => {
    if (!exportFrom || !exportTo) {
      setExportError("Please set a date range");
      return;
    }
    setExportError("");
    setExporting(true);
    try {
      ...
      const blob = new Blob([res.data as any], { type: mime });
  ...
  return (
    <div className={s.section}>
      ...
      <p className={s.sectionDesc}>
        Lock a date range to prevent employees from modifying manual time
        entries. Export payroll-ready CSVs for accountants or QuickBooks
        imports.
      </p>
      ...
      <div className={s.card}>
        ...
        <p className={s.hint}>
          Exports per-employee daily hours (regular + overtime + break) for all
          team members visible to you. Compatible with most payroll/accounting
          systems.
        </p>
      </div>
      ...
      <div className={s.card}>
        ...
        <p className={s.hint}>
          Once locked, employees cannot add or edit manual time entries in this
          date range. Managers can still approve or reject existing pending
          entries.
        </p>
      </div>
      ...
      <ConfirmDialog
        ...
        message={
          unlockTarget
            ? `Unlock "${unlockTarget.label}"? This will allow employees to modify time entries in that range.`
            : ""
        }
        confirmText="Unlock"
        cancelText="Cancel"
