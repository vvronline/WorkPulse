...
import {
    ...
    ArrowRight, CheckCircle2, Circle, Network, Tag,
    DollarSign, ScrollText, Settings as SettingsIcon, AlarmClock
} from "lucide-react";
import {
...
export default function AdminHome({ user, onNavigate }: AdminHomeProps) {
    ...
    const [setup, setSetup] = useState<SetupState>({ tzSet: false, hasDept: false, hasTeam: false, hasPolicy: false, loaded: false });

    useEffect(() => {
        let alive = true;
        getAdminStats().then(r => { if (alive) setStats(r.data as AdminStats); }).catch(() => {});
        getRoleChangeRequests({ status: "pending" })
            .then(r => { if (alive) setPendingRoleRequests(((r.data as any[]) || []).length); })
            .catch(() => {});
        getApprovals({ status: "pending" })
            .then(r => { if (alive) setPendingApprovals((((r.data as any)?.data || r.data || []) as any[]).length); })
            .catch(() => {});
        return () => { alive = false; };
    }, []);
    ...
    useEffect(() => {
        ...
        Promise.allSettled([
            ...
            const depts = deptR.status === "fulfilled" ? ((deptR.value.data as any[]) || []) : [];
            const teams = teamR.status === "fulfilled" ? ((teamR.value.data as any[]) || []) : [];
            const pols = polR.status === "fulfilled" ? ((polR.value.data as any[]) || []) : [];
            setSetup({
