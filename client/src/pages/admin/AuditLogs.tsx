...
export default function AuditLogs() {
  ...
  const serverParams = useMemo(() => {
    ...
    if (filters.range === "custom") {
      if (filters.custom_from)
        p.from = new Date(filters.custom_from).toISOString();
  ...
  const { data } = useQuery({
    queryKey: ["admin", "audit-logs", serverParams],
    queryFn: async () => {
      const r = await getAuditLogs(serverParams);
      return {
        logs: ((r.data as any)?.logs || []) as AuditLog[],
        total: ((r.data as any)?.total || 0) as number,
      };
    },
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "0.85rem" }}>
          ...
          >
            ...
            <input
              ...
              style={{
                ...
                border: "none",
                outline: "none",
                color: filters.actor_id
                  ? "var(--accent)"
                  : "var(--text-primary)",
                font: "inherit",
                fontSize: "0.8rem",
                fontWeight: filters.actor_id ? 600 : 500,
                width: 130,
              }}
            />
            {filters.actor_id && (
      ...
      <table className={s.table}>
        ...
        <tbody>
          {logs.map((log) => (
            <tr key={log.id}>
              <td className={al["audit-time"]}>
                {new Date(log.created_at).toLocaleString()}
              </td>
              <td>{renderActor(log)}</td>
              <td>
                <span className={`${s.badge} ${al["badge-accent"]}`}>
                  {log.action}
                </span>
              </td>
              <td>
      ...
      {totalPages > 1 && (
        <div className={s.pagination}>
          ...
          <span className={su["text-muted-sm"]}>
...
function renderActor(log: AuditLog): React.ReactNode {
  const baseLabel =
    log.actor_name || log.actor_username || `User #${log.actor_id}`;
  if (!log.actor_is_inspector) return baseLabel;
  const realName =
    log.actor_inspector_real_name || log.actor_inspector_username;
  return (
