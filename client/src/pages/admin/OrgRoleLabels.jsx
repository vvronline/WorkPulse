...
import { Plus, Save, Trash2, AlertCircle, Lock, X, Edit3, ShieldCheck, Users } from 'lucide-react';
import { getOrgRoles, createOrgRole, updateOrgRole, deleteOrgRole } from '../../api';
...
const PERMISSION_LEVELS = [
    { value: 1, label: 'Standard member',   hint: 'Basic access only — equivalent to employee.' },
    { value: 2, label: 'Team lead',         hint: 'Can review and manage their own team.' },
    { value: 3, label: 'Manager',           hint: 'Approves leaves and tasks; manages a department.' },
    { value: 4, label: 'HR admin',          hint: 'Full people-ops: invite/remove/manage org members.' },
];

const PERMISSION_LEVEL_LABEL = Object.fromEntries(PERMISSION_LEVELS.map(p => [p.value, p.label]));

export default function OrgRoleLabels({ canEdit }) {
    const { setRoles: pushToCtx } = useRoleLabels();
    const toast = useToast();

    const [roles, setRoles] = useState([]);
    const [loading, setLoading] = useState(true);
    const [savingKey, setSavingKey] = useState(null);  // role_key currently being saved
    const [deletingKey, setDeletingKey] = useState(null);
    const [editing, setEditing] = useState({});         // { role_key: { label, description, color, permission_level } }
    const [adding, setAdding] = useState(null);         // { role_key, label, description, color, permission_level } | null
    ...
    const sorted = useMemo(
        () => [...roles].sort((a, b) => (a.sort_order ?? 0) - (b.sort_order ?? 0) || a.role_key.localeCompare(b.role_key)),
        [roles]
    );
    ...
    const updateField = (role_key, field, value) => {
        setEditing(prev => ({
            ...prev,
    ...
    const saveRole = async (role) => {
        ...
        if (!label || label.length > 40) return toast.error('Label must be 1..40 chars');
        if (draft.color && !HEX_RE.test(draft.color)) return toast.error('Color must be #RRGGBB');
        if (![1, 2, 3, 4].includes(Number(draft.permission_level))) return toast.error('Invalid permission level');
    ...
    const updateAddField = (field, value) => {
        setAdding(prev => ({ ...prev, [field]: value }));
    };
    ...
    const submitAdd = async () => {
        ...
        if (!KEY_RE.test(role_key)) {
            return toast.error('Key must be lowercase letters/numbers/underscores, start with a letter (1..40 chars)');
        }
        if (!label || label.length > 40) return toast.error('Label must be 1..40 chars');
        if (adding.color && !HEX_RE.test(adding.color)) return toast.error('Color must be #RRGGBB');
        if (![1, 2, 3, 4].includes(Number(adding.permission_level))) return toast.error('Invalid permission level');
    ...
    return (
        <div className={s.root}>
            ...
            <div className={s.list}>
                {sorted.map(role => {
                    ...
                    return (
                        <div
                            key={role.role_key}
                            className={s.row}
                            style={{ '--role-color': (isEditing ? draft.color : role.color) || '#6366f1' }}
                        >
                            <div className={s.rowHeader}>
                                <span className={s.swatch} style={{ background: (isEditing ? draft.color : role.color) || '#6366f1' }} />
                                <div className={s.rowMeta}>
                                    ...
                                    {!role.is_system && (
                                        <span className={s.customBadge}>
                                            custom
                                        </span>
                                    )}
                                    {role.user_count > 0 && (
                                        <span className={s.usersBadge} title={`${role.user_count} active user${role.user_count === 1 ? '' : 's'} hold${role.user_count === 1 ? 's' : ''} this role`}>
                                            <Users size={11} /> {role.user_count}
                                        </span>
                                    )}
                                    <span className={s.levelBadge}>
                                        L{role.permission_level} · {PERMISSION_LEVEL_LABEL[role.permission_level] || `level ${role.permission_level}`}
                                    </span>
                                </div>
                            ...
                            {!isEditing && (
                                <div className={s.previewRow}>
                                    ...
                                    {role.description && <span className={s.previewDesc}>{role.description}</span>}
                                </div>
                            )}
                                    ...
                                    <div className={s.fields}>
                                        <label className={s.field}>
                                            ...
                                            <input
                                                ...
                                                className={s.input}
                                                value={draft.label}
                                                onChange={(e) => updateField(role.role_key, 'label', e.target.value)}
                                                maxLength={40}
                                                disabled={isSaving}
                                            />
                                        </label>
                                        ...
                                        <label className={s.field}>
                                            ...
                                            <input
                                                ...
                                                onChange={(e) => updateField(role.role_key, 'description', e.target.value)}
                                                maxLength={200}
                                                disabled={isSaving}
                                            />
                                        </label>
                                        ...
                                        <label className={`${s.field} ${s.colorField}`}>
                                            ...
                                            <div className={s.colorGroup}>
                                                <input
                                                    ...
                                                    value={HEX_RE.test(draft.color || '') ? draft.color : '#6366f1'}
                                                    onChange={(e) => updateField(role.role_key, 'color', e.target.value)}
                                                    disabled={isSaving}
                                                />
                                                <input
                                                    type="text"
                                        ...
                                        <label className={s.field}>
                                            ...
                                            <select
                                                ...
                                                onChange={(e) => updateField(role.role_key, 'permission_level', Number(e.target.value))}
                                                disabled={isSaving}
                                            >
                                                {PERMISSION_LEVELS.map(p => (
                                                    <option key={p.value} value={p.value}>L{p.value} — {p.label}</option>
                                                ))}
                                            </select>
                                        </label>
                                    </div>
                ...
                {canEdit && adding && (
                    <div className={`${s.row} ${s.addRow}`} style={{ '--role-color': adding.color || '#6366f1' }}>
                        <div className={s.rowHeader}>
                            <span className={s.swatch} style={{ background: adding.color || '#6366f1' }} />
                            <div className={s.rowMeta}>
                                <strong>New role</strong>
                            </div>
                            <button type="button" className={s.rowReset} onClick={cancelAdd} disabled={creating}>
                                <X size={12} /> Cancel
                            </button>
                        </div>
                        ...
                        <div className={s.fields}>
                            <label className={s.field}>
                                ...
                                <input
                                    ...
                                    onChange={(e) => updateAddField('role_key', e.target.value.toLowerCase())}
                            ...
                            <label className={`${s.field} ${s.colorField}`}>
                                ...
                                <div className={s.colorGroup}>
                                    <input
                                        type="color"
                                        className={s.colorPicker}
                                        value={HEX_RE.test(adding.color || '') ? adding.color : '#6366f1'}
                            ...
                            <label className={s.field}>
                                ...
                                <select
                                    ...
                                    onChange={(e) => updateAddField('permission_level', Number(e.target.value))}
                                    disabled={creating}
                                >
                                    {PERMISSION_LEVELS.map(p => (
                                        <option key={p.value} value={p.value}>L{p.value} — {p.label}</option>
                        ...
                        <div className={s.actions}>
                            <button type="button" className={s.btnSecondary} onClick={cancelAdd} disabled={creating}>
                                Cancel
                            </button>
                            <button type="button" className={s.btnPrimary} onClick={submitAdd} disabled={creating}>
                ...
                <div className={`${s.row} ${s.rowReadonly}`} style={{ '--role-color': '#ef4444' }}>
                    <div className={s.rowHeader}>
                        <span className={s.swatch} style={{ background: '#ef4444' }} />
                        <div className={s.rowMeta}>
                            <code className={s.roleKey}>super_admin</code>
                            <span className={s.lockedBadge}><Lock size={11} /> system</span>
                            <span className={s.levelBadge}>L5 · Org admin</span>
                        </div>
                    </div>
                    <p className={s.readonlyNote}>
                        Org-wide admin with access to all settings and billing. Always present in every organisation.
                    </p>
                </div>
                <div className={`${s.row} ${s.rowReadonly}`} style={{ '--role-color': '#0f172a' }}>
                    <div className={s.rowHeader}>
                    ...
                    <p className={s.readonlyNote}>
                        Cross-organisation system operator. Not assignable from inside an organisation.
                    </p>
                </div>
            </div>
