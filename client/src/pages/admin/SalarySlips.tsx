...
export default function SalarySlips() {
    ...
    const loadSlips = useCallback(async (periodId: string) => {
        if (!periodId) { setSlips([]); return; }
        try {
            const res = await getSalarySlips({ pay_period_id: periodId } as any);
            setSlips((res.data as any) || []);
            const disbRes = await getDisbursements({ pay_period_id: periodId } as any);
    ...
    const handleDisburse = async () => {
        ...
        try {
            ...
            setMessage(`${data.message} (${data.disbursed} sent, ${data.failed} failed)`);
            loadSlips(selectedPeriod);
        } catch (err: any) {
