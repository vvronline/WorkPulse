...
export default function OrgSettings() {
  ...
  const { data, isLoading: loading } = useQuery({
    ...
    queryFn: async () => {
      const [settingsRes, codesRes] = await Promise.all([
        getRegistrationSettings().catch(() => null),
        getInviteCodes().catch(() => null),
      ]);
      const mode = (settingsRes?.data as any)?.mode || "open";
      const list = Array.isArray(codesRes?.data)
        ? (codesRes!.data as InviteCode[])
        : [];
      return { mode, codes: list };
    },
  });
  ...
  const handleCreateCode = async () => {
    ...
    setCreating(true);
    try {
      const payload: Record<string, unknown> = { role: codeForm.role };
      if (codeForm.max_uses) payload.max_uses = Number(codeForm.max_uses);
      if (codeForm.expires_days)
        payload.expires_days = Number(codeForm.expires_days);
      const res = await createInviteCode(payload);
      setSuccess(`Invite code created: ${(res.data as any).code}`);
      setShowCreate(false);
      ...
      try {
        await navigator.clipboard.writeText((res.data as any).code);
        setCopiedId("new");
  ...
  const copyCode = async (code: string, id: string | number) => {
    try {
  ...
  if (loading)
    return (
      <div style={{ padding: 24, textAlign: "center" }}>
        <Loader2 size={20} className={s.spinner} /> Loading…
      </div>
    );
  ...
  const modes = [
    { value: "open", label: "Open", desc: "Anyone can register and join" },
    {
      value: "invite_only",
      label: "Invite Only",
      desc: "Users need an invite code to register",
    },
          ...
          >
            <label style={{ fontSize: 13 }}>
              ...
              <select
                value={codeForm.role}
                onChange={(e) =>
                  setCodeForm((f) => ({ ...f, role: e.target.value }))
                }
                style={{
                  padding: "6px 10px",
                  borderRadius: 6,
            ...
            <label style={{ fontSize: 13 }}>
              ...
              <input
                ...
                onChange={(e) =>
                  setCodeForm((f) => ({ ...f, expires_days: e.target.value }))
                }
                placeholder="Never"
                style={{
                  padding: "6px 10px",
            ...
            <tbody>
              {[...activeCodes, ...inactiveCodes].map((c) => (
                <tr key={c.id} style={{ opacity: c.is_active ? 1 : 0.5 }}>
                  ...
                  <td>
                    <span className={s.badgeRole} data-role={c.role}>
                      {c.role}
                    </span>
                  </td>
                      ...
                      {c.is_active && (
                        <>
                          ...
                          <button
                            ...
                            onClick={() =>
                              copyCode(
                                `${window.location.origin}/register?invite=${c.code}`,
                                `link-${c.id}`,
                              )
                            }
                          >
