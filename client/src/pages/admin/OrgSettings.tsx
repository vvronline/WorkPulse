...
export default function OrgSettings() {
    ...
    const [codeForm, setCodeForm] = useState<CodeForm>({ role: "employee", max_uses: "", expires_days: "" });
    ...
    const loadCodes = useCallback(async () => {
        try {
            ...
            setCodes(Array.isArray(res.data) ? (res.data as InviteCode[]) : []);
        } catch { /* ignore */ }
    }, []);

    useEffect(() => {
        Promise.all([
            getRegistrationSettings().then(r => setRegMode((r.data as any)?.mode || "open")).catch(() => {}),
            loadCodes(),
    ...
    const handleCreateCode = async () => {
        ...
        try {
            ...
            try { await navigator.clipboard.writeText((res.data as any).code); setCopiedId("new"); setTimeout(() => setCopiedId(null), 2000); } catch {}
        } catch (e: any) {
