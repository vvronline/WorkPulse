...
export default function OrgRoleLabels({ canEdit }: OrgRoleLabelsProps) {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      return Array.isArray((res.data as any)?.roles)
        ? ((res.data as any).roles as any[])
        : [];
    },
  });
  ...
  useEffect(() => {
    if (isSuccess) pushToCtx(roles);
  }, [isSuccess, roles, pushToCtx]);
  ...
  const saveRole = async (role: any) => {
    ...
    try {
      await updateOrgRole(role.role_key, {
        label,
        description: draft.description?.trim() || null,
        color: draft.color,
        permission_level: Number(draft.permission_level),
      });
      queryClient.invalidateQueries({ queryKey: ["admin", "org-roles"] });
  ...
  const deleteRole = async (role: any) => {
    ...
    try {
      await deleteOrgRole(role.role_key);
      queryClient.invalidateQueries({ queryKey: ["admin", "org-roles"] });
  ...
  return (
    <div className={s.root}>
      <div className={s.intro}>
        ...
        <div>
          Each role is pinned to a <em>permission level</em>{" "}
          (Standard&nbsp;member, Team&nbsp;lead, Manager, or HR&nbsp;admin)
          which controls what they can see and do. You can rename roles, add new
          ones, or delete unused ones — the underlying access control rules are
          determined by the permission level you assign.
        </div>
      </div>
      ...
      <div className={s.list}>
                    ...
                    <label className={`${s.field} ${s.colorField}`}>
                      ...
                      <div className={s.colorGroup}>
                        <input
                          ...
                          onChange={(e) =>
                            updateField(role.role_key, "color", e.target.value)
                          }
                          disabled={isSaving}
                        />
                        <input
        ...
        {canEdit && adding && (
          <div
            ...
            style={
              {
                "--role-color": adding.color || "#6366f1",
              } as React.CSSProperties
            }
          >
            <div className={s.rowHeader}>
            ...
            <div className={s.fields}>
              ...
              <label className={`${s.field} ${s.colorField}`}>
                ...
                <div className={s.colorGroup}>
                  <input
                    ...
                    className={s.colorPicker}
                    value={
                      HEX_RE.test(adding.color || "") ? adding.color : "#6366f1"
                    }
              ...
              <label className={s.field}>
                ...
                <select
                  ...
                  onChange={(e) =>
                    updateAddField("permission_level", Number(e.target.value))
                  }
                  disabled={creating}
                >
                  {PERMISSION_LEVELS.map((p) => (
