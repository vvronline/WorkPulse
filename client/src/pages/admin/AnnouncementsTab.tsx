...
export default function AnnouncementsTab({ userRole }: AnnouncementsTabProps) {
  const queryClient = useQueryClient();
  const { data: announcements = EMPTY_ANNOUNCEMENTS, isLoading: loading } =
    useQuery({
      queryKey: ["admin", "announcements"],
      queryFn: async () =>
        ((await getAdminAnnouncements()).data as any).data as Announcement[],
    });
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      await updateAnnouncement(editId as any, payload);
      setEditId(null);
  ...
  return (
    <div className={s.section}>
      ...
      <p className={su["section-desc-muted"]}>
        Create announcements and quotes to display on employees' dashboards. Use
        the "Quote" type for motivational quotes (shown with quote marks and
        author attribution from the message).
      </p>
      ...
      {announcements.length === 0 ? (
        ...
        >
          No announcements yet. Create one above to display messages and quotes
          on employee dashboards.
        </p>
      ) : (
