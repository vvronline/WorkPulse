...
export default function MemberOverview({ data }: MemberOverviewProps) {
                    ...
                    <div className={m.weeklyTrendGrid}>
                        {data.weeklyTrend.map((day: any, i: number) => {
                            const maxMin = Math.max(
                                ...data.weeklyTrend.map((d: any) => d.floorMinutes || 0),
                                480,
                                1,
                            );
                            ...
                            return (
                                <div key={i} className={m.weeklyDayCol}>
                                    ...
                                    <div className={m.weeklyDayHours}>
                                        {formatMin(day.floorMinutes)}
                                    </div>
                                    {day.workMode && (
                                        <div className={m.weeklyDayMode}>
                                            {day.workMode === "remote" ? (
                                                <House size={12} />
                                            ) : (
                                                <Building2 size={12} />
                ...
                <div className={m.overviewSection}>
                    ...
                    <div className={m.perfGrid}>
                        ...
                        <div className={m.perfItem}>
                            ...
                            <span className={m.perfLabel}>Total Work Time</span>
                        </div>
                        <div className={m.perfItem}>
                            <span className={m.perfValue}>{formatMin(stats.avgFloorMinutes)}</span>
                            <span className={m.perfLabel}>Avg Work/Day</span>
                        </div>
                        <div className={m.perfItem}>
                ...
                {taskStats.total !== undefined && (
                    <div className={m.overviewSection}>
                        ...
                        <div className={m.taskStatsGrid}>
                            <div className={m.taskStatCard}>
                                <span className={m.taskStatNum}>{taskStats.total}</span>
                                <span className={m.taskStatLbl}>Total</span>
                            </div>
                            <div className={m.taskStatCard}>
                                <span className={`${m.taskStatNum} ${m.colorGreen}`}>
                                    {taskStats.done}
                                </span>
                                <span className={m.taskStatLbl}>Done</span>
                            </div>
                            <div className={m.taskStatCard}>
                                <span className={`${m.taskStatNum} ${m.colorAmber}`}>
                                    {taskStats.inProgress}
                                </span>
                                <span className={m.taskStatLbl}>In Progress</span>
                            </div>
                            <div className={m.taskStatCard}>
                                <span className={m.taskStatNum}>{taskStats.completionRate}%</span>
                                <span className={m.taskStatLbl}>Completion</span>
                            </div>
                        </div>
                    </div>
                )}
                ...
                {data.leaveBalances && data.leaveBalances.length > 0 && (
                    <div className={m.overviewSection}>
                        ...
                        <div className={m.leaveBalanceList}>
                            {data.leaveBalances.map((lb: any, i: number) => {
                                ...
                                return (
                                    <div key={i} className={m.leaveBalanceRow}>
                                        <div className={m.lbInfo}>
                                            ...
                                            >
                                                <LeaveIconFor type={lb.leave_type} />{" "}
                                                {lb.policy_name || lb.leave_type}
                                            </span>
                                            <span className={m.lbCount}>
                                                {used}/{total} used
                                            </span>
                                        </div>
                                        <div className={m.lbBarTrack}>
                                            <div
                                                className={m.lbBarFill}
                                                style={{
