...
interface TeamAnalyticsProps {
    onSelectMember: (member: Record<string, any>) => void;
}

export default function TeamAnalytics({ onSelectMember }: TeamAnalyticsProps) {
    ...
    useEffect(() => {
        if (range === "custom") {
            if (customFrom && customTo && customFrom <= customTo) {
                getTeamAnalytics(null as any, customFrom, customTo)
                    .then((r) => setData(r.data as Record<string, any>))
                    .catch((e) => console.error(e));
            }
            return;
        }
        getTeamAnalytics(range as any)
            .then((r) => setData(r.data as Record<string, any>))
            .catch((e) => console.error(e));
    ...
    const departments = useMemo(() => {
        ...
        const depts = new Set<string>(
            data.members.map((mem: any) => mem.department_name).filter(Boolean),
        );
        return [...depts].sort();
    }, [data]);
    ...
    const filteredMembers = useMemo(() => {
        if (!data?.members) return [];
        let list = data.members as Record<string, any>[];
        if (search) {
        ...
        <>
            ...
            <div className={m.summaryGrid}>
                ...
                <div className={m.summaryCard}>
                    ...
                    <div className={`${m.summaryValue} ${m.colorAmber}`}>
            ...
            <h3 className={m.sectionTitle}>
                Member Performance — {rangeLabel}{" "}
                <span className={m.memberCount}>({filteredMembers.length})</span>
            </h3>
            <div className={m.tableWrap}>
                <table className={m.analyticsTable}>
                    <thead>
                        <tr>
                            <th onClick={() => handleSort("full_name")} className={m.sortable}>
                                Member
                                <SortIcon col="full_name" />
                            </th>
                            <th>Today</th>
                            <th onClick={() => handleSort("hours")} className={m.sortable}>
                                Total Hours
                                <SortIcon col="hours" />
                            </th>
                            <th
                                onClick={() => handleSort("avgFloorMinutes")}
                                className={m.sortable}
                            >
                                Avg/Day
                                <SortIcon col="avgFloorMinutes" />
                            </th>
                            <th onClick={() => handleSort("tasksDone")} className={m.sortable}>
                                Planner
                                <SortIcon col="tasksDone" />
                            </th>
                            <th
                                onClick={() => handleSort("targetMetPercent")}
                                className={m.sortable}
                            >
                                Target Met
                                <SortIcon col="targetMetPercent" />
                            </th>
                            <th
                                onClick={() => handleSort("punctualityPercent")}
                                className={m.sortable}
                            >
                                Punctuality
                                <SortIcon col="punctualityPercent" />
                            </th>
                            <th>Trend</th>
                            <th></th>
                        </tr>
                    ...
                    <tbody>
                        {filteredMembers.map((mem) => (
                            <React.Fragment key={mem.id}>
                                ...
                                >
                                    <td>
                                        <div className={m.memberInfo}>
                                            {mem.avatar ? (
                                                <img
                                                    src={mem.avatar}
                                                    className={m.memberAvatarSm}
                                                    alt=""
                                                />
                                            ) : (
                                                <span className={m.avatarPlaceholder}>
                                                    {mem.full_name?.charAt(0)}
                                                </span>
                                            )}
                                            <div>
                                                <div className={m.memberNameCol}>
                                                    {mem.full_name}
                                                </div>
                                    ...
                                    <td>
                                        <TodayStatusBadge
                                            status={mem.todayStatus}
                                            minutes={mem.todayHoursMin}
                                        />
                                    </td>
                                    ...
                                    <td className={m.numCell}>
                                        <span className={m.tasksPill}>
                                            {mem.tasksDone}
                                            <span className={m.tasksSep}>/{mem.tasksTotal}</span>
                                        </span>
                                    </td>
                                    <td>
                                        <PercentBar value={mem.targetMetPercent} />
                                    </td>
                                    <td>
                                        <PercentBar value={mem.punctualityPercent} color="blue" />
                                    </td>
                                    <td>
                                        <MiniTrend data={mem.trend} target={data.targetMinutes} />
                                    </td>
                                    <td>
                                        <button
                                            className={m.viewBtn}
                                    ...
                                    <tr className={m.expandedRow}>
                                        <td colSpan={9}>
                                            <MemberExpandedCard
                                                ...
                                                expectedWeekdays={data.expectedWeekdays}
                                            />
                                        </td>
                                    </tr>
                                )}
                        ...
                        {filteredMembers.length === 0 && (
                            <tr>
                                <td colSpan={9} className={m.emptyState}>
