...
export default function TeamAnalytics({ onSelectMember }: TeamAnalyticsProps) {
  ...
  const { data = null } = useQuery({
    ...
    queryFn: async () =>
      (range === "custom"
        ...
        : (await getTeamAnalytics(range as any)).data) as Record<string, any>,
  ...
  const filteredMembers = useMemo(() => {
    ...
    if (filterDept)
      list = list.filter((mem) => mem.department_name === filterDept);
    list = [...list].sort((a, b) => {
      const av = a[sortBy],
        bv = b[sortBy];
      if (typeof av === "string")
