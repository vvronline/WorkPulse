...
export default function EmployeeDashboard({
  ...
  const { data: overview = null, isLoading: loading } = useQuery({
    queryKey: ["manager", "memberOverview", member.id],
    queryFn: async () =>
      (await getMemberOverview(member.id)).data as Record<string, any>,
  });
  ...
  return (
    <div className={s.adminPage}>
      <div className={m.employeeHeader}>
        <button
          onClick={onBack}
          className={`${sf.btnCancel} ${m["btn-back-sm"]}`}
        >
          ← Back
        </button>
        <div className={m.employeeProfile}>
          {user.avatar ? (
            ...
            <span className={`${s.initials} ${m["initials-lg"]}`}>
          ...
          <div>
            ...
            <div className={m.employeeMeta}>
              <span className={m.memberRole}>
                {ROLE_LABELS[user.role] || user.role}
              </span>
              {user.email && <span className={m.metaDot}>· {user.email}</span>}
              {user.department_name && (
                <span className={m.metaDot}>· {user.department_name}</span>
              )}
              {user.team_name && (
                <span className={m.metaDot}>· {user.team_name}</span>
              )}
            </div>
          </div>
        </div>
      ...
      <div className={s.tabs}>
        ...
        <button
          className={`${s.tab} ${tab === "leaves" ? s.active : ""}`}
        ...
        <button
          className={`${s.tab} ${tab === "hours" ? s.active : ""}`}
