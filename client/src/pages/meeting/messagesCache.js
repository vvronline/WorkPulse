...
/** @type {Map<string, Array<object>>} keyed by meeting code (or id) */
const cache = new Map();
 ...
 */
export function getCachedMessages(code) {
    if (!code) return [];
 ...
 */
export function setCachedMessages(code, messages) {
    if (!code) return;
 ...
 */
export function upsertCachedMessage(code, incoming) {
 ...
 */
export function applyCachedMessages(code, updater) {
    if (!code) return;
    const key = String(code);
    const prev = cache.get(key) || [];
 ...
 */
export function clearMessagesCache(code) {
    if (!code) return;
 ...
 */
export function __resetMessagesCacheForTests() {
    cache.clear();
}