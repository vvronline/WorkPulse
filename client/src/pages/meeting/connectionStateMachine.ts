...
export type MeetingState = (typeof STATES)[keyof typeof STATES];
...
const TERMINAL = new Set<MeetingState>([STATES.LEFT, STATES.ENDED, STATES.FAILED]);
 ...
 */
export function nextState(current: MeetingState, event: MeetingEvent | string): MeetingState {
 ...
 *   info | warn | error
 */
export function describeState(state: MeetingState): StateDescription {
    switch (state) {
