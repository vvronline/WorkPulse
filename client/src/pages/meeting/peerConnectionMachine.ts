/**
 * Phase 5.1 — Per-peer connection reducer for the WebRTC mesh.
 *
 * Mesh equivalent of the 1:1 call state machine
 * (`mobile/src/realtime/callStateMachine.ts`). In a mesh, EACH remote peer has
 * its own long-lived `RTCPeerConnection` whose lifecycle is driven from several
 * racing sources: the `onconnectionstatechange` callback (connected /
 * disconnected / failed), the relay-first fast-retry rebuild (Phase 3.1), the
 * 30s connect-timeout (Phase 3.2), ICE restarts (Phase 3.3), the manual
 * `retryPeer`, and the teardown paths (`meeting_participant_left`, `closePeer`,
 * `leaveMeeting`). Because those fire from long-lived listeners / timers that
 * can race teardown, a LATE `connected` (a peer connection that reached
 * "connected" a beat after the peer already left / the call ended) could
 * re-`upsert` a participant tile that was just removed — reviving a torn-down
 * peer. This is the mesh analogue of the 1:1 P3.14 effect race.
 *
...
export type PeerEvent =
    // A fresh PC is being negotiated (createPeerConnection / initial offer/answer).
    | { type: "CONNECTING" }
    ...
    // meeting down (`closePeer` / `leaveMeeting`). Absorbing.
    | { type: "CLOSED" };
