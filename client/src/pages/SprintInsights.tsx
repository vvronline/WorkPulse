...
import { BurndownChart, VelocityChart } from "../components/agile/AgileWorkflowPanels";
...
export default function SprintInsights() {
    ...
    useEffect(() => {
        getSprints().then(r => {
            const list = (r.data as any)?.sprints || [];
            setSprints(list);
            ...
            const requestedExists = requested && list.some((sp: any) => sp.id === requested);
            if (requestedExists) {
    ...
    const reload = () => {
        ...
        Promise.all([
            ...
            getSprintRetrospective(selectedId).then(r => (r.data as any)?.retrospective).catch(() => null),
            getSprintTasks(selectedId).then(r => (r.data as any)?.tasks || []).catch(() => []),
        ]).then(([st, cf, cy, re, tk]) => {
...
function CumulativeFlowChart({ data }: { data: any }) {
    ...
    const maxY = Math.max(1, ...data.series.map((d: any) => order.reduce((a, k) => a + (d[k] || 0), 0)));
    ...
    const areas = order.map((cat) => {
        ...
        const bottoms = data.series.map((d: any) => order.slice(0, idx).reduce((a, k) => a + (d[k] || 0), 0));
        const tops = data.series.map((d: any, i: number) => bottoms[i] + (d[cat] || 0));
        const yScale = (v: number) => padT + innerH - (innerH * v / maxY);
        const xAt = (i: number) => padL + i * xStep;
        const top = tops.map((v: number, i: number) => `${xAt(i).toFixed(1)},${yScale(v).toFixed(1)}`).join(" L ");
        const bottom = bottoms.map((v: number, i: number) => `${xAt(i).toFixed(1)},${yScale(v).toFixed(1)}`).reverse().join(" L ");
...
const STATUS_LABELS: Record<string, string> = { pending: "To Do", in_progress: "In Progress", in_review: "In Review", done: "Done" };
...
function RetrospectivePanel({ sprintId, initial, onSaved }: RetrospectivePanelProps) {
...
function RetroColumn({ label, value, onChange, placeholder, accent }: RetroColumnProps) {
    return (
        <div className={s.retroCol} style={{ "--accent": accent } as React.CSSProperties}>
