...
export default function SprintInsights() {
  ...
  const { data: sprints = EMPTY_SPRINTS } = useQuery({
    ...
    queryFn: async (): Promise<any[]> =>
      ((await getSprints()).data as any)?.sprints || [],
  });

  // Resolve the initially-selected sprint once the list arrives. Honour a
  // deep-link ?sprint_id=… when present and valid; otherwise fall back to the
  // active sprint or the first one. Only auto-select while nothing is chosen
  // yet, so a background refetch never clobbers a manual selection.
  useEffect(() => {
    if (selectedId || !sprints.length) return;
    const requested = Number(searchParams.get("sprint_id"));
    const requestedExists =
      requested && sprints.some((sp: any) => sp.id === requested);
    if (requestedExists) {
      setSelectedId(requested);
    } else {
      const active = sprints.find((sp: any) => sp.status === "active");
      setSelectedId(active?.id || sprints[0]?.id || null);
    }
  ...
  }, [sprints, searchParams, selectedId, setSearchParams]);

  const { data: insights, isLoading: loading } = useQuery({
    queryKey: ["sprints", "insights", selectedId],
    enabled: !!selectedId,
    queryFn: async () => {
      const [stats, cfd, cycle, retro, sprintTasks] = await Promise.all([
        getSprintStats(selectedId!)
          .then((r) => r.data)
          .catch(() => null),
        getSprintCumulativeFlow(selectedId!)
          .then((r) => r.data)
          .catch(() => null),
        getSprintCycleTime(selectedId!)
          .then((r) => r.data)
          .catch(() => null),
        getSprintRetrospective(selectedId!)
          .then((r) => (r.data as any)?.retrospective)
          .catch(() => null),
        getSprintTasks(selectedId!)
          .then((r) => (r.data as any)?.tasks || [])
          .catch(() => []),
      ]);
      return { stats, cfd, cycle, retro, sprintTasks };
    },
  });

  const stats = insights?.stats ?? null;
  const cfd = insights?.cfd ?? null;
  const cycle = insights?.cycle ?? null;
  const retro = insights?.retro ?? null;
  const sprintTasks = insights?.sprintTasks ?? EMPTY_TASKS;

  const selectedSprint = sprints.find((sp: any) => sp.id === selectedId);

  return (
    <div className={s.page}>
      <header className={s.header}>
        <div>
          <Link to="/tasks" className={s.backLink}>
            <ArrowLeft size={14} /> Back to Tasks
          </Link>
          <h1 className={s.title}>Sprint Insights</h1>
          <p className={s.subtitle}>
            Burndown, velocity, cumulative flow, cycle time and retrospectives.
          </p>
        </div>
        <div className={s.headerActions}>
          ...
          >
            {["active", "planned", "completed"].map((status) => {
              const group = sprints.filter((sp: any) => sp.status === status);
      ...
      {!selectedId && (
        <div className={s.empty}>Select a sprint to see insights.</div>
      )}
        ...
        <>
              ...
              <div className={s.summaryGrid}>
                ...
                {selectedSprint && (
                  <Stat
                    ...
                    sub={`${selectedSprint.start_date} → ${selectedSprint.end_date}`}
                  />
                )}
              </div>
            </section>
          ...
          <section className={s.card}>
            <h2 className={s.sectionTitle}>
              <ListChecks size={16} /> Sprint Tickets
            </h2>
          ...
          <section className={s.card}>
            <h2 className={s.sectionTitle}>
              <Layers size={16} /> Cumulative Flow
            </h2>
          ...
          <section className={s.card}>
            <h2 className={s.sectionTitle}>
              <LineChart size={16} /> Cycle &amp; Lead Time
            </h2>
          ...
          <section className={s.card}>
            <h2 className={s.sectionTitle}>
              <MessageSquare size={16} /> Retrospective
            </h2>
            <RetrospectivePanel
              sprintId={selectedId}
              initial={retro}
...
function Stat({ label, value, sub, kind = "default" }: StatProps) {
  return (
 ...
 */
function CumulativeFlowChart({ data }: { data: any }) {
  if (!data) return <div className={s.chartEmpty}>Loading…</div>;
  if (!data.series?.length)
    return <div className={s.chartEmpty}>No data yet for this sprint.</div>;
  ...
  const maxY = Math.max(
    1,
    ...data.series.map((d: any) => order.reduce((a, k) => a + (d[k] || 0), 0)),
  );
  ...
  const areas = order.map((cat) => {
    ...
    const bottoms = data.series.map((d: any) =>
      order.slice(0, idx).reduce((a, k) => a + (d[k] || 0), 0),
    );
    const tops = data.series.map(
      (d: any, i: number) => bottoms[i] + (d[cat] || 0),
    );
    const yScale = (v: number) => padT + innerH - (innerH * v) / maxY;
    const xAt = (i: number) => padL + i * xStep;
    const top = tops
      .map(
        (v: number, i: number) =>
          `${xAt(i).toFixed(1)},${yScale(v).toFixed(1)}`,
      )
      .join(" L ");
  ...
  const tickIdx = [
    0,
    Math.floor(data.series.length / 2),
  ...
  return (
    <div className={s.chartWrap}>
      ...
      <svg
        ...
        role="img"
        aria-label="Cumulative flow chart"
      >
        {/* y-axis grid */}
        ...
        {/* x-axis labels */}
        {tickIdx
          .filter((i) => i >= 0 && i < data.series.length)
          .map((i) => (
            <text
              key={i}
              x={padL + i * xStep}
...
function CycleTimePanel({ data }: { data: any }) {
  if (!data) return <div className={s.chartEmpty}>Loading…</div>;
  if (!data.tasks?.length)
    return <div className={s.chartEmpty}>No completed tickets yet.</div>;
  return (
    <>
      <div className={s.statRow}>
        <Stat
          label="Cycle (avg)"
          value={data.cycle.avg != null ? `${data.cycle.avg} d` : "—"}
          sub={`median ${data.cycle.median ?? "—"} · p90 ${data.cycle.p90 ?? "—"}`}
        />
        <Stat
          label="Lead (avg)"
          value={data.lead.avg != null ? `${data.lead.avg} d` : "—"}
          sub={`median ${data.lead.median ?? "—"} · p90 ${data.lead.p90 ?? "—"}`}
        />
        ...
        <tbody>
          {data.tasks.map((t: any) => (
            <tr key={t.id}>
              <td>
                #{t.id}{" "}
                <a href={`/tasks?task=${t.id}`} className={s.taskLink}>
                  {t.title}
                </a>
              </td>
              <td>
                {t.type_name && (
                  <span
                    ...
                    style={{ color: t.type_color, borderColor: t.type_color }}
                  >
                    {t.type_name}
                  </span>
                )}
...
function SprintTicketsPanel({ tasks }: { tasks: any[] }) {
  if (!tasks.length)
    return <div className={s.chartEmpty}>No tickets in this sprint.</div>;
  return (
...
function RetrospectivePanel({
  ...
  const [teamMood, setTeamMood] = useState<number | null>(null);
  ...
  useEffect(() => {
    ...
    setActionItems(
      Array.isArray(initial?.action_items) ? initial.action_items : [],
    );
  }, [
    initial?.id,
    initial?.went_well,
  ...
  const save = async () => {
    ...
    try {
      ...
      setSaved(true);
      onSaved && onSaved((r.data as any)?.retrospective);
  ...
  const toggleAction = (id: any) => {
    setActionItems(
      actionItems.map((a) => (a.id === id ? { ...a, done: !a.done } : a)),
    );
  };

  const removeAction = (id: any) => {
  ...
  return (
    <div className={s.retro}>
      ...
      <div className={s.retroActions}>
        <h3 className={s.retroSubtitle}>
          <ListChecks size={14} /> Action items
        </h3>
        <div className={s.actionList}>
          {actionItems.length === 0 && (
            <div className={s.empty}>No action items yet.</div>
          )}
          {actionItems.map((a) => (
            <label
              key={a.id}
              className={`${s.actionItem} ${a.done ? s.actionDone : ""}`}
            >
              <input
                type="checkbox"
                checked={a.done}
