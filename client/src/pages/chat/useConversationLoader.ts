...
type UseConversationLoaderOptions = {
  ...
  setLoadMoreError: Dispatch<SetStateAction<string>>;
...
export default function useConversationLoader({
  ...
  const openConversation = useCallback(
    async (conversationId: number | string, data: AnyRecord) => {
      ...
      try {
        ...
        setMessages(
          (current) => mergeTimelineMessages(current, fetched) as Message[],
        );
      ...
      const readStatus = getReadStatus(conversationId).then((readResponse) => {
        if (!isCurrent()) return;
      ...
      const members = getMembers(conversationId).then((memberResponse) => {
        if (!isCurrent()) return;
      ...
      if (readStatusResult.status === "rejected") {
        console.error("Failed to load read status", readStatusResult.reason);
      }
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      setMessages(
        (current) => mergeTimelineMessages(fetched, current) as Message[],
      );
      setHasMore(fetched.length >= 50);
      ...
      setLoadMoreError("Could not load earlier messages. Tap to retry.");
    } finally {
