...
export default function Tasks() {
  ...
  const [stats, setStats] = useState<any>({ total: 0, done: 0, inProgress: 0, percent: 0 });
  ...
  const [orgLabels, setOrgLabels] = useState<any[]>([]);
  ...
  const { confirmDialog, showConfirm, closeConfirm } = useConfirmDialog() as any;
  ...
  const comments = useComments({ showConfirm, closeConfirm, setTasks, setError }) as any;
  const drag = useDragDrop({ tasks, setTasks, showConfirm, closeConfirm, setError }) as any;

  useEffect(() => {
    getAssignableUsers().then((r) => setAssignableUsers(r.data as any)).catch(() => {});
    getTaskLabels().then((r) => setOrgLabels(r.data as any)).catch(() => {});
    ...
    getAvailableSprints()
      .then((r) => {
        ...
        setAvailableSprints(d);
        const active = d.find((sp) => sp.status === "active");
        if (active) setSelectedSprintId(active.id);
        else if (d.length > 0) setSelectedSprintId(d[0].id);
      })
      .catch(() => {});
  ...
  useEffect(() => {
    ...
    if (taskId) {
      consumed = true;
      getTaskDetail(taskId as any).then((res) => detail.openTaskDetail(res.data)).catch(() => {});
    }
  ...
  useEffect(() => {
    (window as any).__openTaskDetail = (task: any) => detail.openTaskDetail(task);
    return () => { try { delete (window as any).__openTaskDetail; } catch { /* ignore */ } };
  ...
  const toggleLabel = (labelId: any, list: any, setter: any) => {
    setter((prev: any[]) => prev.includes(labelId) ? prev.filter((id) => id !== labelId) : [...prev, labelId]);
  };
        ...
        <>
          <div className={s["tasks-progress-card"]}>
            ...
            <div className={s["tasks-progress-bar"]}>
              <div className={s["tasks-progress-fill"]} style={{ "--fill-width": `${stats.percent}%` } as React.CSSProperties} />
            </div>
