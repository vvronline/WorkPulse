...
export default function PlatformAuditLogs() {
    ...
    useEffect(() => {
        getTenants({ limit: 200 } as any).then(r => setTenants((r.data as any).tenants || [])).catch(() => {});
        getPlatformUsers().then(r => setAdmins((r.data as any) || [])).catch(() => {});
    }, []);
    ...
    const fetchLogs = useCallback(() => {
        ...
        getPlatformAuditLogs(params)
            .then(r => { setLogs((r.data as any).logs); setTotal((r.data as any).total); })
                                                        ...
                                                        <div className={s.sessionDetails}>
                                                            ...
                                                            {details.actions?.length > 0 && (
                                                                <div className={s.sessionActionLog}>
                                                                    {details.actions.map((a: any, i: number) => (
