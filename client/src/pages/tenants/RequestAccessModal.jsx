...
import {
    X, Shield, Loader2, Clock, CheckCircle2, AlertTriangle, Copy, KeyRound, Lock,
} from 'lucide-react';
import {
 ...
 */
export default function RequestAccessModal({ tenant, onClose, onError }) {
    const [policy, setPolicy] = useState(null);
    const [step, setStep] = useState('reason');   // reason | waiting | code | done | break_glass
    ...
    useEffect(() => {
        getImpersonationPolicy().then(r => setPolicy(r.data)).catch(() => { });
    }, []);
    ...
    useEffect(() => {
        let cancelled = false;
        listTenantAccessRequests(tenant.id).then(r => {
            if (cancelled) return;
...
function StepIndicator({ step, hasGlass }) {
