...
export default function PlatformDashboard() {
    ...
    useEffect(() => {
        Promise.all([
            ...
            getTenantAlerts().then(r => setAlerts((r.data as any)?.alerts || [])),
            ...
            {by_plan && Object.keys(by_plan).length > 0 && (
                <fieldset className={s.fieldset} style={{ marginBottom: 20 }}>
                    ...
                    <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
                        {Object.entries(by_plan).map(([plan, count]: [string, any]) => (
                            <div key={plan} style={{
                                padding: "12px 20px",
            ...
            {trend_30d && trend_30d.length > 0 && (
                <fieldset className={s.fieldset} style={{ marginBottom: 20 }}>
                    ...
                    <div style={{ display: "flex", gap: 3, alignItems: "flex-end", height: 60, padding: "8px 0" }}>
                        {trend_30d.map((d: any, i: number) => {
                            const max = Math.max(...trend_30d.map((x: any) => parseInt(x.count, 10)), 1);
                    ...
                    <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 4 }}>
                        Total: {trend_30d.reduce((sum: number, d: any) => sum + parseInt(d.count, 10), 0)} new tenants
                    </div>
                </fieldset>
            )}
