...
export default function FaceEnrollment() {
    const [status, setStatus] = useState({ loading: true, enrolled: false, enrolled_at: null });
    const [mode, setMode] = useState('view'); // 'view' | 'capture'
    ...
    async function refreshStatus() {
        try {
            ...
            setStatus({ loading: false, enrolled: !!r.data.enrolled, enrolled_at: r.data.enrolled_at });
        } catch {
    ...
    async function handleEnroll(descriptor) {
        setBusy(true);
        setToast(null);
        try {
    ...
    async function handleClear() {
        if (!window.confirm('Clear your face enrollment? You will need to re-enroll to clock in when attendance verification is on.')) return;
        setBusy(true);
        setToast(null);
        try {
    ...
    return (
        <div className={s.page}>
            <div className={s.header}>
                <Link to="/profile" className={s.back}><ChevronLeft size={16} /> Profile</Link>
            ...
            <div className={`${s.statusCard} ${status.enrolled ? s.ok : s.notEnrolled}`}>
                {status.enrolled
                    ? (<><ShieldCheck size={20} /> <span>Enrolled{status.enrolled_at ? ` on ${new Date(status.enrolled_at).toLocaleDateString()}` : ''}</span></>)
                    : (<><ShieldOff size={20} /> <span>Not enrolled yet</span></>)}
            </div>

            {toast && (
                <div className={`${s.toast} ${s[toast.kind]}`}>{toast.text}</div>
            )}
            ...
            {mode === 'capture' && (
                <div className={s.captureBox}>
                    <h3>Position your face inside the circle and click <em>Enroll Face</em>.</h3>
                    <FaceCapture
                        autoStart
