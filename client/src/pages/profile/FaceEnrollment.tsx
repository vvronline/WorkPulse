...
export default function FaceEnrollment() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["profile", "face-status"],
    queryFn: async () => (await getFaceStatus()).data,
  });
  const status: FaceStatus = {
    loading: isLoading,
    enrolled: !!(data as any)?.enrolled,
    enrolled_at: (data as any)?.enrolled_at ?? null,
  };
  ...
  async function handleEnroll(descriptor: any) {
    ...
    try {
      ...
      const msg =
        err?.response?.data?.error || "Failed to enrol face. Try again.";
  ...
  return (
    <div className={s.page}>
      <div className={s.header}>
        ...
        <p className={s.lede}>
          Enrol your face so you can clock in when your organization requires
          attendance verification. Your photo never leaves this device — only a
          numerical representation is stored.
        </p>
      </div>

      <div
        className={`${s.statusCard} ${status.enrolled ? s.ok : s.notEnrolled}`}
      >
        {status.enrolled ? (
          <>
            <ShieldCheck size={20} />{" "}
            <span>
              Enrolled
              {status.enrolled_at
                ? ` on ${new Date(status.enrolled_at).toLocaleDateString()}`
                : ""}
            </span>
          </>
        ) : (
