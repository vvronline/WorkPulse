...
interface LeaveHistoryProps {
    leaves: LeaveItem[];
    loading?: boolean;
    onWithdraw: (leave: LeaveItem) => void;
}
...
function isPublicHoliday(l: LeaveItem | null | undefined): boolean {
    return (
        !!l &&
...
export default function LeaveHistory({ leaves, loading, onWithdraw }: LeaveHistoryProps) {
    ...
    useEffect(() => {
        ...
        getLeavePolicies()
            .then((r) => {
                if (!cancelled) setPolicies((r.data as any[]) || []);
            })
            .catch(() => {});
        return () => {
            cancelled = true;
    ...
    const lookupType = (val: string) => typeMeta[val] || getLeaveType(val);
    ...
    return (
        <>
            ...
            <div className={s.statsStrip}>
                ...
                <div className={s.statDivider} />
                <div className={s.statItem}>
                    <span className={s.statNum} style={{ color: "#10b981" }}>
                        {personalLeaves.filter((l) => l.status === "approved").length}
                    </span>
                ...
                <div className={s.statDivider} />
                <div className={s.statItem}>
                    <span className={s.statNum} style={{ color: "#ef4444" }}>
                        {personalLeaves.filter((l) => l.status === "rejected").length}
                    </span>
                ...
                {loading ? (
                    ...
                    <div className={s.leaveList}>
                        {filteredLeaves.map((leave) => {
                            ...
                            const status =
                                STATUS_CONFIG[leave.status as string] ?? STATUS_CONFIG.pending;
                            const durLabel =
                                leave.duration === "half"
                                    ? "Half Day"
                                ...
                                <div key={leave.id} className={s.leaveItem}>
                                    <div className={s.leaveItemLeft}>
                                        ...
                                        <div className={s.leaveItemBody}>
                                            ...
                                            {leave.reason && (
                                                <div className={s.leaveReason}>
                                                    "{leave.reason}"
                                                </div>
                                            )}
                                            {leave.reject_reason && (
                                                <div className={s.leaveRejectReason}>
                                                    Rejection reason: {leave.reject_reason}
                                                </div>
                                            )}
                                            {leave.approved_by_name &&
                                                leave.status === "approved" && (
                                                    <div className={s.leaveApprover}>
