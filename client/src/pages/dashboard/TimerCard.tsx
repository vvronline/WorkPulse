...
function getStateLabel(state: string, mode: string): string {
        ...
        <div className={`status-card ${s["dash-timer-card"]}`}>
            ...
            <div className={s["dash-badges-row"]}>
                <span className={`${s["status-badge"]} ${s[isWeekend && state === "logged_out" ? "weekend" : state]}`}>
                    {isWeekend && state === "logged_out" ? (
                        <>
                            <Palmtree size={13} /> Weekend Holiday
                        </>
                    ) : (
                        getStateLabel(state, workMode)
                    )}
                </span>
                {state !== "logged_out" && (
                    <span
                        className={`${s["work-mode-badge"]} ${s[workMode]}`}
                ...
                <div className={s["timer-left-col"]}>
                    <div className={s["circular-progress-wrapper"]}>
                        ...
                        <div className={s["circular-inner"]}>
                            {state === "on_floor" && (
                                <>
                                    <div className={`${s["circular-time"]} ${s["floor-timer"]}`}>
                                        {formatTimeSec(liveFloorSec)}
                                    </div>
                            ...
                            {state === "on_break" && (
                                <>
                                    <div className={`${s["circular-time"]} ${s["break-timer-text"]}`}>
                                        {formatTimeSec(liveBreakSec)}
                                    </div>
                            ...
                            {state === "logged_out" && isWeekend && (
                                <>
                                    <div className={`${s["circular-time"]} ${s["weekend-text"]}`}>
                                        <Palmtree size={36} />
                                    </div>
                        ...
                        {estimatedClockOut && (
                            <div className={s["eta-banner"]}>
                                <span style={{ display: "inline-flex", alignItems: "center", gap: "0.3rem" }}>
                                    <Clock size={14} /> {Math.round(targetMinutes / 60)}hr by{" "}
                                    <strong>{estimatedClockOut}</strong>
                                </span>
                            </div>
                        )}
                        {overtimeMinutes > 0 && (
                            <div className={s["overtime-banner"]}>
                                <span className={s["overtime-icon"]}>
                                    <Zap size={14} />
                                </span>
                                <span>
                                    Overtime: <strong>{formatTime(overtimeMinutes)}</strong>
                                </span>
                            </div>
                        )}
                    </div>
                    ...
                    {state !== "logged_out" && (
                        <div className={s["inline-stats"]}>
                            <div className={s["inline-stat"]}>
                                <span className={`${s["inline-stat-icon"]} ${s["work-icon"]}`}>
                                    <Timer size={18} />
                                </span>
                                <div>
                                    <div className={s["inline-stat-label"]}>Work</div>
                                    <div className={`${s["inline-stat-value"]} ${s.floor}`}>
                                        {formatTime(floorMinutes)}
                                    </div>
                                </div>
                            </div>
                            <div className={s["inline-stat"]}>
                                ...
                                <div>
                                    ...
                                    <div className={`${s["inline-stat-value"]} ${s.break}`}>
                                        {formatTime(Math.floor(liveBreakSec / 60))}
                                    </div>
                                </div>
                            </div>
                            <div className={s["inline-stat"]}>
                                <span className={`${s["inline-stat-icon"]} ${s["total-icon"]}`}>
                                    <Hourglass size={18} />
                                </span>
                                <div>
                                    <div className={s["inline-stat-label"]}>Total</div>
                                    <div className={`${s["inline-stat-value"]} ${s.total}`}>
                                        {formatTime(Math.floor(liveFloorSec / 60) + Math.floor(liveBreakSec / 60))}
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div className={s["action-buttons"]}>
                {state === "logged_out" && !isWeekend && dailyTargetMet && (
                    <div className={s["daily-target-met-state"]}>
                        <p className={s["daily-target-met-msg"]}>
                            <Target size={16} style={{ display: "inline", verticalAlign: "middle" }} /> Daily target
                            complete! Need to keep working?
                        </p>
                ...
                {state === "on_floor" && (
                    <>
                        <button className="btn btn-warning" onClick={handleBreakStart} disabled={!!actionLoading}>
                            {actionLoading === "breakStart" ? (
                                "Starting..."
                            ) : (
                                <>
                                    <CirclePause size={14} /> Break
                                </>
                            )}
                        </button>
