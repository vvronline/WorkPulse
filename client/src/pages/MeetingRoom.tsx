...
export default function MeetingRoom() {
    ...
    useEffect(() => {
        ...
        getMeeting(code as any)
            .then(r => {
                if (cancelled) return;
                joinMeeting({ meetingId: (r.data as any).id, code, meeting: r.data, initialMuted: false, initialVideoOff: false });
            })
            .catch(() => {
    ...
    const tiles = useMemo(() => {
        const list: { key: string; participant: any; isLocal: boolean }[] = [];
        // Local tile
        list.push({
            key: "local",
            participant: {
    ...
    const handleToggleChat = () => setActivePanel((p: string | null) => p === "chat" ? null : "chat");
    const handleToggleParticipants = () => setActivePanel((p: string | null) => p === "participants" ? null : "participants");
    ...
    useEffect(() => {
        ...
        if (diff > 0 && activePanelRef.current !== "chat") {
            ...
            const fromOthers = newMessages.filter((m: any) => m.sender_id !== user?.id).length;
            if (fromOthers > 0) {
