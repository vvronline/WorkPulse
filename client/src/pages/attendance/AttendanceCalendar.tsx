...
function buildMonthMatrix(year: number, month: number /* 0-11 */): (Date | null)[] {
...
/* ---------- component ---------- */
export default function AttendanceCalendar({ refreshKey = 0 }: AttendanceCalendarProps) {
    ...
    const liveFloorMinutes = useMemo(() => {
        ...
        const sec = state === "logged_out" ? (liveStatus?.floorMinutes || 0) * 60 : liveFloorSec;
    ...
    const workDayJsDowSet = useMemo(() => workDaysToJsDowSet(org?.work_days), [org]);
    ...
    return (
        <section className={s.wrap}>
            <header className={s.header}>
                ...
                <div className={s.nav}>
                    <button className={s.navBtn} onClick={goPrev} aria-label="Previous month">
                        <ChevronLeft size={16} />
                    </button>
            ...
            <div className={s.legend}>
                <span className={s.legendItem}>
                    <span className={`${s.swatch} ${s.swPresent}`} /> Present (≥ {minHoursPresent}h)
                </span>
                <span className={s.legendItem}>
