...
export default function AttendanceCalendar({
  ...
  const firstDay = fmtYMD(new Date(year, month, 1));
  const lastDay = fmtYMD(new Date(year, month + 1, 0));

  // Stale-while-revalidate per visible month. Cached attendance (restored from
  // localStorage on a cold start) renders instantly while a background refetch
  // keeps it current. `refreshKey` (bumped by the parent after a manual-entry
  // change) is part of the key so those edits force a refetch.
  const {
    data,
    isLoading: loading,
    isError,
  } = useQuery({
    queryKey: ["attendance", "calendar", firstDay, lastDay, year, refreshKey],
    queryFn: async () => {
      ...
      return {
        history: hRes.status === "fulfilled" ? hRes.value.data || [] : [],
        leaves: lRes.status === "fulfilled" ? lRes.value.data || [] : [],
  ...
  const history: any[] = data?.history ?? EMPTY;
  const leaves: any[] = data?.leaves ?? EMPTY;
  const holidays: any[] = data?.holidays ?? EMPTY;
  ...
  /* day-cell renderer */
  const renderCell = (dt: Date | null, idx: number) => {
    ...
    if (present) {
      ...
      const displayMin = isToday
        ? Math.max(present.floorMinutes, liveFloorMinutes)
        : present.floorMinutes;
    ...
    return (
      ...
      >
        ...
        {kind === "leave" && (
          <span className={`${s.dot} ${s.dotLeave}`} aria-hidden />
        )}
        {kind === "leave-pending" && (
          <span className={`${s.dot} ${s.dotLeavePending}`} aria-hidden />
        )}
        {kind === "absent" && (
          <span className={`${s.dot} ${s.dotAbsent}`} aria-hidden />
        )}
        {kind === "holiday" && (
          <span className={`${s.dot} ${s.dotHoliday}`} aria-hidden />
        )}
      </div>
    );
  };
    ...
    <section className={s.wrap}>
      <header className={s.header}>
        ...
        <div className={s.nav}>
          ...
          <button className={s.navBtn} onClick={goNext} aria-label="Next month">
      ...
      <div className={s.legend}>
        <span className={s.legendItem}>
          <span className={`${s.swatch} ${s.swPresent}`} /> Present (≥{" "}
          {minHoursPresent}h)
        </span>
        <span className={s.legendItem}>
          <span className={`${s.swatch} ${s.swAbsent}`} /> Absent
        </span>
        <span className={s.legendItem}>
          <span className={`${s.swatch} ${s.swLeave}`} /> Leave
        </span>
        <span className={s.legendItem}>
          <span className={`${s.swatch} ${s.swLeavePending}`} /> Leave (pending)
        </span>
        <span className={s.legendItem}>
          <span className={`${s.swatch} ${s.swHoliday}`} /> Holiday / Weekend
        </span>
      </div>
      ...
      <div className={s.grid}>
        ...
        {loading
          ? Array.from({ length: 35 }).map((_, i) => (
              <div key={`sk-${i}`} className={s.cellSkeleton} />
            ))
