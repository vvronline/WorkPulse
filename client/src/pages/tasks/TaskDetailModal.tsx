...
import AcceptanceCriteria from "../../components/agile/AcceptanceCriteria";
import {
    BlockerControl,
    DependenciesPanel,
    ParentChildPanel,
...
interface TaskDetailModalProps {
    ...
    onAddComment: (...args: any[]) => void;
    onEditComment: (...args: any[]) => void;
    ...
    onStatusChange: (task: any, col: any) => void;
...
export default function TaskDetailModal({
    ...
    useEffect(() => {
        ...
        getTaskCustomFieldValues(detailTask.id)
            .then((r) => {
                if (!cancelled) setCustomValues((r.data as any)?.values || {});
            })
            .catch(() => {
    ...
    const isEpic = !!(
        detailTask?.work_item_type_id &&
        typeById[detailTask.work_item_type_id]?.is_epic
    );
    ...
    const swapToTask = (id: number | string) => {
        getTaskDetail(id as any)
            .then((res) => {
                if ((window as any).__openTaskDetail)
                    (window as any).__openTaskDetail(res.data);
    ...
    const [editSprintId, setEditSprintId] = useState<number | string>("");
    const [editLabels, setEditLabels] = useState<Array<number | string>>([]);
    const [editLabelDropdownOpen, setEditLabelDropdownOpen] = useState(false);
    const [editStoryPoints, setEditStoryPoints] = useState<any>(null);
    const [editWorkItemType, setEditWorkItemType] = useState<any>("");
    const [editProjectId, setEditProjectId] = useState<string | number>("");
    ...
    useEffect(() => {
        if (detailEditing && detailTask) {
            ...
            setEditLabels(detailTask.labels?.map((l: any) => l.id) || []);
    ...
    const toggleLabel = (id: number | string) => {
        setEditLabels((prev) =>
            ...
            <div className={s["detail-modal"]} onClick={(e) => e.stopPropagation()}>
                ...
                <div className={s["detail-modal-header"]}>
                    <div className={s["detail-badges"]}>
                        ...
                        {detailTask.date && (
                            <span className={s["detail-date-badge"]}>
                                <CalendarDays
                                    size={12}
                                    style={{ marginRight: 3, verticalAlign: "middle" }}
                                />
                ...
                <div className={s["detail-modal-body"]}>
                        ...
                        <div className={s["detail-edit-section"]}>
                            ...
                            <div className={s["form-extras"]}>
                                ...
                                <SprintSelector
                                    ...
                                    selected={editSprintId as any}
                                    onChange={(id: any) => {
                                        setEditSprintId(id as any);
                                        if (!id) {
                                            setEditDueDate("");
                                        } else {
                            ...
                            <div className={s["detail-edit-buttons"]}>
                                {isBacklogItem || activeTab === "backlog" ? (
                                    ...
                                    <button
                                        className="btn btn-secondary btn-sm"
                                        onClick={() =>
                                            onUnschedule(detailTask.id, detailTask.title, onClose)
                                        }
                                    >
                                        <Package
                                            size={13}
                  ...
                  based on the work item type's is_epic flag. */}
                            ...
                            <CustomFieldsEditor
                                ...
                                onSaved={(v: any) => setCustomValues(v)}
                            />
                        </div>
                    ) : (
                        /* ─── VIEW MODE ─── */
                  ...
                  so users batch their changes and the view stays scannable. */}
                            {detailTask.is_blocked && (
                                <div style={{ marginTop: 8 }}>
                                    <span
                                        className={s["detail-blocker-readonly"]}
                                        title={detailTask.blocked_reason || "Blocked"}
                                    >
                                        ⛔ Blocked
                            ...
                            {Array.isArray(detailTask.acceptance_criteria) &&
                                detailTask.acceptance_criteria.length > 0 && (
                                    <div className={s["detail-readonly-section"]}>
                                        <div className={s["detail-readonly-title"]}>
                                            ...
                                            <span className={s["detail-readonly-progress"]}>
                                                (
                                                {
                                                    detailTask.acceptance_criteria.filter(
                                                        (c: any) => c.done,
                                                    ).length
                                                }
                                        ...
                                        <ul className={s["detail-readonly-list"]}>
                                            {detailTask.acceptance_criteria.map(
                                                (c: any, i: number) => (
                                                    <li
                                                        key={c.id || i}
                                                        className={c.done ? s["ac-done"] : ""}
                                                    >
                                                        <span className={s["ac-check"]}>
                                                            {c.done ? "☑" : "☐"}
                                                        </span>{" "}
                                                        {c.text}
                                                    </li>
                            ...
                            <div className={s["detail-meta-grid"]}>
                                {detailTask.assignee && (
                                    <div className={s["detail-meta-item"]}>
                                        <span className={s["detail-meta-label"]}>
                                            Assigned to
                                        </span>
                                        ...
                                        <div className={s["detail-meta-item"]}>
                                            ...
                                            <span className={s["detail-meta-value"]}>
                                                {detailTask.creator.avatar ? (
                                                    ...
                                                    <span
                                                        className={s["detail-avatar-placeholder"]}
                                                    >
                                                        {(
                                    ...
                                    <div className={s["detail-meta-item"]}>
                                        ...
                                        <span className={s["detail-meta-value"]}>
                                            ...
                                            {availableSprints.find(
                                                (sp: any) => sp.id === detailTask.sprint_id,
                                            )?.name || `Sprint #${detailTask.sprint_id}`}
                                        </span>
                                    </div>
                                )}
                            </div>
                    ...
                    <div className={s["detail-comments"]}>
                        <div className={s["detail-tab-switcher"]}>
                            ...
                            >
                                ...
                                />
                                Comments{" "}
                                <span className={s["detail-tab-count"]}>
                            ...
                            <div className={s["history-list"]}>
                                ...
                                {detailHistory.map((h) => {
                                    ...
                                    const actionText = () => {
                                        ...
                                        if (h.action === "comment_added")
                                            return "added a comment";
                                        if (h.action === "comment_edited")
                                            return "edited a comment";
                                        if (h.action === "comment_deleted")
                                    ...
                                    return (
                                        <div key={h.id} className={s["history-item"]}>
                                            ...
                                            <div className={s["history-content"]}>
                                                ...
                                                <span className={s["history-action"]}>
                                                    {actionText()}
                                                </span>
                                            </div>
