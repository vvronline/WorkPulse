...
export default function ServiceDeskTab() {
  ...
  const {
    ...
    queryKey: ["service-desk", "tickets", filterStatus, filterType],
    queryFn: async () => {
      const params: Record<string, string> = {};
      if (filterStatus) params.status = filterStatus;
      if (filterType) params.ticket_type = filterType;
      const res = await getServiceDeskTickets(params);
      return (res.data as any).tickets as any[];
    },
  });

  const { data: stats = null } = useQuery({
    queryKey: ["service-desk", "stats"],
    queryFn: async () => (await getServiceDeskStats()).data,
  });

  const displayError = error || (ticketsError ? "Failed to load tickets" : "");

  const invalidateServiceDesk = () => {
    queryClient.invalidateQueries({ queryKey: ["service-desk", "tickets"] });
    queryClient.invalidateQueries({ queryKey: ["service-desk", "stats"] });
  };
  ...
  return (
    <div className={s["service-desk"]}>
      {displayError && (
        <div className="error-msg error-msg-mb">{displayError}</div>
      )}
                ...
                {isExpanded && (
                  <div className={s["ticket-details"]}>
                    ...
                    <div className={s["ticket-meta"]}>
                      ...
                      {ticket.tenant_name && (
                        <span>
                          Organization: <strong>{ticket.tenant_name}</strong>
                        </span>
                      )}
                      {ticket.assigned_to && (
                        <span>
