...
export default function PolicyForm({ initial, onClose, onSaved }: PolicyFormProps) {
    const [form, setForm] = useState<Record<string, any>>({ ...defaults, ...initial });
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!isExisting) {
                ...
                if (!slug) {
                    setError("Invalid leave type name");
                    setSaving(false);
                    return;
                }
            ...
            <div className={s.modalBox} onClick={(e) => e.stopPropagation()}>
                ...
                <form onSubmit={handleSave} className={s.modalForm}>
                    ...
                    <div className={s.formField}>
                        ...
                        <div style={{ display: "flex", gap: "0.5rem" }}>
                            <input
                                ...
                                title={
                                    isExisting
                                        ? "Leave-type identifier cannot be renamed once created"
                                        : ""
                                }
                                style={{ flex: 1 }}
                            />
                        ...
                        {isExisting && (
                            ...
                            >
                                The leave-type identifier is locked once created so existing balances
                                and requests stay linked. Delete and re-create the policy to rename
                                it.
                            </p>
                        )}
                    </div>
                    ...
                    <div className={s.formField}>
                        ...
                        >
                            <option value="annual">
                                Annual — all days granted at year start
                            </option>
                            <option value="monthly">Monthly — days accrue each month</option>
                            <option value="quarterly">
                                Quarterly — days accrue each quarter
                            </option>
                        </select>
                    </div>
                        ...
                        <div className={s.checkGroup}>
                            <label className={s.checkItem}>
                                <input
                                    ...
                                    onChange={(e) =>
                                        set("half_day_allowed", e.target.checked ? 1 : 0)
                                    }
                                />
                            ...
                            <label className={s.checkItem}>
                                <input
                                    ...
                                    onChange={(e) =>
                                        set("quarter_day_allowed", e.target.checked ? 1 : 0)
                                    }
                                />
