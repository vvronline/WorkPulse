...
interface Policy {
  ...
  half_day_allowed?: number | boolean;
  quarter_day_allowed?: number | boolean;
  [key: string]: any;
}
...
export default function PoliciesTab() {
  ...
  const { data: policies = EMPTY_POLICIES } = useQuery({
    queryKey: ["leave-policy", "policies"],
    queryFn: async () => (await getLeavePolicies()).data as Policy[],
  });
  ...
  const reload = () =>
    queryClient.invalidateQueries({ queryKey: ["leave-policy", "policies"] });
  ...
  const handleDelete = async (id: number | string) => {
    if (
      ...
      return;
    try {
      await deleteLeavePolicyAPI(id as any);
      reload();
    } catch {
      /* ignore */
    }
      ...
      <div className={s.panelHeader}>
        <div>
          ...
          <p className={s.panelSubtitle}>
            Define quotas, accrual rules, and permissions per leave type
          </p>
        </div>
        <button className={s.primaryBtn} onClick={openAdd}>
          + Add Policy
        </button>
      </div>
      ...
      {policies.length === 0 ? (
        <div className={s.emptyState}>
          ...
          <p className={s.emptyText}>
            Create leave policies to control how employees request time off
          </p>
          <button className={s.primaryBtn} onClick={openAdd}>
            Create First Policy
          </button>
        ...
        <div className={s.policyGrid}>
          {policies.map((p) => {
            ...
            return (
              <div
                key={p.id}
                className={s.policyCard}
                style={
                  { "--pc": meta.color, "--pb": meta.bg } as React.CSSProperties
                }
              >
                <div className={s.policyCardTop}>
                  <div
                    className={s.policyTypeIcon}
                ...
                <div className={s.policyMeta}>
                  ...
                  <div className={s.policyMetaItem}>
                    ...
                    <span className={s.policyMetaVal}>
                      {p.carry_forward_limit} days
                    </span>
                  </div>
                </div>
                <div className={s.policyPerms}>
                  <span
                    className={`${s.permChip} ${
                      p.half_day_allowed ? s.permOn : s.permOff
                    }`}
                  >
                    ½ Half-day
                  </span>
                  <span
                    className={`${s.permChip} ${
                      p.quarter_day_allowed ? s.permOn : s.permOff
                    }`}
                  >
                    ¼ Quarter-day
                  </span>
