...
export default function HolidaysTab({ isHR }: HolidaysTabProps) {
  ...
  const { data: holidays = EMPTY_HOLIDAYS } = useQuery({
    queryKey: ["leave-policy", "holidays", year],
    queryFn: async () => (await getHolidays(year)).data as Holiday[],
  });
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setShowForm(false);
      queryClient.invalidateQueries({ queryKey: ["leave-policy", "holidays"] });
    } catch {
      /* ignore */
    } finally {
      setSaving(false);
      ...
      {holidays.length === 0 ? (
        ...
        <>
          ...
          {past.length > 0 && (
            <section className={s.holidaySection}>
              <h3
                className={`${s.holidaySectionTitle} ${s.holidaySectionMuted}`}
              >
                Past
              </h3>
