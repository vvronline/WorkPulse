...
export default function HolidaysTab({ isHR }) {
    ...
    const load = useCallback(() => {
        getHolidays(year).then(r => setHolidays(r.data)).catch(console.error);
    }, [year]);
    ...
    const handleDelete = async (id) => {
        ...
        try { await deleteHoliday(id); load(); } catch { }
    };

    const now = new Date();
    const upcoming = holidays.filter(h => new Date(h.date + 'T00:00:00') >= now).sort((a, b) => new Date(a.date) - new Date(b.date));
    const past     = holidays.filter(h => new Date(h.date + 'T00:00:00') <  now).sort((a, b) => new Date(b.date) - new Date(a.date));
        ...
        <div className={s.tabPanel}>
            <div className={s.panelHeader}>
                <div>
                    ...
                    <p className={s.panelSubtitle}>Organisation-wide holidays and optional observances</p>
                </div>
                <div className={s.panelHeaderRight}>
                    <select className={s.yearSelect} value={year} onChange={e => setYear(+e.target.value)}>
                        {years.map(y => <option key={y} value={y}>{y}</option>)}
                    </select>
                    {isHR && (
                        <button className={s.primaryBtn} onClick={() => setShowForm(true)}>+ Add Holiday</button>
                    )}
                </div>
            </div>
            ...
            {holidays.length === 0 ? (
                <div className={s.emptyState}>
                    ...
                    {isHR && <button className={s.primaryBtn} onClick={() => setShowForm(true)}>Add First Holiday</button>}
                </div>
            ) : (
                <>
                    {upcoming.length > 0 && (
                    ...
                    {past.length > 0 && (
                        <section className={s.holidaySection}>
                            <h3 className={`${s.holidaySectionTitle} ${s.holidaySectionMuted}`}>Past</h3>
                        ...
                        <form onSubmit={handleAdd} className={s.modalForm}>
                            ...
                            <label className={s.checkItem}>
                                <input
                                    ...
                                    onChange={e => setForm({ ...form, is_optional: e.target.checked ? 1 : 0 })}
                                />
