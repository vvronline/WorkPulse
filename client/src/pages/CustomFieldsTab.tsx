...
import {
    getCustomFieldsAll, createCustomField, updateCustomField, deleteCustomField,
} from "../api";
...
export default function CustomFieldsSection({ canEdit = true }: CustomFieldsSectionProps) {
    const { workItemTypes } = useAgileConfig() as any;
    const { refresh: refreshFields } = useCustomFields() as unknown as { refresh: () => Promise<void> };
    ...
    const startEdit = (row: any) => {
        ...
        setForm({
            ...
            options: Array.isArray(row.options) && row.options.length > 0
                ? row.options.map((o: any) => ({ value: o.value || "", label: o.label || "" }))
                : [{ value: "", label: "" }],
            applies_to_types: Array.isArray(row.applies_to_types) ? row.applies_to_types : [],
        });
    };
    ...
    const setF = (k: keyof CustomFieldForm, v: any) => setForm(prev => ({ ...prev, [k]: v }));
    ...
    const handleDelete = async (row: any) => {
        if (!window.confirm(`Delete custom field "${row.label}"? Existing values on tasks will be lost.`)) return;
        try {
                ...
                <div className={s.formCard}>
                    ...
                    <div className={s.formActions}>
                        ...
                        <button className="btn btn-primary btn-sm" onClick={save} disabled={saving}>
                            ...
                            {saving ? " Saving…" : (editingId === "new" ? " Create field" : " Save changes")}
                        </button>
                    </div>
                </div>
            )}
                ...
                <table className={s.adminTable}>
                    ...
                    <tbody>
                        {rows.map(r => (
                            <tr key={r.id}>
                                <td>{r.label}</td>
                                <td><code style={{ fontSize: "0.75rem" }}>{r.key}</code></td>
                                <td><span className={s.typeBadge}>{r.field_type}</span></td>
                                <td>{r.is_required ? "✓" : "—"}</td>
