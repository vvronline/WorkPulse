...
describe("GlobalSearch - nav items filtering", () => {
    beforeEach(() => {
        mockGlobalSearch
            .mockReset()
            .mockResolvedValue({ data: { tasks: [], notes: [], users: [], events: [], leaves: [], sprints: [] } });
    });
...
describe("GlobalSearch - hr_admin nav items", () => {
    test("hr_admin sees admin nav items", async () => {
        ...
        const NAV_INCLUDES_ADMIN = [{ title: "Admin Panel", minRole: "hr_admin" }];
        const userLevel = ROLE_LEVEL["hr_admin"];
        const visible = NAV_INCLUDES_ADMIN.filter(
            (n) => !n.minRole || userLevel >= (ROLE_LEVEL[n.minRole] || 1)
        );
