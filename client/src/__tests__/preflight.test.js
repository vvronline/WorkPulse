...
function makeFakePC({ candidates = [], constructThrows = null, createOfferRejects = null, asyncTickMs = 0 } = {}) {
...
describe('runPreflight', () => {
    beforeEach(() => { delete globalThis.RTCPeerConnection; });
    afterEach(() => { globalThis.RTCPeerConnection = realPC; });
    ...
    it('marks all four candidate kinds when each appears', async () => {
        globalThis.RTCPeerConnection = makeFakePC({
            candidates: [
    ...
    it('settles on timeout when no candidates arrive', async () => {
        globalThis.RTCPeerConnection = class {
            ...
            async setLocalDescription() { /* never emits candidates */ }
            close() { }
        };
    ...
    it('surfaces createOffer failures as errorCode', async () => {
        globalThis.RTCPeerConnection = makeFakePC({ createOfferRejects: 'simulated' });
        const r = await runPreflight({ timeoutMs: 200 });
        expect(r.ok).toBe(false);
    ...
    it('surfaces constructor failures as pc-construct-failed', async () => {
        globalThis.RTCPeerConnection = makeFakePC({ constructThrows: 'oops' });
        const r = await runPreflight({ timeoutMs: 200 });
...
describe('summarisePreflight', () => {
    ...
    it('no-rtcpeerconnection → friendly browser-unsupported error', () => {
        expect(summarisePreflight({ errorCode: 'no-rtcpeerconnection', ok: false }).label)
            .toMatch(/browser/i);
    });
    it('relay candidate → "ok" with TURN reachable label', () => {
        expect(summarisePreflight({ ok: true, hasRelayCandidate: true })).toMatchObject({
            severity: 'ok',
            label: expect.stringContaining('TURN'),
        });
    });
    it('srflx only → "ok" with STUN reachable label', () => {
        expect(summarisePreflight({ ok: true, hasSrflxCandidate: true })).toMatchObject({
            severity: 'ok',
            label: expect.stringContaining('STUN'),
        });
    });
    it('host-only → "warn" (LAN-friendly note)', () => {
        expect(summarisePreflight({ ok: true, hasHostCandidate: true })).toMatchObject({
            severity: 'warn',
            label: expect.stringContaining('local'),
        });
    });
    it('no candidates at all → "error"', () => {
        expect(summarisePreflight({ ok: false })).toMatchObject({
            severity: 'error',
