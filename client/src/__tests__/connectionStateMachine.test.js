...
import { STATES, nextState, describeState } from '../pages/meeting/connectionStateMachine';

describe('connectionStateMachine', () => {
    it('starts idle and walks the happy path', () => {
        let s = STATES.IDLE;
        s = nextState(s, 'media_acquired'); expect(s).toBe(STATES.JOINING);
        s = nextState(s, 'join_sent'); expect(s).toBe(STATES.JOINING);
        s = nextState(s, 'participant_joined'); expect(s).toBe(STATES.CONNECTING);
        s = nextState(s, 'peer_connected'); expect(s).toBe(STATES.CONNECTED);
