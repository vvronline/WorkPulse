...
describe('useStore (React hook with selectors)', () => {
    function renderWithStore(useStore, selector, equalityFn) {
    ...
    it('subscribing to a single field only re-renders when that field changes', () => {
        ...
        const { renderSpy } = renderWithStore(useStore, (s) => s.muted);
    ...
    it('shallowEqual prevents re-render when a derived object is structurally identical', () => {
        ...
        const { renderSpy } = renderWithStore(useStore, (s) => ({ a: s.a, b: s.b }), shallowEqual);
...
describe('DEFAULT_MEETING_STATE shape contract', () => {
    it('is a frozen object with every public field present', () => {
        ...
        const expectedKeys = [
            ...
            'participants', 'presenterId', 'activeSpeakerId',
            'status', 'fsmState', 'connectionBanner', 'connectionQualities',
            'activePanel', 'raisedHand', 'messages',
        ];
