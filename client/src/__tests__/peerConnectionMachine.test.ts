/**
 * Phase 5.1 — Per-peer connection reducer tests (web mesh).
 *
 ...
 * analogue of the 1:1 P3.14 effect-race fix.
 */
...
import {
    ...
    type PeerPhase,
    type PeerEvent,
} from "../pages/meeting/peerConnectionMachine";

describe("peerConnectionMachine — Phase 5.1 per-peer reducer", () => {
    it("initial phase is connecting", () => {
        expect(initialPeerPhase()).toBe("connecting");
    });

    it("CONNECTED moves connecting → connected", () => {
        expect(peerConnectionReducer("connecting", { type: "CONNECTED" })).toBe(
            "connected",
        );
    });

    it("RECONNECTING moves connected → reconnecting (ICE blip / relay rebuild)", () => {
        expect(
            peerConnectionReducer("connected", { type: "RECONNECTING" }),
        ).toBe("reconnecting");
    });

    it("CONNECTED moves reconnecting → connected (recovery succeeded)", () => {
        expect(
            peerConnectionReducer("reconnecting", { type: "CONNECTED" }),
        ).toBe("connected");
    });

    it("FAILED moves connecting → failed (30s connect timeout / PC failed)", () => {
        expect(peerConnectionReducer("connecting", { type: "FAILED" })).toBe(
            "failed",
        );
    });

    it("RETRY moves failed → connecting (manual per-peer rebuild)", () => {
        expect(peerConnectionReducer("failed", { type: "RETRY" })).toBe(
            "connecting",
        );
    });

    it("CLOSED moves any live phase → closed", () => {
        ...
        for (const phase of live) {
            expect(peerConnectionReducer(phase, { type: "CLOSED" })).toBe(
                "closed",
            );
        }
    });
    ...
    describe("terminal phase is absorbing (the effect-race fix)", () => {
        ...
        it("no event can revive a closed peer", () => {
            for (const event of allEvents) {
                expect(peerConnectionReducer("closed", event)).toBe("closed");
            }
        });

        it("a late CONNECTED after CLOSED never flips back to connected", () => {
        ...
        it("isPeerTerminal reflects closed only", () => {
    ...
    it("a full happy-path peer lifecycle", () => {
    ...
    it("a failed peer recovers via RETRY then connects", () => {
        let phase: PeerPhase = "connecting";
