...
export default function Teams({ orgId, userRole }: TeamsProps) {
  ...
  const departments = teamsData?.departments ?? EMPTY_DEPARTMENTS;
                            ...
                            onClick={async () => {
                              ...
                              setEditForm({
                                ...
                                department_id: t.department_id
                                  ...
                                  : "",
                                lead_id: t.lead_id ? String(t.lead_id) : "",
                                sprint_duration_weeks: 2,
                                sprint_start_date: "",
                                sprint_mode: "manual",
                              });
                              ...
                              setEditPaused(false);
                              try {
                                const sprintRes = await getTeamSprintConfig(
                                  t.id,
                                );
                                setEditForm((prev) => ({
                                  ...prev,
                                  sprint_duration_weeks:
                                    sprintRes.data.sprintDurationWeeks || 2,
                                  sprint_start_date:
                                    sprintRes.data.sprintStartDate || "",
                                  sprint_mode:
                                    sprintRes.data.sprintMode || "manual",
                                }));
                                setEditPaused(!!sprintRes.data.sprintPaused);
                              } catch (err) {
                                console.error(
                              ...
                              try {
                                ...
                                setEditActiveSprint(act.data?.sprint || null);
                                if (act.data?.sprint) {
                                  setEditPaused(
                                    act.data.sprint.status === "paused",
                                  );
                                }
                              } catch {
                                /* non-fatal */
                  ...
                  >
                    <div className={tc["sprint-config-form"]}>
                      ...
                      <div className={tc["sprint-field"]}>
                        ...
                        />
                        <small className={tc["field-hint"]}>
                          First sprint's start date (sprints auto-calculated
                          from this)
                        </small>
                      </div>
                      <div className={tc["sprint-field"]}>
                        ...
                        <small className={tc["field-hint"]}>
                          Auto mode automatically starts, completes, and rotates
                          sprints on the configured cadence.
                        </small>
                      </div>
                      {editForm.sprint_mode === "auto" && editActiveSprint && (
                        <div className={tc["sprint-field"]}>
                          <label className={tc["field-label"]}>
                            ⏯️ Active Sprint — {editActiveSprint.name} (
                            {editPaused ? "Paused" : editActiveSprint.status})
                          </label>
                          <button
                            type="button"
                            className={`${s.btnSmall} ${
                              editPaused ? s.btnAccent : s.btnSecondary
                            }`}
                            onClick={() =>
                              handlePauseResume(editActiveSprint.id, editPaused)
                            }
                          >
                            {editPaused ? "Resume Sprint" : "Pause Sprint"}
                          </button>
                          <small className={tc["field-hint"]}>
                            {editPaused
