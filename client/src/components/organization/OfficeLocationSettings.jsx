...
import { MapPin, Crosshair, ShieldCheck, ShieldOff, Save, Loader2, Search, X, Wifi, Plus, Trash2 } from 'lucide-react';
...
export default function OfficeLocationSettings({ org, onUpdate }) {
    ...
    const [wifiBssids, setWifiBssids] = useState(
    ...
    const [wifiAddErr, setWifiAddErr] = useState(null);
    const [currentWifi, setCurrentWifi] = useState(null); // { ok, bssid, ssid, signal }
    ...
    async function handleAddCurrentWifi() {
        ...
        try {
            ...
            const defaultLabel = info.ssid ? `${info.ssid} (${mac.slice(-5)})` : `Office AP (${mac.slice(-5)})`;
            setWifiBssids(prev => [...prev, {
                bssid: mac,
    ...
    function handleRemoveBssid(mac) {
        setWifiBssids(prev => prev.filter(b => (b.bssid || '').toUpperCase() !== mac.toUpperCase()));
        if (editingBssid && editingBssid.toUpperCase() === mac.toUpperCase()) {
            setEditingBssid(null);
        }
    }

    function startEditLabel(ap) {
        ...
        <div className={s.wrap}>
            ...
            <div className={s.wifiSection}>
                ...
                <p className={s.wifiHint}>
                    ...
                    treated as <strong>at the office</strong> regardless of GPS accuracy.
                    The geofence below acts as a fallback for browsers, ethernet, and
                    mobile network clock-ins.
                </p>

                {wifiBssids.length === 0 && (
                    <div className={s.wifiEmpty}>
                        No office access points registered yet. Add one from the desktop app
                        while connected to the office Wi-Fi.
                    </div>
                )}
                ...
                {wifiBssids.length > 0 && (
                    <ul className={s.wifiList}>
                        {wifiBssids.map(ap => {
                            const isEditing = editingBssid && editingBssid.toUpperCase() === (ap.bssid || '').toUpperCase();
                            return (
                                    ...
                                    <div className={s.wifiItemMain}>
                                        ...
                                        <div className={s.wifiItemText}>
                                            ...
                                            <div className={s.wifiItemMeta}>
                                                <code>{ap.bssid}</code>
                                                {ap.ssid && <span className={s.wifiItemSsid}>SSID: {ap.ssid}</span>}
                                            </div>
                                        </div>
                                    </div>
                                    <button
                    ...
                    <div className={s.wifiManualForm}>
                        <div className={s.wifiManualRow}>
                            ...
                            <input
                                ...
                                className={s.wifiManualInput}
                                maxLength={100}
                                onKeyDown={e => { if (e.key === 'Enter') handleSubmitManualBssid(); }}
                            />
                        </div>
                        <div className={s.wifiManualActions}>
                            ...
                            <button
                                ...
                                onClick={() => { setManualOpen(false); setManualBssid(''); setManualLabel(''); setWifiAddErr(null); }}
                            >
                                Cancel
                            </button>
                        </div>
                ...
                <div className={s.wifiActions}>
                    ...
                    <button
                        type="button"
                        className="btn btn-secondary btn-sm"
                        onClick={() => { setManualOpen(o => !o); setWifiAddErr(null); }}
                    >
                ...
                <div className={s.searchInputWrap}>
                    ...
                    {searchOpen && (searchResults.length > 0 || searchErr) && (
                        <ul className={s.searchDropdown}>
                            ...
                            {searchResults.map(r => (
                                <li key={r.place_id} className={s.searchItem} onClick={() => pickSearchResult(r)}>
