...
function TreeNode({
    ...
    return (
        <div className={oc["tree-node"]} style={{ "--depth": depth } as React.CSSProperties}>
            <div
                className={`${oc["tree-row"]} ${
                    highlight && member.full_name.toLowerCase().includes(highlight.toLowerCase())
                        ? oc["tree-row-hl"]
                        : ""
                }`}
            >
...
export default function OrgChartView({ orgId }: { orgId?: number | string }) {
    ...
    const { childrenMap, roots } = useMemo(() => {
        if (!chart) return { childrenMap: {} as ChildrenMap, roots: [] as OrgMember[] };
        ...
        <div>
            ...
            <div className={oc["chart-controls"]}>
                <div className={oc["view-toggle"]}>
                    ...
                    >
                        <Building2 size={14} style={{ marginRight: 5, verticalAlign: "middle" }} />
                        By Department
                    </button>
                    <button
                        className={`${oc["view-btn"]} ${
                    ...
                    >
                        <Users size={14} style={{ marginRight: 5, verticalAlign: "middle" }} />
                        Reporting Lines
                    </button>
                </div>
                <div className={oc["search-box"]}>
                    ...
                    />
                    {search && (
                        <button className={oc["search-clear"]} onClick={() => setSearch("")}>
                            ✕
                        </button>
                    )}
                </div>
            ...
            {viewMode === "tree" && (
                <div className={oc["tree-root"]}>
                    {roots.length === 0 ? (
                        ...
                        >
                            No reporting lines configured — assign managers to employees to build the
                            hierarchy.
                        </p>
                    ) : (
                        roots.map((r) => (
