...
const ROLE_LABELS_MAP = ROLE_LABELS as Record<string, string>;
...
function MemberAvatar({
  ...
  return src ? (
    <img
      src={src}
      className={size === "md" ? oc["mini-avatar-md"] : oc["mini-avatar-sm"]}
      alt=""
    />
  ) : (
    <span
      className={`${s.initials} ${
        size === "md" ? oc["mini-initials-md"] : oc["mini-initials-sm"]
      }`}
    >
...
interface MemberChipProps {
  member: OrgMember;
  highlight?: string;
  showDeptTeam?: boolean;
}

function MemberChip({ member, highlight, showDeptTeam }: MemberChipProps) {
  const name = member.full_name;
  const hl = highlight?.toLowerCase();
  const escapedHl = hl ? hl.replace(/[.*+?^${}()|[\]\\]/g, "\\$&") : null;
  const hiName =
    escapedHl && name.toLowerCase().includes(hl!)
      ? DOMPurify.sanitize(
          name.replace(new RegExp(`(${escapedHl})`, "gi"), "<mark>$1</mark>"),
        )
      : DOMPurify.sanitize(name);
  const deptTeamLabel = showDeptTeam
    ? [member.department_name, member.team_name].filter(Boolean).join(" › ") ||
      null
    : null;
  return (
    <div
      className={oc["member-chip"]}
      title={`${name}\n${ROLE_LABELS_MAP[member.role] || member.role}${
    ...
    >
      ...
      <div className={oc["member-chip-info"]}>
        ...
        {deptTeamLabel && (
          <span className={oc["member-chip-dept"]}>{deptTeamLabel}</span>
        )}
      </div>
      <span
        className={`${s.badgeRole} ${oc["badge-sm"]}`}
...
type ChildrenMap = Record<string, OrgMember[]>;
...
function TreeNode({
  ...
  return (
    <div
      className={oc["tree-node"]}
      style={{ "--depth": depth } as React.CSSProperties}
    >
      <div
        className={`${oc["tree-row"]} ${
          highlight &&
          member.full_name.toLowerCase().includes(highlight.toLowerCase())
            ? oc["tree-row-hl"]
            : ""
        }`}
      >
...
function DeptCard({
  ...
  return (
    <div className={oc["card-panel"]}>
      <div
        className={oc["dept-header"]}
        style={{ cursor: "pointer" }}
        onClick={() => setOpen((o) => !o)}
      >
        <span className={oc["dept-icon"]}>
          <Building2 size={18} />
        </span>
        <div style={{ flex: 1 }}>
          ...
          {dept.head_name && (
            <div className={su["text-muted-xs"]}>Head: {dept.head_name}</div>
          )}
        </div>
      </div>
      {open && (
        <>
            ...
            return (
              <div key={team.id} className={oc["team-card"]}>
                <div className={oc["team-title"]}>
                  ...
                  />
                  {team.name}
                  <span className={oc["headcount-badge"]}>
                    {tMembers.length}
                  </span>
                  {team.lead_name && (
                    <span className={oc["team-lead-label"]}>
                      {" "}
                      · Lead: {team.lead_name}
                    </span>
                  )}
                ...
                <div className={oc["flex-wrap"]}>
                  {tMembers.length > 0 ? (
                    tMembers.map((m) => (
                      <MemberChip key={m.id} member={m} highlight={highlight} />
                    ))
                  ) : (
          ...
          {deptDirectMembers.length > 0 && (
            <div className={oc["unassigned-section"]}>
              <div className={oc["unassigned-label"]}>
...
// ── Main component ─────────────────────────────────────────────────────────
export default function OrgChartView({ orgId }: { orgId?: number | string }) {
  const { data: chart = null } = useQuery({
    queryKey: ["admin", "orgChart", orgId],
    queryFn: async (): Promise<OrgChartData> =>
      (await getOrgChart(orgId ? { org_id: orgId } : undefined)).data,
  });
  const [viewMode, setViewMode] = useState<"dept" | "tree">("dept"); // 'dept' | 'tree'
  ...
  const filteredMembers = useMemo(() => {
    ...
    return chart.members.filter(
      (m) =>
        ...
        (ROLE_LABELS_MAP[m.role] || m.role).toLowerCase().includes(q) ||
  ...
  const { childrenMap, roots } = useMemo(() => {
    if (!chart)
      return { childrenMap: {} as ChildrenMap, roots: [] as OrgMember[] };
    const allIds = new Set(chart.members.map((m) => m.id));
    const map: ChildrenMap = {};
    for (const m of chart.members) {
      const pid =
        m.manager_id && allIds.has(m.manager_id) ? m.manager_id : null;
  ...
  const totalVisible =
    viewMode === "dept" ? filteredMembers.length : chart.members.length;
  ...
  return (
    <div>
      ...
      <div className={oc["chart-controls"]}>
        <div className={oc["view-toggle"]}>
          <button
            className={`${oc["view-btn"]} ${
              viewMode === "dept" ? oc["view-btn-active"] : ""
            }`}
          ...
          <button
            className={`${oc["view-btn"]} ${
              viewMode === "tree" ? oc["view-btn-active"] : ""
            }`}
        ...
        <span className={oc["member-total"]}>
            ...
            <div className={oc["card-panel"]}>
              ...
              <div className={oc["flex-wrap"]}>
                {filteredUnassigned.map((m) => (
                  <MemberChip key={m.id} member={m} highlight={search} />
                ))}
              </div>
            </div>
          )}
