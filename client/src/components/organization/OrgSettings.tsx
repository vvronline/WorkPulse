...
function parseWorkDaysToSet(value?: string): Set<number> {
...
function setToCsv(set: Set<number>): string {
    return Array.from(set)
        .sort((a, b) => a - b)
        .join(",");
}

export default function OrgSettings({ org, onUpdate, userRole }: OrgSettingsProps) {
    ...
    const toggleWorkDay = (jsDow: number) => {
        ...
        <form onSubmit={handleSave} className={su["form-container-sm"]}>
                ...
                <>
                    ...
                    <div className={sf.formGroup}>
                        ...
                        <input
                            ...
                            onChange={(e) =>
                                setForm({ ...form, work_hours_per_day: e.target.value })
                            }
                        />
                    </div>
                </>
            ...
            <div className={sf.formGroup}>
                ...
                <small style={{ color: "var(--text-muted)", display: "block", marginTop: 4 }}>
                    Days not selected here are treated as <strong>weekend holidays</strong>. By
                    default Saturday &amp; Sunday are off — toggle them on (or any other day) to match
                    your organisation's schedule. Affects clock-in, attendance reports, the "Weekend
                    Holiday" badge, and the analytics denominator.
                </small>
            </div>
            <div className={sf.formGroup}>
            ...
            <div className={sf.formGroup}>
                ...
                />
                <small style={{ color: "var(--text-muted)", display: "block", marginTop: 4 }}>
                    Employees logging fewer than this many hours on a working day are considered
                    absent in attendance reports. Leave blank to use half the daily work hours (
                    {defaultMinHours}h).
                </small>
            </div>
            <div className={sf.formGroup}>
