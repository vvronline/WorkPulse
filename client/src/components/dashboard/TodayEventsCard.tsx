...
function isNowOrPast(isoString: string): boolean {
...
function isJoinable(start: string, end: string): boolean {
    const now = Date.now();
    const startMs = new Date(start).getTime();
...
function MeetingItem({ ev, onJoinMeeting, tick }: { ev: CalEvent; onJoinMeeting: (code: string) => void; tick: number }) {
...
function EventItem({ ev, accentColor }: { ev: CalEvent; accentColor: string }) {
...
const TodayEventsCard = memo(function TodayEventsCard({ events, tomorrowEvents }: { events?: CalEvent[] | null; tomorrowEvents?: CalEvent[] | null }) {
    ...
    const onJoinMeeting = (code: string) => navigate(`/meeting/${code}`);
    ...
    const tomorrowSorted = useMemo(() =>
        tomorrowEvents ? [...tomorrowEvents].sort((a, b) => new Date(a.start_time).getTime() - new Date(b.start_time).getTime()) : [],
        [tomorrowEvents]
    );
