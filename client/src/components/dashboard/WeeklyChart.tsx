...
const WeeklyChart = memo(function WeeklyChart({ weeklyData }: { weeklyData?: WeeklyData | null }) {
                        ...
                        return (
                            <div key={i} className={`${s["weekly-bar-col"]} ${d.isToday ? s.today : ""}`} title={`${d.day}: ${d.hours}h`}>
                                <div className={s["weekly-bar-track"]}>
                                    <div className={s["weekly-bar-fill"]} style={{ "--bar-height": `${barHeight}%`, "--bar-color": barColor } as React.CSSProperties} />
                                </div>
