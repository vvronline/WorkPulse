...
function DeviceSelector({ devices, activeId, onSelect, onClose, label }: DeviceSelectorProps) {
    return (
...
export default function CallOverlay({ callState, user, wsSend, onEnd }: CallOverlayProps) {
    const {
    ...
    const [status, setStatus] = useState<string>(isReconnect ? "reconnecting" : (isIncoming ? "incoming" : "ringing"));
    ...
    const [addPartResults, setAddPartResults] = useState<any[]>([]);
    const [addPartSearching, setAddPartSearching] = useState(false);
    const [swapped, setSwapped] = useState(false);
    const addPartTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);

    // Drag state for local video PiP
    const localVideoDragState = useRef<any>(null);
    const [localVideoPos, setLocalVideoPos] = useState<{ left: number; top: number } | null>(null);
        ...
        if (status === "connected" && pcRef.current) {
            statsIntervalRef.current = setInterval(async () => {
                try {
                    ...
                    let rtt: number | null = null, packetsLost = 0, packetsReceived = 0;
    ...
    const createPeerConnection = useCallback((stream: MediaStream | null, targetUserId: any) => {
    ...
    const handleSignalInternal = useCallback(async (signal: any, fromUserId: any) => {
        ...
        if (signal.type === "offer") {
            ...
            try { await pcRef.current.addIceCandidate(new RTCIceCandidate(signal.candidate)); } catch { /* noop */ }
        }
    ...
    const cleanup = () => {
        try { screenStreamRef.current?.getTracks().forEach(t => { try { t.stop(); } catch { /* noop */ } }); } catch { /* noop */ }
        screenStreamRef.current = null;
        try { localStreamRef.current?.getTracks().forEach(t => { try { t.stop(); } catch { /* noop */ } }); } catch { /* noop */ }
        localStreamRef.current = null;
        try { if (remoteAudioRef.current) remoteAudioRef.current.srcObject = null; } catch { /* noop */ }
        try { if (pcRef.current) pcRef.current.close(); } catch { /* noop */ }
        pcRef.current = null;
        pendingSignalsRef.current = [];
        if (timerRef.current) clearInterval(timerRef.current);
        if (statsIntervalRef.current) clearInterval(statsIntervalRef.current);
        if (addPartTimerRef.current) clearTimeout(addPartTimerRef.current);
    ...
    const toggleVideo = async () => {
        ...
        if (!videoOff) {
            ...
            try {
                const constraints: MediaStreamConstraints = activeVideoDevice
        ...
        if (screenSharing) {
            ...
            try {
                const screenStream = await navigator.mediaDevices.getDisplayMedia({
                    video: { cursor: "always" } as any, audio: false
                });
    ...
    const handleAddPartSearch = (val: string) => {
        setAddPartQuery(val);
        if (addPartTimerRef.current) clearTimeout(addPartTimerRef.current);
    ...
    const handleAddPartInvite = (targetUserId: any) => {
    ...
    const onLocalVideoDragStart = useCallback((e: any) => {
        if (swapped) return;
        e.preventDefault();
    ...
    return (
        ...
        >
            ...
            {/* Avatar / info */}
            {(!isVideoCall || !isConnected || onHold || (status as string) === "reconnecting") && (
