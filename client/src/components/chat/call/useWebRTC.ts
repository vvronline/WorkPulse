 ...
 */
function buildMediaConstraintProfiles(
  isVideoCall: boolean,
  isMobile: boolean,
...
/**
 * Apply the Opus FEC munge to a description BEFORE it is set locally, so the
 * description we hand to `setLocalDescription` and the SDP we signal to the
 * peer are byte-identical.
 *
 * Without `useinbandfec=1` every lost audio packet is an audible gap — the
 * most-noticed form of "jitter" on a call. FEC lets the decoder rebuild a lost
 * packet from redundancy carried in the next one for a few kbps.
 */
function withOpusFec(
  desc: RTCSessionDescriptionInit,
): RTCSessionDescriptionInit {
  if (!desc?.sdp) return desc;
  return { type: desc.type, sdp: preferOpusFec(desc.sdp) };
}
...
export default function useWebRTC({
  ...
  const [remotePeerQuality, setRemotePeerQuality] = useState<
  ...
  // ── Adaptive quality ──────────────────────────────────────────────────
  // ONE controller per call, owned here and shared with useCallControls'
  // stats loop. Previously the connect-time bitrate ramp and the stats loop
  // each kept their own idea of the current encoder target; when they drifted
  // apart (a skipped ramp step, a dropped setParameters) the stats loop saw
  // "already at the right bitrate" and stopped correcting — leaving the call
  // pinned at the ~400 kbps connect cap for its entire duration. Sharing the
  // controller makes that class of bug impossible.
  ...
  const refreshIceConfig = useCallback(async () => {
    try {
      ...
      if (data?.iceServers?.length) {
        ...
        console.log(
          "[call-webrtc] ICE config refreshed (mode:",
          data.mode || "unknown",
          ", expiresAt:",
      ...
      console.warn(
        "[call-webrtc] ICE config fetch failed, using fallback:",
        err?.message || err,
      );
    }
  }, []);
  ...
  const waitForIceConfig = useCallback(
    async (timeoutMs = 2000) => {
      ...
      if (!initialIceConfigLoadedRef.current) {
        ...
        if (!initialIceConfigLoadedRef.current) {
          console.warn(
            "[call-webrtc] ICE config not loaded after",
            timeoutMs,
            "ms — proceeding with fallback",
          );
        }
      }
      ...
      if (!firstNegotiationStartedRef.current && !iceHasRealTurnRef.current) {
        ...
        if (!iceHasRealTurnRef.current) {
          console.warn(
            "[call-webrtc] real TURN not available before first negotiation — proceeding with fallback ICE",
          );
        }
      }
  ...
  const startMedia = useCallback(async (): Promise<MediaStream | null> => {
    if (!navigator.mediaDevices?.getUserMedia) {
      alert(
        "Your browser does not support audio/video calls. Please use Chrome, Edge, Firefox or Safari over HTTPS.",
      );
      return null;
    }
    const profiles = buildMediaConstraintProfiles(
      callType === "video",
      isMobile,
    );
    ...
    for (let i = 0; i < profiles.length; i++) {
      ...
      try {
        ...
        if (i > 0) {
          console.warn(
            "[call-webrtc] media acquired with reduced profile #" + i,
            constraints,
          );
        }
        ...
        lastError = err;
        console.warn(
          "[call-webrtc] getUserMedia profile",
          i,
          "failed:",
          err?.name,
          err?.message,
    ...
    console.error(
      "[call-webrtc] getUserMedia exhausted all profiles:",
      lastError,
    );
    ...
    if (lastError?.name === "NotAllowedError") {
      ...
      alert(
        `Your ${device} is in use by another application (e.g. Zoom, Teams, Skype). Please close that app and try again.`,
      );
    } else if (lastError) {
      alert(
        `Could not access ${device}: ${lastError.message || lastError.name}`,
      );
    }
    return null;
  }, [callType]);
  ...
  const cleanup = useCallback(() => {
    ...
    if (cancelBitrateRampRef.current) {
  ...
  /**
   * Connect-time bitrate ramp. Starts on a modest tier so ICE/DTLS settle and
   * the first frames arrive fast, then walks UP the shared quality ladder.
   *
   * Replaces the previous hand-rolled ramp, which:
   *   • wrote `setParameters` from three concurrent places (its own 3 timers,
   *     the keyframe hack, and the stats loop). Because `getParameters()`
   *     returns a transactionId-stamped snapshot, the racing writes REJECTED
   *     with InvalidStateError into empty `.catch(() => {})` blocks — so ramp
   *     steps were silently dropped and the encoder could stay at the start
   *     cap for the whole call.
   *   • kept ramp state separate from the adaptive controller's state, so the
   *     two disagreed and the controller stopped correcting.
   *
   * `startBitrateRampUp` serialises every write and drives the SAME controller
   * the stats loop reads, so neither failure mode can recur.
   */
  const applyBitrateRampUp = useCallback(
    (pc: RTCPeerConnection | null) => {
      if (!pc) return;
      if (!qualityControllerRef.current) {
        qualityControllerRef.current = createQualityController({ isMobile });
      }
      if (cancelBitrateRampRef.current) cancelBitrateRampRef.current();
    ...
    try {
      if (candidate === null) {
        ...
        try {
          ...
          try {
            await pcRef.current.addIceCandidate({ candidate: "" } as any);
          } catch {
            /* ignore */
          }
        }
      ...
      console.warn(
        "[call-webrtc] addIceCandidate failed (ignored):",
        err?.message || err,
      );
    }
  }, []);
  ...
  const flushPendingIceCandidates = useCallback(async () => {
    ...
    console.log(
      "[call-webrtc] flushing",
      pendingCandidates.length,
      "buffered ICE candidate(s)",
    );
    ...
    for (const track of stream.getTracks()) {
      ...
      if (matchingTr) {
        ...
        try {
          ...
          if (track.kind === "video")
            screenSenderRef.current = matchingTr.sender;
        } catch (err: any) {
          console.warn(
            "[call-webrtc] replaceTrack failed, falling back to addTrack:",
            err?.message || err,
          );
  ...
  const createPeerConnection = useCallback(
    (
      ...
      if (relayOnlyRef.current) {
        ...
        console.log(
          "[call-webrtc] creating PC in RELAY-ONLY mode (TURN-only) — corporate proxy fallback",
        );
      }
      ...
      if (!hasTurn) {
        console.warn(
          "[call-webrtc] No TURN server configured — calls may fail on restrictive networks. Configure TURN_HOST + TURN_STATIC_AUTH_SECRET (coturn) on the server.",
        );
      }
      ...
      pc.onnegotiationneeded = async () => {
        ...
        try {
          ...
          const offer = withOpusFec(await pc.createOffer());
      ...
      pc.ontrack = (e) => {
        ...
        if (
          ...
          remoteAudioRef.current
            ...
            .catch((err: any) =>
              console.warn(
                "[call-webrtc] remote audio autoplay blocked:",
                err?.message || err,
              ),
            );
        }
        ...
        if (remoteVideoRef.current) {
          ...
          remoteVideoRef.current
            ...
            .catch((err: any) =>
              console.warn(
                "[call-webrtc] remote video autoplay blocked:",
                err?.message || err,
              ),
            );
        }
      ...
      pc.onicecandidate = (e) => {
        if (e.candidate) {
          const c = e.candidate;
          console.log(
            "[call-webrtc] local ICE candidate →",
            c.type || "?",
            c.protocol || "?",
      ...
      pc.onicecandidateerror = (e: any) => {
        console.warn(
          "[call-webrtc] ICE candidate error:",
          e.errorCode,
          e.errorText,
          e.url,
        );
      ...
      pc.oniceconnectionstatechange = () => {
        console.log(
          "[call-webrtc] ICE connection state:",
          pc.iceConnectionState,
        );
        if (pc.iceConnectionState === "failed") {
          console.warn(
            "[call-webrtc] ICE failed — will attempt restart. Persistent failures usually mean a TURN server is required (corporate proxy / symmetric NAT).",
          );
          ...
          if (initialNegotiationDone && !iceRestartAttemptedRef.current) {
            setTimeout(() => {
              if (
                ...
                console.log(
                  "[call-webrtc] still disconnected after 2s — issuing ICE restart",
                );
                iceRestartAttemptedRef.current = true;
                pc.createOffer({ iceRestart: true })
                  .then((o) => pc.setLocalDescription(withOpusFec(o)))
                  .then(() =>
                    wsSend("call_signal", {
                      callId: callIdRef.current,
                      conversationId,
                  ...
                  .catch((err) =>
                    console.warn("[call-webrtc] ICE restart failed:", err),
                  );
              }
            }, 2000);
          }
      ...
      pc.onconnectionstatechange = () => {
        ...
        if (pc.connectionState === "connected") {
          ...
          // Shrink the receiver's playout buffer where supported (Chrome).
          // A large default jitter buffer is a major source of the "call feels
          // laggy even though the network is fine" complaint.
          applyLowLatencyPlayout(pc);
          // NOTE: the old `forceKeyframe(pc)` call used to live here. It set
          // `encodings[0].active = false` then back to `true` 50ms later, which
          // TEARS DOWN AND REBUILDS the encoder — a guaranteed 200-500ms black
          // freeze at the exact moment the call connects, i.e. the very freeze
          // it was written to fix. The receiver's PLI/FIR already requests a
          // keyframe whenever it genuinely needs one.
          ...
          if (callType === "video" && !videoStateRequestedRef.current) {
            ...
            remoteVideoWatchdogRef.current = setTimeout(() => {
              ...
              try {
                ...
                console.warn(
                  "[call-webrtc] request-video-state failed:",
                  err?.message || err,
                );
              }
            }, 3000);
          ...
          if (!iceRestartAttemptedRef.current && initialNegotiationDone) {
            ...
            pc.createOffer({ iceRestart: true })
              .then((offer) => {
                return pc.setLocalDescription(withOpusFec(offer));
              })
              .then(() => {
                wsSend("call_signal", {
                  callId: callIdRef.current,
            ...
            console.warn(
              "[call-webrtc] escalating to RELAY-ONLY mode after ICE restart failed",
            );
            ...
            refreshIceConfig().finally(() => {
              ...
              newPc
                .createOffer()
                .then((o) => newPc.setLocalDescription(withOpusFec(o)))
                .then(() => {
                  wsSend("call_signal", {
                    callId: callIdRef.current,
                    conversationId,
                ...
                .catch((err) => {
                  console.error(
                    "[call-webrtc] relay-only rebuild failed:",
                    err,
                  );
                  if (handleEndRef.current) handleEndRef.current();
                });
  ...
  const handleSignalInternal = useCallback(
    async (signal: any, fromUserId: any) => {
      if (!pcRef.current) return;
        ...
        if (signal.type === "offer") {
          ...
          if (offerCollision) {
            if (!politeRef.current) {
              console.warn(
                "[call-webrtc] ignoring colliding offer (impolite peer)",
              );
              return;
            }
            try {
          ...
          const answer = withOpusFec(await pcRef.current.createAnswer());
          ...
          if (pcRef.current.signalingState !== "have-local-offer") {
            console.warn(
              "[call-webrtc] ignoring answer in state:",
              pcRef.current.signalingState,
            );
          } else {
            await pcRef.current.setRemoteDescription(
          ...
          const tgt =
            fromUserId ?? (isIncoming ? callerId : acceptedBy || reconnectTo);
          if (tgt) {
            ...
            try {
              ...
              console.warn(
                "[call-webrtc] reply request-video-state failed:",
                err?.message || err,
              );
            }
          }
  ...
  const handleSignal = useCallback(
    (signal: any, fromUserId: any) => {
      ...
      if (signal.type === "offer" && pcRef.current) {
        ...
        if (needsRebuild) {
          console.warn(
            "[call-webrtc] received offer while PC is",
            cs,
            "/",
            ics,
            "— rebuilding (peer is doing relay-only escalation)",
          );
          relayOnlyRef.current = true;
          try {
            pcRef.current.close();
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn(
        "[call-webrtc] call_peer_ready re-offer failed:",
        err?.message || err,
      );
    }
  }, [
  ...
  useEffect(() => {
    if (accepted && !isIncoming) {
      (async () => {
        console.log(
          "[call-webrtc] call accepted, creating offer. acceptedBy:",
          acceptedBy,
          "hasStream:",
          !!localStreamRef.current,
        );
        ...
        if (!stream) {
          console.log(
            "[call-webrtc] stream not ready yet, deferring offer creation",
          );
  ...
  useEffect(() => {
    const triggerIceRestart = (reason: string) => {
      ...
      if (
        ...
        return;
      console.log(
        "[call-webrtc] network change detected (",
        reason,
        ") — issuing ICE restart",
      );
      ...
      pc.createOffer({ iceRestart: true })
        ...
        .catch((err) =>
          console.warn("[call-webrtc] network-change ICE restart failed:", err),
        );
    };
  ...
  const sendLocalVideoState = useCallback(
    (isVideoOff: boolean) => {
      ...
      try {
        ...
        console.warn(
          "[call-webrtc] sendLocalVideoState failed:",
          err?.message || err,
        );
      }
    },
  ...
  const sendLocalMuteState = useCallback(
    (isMuted: boolean) => {
      const target = isIncoming ? callerId : acceptedBy || reconnectTo;
      if (!target) return;
      try {
        ...
        console.warn(
          "[call-webrtc] sendLocalMuteState failed:",
          err?.message || err,
        );
      }
    },
  ...
  const sendLocalScreenShareState = useCallback(
    (isSharing: boolean) => {
      const target = isIncoming ? callerId : acceptedBy || reconnectTo;
      if (!target) return;
      try {
        ...
        console.warn(
          "[call-webrtc] sendLocalScreenShareState failed:",
          err?.message || err,
        );
      }
    },
  ...
  const sendQualityState = useCallback(
    (quality: "good" | "fair" | "poor" | "unknown") => {
      ...
      try {
        ...
        console.warn(
          "[call-webrtc] sendQualityState failed:",
          err?.message || err,
        );
      }
    },
