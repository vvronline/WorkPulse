...
export default function CallOverlay({
  ...
  // consumers — the Electron PiP window and the document-PiP clone — which
  // are updated from CallDuration's onTick rather than from React state.
  // Mirrors what the mobile client already does.
  ...
  // ─── Duration ───
  // No timer here on purpose — see <CallDuration /> at the bottom of the
  // overlay. A `setInterval(setDuration)` at this level re-rendered the whole
  // tree (video tiles included) once per second for the entire call.

  // Only collect the expensive per-sample debug stats while the panel that
  // displays them is actually open. Previously `setDetailedStats` fired a
  // brand-new object on every stats tick regardless, forcing another full
  // overlay re-render for data nobody was looking at.
  useEffect(() => {
    controls.setStatsPanelOpen?.(showStats);
  }, [showStats, controls.setStatsPanelOpen]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const sendReaction = useCallback(
    (emoji: string) => {
      ...
      const targetUserId = callState.isIncoming
        ? callState.callerId
        : callState.acceptedBy || callState.callerId;
  ...
  const handleLocalVideoDragStart = useCallback((e: any) => {
    ...
    const onMove = (ev: any) => {
      ...
      if (
        Math.abs(cx - dragRef.current.startX) > 10 ||
        Math.abs(cy - dragRef.current.startY) > 10
      ) {
  ...
  useEffect(() => {
    ...
    const offAction = (window.electronAPI as any).callPip.onAction(
  ...
  // Push non-duration state changes into the Electron PiP window. The DURATION
  // is pushed separately from <CallDuration />'s onTick (see pushPipDuration)
  // so the per-second value never has to become React state here.
  useEffect(() => {
  ...
  /**
   * Paint the non-video PiP surfaces (document-PiP clone + Electron PiP window)
   * IMPERATIVELY. Called from <CallDuration />'s onTick each second and from a
   * status/hold change.
   *
   * These two surfaces live outside React's tree, so writing to them from a
   * callback costs nothing; routing the per-second value through `useState`
   * here is exactly what used to re-render the whole overlay every second.
   */
  const paintPipSurfaces = useCallback(
    (seconds: number) => {
      ...
      if (pipWin) {
        try {
          ...
          if (el) {
            ...
            else el.textContent = formatDuration(seconds);
          }
        } catch {
          /* ignore */
        }
      }
      if (hasElectronCallPip && electronPipActive) {
        try {
          (window.electronAPI as any).callPip.updateState({
            remoteName: remoteName || "Call",
            remoteAvatar: remoteAvatar || null,
  ...
  // Repaint immediately on a status / hold change so the PiP text doesn't wait
  // up to a second for the next tick.
  useEffect(() => {
    paintPipSurfaces(durationRef.current);
  }, [paintPipSurfaces]);
  ...
  const externalFloaterActive =
    electronPipActive || nativePipActive || !!docPipWindowRef.current;
    ...
    >
      ...
      {isWinElectron && !minimized && (
        <div className={s.windowControls}>
          ...
          <button
            className={`${s.winBtn} ${s.winCloseBtn}`}
      ...
      {((isVideoCall && controls.videoOff) ||
        (!isVideoCall && webrtc.remoteHasVideo)) && (
        <div
      ...
      {controls.muted &&
        ...
        !swapped && (
          ...
          >
            <div
              className={s.localTileMuteBadge}
              title="You are muted"
      ...
      {isConnected &&
        ...
        !controls.screenSharing && (
          ...
          >
            ...
            {controls.muted && (
              <div
                className={s.selfPreviewMuteBadge}
                title="You are muted"
                aria-label="You are muted"
              >
                <MicOffIcon />
      ...
      {isConnected && (
        <div
          className={`${s.topBar} ${!controlsVisible ? s.topBarHidden : ""}`}
        >
          {controls.screenSharing && (
            <span className={s.sharingBadge}>Screen Sharing</span>
          )}
          ...
          <div className={s.statsPanelBody}>
            ...
            {controls.detailedStats.frameWidth && (
              <div className={s.statRow}>
                ...
                <span>
                  {controls.detailedStats.frameWidth}x
                  {controls.detailedStats.frameHeight}
                </span>
              </div>
            )}
            {controls.detailedStats.audioCodec && (
              <div className={s.statRow}>
                ...
                <span>
                  {controls.detailedStats.audioCodec.replace("audio/", "")}
                </span>
              </div>
            )}
            {controls.detailedStats.videoCodec && (
              <div className={s.statRow}>
                ...
                <span>
                  {controls.detailedStats.videoCodec.replace("video/", "")}
                </span>
              </div>
            )}
      ...
      {reconnectToast && (
        <div
          className={`${s.reconnectToast} ${reconnectToast === "reconnected" ? s.reconnectToastSuccess : ""}`}
        >
          {reconnectToast === "disconnected"
      ...
      {(() => {
        ...
        const callInfoHidden =
          isConnected && !controls.onHold && !controlsVisible;
        return (
          <div
            className={`${s.callInfo} ${callInfoHidden ? s.callInfoHidden : ""}`}
          >
            <div
              ...
              {endMessage ? (
                ...
                <>
                  ...
                  {isConnected && (
                    <CallDuration
                      running={isConnected}
                      prefix={controls.onHold ? "On Hold \u00B7 " : undefined}
                      onTick={paintPipSurfaces}
                    />
                  )}
                </>
              )}
      ...
      {isVideoCall && isConnected && !controls.onHold && (
        <div
          className={`${s.videoOverlayInfo} ${!controlsVisible ? s.videoOverlayInfoHidden : ""}`}
        >
      ...
      {showShortcutsHelp && (
        ...
        >
          <div
            className={s.shortcutsModal}
            onClick={(e) => e.stopPropagation()}
          >
            <div className={s.shortcutsHeader}>
              <span>Keyboard Shortcuts</span>
              <button
                className={s.shortcutsClose}
                onClick={() => setShowShortcutsHelp(false)}
              >
                &times;
              </button>
      ...
      {/* ─── Controls bar ─── */}
      <div
        className={`${s.controls} ${!controlsVisible && isConnected ? s.controlsHidden : ""}`}
      >
        {status === "incoming" ? (
          <>
            <button
              className={`${s.controlBtn} ${s.rejectBtn}`}
              onClick={webrtc.handleReject}
              title="Decline"
            >
              <PhoneIcon rotate />
            </button>
            <button
              className={`${s.controlBtn} ${s.acceptBtn}`}
              onClick={webrtc.handleAccept}
              title="Accept"
            >
              <PhoneIcon />
            </button>
          ...
          <>
            ...
            <button
              className={`${s.controlBtn} ${s.endBtn}`}
              onClick={webrtc.handleEnd}
              title="End call (E)"
            >
              <PhoneIcon rotate />
            </button>
        ...
        <div className={s.miniBar} onClick={(e) => e.stopPropagation()}>
          <div className={s.miniBarInfo}>
            ...
            <span className={s.miniBarMeta}>
              {isConnected ? (
                <CallDuration running={isConnected} onTick={paintPipSurfaces} />
              ) : status === "incoming" ? (
                "Incoming\u2026"
              ) : status === "ringing" ? (
                "Ringing\u2026"
