...
export default function GlobalMeetingNotification() {
    const [notification, setNotification] = useState<MeetingNotificationData | null>(null);
    const navigate = useNavigate();
    const autoDismissRef = useRef<ReturnType<typeof setTimeout> | null>(null);
    const shownMeetingRef = useRef<string | number | null>(null); // dedup: track which meetingId is currently shown
    const { session } = (useMeeting() as any) || {};
    const activeMeetingIdRef = useRef<string | number | null>(null);
    activeMeetingIdRef.current = session?.meetingId ?? null;

    const showNotification = useCallback((data: MeetingNotificationData) => {
    ...
    return (
        <div className={s.card}>
            ...
            <div className={s.meetInfo}>
                ...
                <p className={s.meetHost}>
                    {notification.organizerName} {notification.restarted ? "restarted" : "started"}{" "}
                    this meeting
                </p>
            </div>
            <div className={s.actions}>
                ...
                <button className={s.declineBtn} onClick={handleDismiss}>
                    Dismiss
                </button>
            </div>
        </div>
