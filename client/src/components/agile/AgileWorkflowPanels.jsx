...
import {
    PlayCircle, CheckCircle, AlertCircle, Plus, X, Search,
    LineChart as LineIcon, BarChart3, Link as LinkIcon,
} from 'lucide-react';
import {
    setTaskBlocker, getTaskDependencies, addTaskDependency, removeTaskDependency,
    quicksearchTasks, getSprintBurndown, getRecentVelocity,
    startSprint, completeSprint, getSprints,
    getTaskChildren, getTaskParent, setTaskParent,
} from '../../api';
...
// BlockerControl
// ────────────────────────────────────────────────────────────────────────────
export function BlockerControl({ task, canEdit = true, onChanged }) {
    ...
    const toggle = async () => {
        ...
        setBusy(true);
        try {
            const r = await setTaskBlocker(task.id, false, null);
        ...
        <div className={s.blockerControl}>
            {!editing && (
                <button
                    ...
                    title={task.blocked_reason || (task.is_blocked ? 'Blocked' : 'Mark as blocked')}
                >
                    <AlertCircle size={13} />
            ...
            {editing && (
                <div className={s.blockerForm}>
                    ...
                    />
                    <button className="btn btn-danger btn-sm" onClick={submitBlock} disabled={busy}>
                        Block
                    </button>
...
export function DependenciesPanel({ task, canEdit = true }) {
    const { features } = useAgileConfig();
    const [data, setData] = useState({ blocks: [], blockedBy: [] });
    ...
    const reload = useCallback(async () => {
        ...
        } catch { setData({ blocks: [], blockedBy: [] }); }
    ...
    useEffect(() => {
        ...
        const handle = setTimeout(async () => {
            ...
            try {
                ...
                setResults((r.data.tasks || []).filter(t => t.id !== task?.id));
            } catch { setResults([]); }
    ...
    const link = async (otherId) => {
        try {
            ...
            setAdding(false); setSearch(''); setResults([]);
            reload();
        } catch (e) {
            setError(e?.response?.data?.error || 'Failed to add link');
        }
    };
    ...
    const unlink = async (linkId) => {
        try {
            ...
            setError(e?.response?.data?.error || 'Failed to remove link');
        }
    };

    if (!features.dependencies) return null;
    if (loading) return <div className={s.depLoading}>Loading dependencies…</div>;

    const sections = [
        { title: 'Blocks', items: data.blocks, dirHint: 'This task blocks…' },
        { title: 'Blocked by', items: data.blockedBy, dirHint: 'This task is blocked by…' },
    ];
            ...
            {adding && canEdit && (
                <div className={s.depAddBox}>
                    <div className={s.depAddRow}>
                        <select value={linkType} onChange={e => setLinkType(e.target.value)} className={s.depTypeSel}>
                        ...
                        <ul className={s.depResults}>
                            {results.map(r => (
                                <li key={r.id} onClick={() => link(r.id)}>
                                    <span className={s.depResultId}>#{r.id}</span> {r.title}
                                </li>
                            ))}
                        </ul>
                    )}
                ...
                <div key={sec.title} className={s.depSection}>
                    ...
                    {sec.items.length === 0 ? (
                        ...
                        <ul className={s.depList}>
                            {sec.items.map(it => (
                                <li key={it.link_id} className={`${s.depItem} ${it.is_blocked ? s.itemBlocked : ''}`}>
                                    <span className={s.depBadge} data-type={it.type}>{it.type}</span>
                                    <span className={s.depItemId}>#{it.id}</span>
                                    <span className={s.depItemTitle}>{it.title}</span>
                                    {canEdit && (
                                        <button
...
// ────────────────────────────────────────────────────────────────────────────
export function BurndownChart({ sprintId, height = 200 }) {
    ...
    const chart = useMemo(() => {
        ...
        const idealPoints = (data.ideal || []).map((p, i, arr) => ({
            x: padL + (innerW * i) / Math.max(1, arr.length - 1),
            y: padT + innerH - (innerH * (Number(p.remaining) / Math.max(1, data.startScope))),
            date: p.date,
            value: p.remaining,
        }));

        const actualPoints = (data.snapshots || []).map((sn, i, arr) => {
            ...
            const dayIdx = Math.round((new Date(sn.snapshot_date) - start) / 86400000);
            const totalDays = Math.max(1, idealPoints.length - 1);
            const x = padL + (innerW * Math.max(0, Math.min(dayIdx, totalDays))) / totalDays;
            const y = padT + innerH - (innerH * (Number(sn.remaining_points) / Math.max(1, data.startScope)));
        ...
        return { padL, padR, padT, padB, w, h, innerW, innerH, idealPoints, actualPoints, path, yLabels };
    }, [data, height]);
    ...
    if ((data.snapshots?.length || 0) === 0 && data.startScope === 0) {
        return <div className={s.chartEmpty}>No data yet — add story points & start the sprint to see a burndown.</div>;
    }

    return (
        <div className={s.chartWrap}>
            <div className={s.chartHeader}>
                <span><LineIcon size={13} /> Burndown</span>
                <span className={s.chartLegend}>
                    <span className={s.legIdeal}></span> Ideal
                    <span className={s.legActual}></span> Actual
                </span>
            </div>
            <svg viewBox={`0 0 ${chart.w} ${chart.h}`} className={s.chartSvg} role="img" aria-label="Burndown chart">
                {/* y-axis grid + labels */}
                {chart.yLabels.map((t, i) => (
                    <g key={i}>
                        <line
                            x1={chart.padL} y1={t.y}
                            x2={chart.w - chart.padR} y2={t.y}
                            className={s.gridLine}
                        />
                        <text x={chart.padL - 6} y={t.y + 3} textAnchor="end" className={s.axisLabel}>
                            {t.value}
                        </text>
                    </g>
                ))}
                {/* x-axis */}
                <line
                    x1={chart.padL} y1={chart.h - chart.padB}
                    x2={chart.w - chart.padR} y2={chart.h - chart.padB}
                    className={s.axisLine}
                />
                <text x={chart.padL} y={chart.h - 6} className={s.axisLabel}>{data.sprint.start_date}</text>
                <text x={chart.w - chart.padR} y={chart.h - 6} textAnchor="end" className={s.axisLabel}>{data.sprint.end_date}</text>

                {/* Ideal line */}
                <path d={chart.path(chart.idealPoints)} className={s.idealLine} fill="none" />
                {/* Actual line */}
                <path d={chart.path(chart.actualPoints)} className={s.actualLine} fill="none" />
                {chart.actualPoints.map((p, i) => (
                    <g key={i}>
                        <circle cx={p.x} cy={p.y} r={3} className={s.actualDot} />
                        <title>{p.date}: {p.value} {unitLabel} remaining</title>
                    </g>
                ))}
            </svg>
        </div>
...
// ────────────────────────────────────────────────────────────────────────────
export function VelocityChart({ limit = 6, height = 180 }) {
    const { unitLabel } = useAgileConfig();
    const [data, setData] = useState(null);
    const [loading, setLoading] = useState(true);
    ...
    if (!data || !data.sprints?.length) {
        return <div className={s.chartEmpty}>No completed sprints yet — velocity will appear after the first sprint completes.</div>;
    }
    ...
    const innerW = w - padL - padR;
    const innerH = h - padT - padB;
    const maxVal = Math.max(data.average, ...data.sprints.map(sp => Number(sp.velocity_points) || 0)) || 1;
    const barW = Math.max(8, innerW / data.sprints.length - 14);

    return (
        <div className={s.chartWrap}>
            <div className={s.chartHeader}>
                <span><BarChart3 size={13} /> Velocity</span>
                <span className={s.chartLegend}>
                    Avg: <strong>{data.average}</strong> {unitLabel}
                </span>
            </div>
            <svg viewBox={`0 0 ${w} ${h}`} className={s.chartSvg} role="img" aria-label="Velocity chart">
                {/* Average line */}
                {data.average > 0 && (
                    <line
                        x1={padL} x2={w - padR}
                ...
                {data.sprints.map((sp, i) => {
                    const cx = padL + ((innerW / data.sprints.length) * i) + (innerW / data.sprints.length) / 2;
                    ...
                    return (
                        <g key={sp.id}>
                            <rect x={x} y={y} width={barW} height={bh} className={s.bar} rx={3}>
                                <title>{sp.name}: {v} {unitLabel}</title>
                            </rect>
                            <text x={cx} y={y - 4} textAnchor="middle" className={s.barLabel}>{v}</text>
                            <text x={cx} y={h - 18} textAnchor="middle" className={s.barAxis}>{sp.name.replace('Sprint ', 'S')}</text>
                            {sp.completed_at && (
                                <text x={cx} y={h - 6} textAnchor="middle" className={s.barSubAxis}>
                                    {new Date(sp.completed_at).toLocaleDateString()}
                                </text>
                            )}
                        </g>
                    );
...
// ────────────────────────────────────────────────────────────────────────────
export function SprintLifecycleControls({ sprint, canEdit = true, onChanged }) {
    const [busy, setBusy] = useState(false);
    ...
    useEffect(() => {
        ...
        getSprints().then(r => {
            const list = (r.data?.sprints || []).filter(sp => sp.id !== sprint.id && sp.status !== 'completed');
            setAvailable(list);
        });
    ...
    const start = async () => {
        ...
        try {
            ...
            setError(e?.response?.data?.error || 'Failed to start sprint');
        } finally { setBusy(false); }
    };
    ...
    const complete = async () => {
        ...
        try {
            ...
            setError(e?.response?.data?.error || 'Failed to complete sprint');
        } finally { setBusy(false); }
    };
        ...
        <div className={s.lifecycleWrap}>
            ...
            {sprint.status === 'active' && completing && (
                <div className={s.lifecycleForm}>
                    <label className={s.lifecycleLabel}>Roll over incomplete tickets to:</label>
                    <select
                        value={rolloverTo}
                    ...
                    <button className="btn btn-primary btn-sm" onClick={complete} disabled={busy}>
                        Complete
                    </button>
                    <button className="btn btn-secondary btn-sm" onClick={() => setCompleting(false)} disabled={busy}>
                        Cancel
                    </button>
                </div>
            )}
            {sprint.status === 'completed' && (
                <span className={s.completedBadge}>
                    ...
                    {sprint.velocity_points != null && (
                        <span className={s.velocityChip}>velocity {Number(sprint.velocity_points)}</span>
                    )}
                </span>
            )}
...
//
// On an Epic ticket: shows the list of child tickets with a clickable link
// (uses ?task= query param so the existing Tasks page opens the detail modal).
// On any other ticket: shows the parent (with a "Part of" link) and lets you
// link to a different parent via the quicksearch picker.
//
// `onOpenTask(taskId)` is the parent-supplied opener that swaps the modal to
// the linked ticket. We fall back to a window.location query-string nav if
// not provided so the component stays drop-in.
// ────────────────────────────────────────────────────────────────────────────
export function ParentChildPanel({ task, canEdit = true, onOpenTask, onChanged }) {
    ...
    const isEpic = !!(task?.work_item_type_id && typeById[task.work_item_type_id]?.is_epic);
    ...
    const reload = useCallback(async () => {
        ...
        try {
            ...
            setChildren([]); setRollup(null); setParent(null);
        } finally { setLoading(false); }
    }, [task?.id, isEpic]);
    ...
    //
    // We *prefer* Epic-typed tickets to surface first, but never hide
    // non-Epics — some teams treat any ticket as a potential parent
    // (e.g. a Story can be a child of another Story when broken down).
    // Hard-filtering to Epics-only made the picker show "No matching Epics"
    // even when valid Epics existed, because the quicksearch endpoint
    // didn't return work_item_type_id for older tenants.
    useEffect(() => {
        if (!picking) return;
        if (!search.trim()) { setResults([]); return; }
        const handle = setTimeout(async () => {
            ...
            try {
                ...
                const epicTypeIds = new Set(
                    Object.values(typeById).filter(t => t.is_epic).map(t => t.id)
                );
                const all = (r.data?.tasks || []).filter(t => t.id !== task?.id);
    ...
    const setParentTask = async (parentId) => {
        try {
            await setTaskParent(task.id, parentId);
            setPicking(false); setSearch(''); setResults([]);
            await reload();
            onChanged && onChanged();
        } catch (e) {
            setError(e?.response?.data?.error || 'Failed to set parent');
        }
    };
            ...
            <div className={s.depWrap}>
                        ...
                        {rollup && rollup.totalChildren > 0 && (
                            <span className={s.depRollup}>
                                ...
                                {rollup.totalPoints > 0 && (
                                    <> · {rollup.donePoints}/{rollup.totalPoints} pts ({rollup.percentByPoints}%)</>
                                )}
                            </span>
                        )}
                    </div>
                ...
                {children.length === 0 ? (
                    <div className={s.depEmpty}>
                        No child tickets yet. Open any ticket → Edit → set this Epic as its <em>Parent</em>.
                    </div>
                ) : (
                    <ul className={s.depList}>
                        {children.map(c => (
                            <li key={c.id} className={`${s.depItem} ${c.is_blocked ? s.itemBlocked : ''}`}>
                                {c.type_name && (
                                    <span
                                        className={s.depBadge}
                                        style={{
                                            color: c.type_color || undefined,
                                            background: c.type_color ? `color-mix(in srgb, ${c.type_color} 12%, transparent)` : undefined,
                                        }}
                                    >
                                        {c.type_name}
                                    </span>
                                ...
                                <a
                                    ...
                                    onClick={(e) => { e.preventDefault(); openTask(c.id); }}
                                ...
                                {c.state_name && (
                                    <span
                                        ...
                                        style={{ '--state-color': c.state_color || '#6b7280' }}
                                    >
                                        {c.state_name}
                                    </span>
                                )}
                                {c.story_points != null && c.story_points !== '' && (
                                    <span className={s.depPoints} title="Story points">
    ...
    //
    // No "Parent" header label here — the parent appears inline as a small
    // pill ("Part of #42 Epic Title") with a Change button. The Dependencies
    // panel below provides its own heading, so we don't repeat the framing.
    const epicTypeIds = new Set(
        Object.values(typeById).filter(t => t.is_epic).map(t => t.id)
    );
    return (
        <div className={s.parentInline}>
            {parent ? (
                <span className={s.parentChip}>
                    ...
                    <a
                        ...
                        onClick={(e) => { e.preventDefault(); openTask(parent.id); }}
            ...
            {picking && canEdit && (
                <div className={s.depAddBox} style={{ marginTop: 6, width: '100%' }}>
                    <div className={s.depAddRow}>
                        ...
                        <button
                            className="btn btn-secondary btn-sm"
                            onClick={() => { setPicking(false); setSearch(''); setResults([]); }}
                        >
                            Cancel
                        </button>
                    </div>
                    ...
                    {results.length > 0 && (
                        <ul className={s.depResults}>
                            {results.map(r => {
                                const isEpicType = epicTypeIds.has(r.work_item_type_id);
                                return (
                                    <li key={r.id} onClick={() => setParentTask(r.id)}>
                                        {isEpicType && <span className={s.depResultEpic}>Epic</span>}
                                        <span className={s.depResultId}>#{r.id}</span> {r.title}
                                    </li>
                                );
                            })}
