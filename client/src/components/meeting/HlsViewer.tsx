 ...
 */
export default function HlsViewer({ src, poster, autoPlay = true, muted = false, className = "", onState }: HlsViewerProps) {
    const videoRef = useRef<HTMLVideoElement | null>(null);
    const hlsRef = useRef<any>(null);
    const retryRef = useRef<{ count: number; timer: ReturnType<typeof setTimeout> | null }>({ count: 0, timer: null });
    const [state, setState] = useState("loading"); // loading | live | error | offline

    const setS = (s: string) => { setState(s); onState?.(s); };
    ...
    useEffect(() => {
        ...
        if (video.canPlayType("application/vnd.apple.mpegurl")) {
            ...
            (async () => {
                try {
                    // @ts-ignore - hls.js has no bundled type declarations
