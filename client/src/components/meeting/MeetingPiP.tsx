...
export default function MeetingPiP() {
    const { session, leaveMeeting, localStreamRef, wsRef, joinedAt } = useMeeting() as any;
    const { user } = useAuth();
    ...
    useEffect(() => {
        ...
        if (stream) {
            ...
            const audioOff = !stream.getAudioTracks().some((t: MediaStreamTrack) => t.enabled);
            const vidOff = !stream.getVideoTracks().some((t: MediaStreamTrack) => t.enabled);
            setMuted(audioOff);
            setVideoOff(vidOff);
        }
    ...
    const handleToggleMute = (e: React.MouseEvent) => {
        ...
        if (!stream) return;
        const next = !muted;
        stream.getAudioTracks().forEach((t: MediaStreamTrack) => { t.enabled = !next; });
        setMuted(next);
        // Notify peers via WS
    ...
    const handleToggleVideo = (e: React.MouseEvent) => {
        ...
        if (next) {
            ...
            const liveTracks = stream.getVideoTracks().filter((t: MediaStreamTrack) => t.readyState === "live");
            if (liveTracks.length > 0) {
                liveTracks.forEach((t: MediaStreamTrack) => { t.enabled = true; });
