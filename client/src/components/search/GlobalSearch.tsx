...
const LEAVE_STATUS_COLOR: Record<string, string> = { approved: "#16a34a", pending: "#d97706", rejected: "#dc2626", withdraw_pending: "#0284c7" };
const SPRINT_STATUS_COLOR: Record<string, string> = { active: "#16a34a", planned: "#2563eb", completed: "#6b7280" };
            ...
            <div className={s.modal} role="dialog" aria-label="Global search">
                ...
                {hasResults && (
                    <div className={s.results}>
                                ...
                                {results!.tasks!.map((t: any, i: number) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <span className={`${s.badge} ${(s as Record<string, string>)[`status-${t.status}`] || s.badgeDefault}`}>
                        ...
                        {/* ── Leaves ── */}
                        {(results?.leaves?.length ?? 0) > 0 && (
                            <section>
                                <h4 className={s.sectionTitle}>Leave Requests</h4>
                                {results!.leaves!.map((l: any, i: number) => {
                        ...
                        {(results?.sprints?.length ?? 0) > 0 && (
                            <section>
                                ...
                                {results!.sprints!.map((sp: any, i: number) => {
                        ...
                        {(results?.logs?.length ?? 0) > 0 && (
                            <section>
                                ...
                                {results!.logs!.map((l: any, i: number) => {
