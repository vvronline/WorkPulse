...
interface OneOnOnePrefill {
    report?: { fullName?: string } | null;
    tasks?: Array<{ status?: string; title?: string }>;
 ...
 */
export function buildJournalPrefillHtml(prefill: JournalPrefill): string {
    const parts: string[] = [];
    ...
    if (prefill.tasks && prefill.tasks.length > 0) {
        ...
        if (done.length > 0 || other.length > 0) {
            parts.push(`<ul>`);
                ...
                parts.push(`<li data-list="checked">${escHtml(t.title)}</li>`);
            });
            other.forEach((t) => {
                parts.push(
                    `<li data-list="unchecked">${escHtml(t.title)} <em>(${escHtml(t.status)})</em></li>`
                );
            });
            parts.push(`</ul>`);
        }
    ...
    if (prefill.meetings && prefill.meetings.length > 0) {
        ...
        prefill.meetings.forEach((m) => {
            const time = new Date(m.scheduled_start!).toLocaleTimeString(undefined, {
                hour: "2-digit",
                minute: "2-digit",
            });
    ...
    if (prefill.tasks && prefill.tasks.length > 0) {
        ...
        if (inProgress.length > 0) {
            ...
            inProgress.slice(0, 5).forEach((t) => {
                parts.push(`<li>🔵 ${escHtml(t.title)}</li>`);
            });
            parts.push(`</ul>`);
        }
        if (pending.length > 0) {
