...
class MathBlot extends BlockEmbed {
    ...
    static render(node: any, tex: string) {
        const k = typeof window !== "undefined" ? (window as any).katex : null;
        try {
            if (k && tex) {
...
class DrawioBlot extends BlockEmbed {
    static create(value: any) {
        ...
        node.addEventListener("click", (e: Event) => {
            ...
            try {
                node.dispatchEvent(
                    new CustomEvent("notes:drawio-edit", {
                        bubbles: true,
                        detail: { node },
                    })
                );
    ...
    static renderInner(node: any, svg: string) {
        if (svg) {
...
class AudioBlot extends BlockEmbed {
    ...
    static renderInner(node: any) {
        ...
        node.innerHTML = `<div class="ql-audio-row">
         <span class="ql-audio-icon" aria-hidden="true">🎙️</span>
         <span class="ql-audio-label">${label.replace(
             /[<>&]/g,
...
if (typeof window !== "undefined" && (window as any).hljs) {
    baseModules.syntax = { hljs: (window as any).hljs, interval: 250 };
}
