...
import type { NotePage } from "./notesUtils";
    ...
    switch (filterId) {
        ...
        case SMART_FOLDER_IDS.TODOS:
            return list.filter((p) =>
                /data-list="(?:un)?checked"/.test(p.content || ""),
            );
        case SMART_FOLDER_IDS.TODAY: {
            const cutoff = Date.now() - dayMs;
            return list.filter(
                (p) => new Date(p.updatedAt || 0).getTime() >= cutoff,
            );
        }
        ...
        case SMART_FOLDER_IDS.LIKED:
            return list.filter((p) => {
                const likes = (p.reactions as Record<string, unknown>)?.["👍"];
...
interface UseNotesFiltersOptions {
    pages: NotePage[];
...
export function useNotesFilters({
    ...
    dropdownSearch,
    currentUserId,
}: UseNotesFiltersOptions): UseNotesFiltersResult {
    const activePage = pages.find((p) => p.id === activePageId) || null;

    const wc = useMemo(
        () =>
            activePage
                ? getWordCount(activePage.content)
                : { words: 0, chars: 0 },
