...
/* ──────────────────────────────────────────────────────────
   ...
   ────────────────────────────────────────────────────────── */
function makeCommands(opts: MakeCommandsOpts = {}): SlashCommand[] {
    const { onPickPageLink, onInsertToc, onInsertSprintEmbed, onInsertTimeBlock, onConvertToTask, onNewOneOnOne } = opts;
    const blockFormat = (format: string, value: any = true) => (q: any) => {
        const range = q.getSelection(true);
        q.formatLine(range.index, 1, format, value, "user");
    };

    const insertEmbed = (kind: string, value: any = true) => (q: any) => {
        const range = q.getSelection(true);
    ...
    return [
        ...
        },
        {
            ...
            run: (q) => {
                ...
                const range = q.getSelection(true);
                let text = "";
                headings.forEach((h: any) => { text += `${"  ".repeat(h.level - 1)}• ${h.text}\n`; });
...
/* ──────────────────────────────────────────────────────────
   Hook + component
   ────────────────────────────────────────────────────────── */
export default function SlashMenu({ quillRef, pageId, resetKey, onPickPageLink, onInsertToc, onInsertSprintEmbed, onInsertTimeBlock, onConvertToTask, onNewOneOnOne }: SlashMenuProps) {
    ...
    const [state, setState] = useState<{ open: boolean; pos: { top: number; left: number } | null }>({ open: false, pos: null });
    ...
    const triggerRef = useRef<{ index: number } | null>(null); // { index: number }
    ...
    const computePos = (q: any, idx: number) => {
        try {
    ...
    const repositionFor = (q: any, idx: number) => {
        const newPos = computePos(q, idx);
        if (newPos) setState(st => (st.open ? { open: true, pos: newPos } : st));
    };
    ...
    useEffect(() => {
        ...
        const firstInsert = (delta: any) => {
            let p = 0;
            for (const op of (delta?.ops || [])) {
                if (typeof op.retain === "number") p += op.retain;
                else if (typeof op.delete === "number") return null; // deletion → not an insert
                else if (typeof op.insert === "string") return { pos: p, text: op.insert };
                else if (op.insert) return null; // embed
            }
            return null;
        };

        const onChange = (delta: any, _old: any, source: string) => {
            ...
            const newPos = computePos(q, slashIndex);
            setQuery("");
            setState({ open: true, pos: newPos || { top: 0, left: 0 } });
        };
