...
import {
    Trash2, Copy, ClipboardPaste, Scissors,
...
function detectTarget(el: HTMLElement | null): DetectedTarget | null {
    if (!el) return null;
    ...
    while (node && !node.classList?.contains("ql-editor")) {
        ...
        if (node.tagName === "P" && (node.parentNode as HTMLElement)?.classList?.contains("ql-editor")) {
...
export default function EditorContextMenu({ quillRef, pageId, resetKey, readOnly }: EditorContextMenuProps) {
    const [menu, setMenu] = useState<MenuState | null>(null); // { x, y, target, hasSelection }
    const [subMenu, setSubMenu] = useState<string | null>(null); // for callout variant picker
    ...
    useEffect(() => {
        ...
        const onContext = (e: MouseEvent) => {
            const target = detectTarget(e.target as HTMLElement);
    ...
    const changeCalloutVariant = useCallback((variant: string) => {
        const target = menu?.target;
    ...
    const convertHeadingLevel = useCallback((level: number | false) => {
        const quill = getQuill();
        const target = menu?.target;
        if (!quill || !target) return;
        const blot = Quill_findBlot(quill, target.node);
    ...
    const style: React.CSSProperties = { position: "fixed", left: menu.x, top: menu.y, zIndex: 10000 };
...
function Quill_findBlot(quill: any, node: HTMLElement): any {
    try {
