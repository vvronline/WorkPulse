...
function snippetOf(html?: string, max = 140): string {
...
export default function NotesHome({ store }: NotesHomeProps) {
    ...
    const activePages = useMemo(() => pages.filter((p: any) => !p.archived), [pages]);

    const pinned = useMemo(
        () => activePages.filter((p: any) => p.pinned).slice(0, 6),
        [activePages]
    );
        ...
        return activePages
            .filter((p: any) => {
                ...
                return Object.values(reactions).some(
                    (list: any) => Array.isArray(list) && list.includes(uid)
                );
            })
    ...
    const tagCounts = useMemo(() => {
        ...
        activePages.forEach((p: any) => (p.tags || []).forEach((t: string) => {
                ...
                <nav className={s.sideNav}>
                    ...
                    <div className={s.sideTree}>
                        <FolderTree
                            ...
                            pages={pages}
                            activePageId={null}
                            onSelectPage={(id: any) => openEditor(id)}
                            onNewFolder={handleNewFolder}
                            onNewPage={(folderId: any, title: any) => handleNewPage(folderId, title)}
