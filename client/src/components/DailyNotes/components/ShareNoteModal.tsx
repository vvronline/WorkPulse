 ...
 */
export default function ShareNoteModal({ page, isOpen, onClose }: ShareNoteModalProps) {
    ...
    useEffect(() => {
        ...
        getNoteShare(page.id)
            ...
            .catch((err: any) => { if (!cancelled) setError(err.response?.data?.error || "Failed to load share state"); })
            .finally(() => { if (!cancelled) setLoading(false); });
        return () => { cancelled = true; };
    }, [isOpen, page?.id]);

    const buildUrl = (t: string) => `${window.location.origin}/n/${t}`;

    const create = async () => {
        if (!page?.id) return;
        setBusy(true); setError(""); setCopied(false);
        try {
            const { data } = await createNoteShare(page.id) as any;
            setToken(data.token);
