...
type EntityType = "task" | "calendar_event" | "meeting";
...
function MeetingChip({ meeting }: { meeting: any }) {
...
interface LinkedEntitiesPanelProps {
    ...
    onNavigateTask?: (...args: any[]) => void;
}

export default function LinkedEntitiesPanel({ pageId, onNavigateTask }: LinkedEntitiesPanelProps) {
    const [links, setLinks] = useState<NoteLink[]>([]);
    const [loading, setLoading] = useState(false);
    const [adding, setAdding] = useState<EntityType | null>(null); // 'task' | 'calendar_event' | 'meeting' | null
    const [searchQ, setSearchQ] = useState("");
    const [searchResults, setSearchResults] = useState<any[]>([]);
    const [searching, setSearching] = useState(false);
    const [expanded, setExpanded] = useState(true);
    const [ctxMenu, setCtxMenu] = useState<CtxMenu | null>(null); // { x, y, entityType, entityId }
    ...
    const fetchLinks = useCallback(async () => {
        ...
        setLoading(true);
        try {
            const res = await getNoteLinks(pageId as any);
            setLinks((res.data as any)?.links || []);
        } catch { /* ignore */ }
        setLoading(false);
    }, [pageId]);
    ...
    const handleAdd = async (entityType: EntityType, entityId: string | number) => {
        try {
            await addNoteLink(pageId as any, entityType, entityId as any);
            setAdding(null);
            setSearchQ("");
            setSearchResults([]);
            fetchLinks();
    ...
    const handleRemove = async (entityType: string, entityId: string | number) => {
        try {
            await removeNoteLink(pageId as any, entityType, entityId as any);
    ...
    useEffect(() => {
        ...
        debounceRef.current = setTimeout(async () => {
            ...
            try {
                ...
                setSearchResults((res.data as any)?.[key] || []);
    ...
    const handleContextMenu = (e: React.MouseEvent, entityType: string, entityId: string | number) => {
        e.preventDefault();
