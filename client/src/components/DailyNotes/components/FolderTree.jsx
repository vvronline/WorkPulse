...
function CreateRow({ mode, depth, onSubmit, onCancel }) {
...
/* ── A note row (leaf) ─────────────────────────────────────── */
function NoteRow({ page, depth, isActive, onSelect, onDragStart, onDragEnd, isDragging }) {
    return (
        <div
...
function FolderNode({
    ...
    const [creating, setCreating] = useState(null); // 'folder' | 'page' | null
...
export default function FolderTree({
    ...
    const [dragInfo, setDragInfo] = useState(null);   // { type, id }
    const [dropTarget, setDropTarget] = useState(null); // folderId | 'root'
    const [creatingRoot, setCreatingRoot] = useState(null); // 'folder' | null

    const onToggle = useCallback((id, forceOpen) => {
        setExpanded(prev => ({
            ...prev,
