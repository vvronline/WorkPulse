...
export default function NotesModal({ store, embedded = false }: NotesModalProps) {
    const {
    ...
    const content = (
        ...
        >
            <div className={`${s.modal} ${embedded ? s.modalEmbedded : ""}`}>
                ...
                <div className={s.body}>
                    <ModalEditor
                        ...
                        onSelectFolder={(fid: any) => { setFolderFilter(fid || "all"); }}
            ...
            <ReportPickerModal
                ...
                onSelect={(reportUserId: any) => handleNewOneOnOneWithPrefill(reportUserId)}
            />
        </div>
    );
