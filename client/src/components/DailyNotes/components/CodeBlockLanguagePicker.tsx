...
const LANG_LABEL: Record<string, string> = Object.fromEntries(CODE_LANGUAGES.map((l: any) => [l.id, l.label]));

function getEditor(ref: any): any {
...
function readLang(pre: HTMLElement | null): string {
...
export default function CodeBlockLanguagePicker({ quillRef, pageId, resetKey }: CodeBlockLanguagePickerProps) {
    const [picker, setPicker] = useState<PickerState | null>(null); // { pre, top, left }
    ...
    useEffect(() => {
        ...
        const ensureBadge = (pre: HTMLElement) => {
        ...
        const refreshBadge = (pre: HTMLElement) => {
            const btn = pre.querySelector<HTMLButtonElement>("[data-cb-lang]");
            if (!btn) return;
            const label = readLang(pre);
            btn.innerText = LANG_LABEL[label] || label;
        };
        ...
        const scan = () => {
            editor.querySelectorAll("pre").forEach((pre: HTMLElement) => {
                ensureBadge(pre);
                refreshBadge(pre);
            });
        };
        ...
        const onClick = (e: MouseEvent) => {
            const btn = (e.target as HTMLElement).closest("[data-cb-lang]");
            if (!btn) return;
