...
interface TodoRowProps {
    ...
    onPriority: (id: any, priority: string) => void;
    onDue: (id: any, due: string) => void;
    isDragging: boolean;
...
export default function TodoApp({ store }: TodoAppProps) {
    const {
        todos = [],
    ...
    const overId = useRef<any>(null);
    const [draggingId, setDraggingId] = useState<any>(null);

    const ordered = useMemo(
        () => [...todos].sort((a: any, b: any) => (a.sortOrder ?? 0) - (b.sortOrder ?? 0)),
        [todos]
    );
    ...
    const counts = useMemo(() => {
        ...
        const done = dayItems.filter((t: any) => t.done).length;
        return { total, done, active: total - done };
    }, [dayItems]);

    const visible = useMemo(() => {
        if (filter === "active") return dayItems.filter((t: any) => !t.done);
        if (filter === "completed") return dayItems.filter((t: any) => t.done);
        return dayItems;
    }, [dayItems, filter]);
        ...
        <div className={s.wrap}>
            <header className={s.header}>
                <div className={s.headerTop}>
                    ...
                    <div className={s.progressRing} style={{ "--p": progress } as React.CSSProperties}>
                        <span>{progress}%</span>
                    </div>
                </div>
