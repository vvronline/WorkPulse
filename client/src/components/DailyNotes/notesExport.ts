...
function buildPrintable(page: NotePage): HTMLDivElement {
    const title = (page.title || "Untitled").replace(/[<>&]/g, (c) => escapeMap[c]);
...
export async function savePageAsPdf(page?: NotePage): Promise<void> {
    if (!page) return;
...
export function savePageAsMarkdown(page?: NotePage, orgName?: string): void {
    if (!page) return;
...
export function savePageAsHtml(page?: NotePage): void {
    if (!page) return;
    const title = (page.title || "Untitled").replace(/[<>&]/g, (c) => escapeMap[c]);
    const meta = page.updatedAt
        ? `<div class="meta">Edited ${new Date(page.updatedAt).toLocaleString()}</div>`
...
export function exportAllPagesAsMarkdown(
    pages: NotePage[],
    folders: NoteFolder[],
    orgName?: string
): void {
