...
const QuillLib = (ReactQuill as any).Quill;
...
───────────────────────────────────────────────────────────────────*/
export default function ImageResizer({ quillRef }: { quillRef: any }) {
    const [sel, setSel] = useState<SelState | null>(null); // { img, rect }
    const drag = useRef<any>(null); // active drag state
    const boxRef = useRef<HTMLDivElement | null>(null); // overlay div

    /* ── Recompute overlay rect from current image position ── */
    const syncRect = useCallback((img: HTMLImageElement) => {
        if (!img) return;
    ...
    useEffect(() => {
        ...
        const onClick = (e: MouseEvent) => {
            ...
            if (target.tagName === "IMG") {
                e.preventDefault();
                syncRect(target as HTMLImageElement);
            }
        };
    ...
    const onHandleMouseDown = useCallback(
        (e: React.MouseEvent, dir: string) => {
            ...
            const onMove = (me: MouseEvent) => {
                ...
                const { dir, startX, startY, startW, aspect, img } = drag.current;
...
function commitToQuill(quillRef: any, img: HTMLImageElement) {
    try {
        const q = quillRef?.current?.getEditor?.();
        if (!q) return;
