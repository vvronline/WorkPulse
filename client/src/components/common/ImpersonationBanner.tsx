...
function SessionSummaryModal({ summary, onClose, onConfirmExit, exiting }: SessionSummaryModalProps) {
    return (
            ...
            <div className={s.modal} onClick={(e) => e.stopPropagation()}>
                ...
                <div className={s.modalBody}>
                    <div className={s.summaryStats}>
                        ...
                        <div className={s.summaryStat}>
                            <div className={s.summaryStatValue} style={{ color: "var(--info, #3b82f6)" }}>
                                {summary.reads}
                            </div>
                            <div className={s.summaryStatLabel}>Inspections</div>
                        </div>
                        <div className={s.summaryStat}>
                            <div className={s.summaryStatValue} style={{ color: "var(--warning, #f59e0b)" }}>
                                {summary.writes}
                            </div>
                            ...
                            <div className={s.actionLogList}>
                                {summary.actions.map((a, i) => (
                                    <div key={i} className={s.actionLogItem}>
                                        <span
                                            className={`${s.actionMethod} ${a.type === "write" ? s.methodWrite : s.methodRead}`}
                                        >
                                            {a.method || (a.type === "write" ? "WRITE" : "READ")}
                                        </span>
                                        <span className={s.actionPath}>{a.path}</span>
                                        <span className={s.actionTime}>
                                            {new Date(a.timestamp as any).toLocaleTimeString()}
                                        </span>
                                    </div>
                                ))}
                            </div>
...
export default function ImpersonationBanner() {
    ...
    const [sessionInfo, setSessionInfo] = useState<SessionInfo>({ total: 0, reads: 0, writes: 0, actions: [] });
            ...
            <div className={s.banner}>
                <div className={s.bannerMain}>
                    ...
                    <span className={s.text}>
                        <span className={s.tenantName}>{user.impersonated_by_name || "Platform Admin"}</span> inspecting{" "}
                    ...
                    <div className={s.expandedLog}>
                        {sessionInfo.actions.slice(-10).map((a, i) => (
                            <div key={i} className={s.miniLogItem}>
                                ...
                                <span className={s.miniTime}>{new Date(a.timestamp as any).toLocaleTimeString()}</span>
                            </div>
                        ))}
