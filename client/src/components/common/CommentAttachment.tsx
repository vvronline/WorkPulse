 ...
 * URL is used as-is.
 */
export default function CommentAttachment({ comment }: { comment: any }) {
    if (!comment?.file_url) return null;

    const fullUrl = comment.file_url.startsWith("http") ? comment.file_url : `${serverURL}${comment.file_url}`;
