        ...
        <div className={s.overlay} onClick={(e) => e.target === e.currentTarget && onCancel?.()}>
            <div className={s.modal} ref={modalRef} role="dialog" aria-modal="true" aria-labelledby="confirm-dialog-title">
                <div className={s.header}>
                    <h3 className={s.title} id="confirm-dialog-title">
                        {title}
                    </h3>
                ...
                <div className={s.body}>{typeof message === "string" ? <p>{message}</p> : message}</div>
                <div className={s.footer}>
                    <button className={s.cancelBtn} onClick={onCancel}>
                        {cancelText}
                    </button>
                    <button className={`${s.confirmBtn} ${isDanger ? s.danger : s.primary}`} onClick={onConfirm}>
                        {confirmText}
                    </button>
                </div>
            </div>
