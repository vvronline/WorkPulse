...
//
// Props:
//   total      — total row count (server)
//   limit      — current page size
//   offset     — current zero-based row offset
//   onPageChange(nextOffset)  — called when a page button is clicked
//   onLimitChange(nextLimit)  — optional; if provided, renders a page-size
//                               select with the values in `pageSizeOptions`
//   pageSizeOptions — defaults to [10, 25, 50, 100]
//   compact    — small variant, used inside tight panels (Project tasks)
//   itemLabel  — singular noun for the count summary (default "item")
...
export default function Pagination({
    ...
    return (
        <div style={styles.bar(compact)}>
            <span style={styles.summary}>
                {startRow}–{endRow} of {total} {itemLabel}{total === 1 ? '' : 's'}
            </span>
...
const styles = {
    ...
    summary: { fontSize: 12, color: 'var(--text-secondary, #9ca3af)' },
    controls: { display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' },
    sizeLabel: { display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12, color: 'var(--text-secondary, #9ca3af)' },
    ...
    pager: { display: 'inline-flex', alignItems: 'center', gap: 4 },
    iconBtn: {
        display: 'inline-flex',
        alignItems: 'center',
    ...
    pageInfo: { fontSize: 12, color: 'var(--text-secondary, #9ca3af)', padding: '0 8px' },
};