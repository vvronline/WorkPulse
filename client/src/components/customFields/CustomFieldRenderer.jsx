...
export function CustomFieldRenderer({ field, value }) {
    ...
    switch (field.field_type) {
        ...
        case 'select': {
            ...
            return <span className={s.pill}>{opt?.label || value}</span>;
        }
        case 'multiselect': {
            ...
            return (
                <span className={s.pillRow}>
                    {arr.map(v => {
                        ...
                        return <span key={v} className={s.pill}>{opt?.label || v}</span>;
                    })}
                </span>
            );
        }
...
export function CustomFieldsEditor({
    ...
    const save = async () => {
        ...
        try {
            ...
            const fresh = (r.data && r.data.values) || {};
            setValues(fresh);
            setOriginal(fresh);
            setError('');
...
export function CustomFieldsSummary({ values, onlyShowOnCard = false }) {
    const { fields } = useCustomFields();
